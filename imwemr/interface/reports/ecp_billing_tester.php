...
include_once($GLOBALS['fileroot'].'/library/classes/SaveFile.php');
	...
	//$extracted_on ='2019-04-24 1:50:08';
	...
	$qry="Select cpt_fee_id,cpt4_code,cpt_desc FROM cpt_fee_tbl";
	$rs=imw_query($qry);
	while($res=imw_fetch_array($rs)){
		$arrAllCPTCodes[$res['cpt_fee_id']]['code'] = trim($res['cpt4_code']);
	...
	AND main.gro_id='4' and main.patient_id=152396"; //AND main.gro_id='4'
...
//  AND main.first_posted_date='0000-00-00' 
	...
	$file_name="bb_imedicware_export_tester_".$date.".csv";
	...
	if(sizeof($arrResultData)>0){
		...
		foreach($arryuni as $eid => $chrgObj){
			...
			foreach($chrgObj as $chrId => $detail_obj){
				...
				if(empty($_REQUEST['negative'])==true){
				...
				$strData.= $detail_obj['street'].$pfx;
				$strData.= $detail_obj['street2'].$pfx;
				$strData.= $detail_obj['city'].$pfx;
				$strData.= $detail_obj['state'].$pfx;
				$strData.= $detail_obj['postal_code'].$pfx;
				$strData.= $phone.$pfx;
				$strData.= $email.$pfx;				
...
if(file_exists($csv_file_name) && 5==6){
