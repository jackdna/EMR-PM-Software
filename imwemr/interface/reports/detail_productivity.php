...
if($viewBy=='department'){
	//MAKING OUTPUT DATA
	$file_name="practice_analytic_".time().".csv";
	...
	$data_output.="Practice Analytics Report (Detail)".$pfx;
	...
	$data_output.="Modifiers : $selModifiers".$pfx;
	$data_output.="Adj. Code : $selAdjCode".$pfx;
	$data_output.="Write-off Code :$selWriteoff".$pfx;
	$data_output.="Reg Facility :$dispRegFacility".$pfx;
	$data_output.="\n";
	
	$data_output.="Patient".$pfx;
	$data_output.="ID".$pfx;
	$data_output.=$firstGrpTitle1.$pfx;
	$data_output.=$secGrpTitle1.$pfx;
	...
	$data_output.="Primary Insurance".$pfx;
	$data_output.="Secondary Insurance".$pfx;
	$data_output.="Charges".$pfx;
	$data_output.="Pat. Paid".$pfx;
	$data_output.="Pri. Paid".$pfx;
	$data_output.="Sec. Paid".$pfx;
	...
	foreach($mainResArr as $firstGrpId => $firstGrpData){	
		...
		$firstGrpTitle='Filing Physician';
...
/*			if($viewBy=='physician' || $viewBy=='operator'){
			...
			foreach($secGrpData as $thirdGrpId => $encData){
				...
				foreach($encData as $enc_id => $encDataArr){
					...
					$primaryInsNameCSV  = $arrAllInsCompaniesCSV[$encDataArr[0]['pri_ins_id']];
					$secondaryInsName = $arrAllInsCompanies[$encDataArr[0]['sec_ins_id']];
					$secondaryInsNameCSV = $arrAllInsCompaniesCSV[$encDataArr[0]['sec_ins_id']];
					...
					for($d=0;$d<count($encDataArr);$d++){
						...
						if($pay_location=='1' && ($DateRangeFor=='transaction_date' || $DateRangeFor=='date_of_payment')){
							...
							// TOTAL PAYMENT
							$patientPaidAmt+= $mainEncounterPayArr[$secGrpId][$chgDetId] + ($patCrdDbt + $insCrdDbt);			
							// PATIENT PAID
							$patPaidAmt+= $patPayDetArr[$secGrpId][$chgDetId]['patPaid'] + $patCrdDbt;
							// INSURANCE PAID
							$insPaidAmt+= $patPayDetArr[$secGrpId][$chgDetId]['insPaid'] + $insCrdDbt;
							// PRI INSURANCE PAID
							if($patPayDetArr[$secGrpId][$chgDetId]['priPaid']>0){
								$priPaidAmt+= $patPayDetArr[$secGrpId][$chgDetId]['priPaid'] + $insCrdDbt;
								$insCrdDbt=0;
							}
							// SEC+TER INSURANCE PAID
							$secPaidAmt+= $patPayDetArr[$secGrpId][$chgDetId]['secPaid'] + $patPayDetArr[$secGrpId][$chgDetId]['terPaid'] + $insCrdDbt;
		
							//LAST PAID DOT
							if($patPayDetArr[$secGrpId][$chgDetId]['lastDOT']!=null && $patPayDetArr[$secGrpId][$chgDetId]['lastDOT']!='' && $patPayDetArr[$secGrpId][$chgDetId]['lastDOT']!='00-00-0000'){
								$arrLastPaidDOT[]= strtotime($patPayDetArr[$secGrpId][$chgDetId]['lastDOT']);
							}
						}else{
							//WRITE-OFF & ADJUSTMENTS
							...
							if($patPayDetArr[$chgDetId]['lastDOT']!=null && $patPayDetArr[$chgDetId]['lastDOT']!='' && $patPayDetArr[$chgDetId]['lastDOT']!='00-00-0000'){
								$arrLastPaidDOT[]= strtotime($patPayDetArr[$chgDetId]['lastDOT']);
							}
						}
						...
						$units+= $encDataArr[$d]['units'];
						
						
						if(!$arrTempChgIds[$chgDetId]){
							$arrSecGrpTotal["totalUnits"]+=$encDataArr[$d]['units'];
							$arrSecGrpTotal["totalAmt"]+=$encDataArr[$d]['totalAmt'];
							$arrSecGrpTotal["insuranceDue"]+=$encDataArr[$d]['pri_due'] + $encDataArr[$d]['sec_due'] + $encDataArr[$d]['tri_due'];
							$arrSecGrpTotal["patientDue"]+=$encDataArr[$d]['pat_due'];
							$arrSecGrpTotal["totalBalance"]+=$balAmt;
							$arrSecGrpTotal["creditProcAmount"]+=$encDataArr[$d]['over_payment'];
							$arrTempChgIds[$chgDetId]=$chgDetId;
						}
					}
					...
					$totalBalance = $CLSReports->numberFormat($totalBalance,2,1);
					...
					$strCPT_Desc = implode(', ', $arrCPT_Desc);
					...
					//GET LAST DOT
					if(sizeof($arrLastPaidDOT)>0){
						rsort($arrLastPaidDOT);
						$lastPaidDOT= date($phpDateFormat, $arrLastPaidDOT[0]);
					}
					...
					<tr>
						...
						<td class="text_10" style="background:#FFFFFF;">$strCPT_Desc</td>
...
DATA;
					...
					$data_output.='"'.$secGrpName.'"'.$pfx;
					$data_output.='"'.$arrDeptNames[$thirdGrpId].'"'.$pfx;
					$data_output.='"'.$date_of_service.'"'.$pfx;
					$data_output.='"'.$lastPaidDOT.'"'.$pfx;
					$data_output.='"'.$enc_id.'"'.$pfx;
					$data_output.='"'.$strCPT_CSV.'"'.$pfx;
					$data_output.='"'.$strCPT_Desc.'"'.$pfx;
					$data_output.='"'.$units.'"'.$pfx;
					$data_output.='"'.$strDxCodes.'"'.$pfx;
					$data_output.='"'.$primaryInsNameCSV.'"'.$pfx;
					$data_output.='"'.$secondaryInsNameCSV.'"'.$pfx;
					$data_output.='"'.$totalAmt.'"'.$pfx;
					$data_output.='"'.$patPaidAmt.'"'.$pfx;
					$data_output.='"'.$priPaidAmt.'"'.$pfx;
					$data_output.='"'.$secPaidAmt.'"'.$pfx;
					$data_output.='"'.$patientPaidAmt.'"'.$pfx;
					$data_output.='"'.$creditProcAmount.'"'.$pfx;
					$data_output.='"'.$write_off_amt.'"'.$pfx;
					$data_output.='"'.$adj_amt.'"'.$pfx;
					$data_output.='"'.$nsuranceDue.'"'.$pfx;
					$data_output.='"'.$patientDue.'"'.$pfx;
				...
				$sub_total_balance = $CLSReports->numberFormat($sub_total_balance,2,1);
				...
				<tr>
					<td style="text-align:right; background:#FFFFFF;" class="text_10b" colspan="9">Department Total : </td>
					<td style="text-align:right; background:#FFFFFF;" class="text_10b">'.$sub_total_units.'</td>
			...
			// SECOND GROUP TOTAL
			$sub_total_units = $arrSecGrpTotal['totalUnits'];
			...
			$arrFirstGrpTotal["totalUnits"]+= $sub_total_units;
			...
			<tr><td colspan="$colspan" class="total-row"></td></tr>
			<tr>
				<td style="text-align:right; background:#FFFFFF;" class="text_10b" colspan="9">$secGrpTitle Total : </td>
				<td style="text-align:right; background:#FFFFFF;" class="text_10b">$sub_total_units</td>
				<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
				<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
				<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
...
DATA;
		...
		// FIRST GROUP TOTAL
		$grandTotalAmtArr["totalUnits"]+= $arrFirstGrpTotal["totalUnits"];
		...
		<tr><td colspan="'.$colspan.'" class="total-row"></td></tr>
		<tr>
			<td style="text-align:right; background:#FFFFFF;" class="text_10b" colspan="9">'.$firstGrpTitle.' Total : </td>
			<td style="text-align:right; background:#FFFFFF;" class="text_10b">'.$arrFirstGrpTotal["totalUnits"].'</td>
			<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
			<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
			<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
			...
			<td style="text-align:right; background:#FFFFFF;" class="text_10b">'.$CLSReports->numberFormat($arrFirstGrpTotal["totalBalance"],2,1).'</td>
			'.$tdTotal.'
		</tr>	
		<tr><td colspan="'.$colspan.'" class="total-row"></td></tr>';
		...
		<tr>
			...
			<td width="70" style="text-align:right; background:#FFFFFF;" class="text_10b">'.$CLSReports->numberFormat($arrFirstGrpTotal["totalBalance"],2,1).'</td>
			'.$subTotBlankTD.'
		</tr>	
		<tr>
			<td colspan="15" class="total-row"></td>
	...
	foreach($mainResArr as $firstGrpId => $firstGrpData){	
		...
		foreach($firstGrpData as $secGrpId => $secGrpData){
			...
			foreach($secGrpData as $enc_id => $encDataArr){
				...
				$date_of_service = $encDataArr[0]['date_of_service'];
				$default_facility = $encDataArr[0]['default_facility'];
				//IF PATIENT HAS NO INSURANCE THEN SHOW "SELF PAY"
				if($encDataArr[0]['pri_ins_id']=='0'){
					$primaryInsName= $secondaryInsName="SELF PAY";
					$primaryInsNameCSV= $secondaryInsNameCSV="SELF PAY";
				}else{
					$primaryInsName  = $arrAllInsCompanies[$encDataArr[0]['pri_ins_id']];
					$primaryInsNameCSV  = $arrAllInsCompaniesCSV[$encDataArr[0]['pri_ins_id']];
					$secondaryInsName = $arrAllInsCompanies[$encDataArr[0]['sec_ins_id']];
				...
				for($d=0;$d<count($encDataArr);$d++){
					...
					$subTotalAmtArr["totalUnits"][] = $encDataArr[$d]['units'];
					if($encDataArr[$d]['totalAmt']>0){
						$totalAmtArr[] = $encDataArr[$d]['totalAmt'];
						$subTotalAmtArr["totalAmt"][] = $encDataArr[$d]['totalAmt'];
					}
					...
					if($viewBy=='insurance'){
						...
						if($pay_location=='1'  && ($DateRangeFor=='transaction_date' || $DateRangeFor=='date_of_payment')){
							...
							// TOTAL PAYMENT
							$patientPaidAmt+= $mainEncounterPayArr[$paylocid][$chgDetId] + ($patCrdDbt + $insCrdDbt);
					...
					if($viewBy!='insurance'){
						if($pay_location=='1' && ($DateRangeFor=='transaction_date' || $DateRangeFor=='date_of_payment')){
							// PATIENT PAID
							$patPaidAmt+=$patPayDetArr[$paylocid][$chgDetId]['patPaid'] + $patCrdDbt;
							// INSURANCE PAID
							$insPaidAmt+= $patPayDetArr[$paylocid][$chgDetId]['insPaid'] + $insCrdDbt;
							// PRI INSURANCE PAID
							if($patPayDetArr[$paylocid][$chgDetId]['priPaid']>0){
								$priPaidAmt+= $patPayDetArr[$paylocid][$chgDetId]['priPaid'] + $insCrdDbt;
								$insCrdDbt=0;
							}
							// SEC+TER INSURANCE PAID
							$secPaidAmt+= $patPayDetArr[$paylocid][$chgDetId]['secPaid'] + $patPayDetArr[$paylocid][$chgDetId]['terPaid'] + $insCrdDbt;
						}else{
							// PATIENT PAID
							$patPaidAmt+=$patPayDetArr[$chgDetId]['patPaid'] + $patCrdDbt;
							// INSURANCE PAID
					...
					//LAST PAID DOT
					if($pay_location=='1'  && ($DateRangeFor=='transaction_date' || $DateRangeFor=='date_of_payment')){
						if($patPayDetArr[$paylocid][$chgDetId]['lastDOT']!=null && $patPayDetArr[$paylocid][$chgDetId]['lastDOT']!='' && $patPayDetArr[$paylocid][$chgDetId]['lastDOT']!='00-00-0000'){
							$arrLastPaidDOT[]= strtotime($patPayDetArr[$paylocid][$chgDetId]['lastDOT']);
						}
					}else{
						if($patPayDetArr[$chgDetId]['lastDOT']!=null && $patPayDetArr[$chgDetId]['lastDOT']!='' && $patPayDetArr[$chgDetId]['lastDOT']!='00-00-0000'){
							$arrLastPaidDOT[]= strtotime($patPayDetArr[$chgDetId]['lastDOT']);
					...
					//GRAND TOTAL - TO AVOID DUPLICATE OF CHARGES, CREDIT AND BALANCE IN CASE OF GROUP BY INSURANCE CASE OR WHENEVER RECORD IS REPEATING
					if(!$tempIsChgDetId[$chgDetId]){
						$arrFirstGrpTotal["totalUnits"]+= $encDataArr[$d]['units'];
						$arrFirstGrpTotal["totalAmt"]+= $encDataArr[$d]['totalAmt'];					
						$arrFirstGrpTotal["creditProcAmount"]+= $encDataArr[$d]['creditProcAmount'];					
						$arrFirstGrpTotal["insuranceDue"]+= $encDataArr[$d]['pri_due'] + $encDataArr[$d]['sec_due'] + $encDataArr[$d]['tri_due'];			
						$arrFirstGrpTotal["patientDue"]+= $encDataArr[$d]['pat_due'];					
						$arrFirstGrpTotal["totalBalance"]+= $balAmt;					
						
						$grandTotalAmtArr["totalUnits"]+= $encDataArr[$d]['units'];
...
DATA;
				...
				$data_output.='"'.$patient_name.'"'.$pfx;
				$data_output.='"'.$patient_id.'"'.$pfx;
				$data_output.='"'.$tmpFacilityName.'"'.$pfx;
				$data_output.='"'.$secGrpName.'"'.$pfx;
				$data_output.='"'.$date_of_service.'"'.$pfx;
				$data_output.='"'.$lastPaidDOT.'"'.$pfx;
				$data_output.='"'.$enc_id.'"'.$pfx;
			...
			// SECOND GROUP TOTAL
			$sub_total_units = array_sum($subTotalAmtArr['totalUnits']);
			$sub_total_amt = array_sum($subTotalAmtArr['totalAmt']);
			$sub_pat_paid_amt = array_sum($subTotalAmtArr['pat_paid_amt']);
			$sub_ins_due = array_sum($subTotalAmtArr['insuranceDue']);
			$sub_patient_due = array_sum($subTotalAmtArr['patientDue']);
...
if($chargesForNotPosted>0){
	...
	$totalUnits= $grandTotalAmtArr["totalUnits"] + $unitsForNotPosted;
	...
	<tr><td colspan="'.$colspan.'" class="total-row"></td></tr>
  <tr>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b" colspan="9">Not Posted Amount : </td>
		<td style="text-align:right; background:#FFFFFF;" class="text_10b">'.$unitsForNotPosted.'</td>
		<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
		<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
		<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
	...
	//FOR CSV
//	$arrCSVNotPosted=array();
	
	$CSVBlanks=8;
	if($viewBy=='department')$CSVBlanks=$startColspan+1;
	
	for($i=0;$i<$CSVBlanks-1; $i++){
		$arrCSVNotPosted.="".$pfx;
	}
	$arrCSVNotPosted.="Not Posted Amount :".$pfx;
	$arrCSVNotPosted.='"'.$unitsForNotPosted.'"'.$pfx;
	$arrCSVNotPosted.="".$pfx;
	$arrCSVNotPosted.="".$pfx;
	$arrCSVNotPosted.="".$pfx;
	$arrCSVNotPosted.='"'.$CLSReports->numberFormat($chargesForNotPosted,2).'"'.$pfx;
	$arrCSVNotPosted.="".$pfx;
	$arrCSVNotPosted.="".$pfx;
	$arrCSVNotPosted.="".$pfx;
	$arrCSVNotPosted.="".$pfx;
...
if($viewBy=='insurance'){
	...
	<tr><td colspan="'.$colspan.'" class="total-row"></td></tr>
    <tr>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b" colspan="9">Other Totals : </td>
		<td class="text_10b" style="text-align:right; background:#FFFFFF;"></td>
		<td class="text_10b" style="text-align:right; background:#FFFFFF;"></td>
		<td class="text_10b" style="text-align:right; background:#FFFFFF;"></td>
		<td class="text_10b" style="text-align:right; background:#FFFFFF;"></td>
        ...
        <td style="text-align:right; background:#FFFFFF;" class="text_10b">'.$CLSReports->numberFormat($otherBalance,2,1).'</td>
        '.$tdTotal.'
    </tr>	
    <tr><td colspan="'.$colspan.'" class="total-row"></td></tr>';	
	...
	<tr>
		...
		<td width="70" style="text-align:right; background:#FFFFFF;" class="text_10b">'.$CLSReports->numberFormat($otherBalance,2,1).'</td>
		'.$subTotBlankTD.'
	</tr>	
	<tr><td colspan="'.$colspanPdf.'" class="total-row"></td></tr>';	

	//FOR CSV
	//$arrCSVOtherTotals=array();

	$CSVBlanks=8;
	if($viewBy=='department')$CSVBlanks=$startColspan+1;
	for($i=0;$i<$CSVBlanks-1; $i++){
		$arrCSVOtherTotals.="".$pfx;
	}
	$arrCSVOtherTotals.="Other Totals :".$pfx;
	$arrCSVOtherTotals.="".$pfx;
	$arrCSVOtherTotals.="".$pfx;
	$arrCSVOtherTotals.="".$pfx;
	$arrCSVOtherTotals.="".$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otherCharges,2).'"'.$pfx;
	$arrCSVOtherTotals.="".$pfx;
	$arrCSVOtherTotals.="".$pfx;
	$arrCSVOtherTotals.="".$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otherPayments,2).'"'.$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otherOverPayment,2).'"'.$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otherWriteOff,2).'"'.$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otherAdj,2).'"'.$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otheriInsDue,2).'"'.$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otherPatDue,2).'"'.$pfx;
	$arrCSVOtherTotals.='"'.$CLSReports->numberFormat($otherBalance,2).'"'.$pfx;
	$arrCSVOtherTotals.="\n";
...
if($showTotalAgain==1){
	...
	.$otherTotals
    .'<tr>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b" colspan="9">Total : </td>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b">'.$totalUnits.'</td>
		<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
		<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
		<td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
        ...
        <td style="text-align:right; background:#FFFFFF;" class="text_10b">'.$CLSReports->numberFormat($totalBalance+$otherBalance,2,1).'</td>
        '.$tdTotal.'
    </tr>	
    <tr><td colspan="'.$colspan.'" class="total-row"></td></tr>';	
	 ...
	 .'<tr>
		...
		<td width="70" style="text-align:right; background:#FFFFFF;" class="text_10b">'.$CLSReports->numberFormat($totalBalance+$otherBalance,2,1).'</td>
		'.$subTotBlankTD.'
	</tr>	
	<tr><td colspan="'.$colspanPdf.'" class="total-row"></td></tr>';

	
	//$arrCSVTotalRows=array();
	...
	$arrCSVTotalRows.="Total :".$pfx;
	$arrCSVTotalRows.='"'.$totalUnits.'"'.$pfx;
	$arrCSVTotalRows.="".$pfx;
	$arrCSVTotalRows.="".$pfx;
	$arrCSVTotalRows.="".$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($totalCharges+$otherCharges,2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["patPaidAmt"],2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["priPaidAmt"],2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["secPaidAmt"],2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["pat_paid_amt"]+$otherPayments,2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["creditProcAmount"]+$otherOverPayment,2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["write_off_amt"]+$otherWriteOff,2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["adj_amt"]+$otherAdj,2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["insuranceDue"]+$otherInsDue,2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($grandTotalAmtArr["patientDue"]+$otherPatDue,2).'"'.$pfx;
	$arrCSVTotalRows.='"'.$CLSReports->numberFormat($totalBalance+$otherBalance,2,1).'"'.$pfx;
	$arrCSVTotalRows.="\n";
}
...
<table class="rpt_table rpt rpt_table-bordered rpt_padding">
    ...
    <tr>
        ...
        <td class="rptbx3">Write-off Code : <?php echo $selWriteoff; ?>&nbsp;&nbsp;Reg Facility: <?php echo $dispRegFacility;?></td>
    </tr>
</table>
...
<table class="rpt_table rpt rpt_table-bordered">
    <tr>
        ...
        <td style="text-align:center; width:50px" class="text_b_w">CPT</td>
        <td style="text-align:center; width:50px" class="text_b_w">CPT Desc</td>
        <td style="text-align:center; width:60px" class="text_b_w">Units</td>
    ...
    <tr>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b" colspan="9"><?php echo $totalLabel;?> : </td>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b"><?php echo $grandTotalAmtArr['totalUnits'];?></td>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
        <td style="text-align:right; background:#FFFFFF;" class="text_10b"></td>
        ...
        <td style="text-align:right; background:#FFFFFF;" class="text_10b"><?php print $CLSReports->numberFormat($grandTotalAmtArr['totalBalance'],2,1); ?></td>
        <?php echo $tdTotal;?>
    </tr>	
    <tr><td colspan="<?php echo $colspan;?>" class="total-row"></td></tr>
...
//MAKING STARTING COLSPAN COLUMNS
for($i=0; $i<$CSVBlanks-1; $i++){
	$data_output1.="".$pfx;
}
$data_output1.='"'.$totalLabel.'"'.$pfx;
$data_output1.='"'.$grandTotalAmtArr['totalUnits'].'"'.$pfx;
$data_output1.="".$pfx;
$data_output1.="".$pfx;
$data_output1.="".$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['totalAmt'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['patPaidAmt'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['priPaidAmt'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['secPaidAmt'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['pat_paid_amt'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['creditProcAmount'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['write_off_amt'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['adj_amt'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['insuranceDue'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['patientDue'],2).'"'.$pfx;
$data_output1.='"'.$CLSReports->numberFormat($grandTotalAmtArr['totalBalance'],2).'"'.$pfx;
$data_output1.="\n";

@fwrite($fp,$data_output);
@fwrite($fp,$data_output1);
@fwrite($fp,$arrCSVNotPosted);
@fwrite($fp,$arrCSVOtherTotals);
@fwrite($fp,$arrCSVTotalRows);

/* fputcsv($fp,$arr, ",","\"");	
fputcsv($fp,$arrCSVNotPosted, ",","\""); //CSV NOT POSTED
fputcsv($fp,$arrCSVOtherTotals, ",","\""); //CSV OTHER TOTALS
fputcsv($fp,$arrCSVTotalRows, ",","\""); //TOTAL ROWS */
...
if(count($arrVoidPay)>0){
	//FOR CSV
	$void_data.="Voided Payments".$pfx;
	$void_data.="\n";
	$void_data.="Patient Name-ID".$pfx;
	$void_data.="Facility".$pfx;
	$void_data.="DOS".$pfx;
	$void_data.="E.ID".$pfx;
	$void_data.="CPT".$pfx;
	$void_data.="CPT Desc.".$pfx;
	$void_data.="Charges".$pfx;
	$void_data.="Cash".$pfx;
	$void_data.="Pt Check".$pfx;
	$void_data.="Ins Check".$pfx;
	$void_data.="CC".$pfx;
	$void_data.="EFT".$pfx;
	$void_data.="MO".$pfx;
	$void_data.="VEEP".$pfx;
	$void_data.="Write-off & Adj".$pfx;
	$void_data.="Del Opr".$pfx;
	$void_data.="\n";
	...
	foreach($fristGrpArr as $firstID){
		...
		foreach($secGrpArr as $secID){
			...
			foreach($encArr as $encID=>$arrData){
				...
				$strCPTDesc= implode(', ', $arrData['proc_desc']);
			...
			<tr>
				...
				<td class="text_12" bgcolor="#FFFFFF" align="left" style="width:'.$w_cols.'">'.$strCPTDesc.'</td>
			...
			//FOR CSV
			$void_data.='"'.$arrData['pt_name'].' - '.$arrData['pt_id'].'"'.$pfx;
			$void_data.='"'.$secGrpName.'"'.$pfx;
			$void_data.='"'.$arrData['dos'].'"'.$pfx;
			$void_data.='"'.$encID.'"'.$pfx;
			$void_data.='"'.$strCPT.'"'.$pfx;
			$void_data.='"'.$strCPTDesc.'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['charges'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['cash'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['pt_check'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['ins_check'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['CC'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['EFT'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['MO'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['VEEP'],2).'"'.$pfx;
			$void_data.='"'.$CLSReports->numberFormat($arrData['adjustments'],2).'"'.$pfx;
			$void_data.='"'.$delOprName.'"'.$pfx;
			$void_data.="\n";
		...
		$del_csv .= '
			...
			<tr class="text_12b">
				<td bgcolor="#FFFFFF"  style="text-align:right; width:'.($pat_col1+$fac_col1+$w_cols1+$w_cols1).'%;" colspan="6" >('.$firstGrpName.') '.$firstTitle.' Total: </td>
			...
			<tr><td colspan="'.$total_cols.'" class="total-row"></td></tr>
			<tr class="text_12b">
				<td bgcolor="#FFFFFF"  style="text-align:right" colspan="6">Total Voided Payment: </td>
				...
				<td bgcolor="#FFFFFF"  style="text-align:right" colspan="9"></td>
	...
	$del_csv .= '
			<tr><td colspan="'.$total_cols.'" class="total-row"></td></tr>
			<tr class="text_12b">
				<td bgcolor="#FFFFFF"  style="text-align:right" colspan="6">Grand Total: </td>
	...
	$secColLabel= ($viewBy=='physician')? 'Facility' : 'Physician';
	...
	<tr>
			...
			<td align="center" class="text_b_w" style="width:'.$fac_col.'">'.$secColLabel.'</td>
			...
			<td align="center" class="text_b_w" style="width:'.$w_cols.'">CPT Desc</td>
		...
		<tr>
			<td align="center" class="text_b_w" colspan="7" style="width:'.($pat_col1+$fac_col1+($w_cols1*4)).'%"></td>
	...
	$void_csv_data.="".$pfx;
	$void_csv_data.="".$pfx;
	$void_csv_data.="Deleted Total: ".$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['charges'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['cash'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['pt_check'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['ins_check'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['CC'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['EFT'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['MO'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['VEEP'],2).'"'.$pfx;
	$void_csv_data.='"'.$CLSReports->numberFormat($arrGrandTotal['adjustments'],2).'"'.$pfx;
	$void_csv_data.="".$pfx;
	$void_csv_data.="\n";
	echo $del_paid_csv;
}
@fwrite($fp,$void_data);
@fwrite($fp,$void_csv_data);
...
if(sizeof($mainResArr)>0 && empty($del_paid_csv) == false){
	...
	$grand_data.="\n";
	$grand_data.="".$pfx;
	$grand_data.="Charges".$pfx;
	$grand_data.="Payments".$pfx;
	$grand_data.="Write-off & Adj".$pfx;
	$grand_data.="".$pfx;
	$grand_data.="\n";	

	$grand_data.="Total : ".$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($totalChargesAmt,2).'"'.$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($totalPaymentAmt,2).'"'.$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($totalAdjustmentAmt,2).'"'.$pfx;
	$grand_data.="".$pfx;
	$grand_data.="\n";	
	
	$grand_data.="Total Deleted : ".$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($totalDeletedCharges,2).'"'.$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($totalDeletedPayments,2).'"'.$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($totalDeletedAdjustments,2).'"'.$pfx;
	$grand_data.="".$pfx;
	$grand_data.="\n";
	
	$grand_data.="Grand Total : ".$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($grandCharges,2,1).'"'.$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($grandPayments,2,1).'"'.$pfx;
	$grand_data.='"'.$CLSReports->numberFormat($grandAdjustments,2,1).'"'.$pfx;
	$grand_data.="".$pfx;
	$grand_data.="\n";	
	echo $grand_totals_csv;
}
@fwrite($fp,$grand_data);
...
<page_header>	
    <table width="100%" cellpadding="1" cellspacing="1" border="0" bgcolor="#FFF3E8" class="rpt_padding">
        ...
        <tr>
            ...
            <td class="rpt_headers rptbx3">Write-off Code : <?php echo $selWriteoff; ?>&nbsp;&nbsp;Reg Facility: <?php echo $dispRegFacility;?></td>
        </tr>
    </table>
</page_header>    
    <table width="100%" cellpadding="1" cellspacing="1" border="0" bgcolor="#FFF3E8">
        ...
        <tr>
            ...
            <td style="text-align:center;" width="65" class="text_b_w">CPT Desc.</td>
