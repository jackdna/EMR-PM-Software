...
$dbtemp_name = "Workable No Show Report";
                                ...
                                <div class="rptbox">
                                    <div id="html_data_div" class="row">
										...
										if($_POST['form_submitted']) {
											include('no_show_report_result.php');
										}else{
...
<script type="text/javascript">
	...
	var operator = '<?php echo $report_generator_name; ?>';
	...
	function save_cmnts(appt_id)
	{
		var saved_cmts= $("#saved_appt_comment_"+appt_id).val();
		var cmnts= $("#appt_comment_"+appt_id).val();
		if(saved_cmts==cmnts)return false;
		var u = top.JS_WEB_ROOT_PATH+'/interface/reports/no_show_report_ajax.php';
		var p = {cmnts:cmnts,action:'update',appt_id:appt_id};
		$.ajax({
				url:u,
				type:'post',
				data:p,
				success:function(r){
					$("#appt_comment_"+appt_id).val(r);
					$("#saved_appt_comment_"+appt_id).val(r);
				}
		});
	}
	
	
	function get_operator_name_date(field_id)
	{
		var t = current_date(top.jquery_date_format,2) + ' ' + operator + ': \n' + $("#"+field_id).val();
		$("#"+field_id).val(t);
		set_caret_position(field_id, 13);
	}

	function remove_operator_name_date(appt_id)
	{
		var cmnts =  $("#appt_comment_"+appt_id).val();
		var saved_cmts= $("#saved_appt_comment_"+appt_id).val();
		var match_string = current_date(top.jquery_date_format,2) + ' ' + operator + ':';
		if(cmnts == match_string )$("#appt_comment_"+appt_id).val('');
		
		//now test by removeing name date string
		var myString = cmnts.replace(match_string,'');
		myString = myString.replace("\n",'');
		if($.trim(myString) || saved_cmts)
		{
			if(!$.trim(myString)){$("#appt_comment_"+appt_id).val('');}
			save_cmnts(appt_id);
		}
		else
		{
			$("#appt_comment_"+appt_id).val('');	
		}
	}
