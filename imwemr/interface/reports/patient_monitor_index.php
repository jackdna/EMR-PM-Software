...
$arrUserIds=array();
$usersRs=imw_query("Select id, fname, mname, lname, user_type, delete_status FROM users WHERE user_type IN(1,3,6) ORDER BY delete_status, lname");
while($usersRes= imw_fetch_array($usersRs)){
	
	if(!empty($usersRes['fname']) || !empty($usersRes['mname']) || !empty($usersRes['lname'])){
		if(!empty($usersRes['lname']) || !empty($usersRes['mname'])){
			$Last_name = ', '.$usersRes['fname'];
		}
		else{
			$Last_name = $usersRes['fname'];
		}
		$firstName=ucfirst($usersRes['lname']).' '.$usersRes['mname'];
		$print_val = trim($firstName).ucfirst(trim($Last_name));
		$selected = (in_array($usersRes['id'], $_REQUEST['phyId'])) ? 'selected' : '';
		$colr = ($usersRes['delete_status'] == 1) ? 'red' : '';
		$allUsers .= "<option value='".$usersRes['id']."' ".$selected." style=\"color:$colr\">$print_val</option>";
		$arrUserIds[$usersRes['id']] = $usersRes['id'];
	}
}
                                    ...
                                    <div class="pd5" id="searchcriteria">
										...
										<div class="row">
											...
											<div class="col-sm-6">
												...
												<select name="user_type" id="user_type" class="selectpicker" data-width="100%" data-size="10" onchange="setUsers(this.id,this.value)">
													<option value="">All</option>
													<option value="3" <?php if ($_POST['user_type'] == '3') echo 'selected="selected"'; ?>>Technician</option>
													<option value="1" <?php if ($_POST['user_type'] == '1') echo 'selected="selected"'; ?>>Provider</option>
													<option value="6" <?php if ($_POST['user_type'] == '6') echo 'selected="selected"'; ?>>Sx Coordinator</option>
												</select>
											</div>
											<div class="col-sm-6">
												<label>Procedures</label>
                                                <select name="app_type[]" id="app_type" class="selectpicker" data-width="100%" data-size="10" multiple data-actions-box="true" data-title="Select All">
							...
							<div class="pd5 report-content">
                                <div class="rptbox">
                                    <div id="html_data_div" class="row">
                                        <?php
										if($_POST['form_submitted']) {
											include('patient_monitor_result.php');
										}else{
