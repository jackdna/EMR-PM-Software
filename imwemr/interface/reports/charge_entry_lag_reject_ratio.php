...
$allOprOptions = sizeof(explode('</option>', $operatorOption)) - 1;
...
$facNameArr=array();
$facNameArr[0] = 'No Facility';
$qry = "select pos_facilityies_tbl.facilityPracCode as name, pos_facilityies_tbl.pos_facility_id as id, pos_tbl.pos_prac_code
from pos_facilityies_tbl left join pos_tbl on pos_tbl.pos_id = pos_facilityies_tbl.pos_id order by pos_facilityies_tbl.headquarter desc, pos_facilityies_tbl.facilityPracCode";
$qryRs = imw_query($qry);
while($qryRes  =imw_fetch_array($qryRs)){
	$id = $qryRes['id'];
	$name = $qryRes['name'];
	$pos_prac_code = $qryRes['pos_prac_code'];
	$facNameArr[$id] = $name.' - '.$pos_prac_code;
}
//--- GET FACILITY NAME ----
$facility_id= array_combine($_REQUEST['facility'],$_REQUEST['facility']);
$facilityName = $CLSReports->getFacilityName($facility_id, '1');
$allFacOptions = sizeof(explode('</option>', $facilityName)) - 1;	
...
$dbtemp_name = "Charge Entry Lag - Reject Ratio";
                                    ...
                                    <div class="pd5" id="searchcriteria">
										...
										<div class="row">
                                            ...
                                            <div class="col-sm-12">
                                                ...
                                                <select name="facility[]" id="facility" class="selectpicker" data-width="100%" data-size="10" multiple data-actions-box="true" data-title="Select All" data-container="#common_drop">
                                ...
                                <div class="rptbox">
                                    <div id="html_data_div" class="row">
										...
										if($_POST['form_submitted']) {
											include('charge_entry_lag_reject_ratio_result.php');
										}else{
