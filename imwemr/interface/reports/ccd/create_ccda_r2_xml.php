...
﻿<?php 
/*
FILE : create_ccda_r2_xml.php
PURPOSE : Create XML file for CCD report
...
if(isset($_REQUEST['medications']) && $_REQUEST['medications'] != "")
$arrMedHxMedi = explode("~~",$_REQUEST['medications']);

if(isset($_REQUEST['allergies']) && $_REQUEST['allergies'] != "")
$arrMedHxAller = explode("~~",$_REQUEST['allergies']);

if(isset($_REQUEST['problem_list']) && $_REQUEST['problem_list'] != "")
$arrMedHxProbList = explode("~~",$_REQUEST['problem_list']);

if(isset($_REQUEST['create_type']) && ($_REQUEST['create_type'] == "attachment" || $_REQUEST['create_type'] == "printing")){
$arrData = json_decode($_REQUEST['arrData'],1);
	if($_REQUEST['app_services'] && isset($_REQUEST['phyId']) && $_REQUEST['phyId']!=""){
		$_SESSION['authId'] = $_REQUEST['phyId'];
	}
}


$arrAllFiles = array(); 
$arrOptionsExclude = explode(",",$_REQUEST['ccdDocumentOptions']);
...
if($_REQUEST['patientId']) {
	$andPdQry 			.=  " AND patient_data.pid IN(".$_REQUEST['patientId'].") ";
	$andPdHcObsQry		.=  " AND pt_id IN(".$_REQUEST['patientId'].") ";
	$andPdHcConcernQry	.=  " AND obs.pt_id IN(".$_REQUEST['patientId'].") ";
	$andPdHcRelObsQry	.=  " AND obs.pt_id IN(".$_REQUEST['patientId'].") ";
	$andCmtQry 			.=  " AND patient_id IN(".$_REQUEST['patientId'].") ";
	$andCmtJnQry 		.=  " AND cmt.patient_id IN(".$_REQUEST['patientId'].") ".$andDosJnQry;	
	$andSchQry 			.=  " AND sa.sa_patient_id IN(".$_REQUEST['patientId'].") ".$andDosJnQry;
	$andSmokeQry		.=  " AND patient_id IN(".$_REQUEST['patientId'].") ";
	$andVsmQry			.=  " AND vsm.patient_id IN(".$_REQUEST['patientId'].") ";
	$andRadQry			.=  " AND rad_patient_id IN(".$_REQUEST['patientId'].") ";
	$andLabQry			.=  " AND ltd.lab_patient_id IN(".$_REQUEST['patientId'].") ";
	$andDprQry			.=  " AND dpr.p_id IN(".$_REQUEST['patientId'].") ";
	$andOsacndQry		.=  " AND osacn.patient_id IN(".$_REQUEST['patientId'].") ";
	$andPhsQry 			.=  " AND patient_id IN(".$_REQUEST['patientId'].") ";
	$andListQry			.=  " AND pid IN(".$_REQUEST['patientId'].") ";
	$andClchQry			.=  " AND patient_id IN(".$_REQUEST['patientId'].") ";
	$andCapQry			.=  " AND patient_id IN(".$_REQUEST['patientId'].") ";
	$andimmunQry		.=  " AND patient_id IN(".$_REQUEST['patientId'].") ";
	$andCsteQry			.=  " AND patient_id IN(".$_REQUEST['patientId'].") ";
}
...
/*
$pdQry = "select patient_data.*,users.fname as ptProviderFName,users.mname as ptProviderMName,users.lname as ptProviderLName,users.user_npi as ptProviderNPI,
	refferphysician.Title as ptRefferPhyTitle,refferphysician.FirstName as ptRefferPhyFName,refferphysician.MiddleName as ptRefferPhyMName,
	refferphysician.LastName as ptRefferPhyLName,refferphysician.physician_phone as ptRefferPhyPhone
	from patient_data LEFT JOIN users on users.id = patient_data.providerID
	LEFT JOIN refferphysician ON refferphysician.physician_Reffer_id = patient_data.primary_care_id 
	where patient_data.pid != '0'";
$pdRes = imw_query($pdQry);
$pidRowArr = array();
if(imw_num_rows($pdRes)>0) {
	while($pdRow = imw_fetch_assoc($pdRes)) {
		$pdPId = $pdRow["pid"];
		$pidRowArr[$pdPId] = $pdRow;
	}
}
*/
...
/*
$immunRowArr = array();
$immunQry = "SELECT * FROM immunizations ORDER BY patient_id ";
$immunRes = imw_query($immunQry);
...
foreach($arrData as $key=>$arr){
	$pat_id = $arr['pat_id'];
	$form_id = $arr['form_id'];
	$ccd_type = (isset($arr['ccd_type']) && trim($arr['ccd_type'])!='') ? trim($arr['ccd_type']) : 'ccd';
	if($form_id == "all")
	$form_id = "";
	$dos = $arr['dos'];
	...
	//echo "<br>".$qry;	
	$rsPatient = imw_query($qry) or die(imw_error().$qry);
	$rowPatient = imw_fetch_assoc($rsPatient);
	*/
	
/* BEGIN PATIENT DATA */
//if(in_array('mu_data_set',$arrOptions)){		
...
if(in_array('mu_data_set_care_team_members',$arrOptions) || in_array('provider_info',$arrOptions)){
	...
	/*
	$sql = "SELECT `provIds` FROM `chart_master_table` WHERE `id`='".$form_id."' AND `patient_id`='".$pid."'";
	$resp = imw_query($sql);
	*/
	if(trim($providerIds))//if( $resp && imw_num_rows($resp) === 1 )
	{
		...
		/*
		$sql_patient = "SELECT * FROM patient_data WHERE id = '".$pid."' LIMIT 0,1";
		...
		*/
		...
		//$arr_careTeamIDs= array('users'=>$row_patient['providerID'],'refferphysician'=>$row_patient['primary_care_id']);
		//$data_careTeamIDs = getCareTeamDetails($arr_careTeamIDs);
		/*if(isset($form_id) && !empty($form_id)){
			$sql_cmt = "SELECT providerId FROM chart_master_table WHERE id = '".$form_id."'";
			$result_cmt = imw_query($sql_cmt);
			$row_cmt = imw_fetch_assoc($result_cmt);
			$providerID = $row_cmt['providerId'];
		}*/
		...
		if(in_array('mu_data_set_care_team_members',$arrOptions)){
			...
			//$qry_reff = "SELECT * FROM refferphysician WHERE physician_Reffer_id = '".$rowPatient['primary_care_id']."'";  			
			// REFERRING PHYSICIAN
			...
			//$qry_reff = "SELECT * FROM refferphysician WHERE physician_Reffer_id = '".$rowPatient['primary_care_phy_id']."'"; // PCP PHYSICIAN
			//$res_reff = imw_query($qry_reff);
			if(count($row_reff) > 0){
				//$row_reff = imw_fetch_assoc($res_reff);
		...
		if(in_array('mu_data_set_care_team_members',$arrOptions)){
			//$qry_provider = "SELECT * FROM users WHERE id = '".(($tempTechnicianID!==false)?$tempTechnicianID:$rowPatient['assigned_nurse'])."'";  // PRIMARY PHYSICIAN
...
//if(in_array('mu_data_set',$arrOptions)){
		...
		/*$XML_author_data .= '<addr>';
		$XML_author_data .= '<streetAddressLine nullFlavor="NI"/>';
		$XML_author_data .= '<city nullFlavor="NI"/>';
		$XML_author_data .= '<state nullFlavor="NI"/>';
		$XML_author_data .= '<postalCode nullFlavor="NI"/>';
		$XML_author_data .= '<country nullFlavor="NI"/>';
		$XML_author_data .= '</addr>';
		$XML_author_data .= '<telecom nullFlavor="NI"/>';*/
...
//if(in_array('mu_data_set',$arrOptions)){
		...
		/*$XML_data_enterer_data .= '<addr>';
		$XML_data_enterer_data .= '<streetAddressLine nullFlavor="NAV"/>';
		$XML_data_enterer_data .= '<city nullFlavor="NAV"/>';
		$XML_data_enterer_data .= '<state nullFlavor="NAV"/>';
		$XML_data_enterer_data .= '<postalCode nullFlavor="NAV"/>';
		$XML_data_enterer_data .= '<country nullFlavor="NAV"/>';
		$XML_data_enterer_data .= '</addr>';*/
...
if(in_array('mu_data_set_smoking',$arrOptions)){
		...
		$qry = "SELECT smoking_status,
				...
				DATE_FORMAT(smoke_start_date,'%Y%m%d') as smoking_start_dt,DATE_FORMAT(smoke_start_date,'%M %d, %Y') as smoking_start_dt_show, 
				DATE_FORMAT(smoke_end_date,'%Y%m%d') as smoking_end_dt,DATE_FORMAT(smoke_end_date,'%M %d, %Y') as smoking_end_dt_show  
				FROM social_history WHERE patient_id = '".$pid."'";		
...
if(in_array('mu_data_set_medications',$arrOptions)){
		...
		if(count($arrMedication)>0){
			...
			/*$XML_medication_section .= '<item><content ID="medication'.$medication['id'].'">'.$medication['title'].' - '.$medication['destination'].' (Begin '.$medication['begdate'].' End '.$medication['enddate'].') Site :'.getSite($medication['sites']).'</content></item>';*/
			}
			//$XML_medication_section .= '</list>';
			$XML_medication_section .= ' </tbody>';
		}
		if($flag == 0)
		{
		...
		$XML_medication_section .= '</text>';
				...
				/*  BEGIN MEDICATION ENTRY  */
					...
					/*$XML_medication_activity_entry .= '<originalText>
														<reference value = "#MEDROUTE'.$medication['id'].'"/>
													   </originalText>';	*/											
					...
					/*$XML_medication_activity_entry .= '<administrationUnitCode code="C42953"
														displayName="LIQUID"
...
if(in_array('visit_medication_immu',$arrOptions)){/*
				...
				if(imw_num_rows($res_list)>0){
					...
					while($medication = imw_fetch_assoc($res_list)){	
						...
						$XML_medication_admin_section .='</td>
														<td ID = "MEDROUTE'.$medication['id'].'">'.getSite($medication['sites']).'</td>
														<td ID = "MEDFORM'.$medication['id'].'">'.$medication['destination'].'</td>
				...
				$XML_medication_admin_section .= ' <tr><td colspan="7">No Medication information.</td></tr>';
				}
				$XML_medication_admin_section .= ' </tbody>';
...
*/}
/* END MEDICATIONS ADMINISTERED SECTION */
...
if(in_array('mu_data_set_allergies',$arrOptions)){
		...
		foreach($arrAllergies as $allergy){		
			...
			//$XML_allergies_section .= '<item><content ID="allergy'.$allergy['id'].'">'.$allergy['title'].$strAllerType .'</content></item>';
		}
		}
		if($flag == 0)
		{
		...
		*/
		...
		$XML_allergies_section .= '</text>';
				...
				foreach($arrAllergies as $allergy){
					...
					//lINE BELOW IS COMMENTED AFTER 2.1 IMPLEMENTATION
					...
					if($allergy['ag_occular_drug'] == "fdbATIngredient" || $allergy['ag_occular_drug'] == "fdbATAllergenGroup"){ // Food Allergy
						...
						if($allergy['ccda_code'] != "")	{
							...
							//$ccda_code_aller = "QE1QX6B99R";
							//$ccda_display_name_aller = "PEANUT";	
						}
						//if($ccda_code_aller != "" && $ccda_display_name_aller!=""){
						$XML_allergies_problem_act .= '<code code="'.$ccda_code_aller.'" displayName="'.$ccda_display_name_aller.'"
...
$qry_immu = '';
//$qry_immu = "SELECT * FROM immunizations immu WHERE patient_id = '".$pid."'";
...
if(count($res_immu)){
	...
	foreach($res_immu as $row_immu) {
		...
		/*if($row_immu['site']!=""){
			$arrCCDA = getApproachSiteCode($row_immu['site']);
			$ccda_code_route = $arrCCDA['ccda_code'];
			$ccda_display_name_route = $arrCCDA['ccda_display_name'];
		}
		if($ccda_code_route != "" && $ccda_display_name_route!=""){
			$XML_immunization_entry .= '<approachSiteCode code="'.$ccda_code_route.'" codeSystem="2.16.840.1.113883.6.96"
									codeSystemName="SNOMED CT"
									displayName="'.$ccda_display_name_route.'"/>';
		}else{
			$XML_immunization_entry .= '<approachSiteCode nullFlavor="NI"/>';
		}*/
		...
		$XML_immunization_entry .= '</consumable>';
			...
			//
			/*$XML_immunization_entry .= '<addr>';
			$XML_immunization_entry .= '<streetAddressLine nullFlavor="NI"/>';
			$XML_immunization_entry .= '<city nullFlavor="NI"/>';
			$XML_immunization_entry .= '<state nullFlavor="NI"/>';
			$XML_immunization_entry .= '<postalCode nullFlavor="NI"/>';
			$XML_immunization_entry .= '<country nullFlavor="NI"/>';
			$XML_immunization_entry .= '</addr>';
			$XML_immunization_entry .= '<telecom nullFlavor="NI"/>';	*/							
...
if(in_array('mu_data_set_vs',$arrOptions)){
		...
		$sql_vital = "SELECT vsp.*,vsl.vital_sign,vsm.date_vital FROM vital_sign_master vsm 
			...
			$sql_vital = "SELECT vsp.*,vsl.vital_sign,vsm.date_vital FROM vital_sign_master vsm 
					...
					WHERE vsm.patient_id = '".$pid."' AND  vsm.status = 0 
						AND vsm.date_vital = '".$cmtDOSArr[$form_id]."'
					ORDER BY vsp.id ASC";
		}
	...
	//		$XML_vital_section .= '<templateId root="2.16.840.1.113883.10.20.22.2.4"/>';
			...
			foreach($result_vital as $row_vital) {
				...
				/*$XML_vital_section .= '<item><content ID="vital'.$row_vital['id'].'">'.$row_vital['vital_sign'].' - '.$row_vital['range_vital'].' '.html_entity_decode($row_vital['unit']).'</content></item>';*/
				...
				foreach($lab_test_results as $lab_result_rs){
					...
					$XML_results_section .= '
											<referenceRange>
												<observationRange>
												';
					if($place_observation_range){
						$temp_result_range = explode('-',$lab_result_rs['result_range']);
						if(trim($lab_result_rs['uom'])!='') $unit_measure = ' unit="'.trim($lab_result_rs['uom']).'"';
						else $unit_measure = '';
						
						if(count($temp_result_range)==2)
							$XML_results_section .= '<value xsi:type="IVL_PQ">
														<low value="'.trim(htmlentities($temp_result_range[0])).'"'.$unit_measure.'/>
														<high value="'.trim(htmlentities($temp_result_range[1])).'"'.$unit_measure.'/>
													</value>';
					}else{
						if(strtolower(trim($lab_result_rs['result_range']))=='neg' || strtolower(trim($lab_result_rs['result_range']))=='negative'){
							$XML_results_section .= '
								<value xsi:type="CO" code="260385009" codeSystemName="SNOMED-CT" displayName="Negative" codeSystem="2.16.840.1.113883.6.96">'.trim($lab_result_rs['result']).'</value>
							';
						}else{
							$XML_results_section .= '
								<value xsi:type="ST">'.trim($lab_result_rs['result']).'</value>
							';
						}
					}
					$XML_results_section .= '
...
/*
$goal_qry = 'SELECT `id`, `patient_id`, `form_id`, `goal_set`, `loinc_code`, `goal_data`, `goal_data_type`, `gloal_data_type_unit`, `operator_id`,  DATE_FORMAT(`goal_date`, \'%b %d, %Y\') AS \'goal_date\',DATE_FORMAT(`goal_date`, \'%Y%m%d\') AS \'goal_date_show\' from patient_goals where  patient_id="'.$pid.'" AND delete_status = 0 '.$qry_form_id;
$row_arr = array();
$res = imw_query($goal_qry);
...
if(is_array($row_arr) && count($row_arr)>0){
	...
	foreach($row_arr as $res_arr_goal){
		...
		//$qry_user = imw_query("Select fname,lname,mname,pro_suffix from users where id='".$operator_id."'");
		//$fname = $lname = $mname = $pro_suffix = '';
		if(count($res_user) > 0){
			//$res_user = imw_fetch_assoc($qry_user);
...
/*
$qry_form_id='';if(!empty($form_id)){$qry_form_id=' AND form_id ="'.$form_id.'"';}
...
$res_sql = imw_query($observatin_qry);
$resp_observation = array();
if($res_sql && imw_num_rows($res_sql) > 0){
	while($row = imw_fetch_assoc($res_sql)){
...
/*
$qry_form_id='';if(!empty($form_id)){$qry_form_id=' AND `obs`.form_id ="'.$form_id.'"';}
$res_qry = "SELECT
				`c`.`id`,
				`c`.`concern`,
...
$res_sql = imw_query($rel_observaton_qry);
...
if(in_array('mu_data_set_ap',$arrOptions) || in_array('future_appointment',$arrOptions) || in_array('clinical_instruc',$arrOptions) || in_array('future_sch_test',$arrOptions) || in_array('recommended_patient_decision_aids',$arrOptions)){
		...
		if(in_array('future_appointment',$arrOptions) || in_array('future_sch_test',$arrOptions) || in_array('provider_referrals',$arrOptions)){
			...
			/*$qry= "select id,procedureid, (Select name from facility WHERE id=sa_facility_id) as sa_facility_id, TIME_FORMAT(sa_app_starttime, '%h:%i %p') as sa_app_starttime, sa_app_start_date as sa_app_start_date,(SELECT concat(lname,', ',fname,' ',mname )as user_name from users WHERE id=sa_doctor_id) as sa_doctor_id,(SELECT user_type_name as user_type from user_type join users on users.user_type = user_type.user_type_id  WHERE users.id=sa_doctor_id) as user_type, sa_patient_app_status_id,sa_comments from schedule_appointments where (sa_patient_id='".$pid."'  and sa_patient_app_status_id NOT IN('18', '201')) AND (sa_app_start_date >'".$current_date."' and sa_app_starttime >'".$current_time."' and sa_patient_id='".$pid."' and sa_patient_app_status_id NOT IN('18', '201','203')) order by sa_app_start_date";*/
			/*
			$sql = "SELECT date_of_service FROM chart_master_table WHERE id = '".$form_id."'";
			$res = imw_query($sql);
			$row = imw_fetch_assoc($res);
			...
			*/
			...
			if(count($csteRowPtArr[$pid])>0) {
				foreach($csteRowPtArr[$pid] as $csteRowPt) {
					if($form_id != "" && strtotime($csteRowPt["schedule_date"]) >= strtotime($dos)){
						$row = 	$csteRowPt;
					}else if(strtotime($csteRowPt["schedule_date"]) >= strtotime(date('Y-m-d'))) {
						$row = 	$csteRowPt;	
					}
				}
			}
			...
			*/
			...
			foreach($rowCste as $row) {
				/*if($row['user_type'] == "Test" && in_array('future_sch_test',$arrOptions)){
					$flag = 1;
					$XML_plan_of_care_section .= '<tr><td>Future Sch Test : '.$row['sa_comments'].'</td>
														<td>Future Sch Test</td>
														<td>'.$row['sa_app_start_date'].'</td>
														</tr>';
				}
				if($row['user_type'] != "Test" && in_array('future_appointment',$arrOptions)){
					$flag = 1;
					$XML_plan_of_care_section .= '<tr><td>Future Sch Appoint : '.$row['sa_comments'].'</td>
														<td>Future Scheduled Appointment</td>
														<td>'.$row['sa_app_start_date'].'</td></tr>';
				}*/
		...
		if(in_array('diagnostic_tests_pending',$arrOptions)){
			//$qry = "SELECT * FROM rad_test_data WHERE rad_patient_id = '".$pid."' AND rad_status != 3";									
			//$res = imw_query($qry);
			//while($row = imw_fetch_assoc($res))
			$res = $radRowArr[$pid];
			foreach($res as $row) {	
		...
		if(in_array('future_appointment',$arrOptions) || in_array('future_sch_test',$arrOptions)){
			$current_date = date("Y-m-d");
			$current_time = date("H:i:s");
			/*$qry= "select id,procedureid, (Select name from facility WHERE id=sa_facility_id) as sa_facility_id, TIME_FORMAT(sa_app_starttime, '%h:%i %p') as sa_app_starttime, sa_app_start_date as sa_app_start_date,(SELECT concat(lname,', ',fname,' ',mname )as user_name from users WHERE id=sa_doctor_id) as sa_doctor_id,(SELECT user_type_name as user_type from user_type join users on users.user_type = user_type.user_type_id  WHERE users.id=sa_doctor_id) as user_type, sa_patient_app_status_id from schedule_appointments where (sa_patient_id='".$pid."'  and sa_patient_app_status_id NOT IN('18', '201')) AND (sa_app_start_date >'".$current_date."' and sa_app_starttime >'".$current_time."' and sa_patient_id='".$pid."' and sa_patient_app_status_id NOT IN('18', '201','203')) order by sa_app_start_date";*/
			/*
			if(count($csteRowPtArr[$pid])>0) {
				foreach($csteRowPtArr[$pid] as $csteRowPt) {
					if($form_id != "" && strtotime($csteRowPt["schedule_date"]) >= strtotime($dos)){
			...
			while($row = imw_fetch_assoc($res)){
				...
				$XML_plan_of_care_entry ='<entry typeCode="DRIV">
												<act moodCode="RQO" classCode="ACT">
													...
													<statusCode code="new"/>
													<effectiveTime>
													<center value="'.str_replace("-","",$row['sa_app_start_date']).'"/>
													</effectiveTime>
												</act>
											</entry>';	
				}else if($row['user_type'] != "Test" && in_array('future_appointment',$arrOptions)){
					$XML_plan_of_care_entry ='<entry typeCode="DRIV">
												<act moodCode="RQO" classCode="ACT">
													<templateId root="2.16.840.1.113883.10.20.22.4.39"/>
													<!-- Plan of Care Activity Act -->
...
//if(in_array('mu_data_set',$arrOptions)){
		/*
		$sql = "SELECT * FROM patient_health_status WHERE LOWER(status_type) = 'functional' 
				AND del_status='0' AND patient_id = '".$pid."'";
		if($form_id != '') $sql .= " AND form_id = '".$form_id."'";
...
//if(in_array('mu_data_set',$arrOptions)){
		/*
		$sql = "SELECT * FROM patient_health_status WHERE LOWER(status_type) = 'cognitive' 
				AND del_status='0' AND patient_id = '".$pid."'";
		if($form_id != '') $sql .= " AND form_id = '".$form_id."'";
...
if(in_array('mu_data_set_superbill',$arrOptions)){	
	...
	/*		$XML_procedures_entry .= '				<addr use="WP">';
			$XML_procedures_entry .= '					<streetAddressLine>209 County Line Rd</streetAddressLine>';
			$XML_procedures_entry .= '					<city>Springfield</city>';
			$XML_procedures_entry .= '					<state>VA</state>';
			$XML_procedures_entry .= '					<postalCode>22150</postalCode>';
			$XML_procedures_entry .= '				</addr>';
			$XML_procedures_entry .= '				<telecom use="WP" value="tel:+1(571)555-0155"/>';
...
if(in_array('provider_referrals',$arrOptions) || $ccd_type=='rn'){
		/*
		$sql = "SELECT * FROM chart_assessment_plans WHERE patient_id = '".$pid."' Order By form_id DESC Limit 0, 1";
		$res = imw_query($sql);
		$row = imw_fetch_assoc($res);
		*/
		$row = $capRowArr[$pid];
...
/* END REASON FOR REFERRAL SECTION */	
		...
		//if(in_array('mu_data_set',$arrOptions) || in_array('provider_info',$arrOptions) || in_array('location_info',$arrOptions)){
		...
		//$XML_cda_body .= '<!-- IMMUNIZATIONS  ADMINISTERED DURING VISIT SECTION -->';  // INCLUDED STATIC
		//$XML_cda_body .= $XML_immunization_section;
		}
...
/*if(strlen($enc_key)<16){
	//$enc_key = str_pad($enc_key,16,"X",STR_PAD_LEFT);
}*/
if($enc_key==""){$enc_key="1234567890123456";}
$enc_key = trim($enc_key);
if(isset($_REQUEST['create_type']) && $_REQUEST['create_type'] == "attachment"){
	$plainFileName = "imedic-".$pid."-PLAIN-AMB-".strtoupper($ccd_type)."-R21-".time();
	$plain_file_name = $plainFileName.".xml";
	$save_directory = set_attach_dir_path();
	...
	$file_target = data_path()."xml/temp";
	$xmlDir = data_path()."xml";
	if(!is_dir($xmlDir)){
		mkdir( $xmlDir, 0755, true );
		chown( $xmlDir, 'apache' );
	}
	if(!is_dir($file_target)){
		mkdir( $file_target, 0755, true );
		chown( $file_target, 'apache' );
	}
	
	$fileToCreate = $file_target."/".$encFileName.".xml";
	
	if($zip_encrypt == '1') {//IF MULTIPLE XML EXIST THEN DO NOT ENCRYPT XML FILES BUT ENCRYPT A ZIP FILE
		file_put_contents($fileToCreate,file_get_contents($XML_file_name));					
	}else {//IF SINGLE XML EXISTS THEN ENCRYPT XML
		file_put_contents($fileToCreate,$encData);						
	}
	$encDataHashVal = hash('sha256',file_get_contents($fileToCreate));
	
	/*$hashFileName = "imedic-HASH-".$timeVal.".txt";
	$fileToCreateHASH = "temp/".$hashFileName;
	file_put_contents($fileToCreateHASH,$encDataHashVal);	*/				
	
	/*$plainFileName = "imedic-PLAIN-".$timeVal;
	$fileToCreatePlain = "temp/".$plainFileName.".xml";			
	file_put_contents($fileToCreatePlain,file_get_contents($XML_file_name));
	
	$encDataHashValPLAIN = hash('sha256',file_get_contents($fileToCreatePlain));*/
	$responseData = "";
	$responseData = $encDataHashVal."~~".$encFileName.".xml";		
	...
	imw_query("INSERT INTO pt_printed_records  SET patient_id='".$pid."', form_id = '".$form_id."',date_time = '".date('Y-m-d H:i:s')."',print_by = '".$_SESSION['authId']."',dos = '".$dos."',sending_application = 'iDoc',export_type = 'CCDA' ");
		...
		//$arrAllFiles[] = $fileToCreateHASH;
		//$arrAllFiles[] = $fileToCreatePlain;
			$count++;
	}
	if(count($arrData)<=0){
...
//Make zip --
//pre($arrAllFiles,1);
if($ccda_cal == "cron_tab") {
	$zip_dir = data_path()."xml/cron_zip";
	if(!is_dir($zip_dir)){
		mkdir( $zip_dir, 0755, true );
		chown( $zip_dir, 'apache' );
	}			
}
if(count($arrAllFiles) > 0){
	
	if(count($arrAllFiles) > 1 || $zip_encrypt == '1'){
		$sid = session_id();
		$files = $arrAllFiles;
		$zipPath = data_path().'UserId_'.$_SESSION['authId'].'/tmp/';
		if(!is_dir($zipPath)) mkdir($zipPath, 0777, true);
		$zipname = $zipPath.'fileCCD-'.time().'.zip';
		$encDataHashVal = hash('sha256',file_get_contents($zipname));
		$fileImpld = implode(" ",$files);
		//if($fileImpld) { $fileImpld .= ' CDA.xsl';}
		$fileImpld = trim($fileImpld);
		if($zip_encrypt == '1') {
			if(strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
				//$zip_command = '7zip/7za a -t7z '.$zipname.' '.$fileImpld.' -p'.$enc_key;
				$zip_command = '7zip\7za a '.$zipname.' '.$fileImpld.' -p'.$enc_key;
			}else {
				$zip_command = 'zip -j -P '.$enc_key.' '.$zipname.' '. $fileImpld;				  
			}
			//echo $zip_command;
			exec($zip_command);
		}else {
		...
		//
		if($ccda_cal == "cron_tab" && file_exists($zipname)) {
			$baseZipFileName  = basename($zipname);
			if(copy($zipname,$zip_dir."/".$baseZipFileName)) {
				$save_log_qry = "INSERT INTO ccda_download_log SET
									file_name 	= '".$baseZipFileName."',
									file_path 	= '/xml/cron_zip/".$baseZipFileName."',
									...
									execute_cron_status = '1',
									schedule_date_time = '".$schedule_date." ".$schedule_time."',
									ccda_export_schedule_id = '".$ccda_export_schedule_id."',
									ccda_export_schedule_operator_id = '".$ccda_export_schedule_operator_id."' 
								";			
				$save_log_res = imw_query($save_log_qry);	
			}
		}
		
		//Show Links --
		if(isset($_REQUEST['create_type']) && $_REQUEST['create_type'] == "printing"){
			//echo "<a href='../reports/ccd/download_zip.php?fileName=$zipname&zip=1' class='a_clr1'>Click to download CCDA zip</a>~~$encDataHashVal";
			echo "<a class='btn btn-primary' href='javascript:void[0]' onclick='window.location.href=\"../../reports/ccd/download_zip.php?fileName=$zipname&zip=1\"'>Click to download CCDA zip</a>~~$encDataHashVal";
		}else{
		echo "<tr>
			<td class='text_12' bgcolor='#FFFFFF' width='2%'>&nbsp;</td>
			<td class='text_12' bgcolor='#FFFFFF' width='5%'>1.</td>				
			<td class='text_12' bgcolor='#FFFFFF' width='45%'><a class='btn btn-primary' href='javascript:void[0]' onclick='top.fmain.ccda_export_report.location.href=\"download_zip.php?fileName=$zipname&zip=1\"' >Click to download CCDA zip</a></td>
			<td class='text_12' bgcolor='#FFFFFF' width='45%'>$encDataHashVal</td>
			</tr>";	
		}
		
		
	}elseif(count($arrAllFiles) == 1){
		if($ccda_cal == "cron_tab" && file_exists($fileToCreate)) {
			if(copy($fileToCreate,$zip_dir."/".$encFileName.".xml")) {
				$save_log_qry = "INSERT INTO ccda_download_log SET
									file_name 	= '".$encFileName.".xml',
									file_path 	= '/xml/cron_zip/".$encFileName.".xml',
									facility_id = '".$facility_id."',
									provider_id = '".$provider_id."',
									date_from 	= '".$date_from."',
									date_to 	= '".$date_to."',
		...
		if(isset($_REQUEST['create_type']) && $_REQUEST['create_type'] == "printing"){
			//echo "<a href='../reports/ccd/download_CCD.php?fileName=$encFileName.xml' class='a_clr1'>Click to download CCDA</a>~~$encDataHashVal";
		...
		echo "<tr>
			...
			<td class='text_12' bgcolor='#FFFFFF'><a href='javascript:void[0]' class='btn btn-primary' onclick='top.fmain.download_ccd_export(\"".base64_encode($fileToCreate)."\")'>Click to download CCDA</a></td>
...
//echo $responseData."~~".$encDataHashValPLAIN."~~".$ASEKEY."~~".$plainFileName.".xml"."~~".$patFileName."~~".$hashFileName."##"."1";	
//die;
//--------------------------


/*header('Content-disposition: attachment; filename="ccda_r2_xml.xml"');
header('Content-type: "text/xml"; charset="utf8"');
readfile('ccda_r2_xml.xml');*/
?>
