               ...
               <xsl:if test="not(n1:id/@nullFlavor)">
                  ...
                  <xsl:if test="n1:patient/n1:raceCode | (n1:patient/n1:ethnicGroupCode)">
                     ...
                     <tr>
                        ...
                        <td width="15%" bgcolor="#3399ff">
                           <span class="td_label">
                              <xsl:text>Pref Language Code</xsl:text>
                           </span>
                        </td>
                        <td>
                           <xsl:choose>
                              <xsl:when test="//n1:patient/n1:languageCommunication/n1:languageCode/@code">
                                 <span class="greentext"><xsl:value-of select="//n1:patient/n1:languageCommunication/n1:languageCode/@code"/></span>
                              </xsl:when>
                              <xsl:otherwise>
                                 <span class="redtext">
                                    <xsl:text>Language code missing</xsl:text></span>
                              </xsl:otherwise>
                           </xsl:choose>
                        </td>
                     </tr>
   ...
   <xsl:template name="show-gender">
      <xsl:choose>
         <xsl:when test="@code   = &apos;M&apos;">
            <xsl:text>Male; Birth Sex: Male</xsl:text>
         </xsl:when>
         <xsl:when test="@code  = &apos;F&apos;">
            <xsl:text>Female; Birth Sex: Female</xsl:text>
         </xsl:when>
         ...
         <xsl:when test="$asgnEntity/n1:assignedPerson/n1:name">
            ...
            <xsl:if test="$asgnEntity/n1:representedOrganization/n1:name">
               ...
               <xsl:value-of select="$asgnEntity/n1:representedOrganization/n1:name"/>
				...
				<xsl:text>, </xsl:text>
				<xsl:value-of select="$asgnEntity/n1:representedOrganization/n1:addr/n1:streetAddressLine"/>
				
				<xsl:text>, </xsl:text>
				<xsl:value-of select="$asgnEntity/n1:representedOrganization/n1:addr/n1:city"/>
				
				<xsl:text>, </xsl:text>
				<xsl:value-of select="$asgnEntity/n1:representedOrganization/n1:addr/n1:state"/>
				
				<xsl:text>, </xsl:text>
				<xsl:value-of select="$asgnEntity/n1:representedOrganization/n1:addr/n1:postalCode"/>
				
				<xsl:text>, </xsl:text>
				<xsl:value-of select="$asgnEntity/n1:representedOrganization/n1:addr/n1:country"/>
            </xsl:if>
         </xsl:when>
