...
while($res = imw_fetch_assoc($rs)){
	...
	switch($trans_type){
		...
		case 'debit':
			...
			}elseif($pay_location=='1'){
				if($trans_by=='insurance'){
					$pay_crd_deb_arr[$payloc][$chgDetId]['Insurance']+= $crddbtamt + $prevFetchedAmt;
					if($res['trans_del_operator_id']<=0)$patPayDetArr[$payloc][$chgDetId]['lastDOT'] = $res['trans_dot'];					
				}else{
...
if($no_del_amt=='1'){
	if($DateRangeFor=='transaction_date'){
		$qry.=" AND (main.del_status='0' OR (main.del_status='1' AND DATE_FORMAT(main.trans_del_date, '%Y-%m-%d')>'$End_date')) ".$qryBigPart;
	}
}
	...
	while($rs = imw_fetch_assoc($res)){
		...
		|| ($rs['del_status']=='1' && $first_posted_date!='0000-00-00' && $rs['trans_del_date'] >= $first_posted_date)){
			...
			if($viewBy=='facility'){
				...
				if($rs['pri_ins_id']>0){
					$firstGrpBy= ($arrInsMapInsGroups[$rs['pri_ins_id']]>0) ? $arrInsMapInsGroups[$rs['pri_ins_id']] : 'Group Not Mapped';
				}else{
					$firstGrpBy='SELF PAY';
				}
				$secGrpBy= $primaryProviderId;
			...
			if($processReport == "Summary"){
				...
				if($viewBy=='department'){		
					if($pay_location=='1' && sizeof($tempTransPayLoc[$charge_list_detail_id])>0){
						foreach($tempTransPayLoc[$charge_list_detail_id] as $payloc){
							$mainResArr[$firstGrpBy][$payloc][$deptId][$encounter_id][] = $rs;
						}
					}else{
					...
					if($viewBy=='insurance'){
						...
						if($pay_location=='1' && sizeof($tempTransPayLoc[$charge_list_detail_id])>0 && $facGrpNo>0){
							foreach($tempTransPayLoc[$charge_list_detail_id] as $payloc){
								if($facGrpNo=='1')$firstGrpBy=$payloc;else $secGrpBy=$payloc;
								$mainResArr[$firstGrpBy][$secGrpBy][$encounter_id][] = $rs;	
							}
						}else{
							$mainResArr[$firstGrpBy][$secGrpBy][$encounter_id][] = $rs;
...
if($viewBy!='insurance'){
	if($pay_location=='1'){
		foreach($tempArrInsPay as $loc_id => $payLocDet){
			foreach($payLocDet as $encounter_id => $encPayDet){
				foreach($encPayDet as $chgDetId => $insDet){
					foreach($insDet as $insCompId => $paidForProc){
						
						if($arrInsOfEnc[$encounter_id][$insCompId]['primary']){
							$patPayDetArr[$loc_id][$chgDetId]['priPaid']+= $paidForProc;
						}else if($arrInsOfEnc[$encounter_id][$insCompId]['secondary']){
							$patPayDetArr[$loc_id][$chgDetId]['secPaid']+= $paidForProc;
						}else if($arrInsOfEnc[$encounter_id][$insCompId]['tertiary']){
							$patPayDetArr[$loc_id][$chgDetId]['terPaid']+= $paidForProc;
						}
					}
				}
			}
