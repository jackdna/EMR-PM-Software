...
if(empty($form_submitted) == false){
	...
	if($date_range_for=='DOS'){
		...
		$chg_query = "select patient_charge_list.charge_list_id,patient_charge_list.totalAmt, patient_charge_list.date_of_service, 
					...
					where patient_charge_list.del_status='0' 
		...
		if(empty($credit_physician) === false){
			$chg_query.= " and patient_charge_list.secondaryProviderId IN ($credit_physician)";
		}
		if($chksamebillingcredittingproviders==1){
			$chg_query.= " and patient_charge_list.primary_provider_id_for_reports!=patient_charge_list.secondaryProviderId";							
		}		
		if(trim($sc_name) != ''){
		...
		while($chgQryRes = imw_fetch_array($chgQryRs)){
			...
			$doctor_id = $chgQryRes['primaryProviderId'];
			$fac_id = $chgQryRes['facility_id'];
			
			if(empty($Physician)==true && empty($credit_physician) === false){
				$doctor_id=$chgQryRes['secondaryProviderId'];
			}
					
			$pro_id_arr[$doctor_id] = $doctor_id;
			$fac_id_arr[$fac_id] = $fac_id;
			$firstGroupId = $doctor_id;
		...
		for($p=0;$p<count($providerIdArr);$p++){
			...
			for($y=0;$y<count($year_arr);$y++){
				...
				for($m=0;$m<count($month_arr);$m++){
					...
					//	GET CREDIT AMOUNT
					$credit_qry_rs = $CLSReports->__getCreditappliedRept($str_charge_list_id, $mode='CHARGELISTID');
					...
					//	GET DEBIT AMOUNT
					$debit_qry_rs = $CLSReports->__getDebitappliedData($str_charge_list_id, $mode='CHARGELISTID');
		...
		patchg.facility_id, patchg.gro_id, patChg.secondaryProviderId FROM patient_charge_list patchg
		JOIN patient_data ON patient_data.id=patchg.patient_id WHERE 1=1";
		...
		while($res=imw_fetch_array($rs)){
			...
			$tempEncInfo[$eid]['physician']=$doctor_id;
		...
		if(sizeof($allEncArrTemp)>0){
			...
			foreach($arrYearMonth as $date_range){
				...
				while($writeOffQryRes = imw_fetch_assoc($writeOffQryRs)){
					$eid = $writeOffQryRes['encounter_id'];
				...
				//APPOINTMENTS
				$appt_physician=$Physician;
				if(empty($Physician)==true && empty($credit_physician)==false){
					$appt_physician=$credit_physician;
				}
				...
				if(empty($appt_physician)==false){
					$qry.= " AND sa_doctor_id IN(".$appt_physician.")";
				}
	...
	if(count($provider_data_arr) > 0){
		...
		$proQryRes = array();
		$rs = imw_query($pro_query);
		while($res = imw_fetch_assoc($rs)){
			$proQryRes[] = $res;
		}
		
		$pro_name_arr = array();
		for($p=0;$p<count($proQryRes);$p++){
			$proNameArr = array();
			$proNameArr["LAST_NAME"] = $proQryRes[$p]['lname'];
			$proNameArr["FIRST_NAME"] = $proQryRes[$p]['fname'];
			$proNameArr["MIDDLE_NAME"] = $proQryRes[$p]['mname'];
			$id = $proQryRes[$p]['id'];
			$pro_name_arr[$id] = changeNameFormat($proNameArr);
			$providerIdArr[]=$id;
		}
				...
				for($y=0;$y<count($year_detail_arr);$y++){
					...
					if($dt_year==$lastDataYear){
						...
						for($i=$startDataYear; $i<$lastDataYear; $i++){
							...
							$curCSVPart.='<td class="text_10b text_purple link_cursor" bgcolor="#FFFFFF"  style="text-align:center" onclick="toggleTblNew(\''.$firstGroupId.'_'.$i.'\');">'.$i.'</td>';			
						}
					...
					<td width="100%">
						<table width="100%" cellpadding="1" cellspacing="1" bgcolor="#FFF3E8" >
						<tr>
					...
					$pdf_content .= '
					<tr>
						<td class="text_b_w" colspan="'.$firstPartCol.'" style="text-align:left; height:15px; width:'.$firstColW.'px">&nbsp;'.$firstGroupTitle.' : '.$firstGroup_name.'</td>
						<td class="text_b_w" colspan="'.$secPartCol.'">&nbsp;Year : '.$dt_year.'</td>
					</tr>
...
DATA;
		...
		<table class="rpt rpt_table rpt_table-bordered rpt_padding" width="1050px" >
            <tr class="rpt_headers">
                <td class="rptbx1" style="width:350px;">&nbsp;Yearly Report</td>	
				<td class="rptbx2" style="width:350px;">&nbsp;'.$date_range_for.' : '.$dispDateFrom.' To '.$dispDateEnd.'</td>			
		...
		$PAGE_DATA =
		$header_data.
		'<table class="rpt_table rpt rpt_table-bordered" width="100%" cellpadding="1" cellspacing="1" bgcolor="#FFF3E8" style="border:1px solid #999999;">'
			.$page_content.'
		</table>';
		...
		$GRAND_TOTAL.=
		'<table class="rpt_table rpt rpt_table-bordered" width="100%" cellpadding="1" cellspacing="1" bgcolor="#FFF3E8" style="border:1px solid #999999; margin-top:8px;margin-bottom:8px;">
			<tr><td  colspan="11" class="text_b_w">Practice Summary</td></tr>
			<tr >
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:110px;">Practice</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:40px;">Appt</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Charges</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Pat. Paid</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Ins. Paid</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Total Paid</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Credit</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Adjustment</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Pat. Due</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Ins. Due</td>
				<td bgcolor="#FFFFFF" class="text_10b" style="text-align:center;width:88px;">Balance</td>
			</tr>';
			$arrTot=array();
			...
			$GRAND_TOTAL.='
			<tr><td colspan="11" style="height: 2px; padding: 0px; background: #009933;"></td></tr>
			<tr>
		...
		<page backtop="5mm" backbottom="5mm">
			...
			<table class="rpt_table rpt rpt_table-bordered" width="100%" cellpadding="1" cellspacing="1" bgcolor="#FFF3E8">
				$pdf_content
			</table>
			$GRAND_TOTAL
		</page>
