...
if($rqBackDay > 0){
	...
	if(count($arrPatId) > 0){
		...
		if(imw_num_rows($patRs) > 0){
			...
			$content = '"mr_no"'.','.'"Patient Name"'.','.'"Clinic"'.','.'"Provider Name"'.','.'"provider_num"'.','.'"referring_md_name"'.','.'"referral_num"'.','.'"DOS"'.','.'"Time"'.','.'"Reason"'.','.'"Insurance"'.','.'"insurance_rco_code"'.','.'"Type"'.','.'"Address"'.','.'"City, State"'.','.'"Zip"'.','.'"Phone"'.','.'"DOB"'.','.'"Language"'.','.'"Race"'.','.'"Sex"'.','.'"Marital Status"'.','.'"Scheduled Date"'.','.'"dosz"'.','.'"timez"'.','.'"iMW_MRN"'.','.'"pcp_md_name"'.','.'"pcp_num"'.','.'"DOBz"';
			fwrite($fpH1, $content."\n");
			$arrTemp = array();
			while($patRes = imw_fetch_array($patRs)){
				...
				if($intCont == 1){
					...
					$content = $patMRN.','.$patName.','.$strFacCode.','.$patProName.','.$patProNo.','.$patRfPhyName.','.$patRfPhyNo.','.$patDOS.','.$patDOSTime.','.trim($strComment).','.$patInsurance.','.$patInsuranceRCO.','.$patType.','.$patAddress.','.$patCityState.','.$patZip.','.$patPhome.','.$patDOB.','.$patLanguage.','.$patRace.','.$patGender.','.$patMaritalStatus.','.$patAppDaySch.','.$patDOSZ.','.$patTimez.','.$patiMW_MRN.','.$patPCPName.','.$patPCPNo.','.$patDOB2;
					fwrite($fpH1, $content."\n");
