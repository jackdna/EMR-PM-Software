...
/* 	if($_REQUEST['start_date']!='' && $_REQUEST['end_date']!=''){
	...
	AND main.del_status='0' AND main.gro_id='4'";
...
//  AND main.first_posted_date='0000-00-00' 
	...
	$file_name="bb_imedicware_export_analytics_".$date.".csv";
	...
	if(sizeof($arrResultData)>0){
		...
		foreach($arryuni as $eid => $chrgObj){
			...
			foreach($chrgObj as $chrId => $detail_obj){
				...
				$strData.=$arrAllGroups[$detail_obj['gro_id']].$pfx;
				$strData.=$arrAllFacilities[$detail_obj['facility_id']].$pfx;
				$strData.=$fac_id_arr[$detail_obj['billing_facility_id']].$pfx;
				...
				$strData.=trim($detail_obj['suffix']).$pfx;
