...
$pt_coll_qry = "select 
		...
		patient_data.pat_account_status,report_enc_detail.encounter_id,
...
while ($pt_coll_row = imw_fetch_array($pt_coll_res)) {	
	...
	$mainPatTotAmtArr[$pt_coll_row['patient_id']][] = '';
...
if(count($mainEncResArr)>0){
	...
	// Get Collections ID and set to Patients
	if(sizeof($main_patient_id_arr)>0){
		// GET ID OF 1st Collection Letter and set it
		$rs=imw_query("Select id FROM patient_next_action WHERE LOWER(action_status)='1st collection letter' AND del_status='0'");
		$res=imw_fetch_array($rs);
		$firstCollLetterId =$res['id'];
		if($firstCollLetterId>0){
			$patRs=imw_query("Update patient_data SET next_action_status='".$firstCollLetterId."' WHERE next_action_status=0 AND id IN(".$pat_imp.")");
		}
		
		// UPDATE ALL ENCOUNTERS OF PATIENTS HAVING totalBalance > 0
		$rs=imw_query("Select encounter_id FROM patient_charge_list WHERE collection!='true' AND totalBalance>0 AND patient_id IN(".$pat_imp.")");
		...
		if(sizeof($arrTempEnc)>0){
			...
			$updateStr = "UPDATE patient_charge_list SET collection = 'true',collectionAmount=totalBalance,collectionDate = '".$currentDate."' WHERE encounter_id IN (".$strTempEnc.")";
			$updateRs = imw_query($updateStr);

			$updateStr = "UPDATE report_enc_detail SET collection = 'true',collectionAmount=proc_balance,collectionDate = '".$currentDate."' WHERE encounter_id IN (".$strTempEnc.")";
			$updateRs = imw_query($updateStr);	
		}
	}
	//--------------------------------------
	...
	foreach($main_patient_id_arr  as $pat_key => $pat_val){	
		...
		$collection_data_pdf = str_ireplace('{TOTAL OUTSTANDING CHARGES}',$showCurrencySymbol.number_format($mainPatTotBalArr[$patient_id],2),$collection_data_pdf);
	...
	if(strtolower($billing_global_server_name)!='liesc'){
		$data_pdf=utf8_decode($data_pdf);
	}
	$letter_file_location = write_html($data_pdf,'letter_pdf.html');
}
?>
<script>
