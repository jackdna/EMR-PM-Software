...
<?php
/*
FILE : payrollChargeList.php
PURPOSE :  REPORT FOR PAYROLL CHARGES
ACCESS TYPE : DIRECT
*/
$widthChg='150px';
$widthPay='150px';
...
if($rvu=='1'){
	...
	$widthRVU='110px';
	$colspan='7';
	$titleTDRVU='<td class="text_b_w" style="text-align:right; width:'.$widthRVU.'">Non Facility Pricing Amount</td>';
	$titleTDRVU_PDF='<td class="text_b_w" style="text-align:right; width:'.$widthRVU.'">Non Fac. Pricing Amt.</td>';
	$blankTDRVU='<td class="text_10" style="text-align:right; width:'.$widthRVU.'"></td>';
}

//---- GET DATA FROM MAIN CHARGE LIST TABLE REGARDING DATE OF SERVICE AND POSTED DATE ------
$qry = "select main.encounter_id, users.lname as physicanLname,
users.mname as physicanMname,users.fname as physicanFname,
main.primary_provider_id_for_reports as 'primaryProviderId',
main.charge_list_detail_id, main.units, (main.charges * main.units) as procCharges,	cpt_fee_tbl.cpt4_code,cpt_fee_tbl.cpt_desc 
...
WHERE users.user_type='1' AND main.del_status='0'";
...
$chargesQryRs = imw_query($qry);

$mainResArr = array();
$providerNameArr = array();
$cpt4_code_arr = array();
$main_encounter_arr = array();
$providerIdArr = array();
$ref_cpt_arr = array();
while($chargesQryRes=imw_fetch_array($chargesQryRs)){
	$primaryProviderId = $chargesQryRes['primaryProviderId'];
	$encounter_id = $chargesQryRes['encounter_id'];
	$charge_list_detail_id = $chargesQryRes['charge_list_detail_id'];
	$main_encounter_arr[$encounter_id] = $encounter_id;
	$physicanName = $chargesQryRes['physicanLname'].', ';
	$physicanName .= $chargesQryRes['physicanFname'].' ';
	$physicanName .= $chargesQryRes['physicanMname'];
	$physicanName = ucfirst(trim($physicanName));
	if($physicanName[0] == ','){
		$physicanName = substr($physicanName,1);
	}
	$providerNameArr[$primaryProviderId] = $physicanName;
	$cpt4_code = trim($chargesQryRes['cpt4_code']);
	$cpt_desc = trim($chargesQryRes['cpt_desc']);
	$cpt4_code_arr[$cpt4_code] = $cpt_desc;
	$cpt4_code_chg_arr[$charge_list_detail_id]=$cpt4_code;
	$mainResArr[$primaryProviderId][$cpt4_code][] = $chargesQryRes;
	$providerIdArr[$charge_list_detail_id] = $primaryProviderId;
	$providerIdEncArr[$encounter_id] = $primaryProviderId;
	$charge_list_detail_id_arr[$charge_list_detail_id] = $charge_list_detail_id;
	if($cpt4_code == 92015){
		$ref_cpt_arr[$charge_list_detail_id] = $charge_list_detail_id;
	}
}
imw_free_result($chargesQryRs);

$main_encounter_str = join(',',$main_encounter_arr);
//---- GET ALL PAYMENTS -----
$mainPaymentArr = array();
$proPaymentArr = array();
$patient_copay_arr = array();
$ref_payment_arr = array();
$writeOffArr = array();
$splitted_encounters = array_chunk($main_encounter_arr,5000);
...
foreach($splitted_encounters as $arr){
	...
	trans.trans_type, trans.trans_amount, trans.trans_code_id, trans.trans_dot, trans.trans_qry_type, trans.trans_method,
	DATE_FORMAT(trans.trans_dot,'".$dateFormat."') as trans_dot, trans.trans_del_operator_id, trans.parent_id   
	FROM report_enc_trans trans 
	WHERE trans.encounter_id IN(".$str_splitted_encs.") 
	AND LOWER(trans.trans_type)!='charges' ORDER BY trans.trans_dot, trans.trans_dot_time";
	...
	while($res = imw_fetch_assoc($rs)){
		...
		$tempTransInsCompArr[$chgDetId]=$insCompId;
...
unset($encounter_id);
unset($provider_id);
unset($charge_list_detail_id);
unset($paidForProc);
unset($payment_mode);
unset($insProviderId);
unset($tempTransInsCompArr);


//---- CREATE VARIABLE FOR PDF PRINTING ----
//---- START PHYSICIAN LOOP ------
	...
	for($p=0;$p<count($providerIdArr);$p++){
		...
		for($i=0;$i<count($procIdArr);$i++){
			...
			for($pr=0;$pr<count($proc_data_arr);$pr++){
				...
				if(count($mainPaymentArr[$charge_list_detail_id])>0 || count($pay_crd_deb_arr[$charge_list_detail_id])>0){
			...
			$rvuTd='';
			if($rvu=='1'){
				$work_rvu = $allRVUValues[$proc_code]['work_rvu'];
				$pe_rvu = $allRVUValues[$proc_code]['pe_rvu'];
				$mp_rvu = $allRVUValues[$proc_code]['mp_rvu'];

				$facPricingAmt = round((((($work_rvu * $bugdet_neu_adj_gpci) * $work_gpci) + ($pe_rvu * $practice_expense_gpci) + ($mp_rvu * $malpractice_gpci))  * $convFactor)  * $procedure_count, 2);
				
				if($facPricingAmt<=0){ $facPricingAmt='';}
				$rvuTd='<td class="text_10" style="text-align:right; width:'.$widthRVU.'">'.$facPricingAmt.'</td>';
			}
			
			$procedure_content .= <<<DATA
				<tr bgcolor="#FFFFFF">
					<td class="text_10" style="width:250px">$proc_code - $procedure_name</td>
					<td class="text_10" style="text-align:right; width:25px">$procedure_count</td>
					<td class="text_10" style="text-align:right; width:$widthChg">$procedure_charges</td>
					$rvuTd
					<td class="text_10" style="text-align:right; width:$widthPay">$procedure_paid_for</td>
					<td class="text_10" style="text-align:right; width:$widthWrite">$procedure_write_off</td>
					<td class="text_10" style="text-align:right; width:$widthRef">$procedure_refund_amount</td>
				</tr>
DATA;
		}
		...
		$physician_name = ucwords($providerNameArr[$physician_id]);
...
DATA;
		...
		$pdfData .= <<<DATA
			...
			<page_header>
				...
				<table  class="rpt_table rpt_table-bordered rpt_padding">
					<tr>
						<td class="text_b_w" style="text-align:left;" colspan="$colspan">Physician Name: $physician_name</td>
					</tr>					
					<tr >
						<td class="text_b_w" style="text-align:center; width:250px">Procedure</td>
						<td class="text_b_w"style="text-align:right; width:25px">Count</td>
						<td class="text_b_w" style="text-align:right; width:$widthChg">Charges</td>
						$titleTDRVU_PDF
						<td class="text_b_w" style="text-align:right; width:$widthPay">Payment</td>
						<td class="text_b_w" style="text-align:right; width:$widthWrite">Write-off</td>
						<td class="text_b_w" style="text-align:right; width:$widthRef">Refund</td>
					</tr>
				</table>
			</page_header>
			<table  class="rpt_table rpt_table-bordered rpt_padding">
				$procedure_content
				<tr>
					<td class="text_10" align="left" style="width:250px">COPAY</td>
					<td class="text_10" style="text-align:right; width:25px">$total_copay_count</td>
					<td class="text_10" style="text-align:right; width:$widthChg"></td>
					$blankTDRVU
					<td class="text_10" style="text-align:right; width:$widthPay">$total_copay</td>
					<td class="text_10" style="text-align:right; width:$widthWrite">$total_copay_write_off</td>
					<td class="text_10" style="text-align:right; width:$widthRef"></td>
				</tr>
				<tr><td colspan="'.$colspan.'" class="total-row"></td></tr>				
				<tr>
					<td class="text_10b" style="text-align:right;">$physician_name</td>
					<td class="text_10b" style="text-align:right;">$total_count</td>
					<td class="text_10b" style="text-align:right;">$total_procedure_charges</td>
					$blankTDRVU
					<td class="text_10b" style="text-align:right;">$total_paid_for</td>
					<td class="text_10b" style="text-align:right;">$total_write_off</td>
					<td class="text_10b" style="text-align:right;">$total_refund_amount</td>
				</tr>
				<tr><td colspan="'.$colspan.'" class="total-row"></td></tr>				
			</table>
			<table  class="rpt_table rpt_table-bordered rpt_padding">
				<tr>
					<td class="text_b_w" style="text-align:center;">Payment details</td>
				...
				<tr><td colspan="11" height="1px" bgcolor="#009933"></td></tr>
				<tr>
					<td class="text_10">Commercial Ins. Det</td>
					<td class="text_10" style="text-align:right;">$comercial_count</td>
				...
				<tr>
					...
					<td class="text_10" style="text-align:right;">$medicaid_count</td>
				...
				<tr>
					...
					<td class="text_10" style="text-align:right;">$medicare_count</td>
				...
				<tr>
					...
					<td class="text_10" style="text-align:right;">$patient_count</td>
				...
				<tr>
					...
					<td class="text_10" style="text-align:right;">$patient_copay_count</td>
				...
				<tr>
					...
					<td class="text_10" style="text-align:right;">$patient_ref_count</td>
				...
				<tr>
					<td  class="text_10b" style="text-align:right;" width="90">$physician_name</td>
					<td  class="text_10b" style="text-align:right;">$physicain_total_cnt</td>
...
//		FOR CSV DATA
		$csv_data .= <<<DATA
			...
			<table style="width:100%" class="rpt_table rpt_table-bordered">
				...
				<tr bgcolor="#FFFFFF"><td height="2px" colspan="$colspan"></td></tr>
				<tr>
					<td class="text_b_w" width="200" style="text-align:center;">Procedure</td>
					<td class="text_b_w" width="20" style="text-align:right;">Count</td>
					<td class="text_b_w" width="$widthChg" style="text-align:right;">Charges</td>
					$titleTDRVU
					<td class="text_b_w" width="$widthPay" style="text-align:right;">Payment</td>
					<td class="text_b_w" width="$widthWrite" style="text-align:right;">Write-off</td>
					<td class="text_b_w" width="$widthRef" style="text-align:right;">Refund</td>
				</tr>
				$procedure_content
				<tr bgcolor="#FFFFFF">
					<td class="text_10">COPAY</td>
					<td class="text_10" style="text-align:right;">$total_copay_count</td>
					<td class="text_10" style="text-align:right;"></td>
					$blankTDRVU
					<td class="text_10" style="text-align:right;">$total_copay</td>
					<td class="text_10" style="text-align:right;">$total_copay_write_off</td>
					<td class="text_10" style="text-align:right;"></td>
				</tr>
				<tr><td height="5px" bgcolor="#FFFFFF" colspan="$colspan"></td></tr>
				<tr><td height="1px" bgcolor="#009933" colspan="$colspan"></td></tr>
				<tr bgcolor="#FFFFFF">
					<td class="text_10b" style="text-align:right;">$physician_name</td>
					<td class="text_10b" style="text-align:right;">$total_count</td>
					<td class="text_10b" style="text-align:right;">$total_procedure_charges</td>
				...
				<tr><td height="1px" bgcolor="#009933" colspan="$colspan"></td></tr>
				<tr bgcolor="#FFFFFF"><td colspan="$colspan">&nbsp;</td></tr>
			</table>
			<table style="width:100%" class="rpt_table rpt_table-bordered">
				<tr>
					<td class="text_b_w" style="text-align:center;">Payment details</td>
				...
				<tr>
					<td class="text_10" width="150">Balance Write off</td>
					<td class="text_10" width="30" style="text-align:right;">$total_write_off_count</td>
					<td class="text_10" width="100" style="text-align:center;"></td>
					<td class="text_10" width="100" style="text-align:center;"></td>
					<td class="text_10" width="100" style="text-align:center;"></td>
					<td class="text_10" width="100" style="text-align:center;"></td>
					<td class="text_10" width="100" style="text-align:center;"></td>
				...
				<tr><td colspan="11" height="1px" bgcolor="#009933"></td></tr>
				<tr>
					<td class="text_10">Commercial Ins. Details</td>
					<td class="text_10" style="text-align:right;">$comercial_count</td>
					<td class="text_10" style="text-align:right;">$comercial_cc</td>
					<td class="text_10" style="text-align:right;">$comercial_cash</td>
					<td class="text_10" style="text-align:right;">$comercial_check</td>
				...
				<tr><td colspan="10" height="1px" bgcolor="#009933"></td></tr>
				<tr>
					<td class="text_10">Patient CoPay Details</td>
					<td class="text_10" style="text-align:right;">$patient_copay_count</td>
					<td class="text_10" style="text-align:right;">$patient_copay_cc</td>
					<td class="text_10" style="text-align:right;">$patient_copay_cash</td>
					<td class="text_10" style="text-align:right;">$patient_copay_check</td>
...
$csv_data .= <<<DATA
	<table style="width:100%" class="rpt_table rpt_table-bordered">
		...
		<tr>
			<td class="text_b_w" width="200" style="text-align:center;"></td>
			<td class="text_b_w" width="30" style="text-align:center;">Count</td>
			<td class="text_b_w" width="200" style="text-align:center;">Charges</td>
			<td class="text_b_w" width="200" style="text-align:center;">Payment</td>
			<td class="text_b_w" width="200" style="text-align:center;">Write-off</td>
			<td class="text_b_w" width="200" style="text-align:center;">Refund</td>
		</tr>
		
		<tr>
			<td  class="text_10b" style="text-align:left;">Physican Total</td>
			<td  width="30" class="text_10b" style="text-align:right;">$gross_total_count</td>
