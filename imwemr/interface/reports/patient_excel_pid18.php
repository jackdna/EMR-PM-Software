...
$rqBackDay = 0;
$rqBackDay = (int)$_REQUEST['futureday'];
if($rqBackDay > 0){
	...
	/*$strPatQryPrev = "SELECT sch_id, patient_id from previous_status where status = 0 AND old_status = 0 AND ".$strFacQryPre."
					YEAR(dateTime) = '".$dbYr."' 
					AND MONTH(dateTime) = '".$dbMon."' 
					AND DAYOFMONTH(dateTime) = '".$dbDay."' 
					GROUP BY  patient_id ORDER BY id";
	...
	if($checkDate){
		...
		$patQry="SELECT 
			...
			pd.email as email,
			fac.name as facName,
			sch.hl7_appt_external_encounter_id as pid18
			...
			WHERE sch.sa_app_start_date ='".$checkDate."'  
		...
		if(imw_num_rows($patRs) > 0){
			...
			//$content = '"mr_no"'.','.'"mr_no2"'.','.'"Patient Name"'.','.'"Clinic"'.','.'"Provider Name"'.','.'"provider_num"'.','.'"referring_md_name"'.','.'"referral_num"'.','.'"DOS"'.','.'"Time"'.','.'"Reason"'.','.'"Insurance"'.','.'"insurance_rco_code"'.','.'"Type"'.','.'"Address"'.','.'"City, State"'.','.'"Zip"'.','.'"Phone"'.','.'"Email"'.','.'"DOB"'.','.'"Language"'.','.'"Race"'.','.'"Sex"'.','.'"Marital Status"'.','.'"Scheduled Date"'.','.'"dosz"'.','.'"timez"'.','.'"iMW_MRN"'.','.'"pcp_md_name"'.','.'"pcp_num"'.','.'"Account number(PID18)"';   
			$content ='"Account number(PID18)"'.','.'"MRN"'.','.'"Patient Name"'.','.'"Clinic Code"'.','.'"Language"'.','.'"Email"'.','.'"Phone"'.','.'"Physician"'.','.'"Appointment Date"'.','.'"Appointment Time"'.','.'"Physician SMS ID"';
			fwrite($fpH1, $content."\n");
			$arrTemp = array();
			while($patRes = imw_fetch_array($patRs)){
				...
				//die;
				$patMRN2 = NULL;
				if(empty($patRes['mr_no2']) == false){
					$patMRN2 = (string)$patRes['mr_no2'];
					$patMRN2 = '"'.trim($patMRN2).'"';
				...
				$patEmail = NULL;
				if(empty($patRes['email']) == true){
					$patEmail = '"'." ".'"';
				}
				else{
					$patEmail = (string)$patRes['email'];
				...
				if(empty($patRes['language']) == true){
					...
					$patLanguage = '"'.trim(strtoupper($patLanguage)).'"';
				}
				...
				if(empty($patRes['pcp_num']) == true){
					...
					$patPCPNo = '"'.trim($patPCPNo).'"';
				}
				
				$pid18_val = NULL;
				if(empty($patRes['pid18']) == true){
					$pid18_val = '"'." ".'"';
				}
				else{
					$pid18_val = (string)$patRes['pid18'];
					$pid18_val = '"'.trim($pid18_val).'"';
				}
			...
			/*	$intCont = 0;
				...
				//if($intCont == 1){
					...
					//$content = $patMRN.','.$patMRN2.','.$patName.','.$strFacCode.','.$patProName.','.$patProNo.','.$patRfPhyName.','.$patRfPhyNo.','.$patDOS.','.$patDOSTime.','.trim($strComment).','.$patInsurance.','.$patInsuranceRCO.','.$patType.','.$patAddress.','.$patCityState.','.$patZip.','.$patPhome.','.$patEmail.','.$patDOB.','.$patLanguage.','.$patRace.','.$patGender.','.$patMaritalStatus.','.$patAppDaySch.','.$patDOSZ.','.$patTimez.','.$patiMW_MRN.','.$patPCPName.','.$patPCPNo.','.$pid18_val;
					$content = $pid18_val.','.$patMRN.','.$patName.','.$strFacCode.','.$patLanguage.','.$patEmail.','.$patPhome.','.$patProName.','.$patDOSZ.','.$patTimez.','.$patProNo;
					fwrite($fpH1, $content."\n");
				//}
			}
			fclose($fpH1);
