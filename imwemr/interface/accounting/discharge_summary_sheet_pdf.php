...
$updir=data_path()."PatientId_".$_SESSION['patient']."/discharge_summary";
//$srcDir=data_path(1)."PatientId_".$_SESSION['patient']."/discharge_summary";
$srcDir="../../data/".constant("PRACTICE_PATH")."/PatientId_".$_SESSION['patient']."/discharge_summary";

$oSaveFile = new SaveFile($patient_id);
$oSaveFile->ptDir("PatientId_".$_SESSION['patient']."/discharge_summary");
$ascId= $_REQUEST['ascId'];
$scemr_fac_id= $_REQUEST['sch_facility_id'];
$superBillId	=	$_REQUEST['superBillId'];
$superBillType	=	$_REQUEST['billType'];
$superBillProcArr	=	array();
if($superBillId) 
{
	$superBillProcQry	=	"Select * From procedureinfo Where idSuperBill = '".$superBillId."' ";
	$superBillProcSql		=	imw_query($superBillProcQry) or die('Error '.$superBillProcQry	.': '.imw_error());	
	$superBillProcCnt		=	imw_num_rows($superBillProcSql);
	if($superBillProcCnt > 0 )
	{
		while($superBillProcRow	=	imw_fetch_object($superBillProcSql))
		{
			$superBillProcArr[]		=	$superBillProcRow;	
		}
	}
}
include("connect_sc.php");

$Qry1="select patientId,patientConfirmationId from patientconfirmation where ascId= '$ascId'";
$recordQry1=imw_query($Qry1) or die(imw_error());
$DischargeResult1=imw_fetch_array($recordQry1);
$patient_id=$DischargeResult1['patientId'];
$pconfId=$DischargeResult1['patientConfirmationId'];
...
$tableDSummery.='
	<table style="width:740px;border:1px solid #C0C0C0;" cellpadding="0" cellspacing="0">
				...
				<table valign="top" style="width:370px; border-left:1px solid #C0C0C0;border-right:1px solid #C0C0C0;" cellpadding="0" cellspacing="0">
					...
					if($diagids_length>0 && $diag_ids[0]){
						for($c=0;$c<$diagids_length;$c++){
							...
							if($diagcodeslist[1] <> $diag_names[$diagcodeslist[0]])
							{
						...
						while($icd10Row = imw_fetch_assoc($icd10Res)) {
							...
							if($db_icd10_desc <> $icd10_name[$db_icd10_id])
							{
								...
								<td colspan="3" class="bdrbtm pl5 cbold" style="width:350px;">
									<table style="width:350px;" cellpadding="0" cellspacing="0">
										...
										if($dis_ScanUpload!='') {
											...
											$saveScanUploadPath = $updir."/disSummarySheet_".$_SESSION['patient'].".jpg";
											imagejpeg($bakImgResourceDischarge,$saveScanUploadPath);
											$tableDSummery.='<td style="width:175px; text-align:center;"><img src="'.$srcDir.'/disSummarySheet_'.$_SESSION['patient'].'.jpg" style="height:100px;width:100px; border:1px solid #C0C0C0;"></td>';
										}
										if($dis_ScanUpload2!='') {
											$bakImgResourceDischarge1 = imagecreatefromstring($dis_ScanUpload2);
											$saveScanUpload2Path = $updir."/disSummarySheet1_".$_SESSION['patient'].".jpg";
											imagejpeg($bakImgResourceDischarge1,$saveScanUpload2Path);
											$tableDSummery.='<td style="width:175px; text-align:center; border:1px solid #C0C0C0;"><img src="'.$srcDir.'/disSummarySheet1_'.$_SESSION['patient'].'.jpg" style="height:100px;width:100px; float:left;"></td>';
										}
								$tableDSummery.='			
										</tr>	
									</table>
						...
						if($iol_ScanUpload!='' || $iol_ScanUpload2!=''){
							$tableDSummery.='
								...
								if($iol_ScanUpload!=''){
									...
									$saveIolScanUploadPath = $updir."/oproom_".$_SESSION['patient'].".jpg";
									imagejpeg($bakImgResourceOproom,$saveIolScanUploadPath);
									if(file_exists($saveIolScanUploadPath)){//die($saveIolScanUploadPath.'<br>'.$srcDir.'/oproom'.$_SESSION['patient'].'.jpg');
										$tableDSummery.='
												<td style="width:175px;text-align:center; border:1px solid #C0C0C0;">
													<img src="'.$srcDir.'/oproom_'.$_SESSION['patient'].'.jpg" style="width:100px; height:100px;">
												</td>
											';
									}
								...
								if($iol_ScanUpload2!=''){
									...
									$saveIolScanUpload2Path = $updir."/oproom1_".$_SESSION['patient'].".jpg";
									imagejpeg($bakImgResourceOproom,$saveIolScanUpload2Path);
									if(file_exists($saveIolScanUpload2Path)){
										$tableDSummery.='
													<td style="width:175px;text-align:center; border:1px solid #C0C0C0;">
														<img src="'.$srcDir.'/oproom1_'.$_SESSION['patient'].'.jpg" style="width:100px; height:100px;">
													</td>';
									}
		...
		<tr>
			...
			if($DischargeResult['signSurgeon1Status']){	
				$tableDSummery.='
					<b>Surgeon:&nbsp;</b>'.$surgeon.'
					<br><b>Signature on File:&nbsp;</b>'.$DischargeResult['signSurgeon1Status'].'
					<br><b>Signature Date:&nbsp;</b>'.$DischargeResult['signSurgeon1DateTimeFormat'];
					
			}else{
				$tableDSummery.='
					<b>Surgeon:&nbsp;</b>______
					<br><b>Signature on File:&nbsp;</b>________
	 ...
	 <table style="width:740px; border:apx solid #C0C0C0;" cellpadding="0" cellspacing="0">';
			...
			if($dis_ScanUpload!=''){
				$bakImgResourceDischarge = imagecreatefromstring($dis_ScanUpload);
				$saveDisScanUploadPath = $updir."/disSummarySheetLarge_".$_SESSION['patient'].".jpg";
				imagejpeg($bakImgResourceDischarge,$saveDisScanUploadPath);
				
				$newSize=' width="150" height="100"';
				$priImageSize=array();
				if(file_exists($saveDisScanUploadPath)) {
					$priImageSize = getimagesize($saveDisScanUploadPath);
					if($priImageSize[0] > 395 && $priImageSize[1] < 840){
						$newSize = imageResize(680,400,400);						
						$priImageSize[0] = 500;
					}					
					elseif($priImageSize[1] > 840){
						$newSize = imageResize($priImageSize[0],$priImageSize[1],500);						
						$priImageSize[1] = 600;
					}
					else{					
				...
				$tableDSummery.='<tr><td style="width:700px;text-align:center;" class="bdrbtm"><img src="'.$srcDir.'/disSummarySheetLarge_'.$_SESSION['patient'].'.jpg" '.$newSize.'></td></tr>';
			}
		
			if($dis_ScanUpload2!=''){
				$bakImgResourceDischarge1 = imagecreatefromstring($dis_ScanUpload2);
				$saveDisScanUpload2Path = $updir."/disSummarySheetLarge1_".$_SESSION['patient'].".jpg";
				imagejpeg($bakImgResourceDischarge1,$saveDisScanUpload2Path);
				$priImageSize=array();
				if(file_exists($saveDisScanUpload2Path)) {
					$priImageSize = getimagesize($saveDisScanUpload2Path);
					$newSize = ' width="150" height="100"';
					if($priImageSize[0] > 395 && $priImageSize[1] < 840){
						$newSize = imageResize(680,400,400);						
						$priImageSize[0] = 500;
				...
				$tableDSummery.='<tr><td style="width:700px; text-align:center;" class="bdrbtm"><img src="'.$srcDir.'/disSummarySheetLarge1_'.$_SESSION['patient'].'.jpg" '.$newSize.'></td></tr>';
			}	
		$tableDSummery.='
			</table>
			 ';			
	...
	if($ViewOpRoomRecordNumRow>0 && ($iol_ScanUpload!='' || $iol_ScanUpload2!='')) {
	 ...
	 <table style="width:740px; border:apx solid #C0C0C0;" cellpadding="0" cellspacing="0">';
			...
			if($iol_ScanUpload!=''){
				$bakImgResourceOproom = imagecreatefromstring($iol_ScanUpload);
				$saveIolScanUploadPath = $updir."/oproomLarge_".$_SESSION['patient'].".jpg";
				imagejpeg($bakImgResourceOproom,$saveIolScanUploadPath);
				$newSize=' height="100"';
				$priImageSize=array();
				if(file_exists($saveIolScanUploadPath)) {
					$priImageSize = getimagesize($saveIolScanUploadPath);
					if($priImageSize[0] > 395 && $priImageSize[1] < 840){
						$newSize = imageResize(680,400,400);						
						$priImageSize[0] = 400;
					}					
				...
				$tableDSummery.='<tr><td style="width:700px; text-align:center;" class="bdrbtm"><img src="'.$srcDir.'/oproomLarge_'.$_SESSION['patient'].'.jpg" '.$newSize.'></td></tr>';
			}
		
			if($iol_ScanUpload2!=''){
				$bakImgResourceOproom1 = imagecreatefromstring($iol_ScanUpload2);
				$saveIolScanUpload2Path = $updir."/oproomLarge1_".$_SESSION['patient'].".jpg";
				imagejpeg($bakImgResourceOproom1,$saveIolScanUpload2Path);
				
				$priImageSize=array();
				if(file_exists($saveIolScanUpload2Path)) {
					$priImageSize = getimagesize($saveIolScanUpload2Path);
					$newSize = 'height="100"';
					if($priImageSize[0] > 395 && $priImageSize[1] < 840){
						$newSize = imageResize(680,400,400);						
						$priImageSize[0] = 400;
				...
				$tableDSummery.='<tr><td style="width:700px; text-align:center;" class="bdrbtm"><img src="'.$srcDir.'/oproomLarge1_'.$_SESSION['patient'].'.jpg" '.$newSize.'></td></tr>';
			}
	$tableDSummery.='</table>';		
	}
...
//$URL='http://'. $_SERVER['HTTP_HOST'].'/accounting/pdfFile.html';
if($disSummryFormStatus=='completed' || $disSummryFormStatus=='not completed') {
	$file_path = write_html($tableDSummery);
	//file_put_contents('../reports/new_html2pdf/discharge_report.html',$tableDSummery);
?>
...
<script type="text/javascript">
	...
	//window.open('../reports/new_html2pdf/createPdf.php?op=p&file_name=discharge_report','_parent','menubar=0,resizable=yes');
</script>
<?php
}else {
