...
if($callFromInterface != 'admin'){
	...
	$html_file_name = $test_name.'/'.$date_f.'/cellcount_test_print_'. $_SESSION['patient']."_".$_SESSION['authId']."_".$rand;
	...
	//Previous Test --
	$oChartTestPrev = new ChartTestPrev($patient_id,"CELL COUNT");
	...
	if(!empty($form_id)){
		...
		$q = "SELECT * FROM test_cellcnt WHERE patientId = '$patient_id' AND formId = '$form_id' AND purged='0' AND del_status='0'";
		$res = imw_query($q);
		$row = imw_fetch_assoc($res);
	}else if(isset($tId) && !empty($tId)){//Get record based on patient id and test id
			$q = "SELECT * FROM test_cellcnt WHERE patientId = '$patient_id' AND test_cellcnt_id = '".$tId."'";
			$res = imw_query($q);
			$row = imw_fetch_assoc($res);
	}else{
	...
	if($row != false){
		$cellcnt_pkid = $row["test_cellcnt_id"];
		$test_form_id = $row["formId"];
		$elem_examDate = ($cellcnt_mode != "new") ?  get_date_format($row["examDate"]) : $elem_examDate;
		$elem_examTime = ($cellcnt_mode != "new") ? $row["examTime"] : $elem_examTime ;
		$elem_cellcntEye = $row["test_cellcnt_eye"];
		$elem_performedBy = ($cellcnt_mode != "new") ? $row["performedBy"] : "";
		...
		$elem_physician = ( $cellcnt_mode == "update" ) ? $row["phyName"] : "" ;
	...
	//Super bill init() --
	$superLen = "1";
	$sb_testName = 'CellCount';
	
	//Cpt Code Desc
	$thisCptDescSym = "Cell Count";

	//Prev + Next Records --
	$tmp = getDateFormatDB($elem_examDate);//Exam Date
	
	$tstPrevId = $oChartTestPrev->getPrevId($tmp,$cellcnt_pkid);//getPervId
	$tstNxtId = $oChartTestPrev->getNxtId($tmp,$cellcnt_pkid);//getNextId
}
?>
<!DOCTYPE html>
<html lang="en">
...
<?php if($callFromInterface != 'admin'){ ?>
	function printTest(){
        ...
        //window.open('../../library/html_to_pdf/createPdf.php?onePage=false&op=l&file_location=<?php echo $final_html_file_name_path;?>&mergePDF=1&name=test_'+tId+'_pdf&testIds='+tId+'&saveOption=F','vf_Rpt','menubar=0,resizable=yes');	
	}

	// Prev Data
	var oPrvDt;
	function getPrevTestDt(ctid,cdt,ctm,dir){
		getPrevTestDtExe(ctid,cdt,ctm,dir,"CELL COUNT");
	}
<?php }?>

function selectOs(){
	var arrTestOd = new Array("elem_descOd","elem_numOd",
							"elem_cdOd","elem_avgOd","elem_sdOd","elem_cvOd","elem_mxOd",
							"elem_mnOd","elem_6aOd","elem_cctOd","elem_ppyOd","elem_ppnOd");

	var arrTestOs = new Array("elem_descOs","elem_numOs",
							"elem_cdOs","elem_avgOs","elem_sdOs","elem_cvOs","elem_mxOs",
							"elem_mnOs","elem_6aOs","elem_cctOs","elem_ppyOs","elem_ppnOs");
...
<?php if($callFromInterface != 'admin'){?>
    <input type="hidden" name="elem_saveForm" id="elem_saveForm" value="CellCount">
    ...
    <input type="hidden" id="elem_edMode" name="hd_cellcnt_mode" value="<?php echo $cellcnt_mode;?>">
    <input type="hidden" id="elem_testId" name="elem_cellCntId" value="<?php echo $test_edid;?>">
                            ...
                            <div class="tstopt">
                                <div class="row">
                                    ...
                                    <div class="col-sm-5 testopt">
                                        <ul>
                                            <li class="ouc"><label><input type="radio" name="elem_cellCntEye" value="OU" onClick="setReli_Test(this.value)" <?php echo (!$elem_cellcntEye || $elem_cellcntEye == "OU") ? "checked" : "" ;?>><span class="drak_purple_color label_txt">OU</span></label></li>
                                <li class="odc"><label><input type="radio" name="elem_cellCntEye" value="OD" onClick="setReli_Test(this.value)" <?php echo ($elem_cellcntEye == "OD") ? "checked" : "" ;?>><span class="blue_color label_txt">OD</span></label></li>
                                <li class="osc"><label><input type="radio" name="elem_cellCntEye" value="OS" onClick="setReli_Test(this.value)" <?php echo ($elem_cellcntEye == "OS") ? "checked" : "" ;?>><span class="green_color label_txt">OS</span></label></li>
                                        </ul>
                                    </div>
                                    <div class="col-sm-4">
                                        <?php
                                            ...
                                            if($callFromInterface != 'admin' && constant("ZEISS_FORUM") == "YES"){
                                                $procedure_opts = $objTests->zeissProcOpts(3);
                                            ?>
                    ...
                    <div class="row">
                        <div class="col-sm-7">
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        <label for="elem_diagnosis">Diagnosis </label>
                                        ...
                                        <?php
                                        $arrDigOpts = $objTests->getDiagOpts('1','cellcount',$test_master_id);
                        ...
                        <tr>
                            ...
                            <td class="pd5 tstbx tstrstopt">
                                <div class="row">
                                    ...
                                    <div class="col-sm-8"><input type="text" id="elem_numOd" name="elem_numOd" value="<?php echo $elem_numOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">CD</div>
                                    <div class="col-sm-8"><input type="text" id="elem_cdOd" name="elem_cdOd" value="<?php echo $elem_cdOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">AVG</div>
                                    <div class="col-sm-8"><input type="text" id="elem_avgOd" name="elem_avgOd" value="<?php echo $elem_avgOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">SD</div>
                                    <div class="col-sm-8"><input type="text" id="elem_sdOd" name="elem_sdOd" value="<?php echo $elem_sdOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">CV</div>
                                    <div class="col-sm-8"><input type="text" id="elem_cvOd" name="elem_cvOd" value="<?php echo $elem_cvOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">Max</div>
                                    <div class="col-sm-8"><input type="text" id="elem_mxOd" name="elem_mxOd" value="<?php echo $elem_mxOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">Min</div>
                                    <div class="col-sm-8"><input type="text" id="elem_mnOd" name="elem_mnOd" value="<?php echo $elem_mnOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">6A</div>
                                    <div class="col-sm-8"><input type="text" id="elem_6aOd" name="elem_6aOd" value="<?php echo $elem_6aOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">CCT</div>
                                    <div class="col-sm-8"><input type="text" id="elem_cctOd" name="elem_cctOd" value="<?php echo $elem_cctOd;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <?php if($elem_ppOd == "Y"){$elem_ppyOd="checked";} if($elem_ppOd == "N"){$elem_ppnOd="checked";}?>
                                    <div class="col-sm-5">Pleomorphism Present</div>
                                    <div class="col-sm-3"><label><input type="checkbox" id="elem_ppyOd" name="elem_ppyOd" value="Y" <?php echo $elem_ppyOd;?>
                    onclick="if(this.checked){document.getElementById('elem_ppnOd').checked=!this.checked;}"><span class="label_txt">Yes</span></label>
                                    </div>
                                    <div class="col-sm-4"><label><input type="checkbox" id="elem_ppnOd" name="elem_ppnOd" value="N" <?php echo $elem_ppnOd;?>
                    onclick="if(this.checked){document.getElementById('elem_ppyOd').checked=!this.checked;}"><span class="label_txt">No</span></label>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">&nbsp;Comments</div>
                                    <div class="col-sm-8"><textarea cols="30" rows="2" id="elem_descOd" name="elem_descOd" style="width:100%; height:50px;"><?php echo $elem_descOd;?></textarea></div>
                                </div>                        
                            </td>
                            <td class="pd5 tstbx tstrstopt">
                                <div class="row">
                                    <div class="col-sm-4">NUM</div>
                                    <div class="col-sm-8"><input type="text" id="elem_numOs" name="elem_numOs" value="<?php echo $elem_numOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">CD</div>
                                    <div class="col-sm-8"><input type="text" id="elem_cdOs" name="elem_cdOs" value="<?php echo $elem_cdOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">AVG</div>
                                    <div class="col-sm-8"><input type="text" id="elem_avgOs" name="elem_avgOs" value="<?php echo $elem_avgOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">SD</div>
                                    <div class="col-sm-8"><input type="text" id="elem_sdOs" name="elem_sdOs" value="<?php echo $elem_sdOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">CV</div>
                                    <div class="col-sm-8"><input type="text" id="elem_cvOs" name="elem_cvOs" value="<?php echo $elem_cvOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">Max</div>
                                    <div class="col-sm-8"><input type="text" id="elem_mxOs" name="elem_mxOs" value="<?php echo $elem_mxOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">Min</div>
                                    <div class="col-sm-8"><input type="text" id="elem_mnOs" name="elem_mnOs" value="<?php echo $elem_mnOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">6A</div>
                                    <div class="col-sm-8"><input type="text" id="elem_6aOs" name="elem_6aOs" value="<?php echo $elem_6aOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">CCT</div>
                                    <div class="col-sm-8"><input type="text" id="elem_cctOs" name="elem_cctOs" value="<?php echo $elem_cctOs;?>" class="form-control"></div>
                                </div>
                                <div class="row">
                                    <?php if($elem_ppOs == "Y"){$elem_ppyOs="checked";} if($elem_ppOs == "N"){$elem_ppnOs="checked";}?>
                                    <div class="col-sm-5">Pleomorphism Present</div>
                                    <div class="col-sm-3"><label><input type="checkbox" id="elem_ppyOs" name="elem_ppyOs" value="Y" <?php echo $elem_ppyOs;?>
                    onclick="if(this.checked){document.getElementById('elem_ppnOs').checked=!this.checked;}"><span class="label_txt">Yes</span></label>
                                    </div>
                                    <div class="col-sm-4"><label><input type="checkbox" id="elem_ppnOs" name="elem_ppnOs" value="N" <?php echo $elem_ppnOs;?>
                    onclick="if(this.checked){document.getElementById('elem_ppyOs').checked=!this.checked;}"><span class="label_txt">No</span></label>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4">&nbsp;Comments</div>
                                    <div class="col-sm-8"><textarea cols="30" rows="2" id="elem_descOs" name="elem_descOs" style="width:100%; height:50px;"><?php echo $elem_descOs;?></textarea></div>
                                </div>
                            </td>	
                        </tr>
                    </table>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            <div class="col-sm-12 supperbill">
                                ...
                                <!-- Superbill -->
                                <div class="clearfix"></div>
                				<?php echo $objTests->get_zeiss_forum_button('CELLCOUNT',$test_edid,$patient_id);?>
                            </div>
                            <?php }?>
                        </div>
                    </div>
...
<?php if($callFromInterface != 'admin'){?>
	...
	<?php if(($elem_cellcntEye == "OD") || ($elem_cellcntEye == "OS")){echo "setReli_Test(\"".$elem_cellcntEye."\");";}?>
...
if($callFromInterface != 'admin'){
    ...
    if($_GET["tId"]!='') {	// position of include file cannot go above than these lines
        include 'test_cell_count_print.php';
    }
}
?>
