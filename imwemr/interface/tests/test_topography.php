...
if($callFromInterface != 'admin'){
	...
	$html_file_name = $test_name.'/'.$date_f.'/topography_test_print_'. $_SESSION['patient']."_".$_SESSION['authId']."_".$rand;
	...
	$rand=rand(0,500);
	$test_name="chart_tests";
	$html_file_name = $test_name.'/'.$date_f.'/topography_test_print_'.$_SESSION['authId']."_".$rand;
	...
	//Previous Test --
	$oChartTestPrev = new ChartTestPrev($patient_id,"TOPOGRAPHY");
	...
	if($row != false){
		...
		$elem_topoMeter = $row["topoMeter"];
		$elem_topoMeterEye = $row["topoMeterEye"];
		...
		$elem_treatOther = $row["treat_other"];
		$purged=$row["purged"];
	...
	//Super bill init() --
	$superLen = "1";
	$sb_testName = 'Topography';
	
	//Cpt Code Desc
	$thisCptDescSym = "Topography";
...
<?php if($callFromInterface != 'admin'){?>
	...
	function getPrevTestDt(ctid,cdt,ctm,dir){
		getPrevTestDtExe(ctid,cdt,ctm,dir,"TOPOGRAPHY");
	}
...
//Select OS
function selectOs(){
	var arrTestOd = new Array("elem_descOd");
	var arrTestOs = new Array("elem_descOs");
...
<?php if($callFromInterface != 'admin'){?>
    <input type="hidden" name="elem_saveForm" id="elem_saveForm" value="Topography">
    ...
    <input type="hidden" id="elem_testId" name="elem_topoId" value="<?php echo $test_edid;?>">
                                    ...
                                    <div class="col-sm-4">
                                        <?php
                                            ...
                                            if($callFromInterface != 'admin' && constant("ZEISS_FORUM") == "YES"){
                                                $procedure_opts = $objTests->zeissProcOpts(11);
                                            ?>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <?php
                                        $arrDigOpts = $objTests->getDiagOpts(1,"topo",$test_master_id);
										foreach($arrDigOpts  as $key=>$val){
                                            $sel = (strtolower($elem_diagnosis) == strtolower($val)) ? "SELECTED" : "";
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="whitebox">
                                ...
                                <div class="tstrstopt">
                                	<div class="row tstrstopt">
                                    	<div class="col-sm-1">Treatment</div>
                                        <div class="col-sm-3">
                                            <select class="form-control minimal" id="elem_treat" name="elem_treat" onChange="checkTreatment(this.value)" style="display:<?php echo ($elem_treat == "Other") ? "none" : "block" ;?>;">
                                            <option>--Select--</option>
                                            <?php
                                            $arrDigOpts = $objTests->getDiagOpts(2,"topo_treat",$test_master_id);
                                            foreach($arrDigOpts  as $key=>$val){
                                                $sel = (strtolower($elem_treat) == strtolower($val)) ? "SELECTED" : "";
                                                echo "<option value=\"".$val."\" ".$sel.">".$val."</option>";
                                            }
            
                                            ?>
                                            </select>
                                            <div id="td_treatOther" style="display:<?php echo ($elem_treat == "Other") ? "inline-block" : "none" ;?>;">
                                                <div class="col-sm-10"><input type="text" name="elem_treatOther" value="<?php echo ($elem_treatOther);?>" class="form-control"></div>
                                                <div class="col-sm-2"><img src="<?php echo $library_path;?>/images/close14.png" title="Change" onClick="checkTreatment('');" style="cursor:hand; padding:0px;"></div>
                                            </div>
                                        </div>
                                    </div>
                                    <?php
										if(!empty($elem_prog)){
											$arrTmp = explode(",",$elem_prog);
									...
									?>
                                    <div class="row tstrstopt">
                                        <div class="col-sm-4"><label><input type="checkbox" name="elem_prog[]" value="Stable" <?php echo $opt1;?> ><span class="label_txt">Stable</span></label></div>
                                        <div class="col-sm-4"><label><input type="checkbox" name="elem_prog[]" value="Progressive" <?php echo $opt2;?> ><span class="label_txt">Progressive</span></label></div>
                                    ...
                                    <div class="row tstrstopt">
                                        ...
                                        <div class="col-sm-4"><label><input type="checkbox" name="elem_prog[]" value="Repeat Scan" <?php echo $opt3;?> ><span class="label_txt">Repeat Scan</span></label></div>
                                    </div>
                                </div>
                                <div class="clearfix"></div>
                                <div>
                            ...
                            <div class="col-sm-12 supperbill">
                                ...
                                <!-- Superbill -->
                                <div class="clearfix"></div>
                				<?php echo $objTests->get_zeiss_forum_button('TOPOGRAPHY',$test_edid,$patient_id);?>
                            </div>
                        	<?php }?>
                        </div>
                    </div>
...
if($callFromInterface != 'admin'){
	...
	{
		include 'test_topography_print.php';
	}
}
?>
</body>
