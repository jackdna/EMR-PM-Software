...
if($callFromInterface != 'admin'){
	...
	$html_file_name = $test_name.'/'.$date_f.'/test_template_print_'. $_SESSION['patient']."_".$_SESSION['authId']."_".$rand;
	...
	//Previous Test --
	$oChartTestPrev = new ChartTestPrev($patient_id,"TEMPLATETESTS",$test_template_id);
	...
	if(!empty($form_id)){
		...
		$q = "SELECT * FROM test_other WHERE patientId = '$patient_id' AND test_template_id > 0 AND formId = '$form_id' AND purged='0' AND del_status='0'";
		$res = imw_query($q);
		$row = imw_fetch_assoc($res);
	}else if(isset($tId) && !empty($tId)){//Get record based on patient id and test id
			$q = "SELECT * FROM test_other WHERE patientId = '$patient_id' AND test_other_id = '".$tId."' AND test_template_id > 0";
			$res = imw_query($q);
			$row = imw_fetch_assoc($res);
	}else{
	...
	//Super bill init() --
	$superLen = "1";
	$sb_testName = $this_test_properties['test_name'];
	
	//Cpt Code Desc
	$thisCptDescSym = "TemplateTests";
...
<?php if($callFromInterface != 'admin'){?>
	...
	function getPrevTestDt(ctid,cdt,ctm,dir){
		getPrevTestDtExe(ctid,cdt,ctm,dir,"TEMPLATETESTS",'<?php echo $test_template_id;?>');
	}
...
<?php if($callFromInterface != 'admin'){?>
    <input type="hidden" name="elem_saveForm" id="elem_saveForm" value="TemplateTests">
    ...
    <input type="hidden" name="elem_test_template_id" id="elem_test_template_id" value="<?php echo $this_test_properties['id'];?>">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4">
                                    <div class="form-group">
                                        ...
                                        <?php
                                        $arrDigOpts = $objTests->getDiagOpts('1','template',$test_master_id);
...
<?php if($callFromInterface != 'admin'){?>
	top.set_header_title('<?php echo $this_test_screen_name;?> Test');
