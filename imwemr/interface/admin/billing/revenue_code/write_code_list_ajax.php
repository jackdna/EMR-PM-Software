...
$so		= isset($_REQUEST['so']) ? trim($_REQUEST['so']) : 'w_code';
$soAD	= (strtoupper($_REQUEST['soAD'])=='DESC') ? 'DESC' : 'ASC';	/* Prevent arbitrary values - Security Fix  */
$table	= "write_off_code";
$pkId	= "w_id";
$chkFieldAlreadyExist = "w_code";
switch($task){
	case 'delete':
		$id = $_POST['pkId'];
	...
	case 'save_update':
		...
		unset($_POST['task']);
		$query_part = "";
		if(!$_POST['w_default']){$_POST['w_default']='no';}
		...
		break;
	case 'show_list':
		$q = "SELECT w_id,w_code,w_desc,w_default FROM ".$table." ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
