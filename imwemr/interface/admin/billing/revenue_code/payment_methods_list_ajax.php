...
$pkId	= "pm_id";
$chkFieldAlreadyExist = "pm_name";
$operator_id=$_SESSION['authId'];
$date_time = date('Y-m-d H:i:s');
switch($task){
	case 'delete':
		$id = $_POST['pkId'];
		$q 		= "update ".$table." set del_status='1',del_operator_id='$operator_id',del_date_time='$date_time' WHERE ".$pkId." IN (".$id.")";
		$res 	= imw_query($q);
		if($res){
			echo '1';
		}else{
	...
	case 'save_update':
		...
		foreach($_POST as $k=>$v){
			$query_part .= $k."='".str_replace('-',' ',addslashes($v))."', ";
		}
		...
		$q_c="SELECT ".$pkId." from ".$table." WHERE del_status ='0' AND ".$chkFieldAlreadyExist."='".$_POST[$chkFieldAlreadyExist]."'".$qry_con;
		$r_c=imw_query($q_c);
		if(imw_num_rows($r_c)==0){		
			...
			if($id==''){
				...
				$q = "UPDATE ".$table." SET ".$query_part.",modified_by='$operator_id',modified_date='$date_time' WHERE ".$pkId." = '".$id."'";
			}
			$res = imw_query($q);
			if($res){
		...
		break;
	case 'show_list':
		$q = "SELECT * FROM ".$table." where del_status='0' ORDER BY default_method desc, $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
