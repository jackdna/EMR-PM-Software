...
switch($task){
	...
	case 'save_update':
		...
		if($query_part) {
			$query_part .= " proc_code_operator_id = '".$_SESSION['authId']."', ";
		}
		...
		break;
	case 'show_list':
		$q = "SELECT proc_code_id,proc_code,proc_code_description,date_format(proc_code_enter_date,'".get_sql_date_format()."') as enter_date FROM ".$table." ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
