...
$_REQUEST['so'] = xss_rem($_REQUEST['so'], 2, 'sanitize');	/* Sanitization to prevent arbitrary values - Security Fix */

$task	= isset($_REQUEST['task']) ? trim($_REQUEST['task']) : '';
$so		= isset($_REQUEST['so']) ? trim($_REQUEST['so']) : 'DepartmentCode';
...
switch($task){
	...
	case 'save_update':
		...
		break;
	case 'show_list':
		$q = "SELECT DepartmentId,DepartmentCode,DepartmentDesc FROM ".$table." ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
