...
if($btn_sub || $btn_enter){
	...
	foreach($getResultArr as $getResult){
		...
		$address = trim($getResult['Address1'].' '.$getResult['Address2']);
		$city = !empty($getResult['City']) ? ', '.$getResult['City'] : '';
		$id = $getResult['id'];
		$physician_Reffer_id = $getResult['physician_Reffer_id'];
		$data .='			
		    <tr onClick="selpidtt(\''.core_refine_user_input($patientname).'\','.$physician_Reffer_id.')" style="cursor:pointer;">
			   <td class="text-left">'.$patientname.'</td>
			   <td class="text-left">'.$address.$city.' '.$getResult['ZipCode'].'</td>
			   <td class="text-left">'.$physician_Reffer_id.'</a></td>
		   </tr>
		';
	}
}
...
<script type="text/javascript">
	...
	function selpidtt(name,id){
		...
		opener.get_pcp_name_from_search(name,id)
		window.close();
	}

	$(document).ready(function(){
		$('tbody tr:nth-child(even)').addClass('bg3');
		$('#result1').height($(window).height()-100);
	});	
</script>
</head>
<body>
<div class="whtbox">
    <div class="row boxheadertop">
        <div class="col-sm-8 text-right">
        	...
        	<form name="frm_sel" action="searchPCP.php" method="post" onSubmit="return chk(this.form)">
            <div class="row">
    ...
    <div class="row">
        <div class="col-sm-12">
            <div id="result1" style="height:370px; overflow:auto">
           	<?php
				if($data != "")
				{
					?>
