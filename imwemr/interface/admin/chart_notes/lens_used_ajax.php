...
$table	= "admn_lens_used";
$pkId	= "id";
$chkFieldAlreadyExist = "name";
switch($task){
	case 'delete':
		$id = $_POST['pkId'];
		$q 		= " UPDATE ".$table." set del_by = '".$_SESSION["authId"]."', del_tm='".date("Y-m-d H:i:s")."' WHERE ".$pkId." IN (".$id.")";
		$res 	= imw_query($q);
		if($res){
			echo '1';
		}else{
	...
	case 'save_update':
		...
		break;
	case 'show_list':
		$q = "SELECT id,name FROM ".$table." WHERE del_by='0' ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
