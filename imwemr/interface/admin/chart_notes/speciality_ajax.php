...
$table	= "admn_speciality";
$pkId	= "id";
$chkFieldAlreadyExist = "name";
switch($task){
	case 'delete':
		$id = $_POST['pkId'];
		$q 		= " UPDATE ".$table." set status = '2' WHERE ".$pkId." IN (".$id.")";
		$res 	= imw_query($q);
		if($res){
			echo '1';
		}else{
	...
	case 'save_update':
		...
		$q_c="SELECT ".$pkId." from ".$table." WHERE ".$chkFieldAlreadyExist."='".$_POST[$chkFieldAlreadyExist]."' AND status='0' ".$qry_con;
		$r_c=imw_query($q_c);
		...
		break;
	case 'show_list':
		$q = "SELECT id,name FROM ".$table." WHERE status='0' ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
