...
$so		= isset($_REQUEST['so']) ? trim($_REQUEST['so']) : 'testing_nm';
$soAD	= (strtoupper($_REQUEST['soAD'])=='DESC') ? 'DESC' : 'ASC';	/* Prevent arbitrary values - Security Fix */

$table	= "testing";
$pkId	= "id";
$chkFieldAlreadyExist = "testing_nm";
switch($task){
	case 'delete':
		$id = $_POST['pkId'];
		$opid = $_SESSION["authId"];
		$dt = date("Y-m-d H:i:s");
		$q 		= "UPDATE ".$table." SET del_by='".$opid."', del_on='".$dt."' WHERE ".$pkId." IN (".$id.")";
		$res 	= imw_query($q);
		if($res){
			echo '1';
		}else{
	...
	case 'save_update':
		...
		break;
	case 'show_list':
		$q = "SELECT id,testing_nm FROM ".$table." where del_by='0' ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
