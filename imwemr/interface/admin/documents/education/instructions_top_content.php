	...
	<div class="row">
		<div class="col-sm-3">
			<div class="form-group">
				...
				<input class="form-control" value="<?php echo $name; ?>" data-preview-template="<?php echo $name; ?>" name="name" id="templateName" type="text">	
			</div>	
		</div>
		...
		<div class="col-sm-6">
			<div class="row">
				...
				<div class="col-sm-4">
					<div class="form-group">
						<label>Dx Code</label>
						<select name="dx[]" id="dx" class="selectpicker" multiple="multiple" data-width="100%" data-title="Select" data-live-search="true" data-actions-box="true">
						<?php 
							if(trim($dx)){
							...
							$dx_opt_str = '';
							foreach($icd_10_arr as $key => $val){
								$sel = '';
								if(in_array($key,$dx_code_str)){
									$sel = 'selected';
								}
								$dx_opt_str .= '<option value="'.$key.'" '.$sel.'>'.$key.' - '.$val.'</option>';
							}
							echo $dx_opt_str;
						?>	
						</select>
