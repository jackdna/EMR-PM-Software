...
//Data variables
$template_data = $doc_obj->get_template_data($doc_obj->cur_tab_table,'0','0','collection_name , id');
$variable_data = $doc_obj->get_variable_tags('collection');

//If Edit id exists
$template_preview = '';
if(isset($_REQUEST['edit_id']) && empty($_REQUEST['edit_id']) == false){
	$temp_edit_id = $_REQUEST['edit_id'];
	$edit_temp_data = $doc_obj->get_template_data($doc_obj->cur_tab_table,'id',$temp_edit_id,'collection_name , id');
									...
									<?php 
										...
										if(count($template_data) > 0){
											foreach($template_data as $key => $val){
												$temp_names .= '<li class="list-group-item pointer "><span onClick="selectTemplate('.$val['id'].',\''.addslashes($val['collection_name']).'\',\''.$doc_obj->current_tab.'\')">'.$val['collection_name'].'</span> <span class="delImg glyphicon glyphicon-remove pull-right" onclick="delTemplate('.$val['id'].')"></span></li>';
											}
										}else{
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group">
								...
								<input class="form-control" data-preview-template="<?php echo $edit_data['collection_name']; ?>" value="<?php echo $edit_data['collection_name']; ?>" name="templateName" id="templateName" type="text">	
							</div>	
						</div>
						<div class="col-sm-12 pt10">
							<textarea id="content" name="content" class="ckeditor_textarea"><?php echo stripslashes($edit_data['collection_data']); ?></textarea>	
						</div>	
					</div>
				</div>
			</div>
