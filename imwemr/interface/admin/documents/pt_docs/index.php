...
//Data variables
$pt_doc_data = $doc_obj->get_pt_docs_data();
$template_data = $pt_doc_data['data'];
$category_arr = $pt_doc_data['category'];
$variable_data = $doc_obj->get_variable_tags('pt_docs');
$temp_type_arr = array(""=>"Default","yes"=>"Facesheet","postcard"=>"Post Card","temp_key"=>"Patient Portal Key");

//Loading requested form data
if(isset($_REQUEST['edit_id']) && empty($_REQUEST['edit_id']) == false){
	$temp_edit_id = $_REQUEST['edit_id'];
	$edit_temp_data = $doc_obj->get_template_data($doc_obj->cur_tab_table,"pt_docs_template_status = '0' and pt_docs_template_id",$temp_edit_id);
...
<div class="tblBg">
	<div class="row pt10">
		<input type="hidden" name="pt_docs_template_id" id="pt_docs_template_id" value="<?php echo $edit_data['pt_docs_template_id']; ?>">
		<input type="hidden" name="pt_docs_cat_id" id="pt_docs_cat_id" value="<?php echo $edit_data['pt_docs_template_category_id']; ?>">
		<input type="hidden" name="edit_id" id="edit_id" value="<?php echo $edit_data['pt_docs_template_id']; ?>">
									...
									<div class="panel panel-default">
										<?php 
											...
											foreach($template_data as $key => $val){
												...
												if(count($val) > 0){
													foreach($val as $obj){	
														if($obj['cat_id'] == $edit_data['pt_docs_template_category_id']){
															$selected_tab = 'in';
														}
														$temp_names .= '<li class="list-group-item pointer ">
															<div class="row">
																<div class="col-sm-10">
																	<span onClick="selectTemplate('.$obj['doc_id'].',\''.addslashes($obj['doc_name']).'\',\''.$doc_obj->current_tab.'\')">'.$obj['doc_name'].'</span>
																</div>	
																<div class="col-sm-2">
																	<span class="delImg glyphicon glyphicon-remove pull-right" onclick="delTemplate('.$obj['doc_id'].')"></span>
																</div>	
															</div>
														</li>';
													}
					...
					<div class="row">
							...
							<div class="row">
								<div class="col-sm-4">
									<div class="form-group">
										<label>Name</label>
										<input name="templateName" data-preview-template="<?php echo $edit_data['pt_docs_template_name']; ?>" id="templateName" type="text" class="form-control" value="<?php print $edit_data['pt_docs_template_name']; ?>">	
									</div>
								</div>
								<div class="col-sm-4">
									<div class="form-group">
										<label>Category</label>
										<select class="selectpicker" data-width="100%" name="pt_docs_category" id="pt_docs_category" data-title="Select" data-size="6">
											<?php 
												$dp_str = '';
												$consent_cat_arr = array();
												foreach($category_arr as $key => $val){
													$sel ='';
													if($key == $edit_data['pt_docs_template_category_id']){
														$sel = 'selected';
													}
								...
								<div class="col-sm-4">
									<div class="form-group">
										...
										<select name="pt_docs_enable_facesheet" id="pt_docs_enable_facesheet" class="selectpicker" data-width="100%" data-title="Select">
											<?php 
												$type_opt = '';
												foreach($temp_type_arr as $key => $val){
													$sel = '';
													if($key == $edit_data['pt_docs_template_enable_facesheet']){
														$sel = 'selected';
													}
													$type_opt .= '<option value="'.$key.'" '.$sel.'>'.$val.'</option>';
												}
												echo $type_opt;
											?>	
										</select>	
						...
						<div class="col-sm-12 pt10">
							<textarea id="content" name="content" class="ckeditor_textarea">
								<?php echo stripslashes($edit_data['pt_docs_template_content']); ?>
							</textarea>
						</div>	
					</div>
				</div>
