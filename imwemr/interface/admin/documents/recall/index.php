...
//Data variables
$template_data = $doc_obj->get_template_data($doc_obj->cur_tab_table,'0','0','recallTemplateName , recallLeter_id','=','ASC');
$variable_data = $doc_obj->get_variable_tags('recall');
...
if(isset($_REQUEST['edit_id']) && empty($_REQUEST['edit_id']) == false){
	...
	$edit_temp_data = $doc_obj->get_template_data($doc_obj->cur_tab_table,'recallLeter_id',$temp_edit_id);
	$edit_data = $edit_temp_data[0];
	$template_preview = $doc_obj->get_template_preview($edit_data['recallLeter_id']);
}

?>
<div class="tblBg">
	<div class="row pt10">
		<input type="hidden" name="edit_id" id="edit_id" value="<?php echo $edit_data['recallLeter_id']; ?>">
		<input type="hidden" name="recallLeter_id" id="recallLeter_id" value="<?php echo $edit_data['recallLeter_id']; ?>">
										...
										if(count($template_data) > 0){
											foreach($template_data as $key => $val){
												$temp_names .= '<li class="list-group-item pointer ">
													<div class="row">
														<div class="col-sm-11">
															<span onClick="selectTemplate('.$val['recallLeter_id'].',\''.addslashes($val['recallTemplateName']).'\',\''.$doc_obj->current_tab.'\')">'.$val['recallTemplateName'].'</span>
														</div>
														<div class="col-sm-1 text-center">
															<span class="delImg glyphicon glyphicon-remove pull-right" onclick="delTemplate('.$val['recallLeter_id'].')"></span>
														</div>
													</div>
												</li>';
											}
					...
					<div class="row">
						<div class="col-sm-12">
							<div class="form-group">
								...
								<input class="form-control" data-preview-template="<?php echo $edit_data['recallTemplateName']; ?>" value="<?php echo $edit_data['recallTemplateName']; ?>" name="templateName" id="templateName" type="text">	
							</div>	
						</div>
						<div class="col-sm-12 pt10">
							<textarea id="content" name="content" class="ckeditor_textarea"><?php echo stripslashes($edit_data['recallTemplateData']); ?></textarea>	
						</div>	
					</div>
				</div>
			</div>
