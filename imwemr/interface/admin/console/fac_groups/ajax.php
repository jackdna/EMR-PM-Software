...
$table	= "fac_group_tbl";
$pkId	= "fac_group_id";
$chkFieldAlreadyExist = "fac_group_name";
...
switch($task){
	case 'delete':
		$id = $_POST['pkId'];
		$q 		= "UPDATE ".$table." set fac_group_status='1', modified_operator_id = '".$op_id."', modified_date = '".$curDate."' WHERE ".$pkId." IN (".$id.")";
		$res 	= imw_query($q);
		if($res){
			echo '1';
		}else{
	...
	case 'save_update':
		...
		break;
	case 'show_list':
		$q = "SELECT fac_group_id,fac_group_name,fac_id,if(fac_group_status='0','Active','In-Active') as status,fac_group_status FROM ".$table." ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
		$ref_code_arr=fac_code_arr();
		...
		echo json_encode(array('records'=>$rs_set,'fac_code_arr'=>$ref_code_arr));
		break;
	default: 
}
function fac_code_arr(){
//"select FirstName,MiddleName,LastName,physician_Reffer_id from refferphysician where delete_status = '0'";	
	
	$prac_code_qry = "select id,IF(name!='',CONCAT(name,'; '),'') as fac_name from facility where name!= '' ORDER BY name";
