...
$so		= isset($_REQUEST['so']) ? trim($_REQUEST['so']) : 'lense_code';
$soAD	= isset($_REQUEST['soAD']) ? trim($_REQUEST['soAD']) : 'ASC';
$table	= "contactlensecode";
$pkId	= "code_id";
$chkFieldAlreadyExist = "lense_code";
switch($task){
	case 'delete':
		$id = $_POST['pkId'];
	...
	case 'save_update':
		...
		break;
	case 'show_list':
		$q = "SELECT code_id,lense_code,lense_code_name FROM ".$table." ORDER BY $so $soAD";
		$r = imw_query($q);
		$rs_set = array();
