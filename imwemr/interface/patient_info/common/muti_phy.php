...
if(strtolower($_REQUEST["mode"]) == "get"){
		...
		$web_root = $web_root."/data/".strtoupper(PRACTICE_PATH);
		
		$body	=	'';
		$onCloseClick  = 'onClick="top.fmain.showMultiPhy(0,'.$phyType.');"';
		$headerTitle   = "";
        ...
        if($phyType == 3 || $phyType == 4) {
            $where= ' and pmrf.phy_type IN (3,4) ';
        } else {
            $where= ' and pmrf.phy_type = '.$phyType.' ';
        }
		...
		$qrySelpatPhy = "select TRIM(CONCAT(refPhy.LastName, ', ', refPhy.FirstName, ' ', refPhy.MiddleName,if(refPhy.MiddleName!='',' ',''),refPhy.Title)) as refName, 
				refPhy.Address1, refPhy.Address2, refPhy.ZipCode, refPhy.City, refPhy.State, refPhy.physician_phone, refPhy.physician_fax, refPhy.comments, 
				refPhy.physician_email,refPhy.delete_status, pmrf.id as pmrfDataId, pmrf.ref_phy_id as pmrfRefId 
				from patient_multi_ref_phy pmrf INNER JOIN refferphysician refPhy ON pmrf.ref_phy_id = refPhy.physician_Reffer_id 
				where pmrf.patient_id = '".$_SESSION['patient']."'  ".$where."  and pmrf.status = '0' 
				ORDER BY pmrf.id";
		...
		if(imw_num_rows($rsSelpatPhy) > 0)
		{
            ...
            if(in_array((int)$rowSelpatPhy["pmrfRefId"], $arrTemp) == false){
				...
				$strPhyVal = trim(stripslashes($OBJCommonFunction->getRefPhyName($rowSelpatPhy["pmrfRefId"])));
				$address = "";
				...
				{
					$body .= '
						<div id="divTR-'.$phyType.'-'.$intTemp.'" class="col-xs-12" style="margin-bottom:5px;">
							...
							<div class="col-xs-9" title="'.$address.'">
								...
								<input type="text" name="txt'.$fieldName.'Arr[]" id="txt'.$fieldName.'Arr-'.$intTemp.'" value="'.$strPhyVal.'" class="'.$phyClass.'" onkeyup="top.loadPhysicians(this,\'hid'.$fieldName.'Arr-'.$intTemp.'\', \''.$web_root.'/xml/refphy\',\'\',\''.$callFrom.'\')" onfocus="top.loadPhysicians(this,\'hid'.$fieldName.'Arr-'.$intTemp.'\', \''.$web_root.'/xml/refphy\',\'\',\''.$callFrom.'\')"/>
			...
			if($intTemp > 1)
			{
				$body .= '
					<div id="divTR-'.$phyType.'-'.$intTemp.'" class="col-xs-12" style="margin-bottom:5px;">
						<div class="col-xs-2 text-center" >'.$intTemp.'</div>
						<div class="col-xs-9" >
							<input type="text" name="txt'.$fieldName.'Arr[]" id="txt'.$fieldName.'Arr-'.$intTemp.'" value="" class="form-control" onkeyup="top.loadPhysicians(this,\'hid'.$fieldName.'Arr-'.$intTemp.'\', \''.$web_root.'/xml/refphy\',\'\',\''.$callFrom.'\')" onfocus="top.loadPhysicians(this,\'hid'.$fieldName.'Arr-'.$intTemp.'\', \''.$web_root.'/xml/refphy\',\'\',\''.$callFrom.'\')" />
	...
	<div class="modal-dialog modal-md">
		...
		<div class="modal-content">
			<div class="modal-header bg-primary">
				<button type="button" class="close" data-dismiss="modal" <?php echo $onCloseClick; ?> style="padding-top:5px;">&#10005;</button>
				<h4 class="modal-title" id="modal_title"><?php echo $headerTitle; ?></h4>
			</div>
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-xs-12">
						...
						<div class="col-xs-9"><label><?php echo $columnTitle; ?></label></div>
						<div class="col-xs-1">&nbsp;</div>
					</div>
					<div class="clearfix">&nbsp;</div>
				</div>
				
				<div class="row" style="height:265px; overflow:hidden; overflow-y:scroll;" id="divMultiPhyInner<?php echo $phyType; ?>">
					<input type="hidden" name="hidDelete<?php echo $phyType == 3 ? 'PCP' : $fieldName; ?>" id="hidDelete<?php echo $phyType == 3 ? 'PCP' : $fieldName; ?>" />
					<?php echo $body; ?>
				</div>	
			</div>
		  
			<div class="modal-footer">
				<button type="button" class="btn btn-danger" name="btSaveCloseMultiPhy" id="btSaveCloseMultiPhy" onClick="top.fmain.showMultiPhy(2,<?php echo $phyType; ?>,'<?php echo $WRP; ?>');">Save & Close</button>
			</div>
...
elseif(strtolower($_REQUEST["mode"]) == "save"){
	if($_REQUEST["phyType"] == 1){
					...
					if(empty($arrHidId[$intRefPhyArrKey]) == true){
						if(empty($strRefPhyArrVal) == false){
							...
							$qryInsertRefPhy = "insert into patient_multi_ref_phy 
														...
														('".$_SESSION['patient']."', '".$arrRefPhyID[$intRefPhyArrKey]."', '".addslashes($strRefPhyArrVal)."', '1','".$_SESSION['authId']."', '".date('Y-m-d H:i:s')."')";
							$rsInsertRefPhy = imw_query($qryInsertRefPhy);
						}
					}
					elseif($arrHidId[$intRefPhyArrKey]){
						$qryUpdateRefPhy = "update patient_multi_ref_phy set patient_id = '".$_SESSION['patient']."', ref_phy_id = '".$arrRefPhyID[$intRefPhyArrKey]."',
													ref_phy_name = '".addslashes($strRefPhyArrVal)."', modified_by = '".$_SESSION['authId']."', modified_by_date_time = '".date('Y-m-d H:i:s')."' 
													where id = '".$arrHidId[$intRefPhyArrKey]."'";
		...
		if(imw_num_rows($rsSelpatRefPhy) > 0){
			while($rowSelpatRefPhy = imw_fetch_array($rsSelpatRefPhy)){
				...
				$strRef4Txt .= trim(stripslashes($OBJCommonFunction->getRefPhyName($rowSelpatRefPhy["physician_Reffer_id"])))."; ";
				if($intRefId4Hid == 0){
		...
		echo "DONE"."-!-".$strRef4Txt."-!-".$intRefId4Hid;
	}
					...
					if(empty($arrHidId[$intCoPhyArrKey]) == true){
						if(empty($strCoPhyArrVal) == false){
							...
							$qryInsertCoPhy = "insert into patient_multi_ref_phy 
														(patient_id, ref_phy_id, ref_phy_name, phy_type, created_by, created_by_date_time) Values 
														('".$_SESSION['patient']."', '".$arrCoPhyID[$intCoPhyArrKey]."', '".addslashes($strCoPhyArrVal)."', '2','".$_SESSION['authId']."', '".date('Y-m-d H:i:s')."')";
	...
	elseif($_REQUEST["phyType"] == 3){
		$arrHidDelete = array();
		$arrHidDelete = explode('-', $_REQUEST['hidDeletePCPVal']);
		foreach($arrHidDelete as $intHidDeleteKey => $strHidDeleteVal){
			$arrHidDeleteVal = array();
			$arrHidDeleteVal = explode("~~", $strHidDeleteVal);
			$intHidDeleteVal = $intCounterHid = 0;
					...
					if(empty($arrHidId[$intPCPMedHxArrKey]) == true){
						if(empty($strPCPMedHxArrVal) == false){
							...
							$qryInsertPCPMedHx = "insert into patient_multi_ref_phy 
														(patient_id, ref_phy_id, ref_phy_name, phy_type, created_by, created_by_date_time) Values 
														('".$_SESSION['patient']."', '".$arrPCPMedHxID[$intPCPMedHxArrKey]."', '".addslashes($strPCPMedHxArrVal)."', '3','".$_SESSION['authId']."', '".date('Y-m-d H:i:s')."')";
					...
					if(empty($arrHidId[$intPCPDemoArrKey]) == true){
						if(empty($strPCPDemoArrVal) == false){
							...
							$qryInsertPCPDemo = "insert into patient_multi_ref_phy 
														(patient_id, ref_phy_id, ref_phy_name, phy_type, created_by, created_by_date_time) Values 
														('".$_SESSION['patient']."', '".$arrPCPDemoID[$intPCPDemoArrKey]."', '".addslashes($strPCPDemoArrVal)."', '4','".$_SESSION['authId']."', '".date('Y-m-d H:i:s')."')";
