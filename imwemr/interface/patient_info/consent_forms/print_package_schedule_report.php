...
*/
//---- get Patient details ---------
$qry = "select *,date_format(DOB,'%m-%d-%Y') as pat_dob,date_format(date,'%m-%d-%Y') as reg_date
...
$qry = "select *,date_format(dob,'%m-%d-%Y') as res_dob 
...
if($packageCategoryId) {
	...
	$barCodeImgPath = $GLOBALS['fileroot'].'/data/'.PRACTICE_PATH.'/PatientId_'.$patient_id.'/consent_forms/bar_code_images/'.$img_name.'.png';
...
if(count($consentDetail) == 0){
	...
	for($i_csn=0;$i_csn<count($consentDetail);$i_csn++){
		...
		if($htmlV2Class==true){
			$pageTag='<page backtop="10mm" backbottom="5mm">
			<page_header>
			<table style="width:700px;" cellpadding="0" cellspacing="0"><tr><td style="text-align:right;width:700px;">'.$barCodeContent.'</td></tr><tr><td style="text-align:right;width:700px;">'.$img_name.'</td></tr></table>
			</page_header>
			</page>
			';
		}else {
...
for($i=0;$i<count($consentDetail);$i++){	
	...
	$consent_form_content = str_ireplace('{DATE}',date('m-d-Y'),$consent_form_content);
