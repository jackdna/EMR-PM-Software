...
$fileData .= 'BHT*0019*00*'.$Interchange_controls.'*'.date('Ymd').'*'.date('hm').'*CH~';
...
while($rs = imw_fetch_assoc($chrQryRes)){
	$main_charge_list_id_arr[] = $rs['charge_list_id'];
}
...
//				$selfCode1 = '';
			...
			$fileData .= '*'.substr(trim($subscriberDetails->subscriber_mname),0,1).'***MI';
			...
			$fileData .= $state_name.'*'.trim($otherInsDetail->Zip).'~';			
		...
		//	$fileData .= "N4*$pos_facility_city*$pos_facility_state*$pos_facility_zip~";
			...
			if(is_object($subscriberDetails2) === true){
				...
				if($primaryFile == false){
					...
					//$fileData .= 'AMT*EAF*'.$totalAmt.'~';//COMMENTED IN 5010 ON 6th Feb 2013
				}
			...
			//	$fileData .= 'DMG*D8*'.$DOB2.'*'.substr($subscriber_sex2,0,1).'~';
				...
				$fileData .= '*'.trim($patientMname2).'**'.$subcriber_suffix.'*MI';
	...
	if($files == true){
		...
		$sel = imw_query("select Batch_file_submitte_id from batch_file_submitte where Batch_file_submitte_id = '$fileNameID' and delete_status = '0'");
		$sel_res = imw_fetch_assoc($sel);
		if(count($sel_res)>0){ 
			$created_file_id2 = $sel_res[0]["Batch_file_submitte_id"];
			$qry = "update batch_file_submitte set file_data = '".strtoupper($fileData)."',
				...
				file_error_msg = '".join(":", $error)."',
				file_error = '".(($error_reported2 == true) ? 1 : 0)."',
				file_parent_id = '".$fileDetail->Batch_file_submitte_id."',
				file_invalid_chargelist_id = '".join(":", $inValidChargeListId)."', 
				segment_end = '$segmentCountEnds',ins_company_id = '$insCompanyIds' where Batch_file_submitte_id = '".$created_file_id2."'";
			$updateQry = imw_query($qry);
			if($created_file_id2){batch_file_log($created_file_id2,"regenrate");}
		}else{			
			...
			$record_data_arr['file_error'] = (($error_reported2 == true) ? 1 : 0);
			$record_data_arr['file_error_msg'] = join(":", $error);
			$record_data_arr['file_parent_id'] = $fileDetail->Batch_file_submitte_id;
			$record_data_arr['file_invalid_chargelist_id'] = join(":", $inValidChargeListId);
			$created_file_id2 = AddRecords($record_data_arr,'batch_file_submitte');
			if($created_file_id2){batch_file_log($created_file_id2,"created");}
			//---- INSERT FILE NAME COUNT -----------
			//$record = array("submit_date"=>date("Y-m-d"),"file_status"=>"1");
			//$insertData = $objManageData->AddRecords($record,'batch_file_detail');
		}
		...
		if(empty($err) === true && $error_reported2 == false){
