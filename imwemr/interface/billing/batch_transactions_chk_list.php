...
<?php 
	$str .= '<table class="table table-bordered">
				<input type="hidden" name="edit_page" id="edit_page" value="batch_transactions_chk_list">
				<tr class="grythead">
					<th class="pointer" onclick="load_batch_file(\'batch_transactions_chk_list\',\'b_id='.$b_id.'&sort_by=id&chk_view=yes\')">
						#
					</th>
					<th class="pointer" onclick="load_batch_file(\'batch_transactions_chk_list\',\'b_id='.$b_id.'&sort_by=patient&chk_view=yes\')">
						Patient ID
					</th>
					<th class="pointer" onclick="load_batch_file(\'batch_transactions_chk_list\',\'b_id='.$b_id.'&sort_by=encounter&chk_view=yes\')">
						E. Id
					</th>
					<th>C.P.T.</th>
					<th>Check No.</th>
		...
		$sel_trans_rec=imw_query("select manual_batch_transactions.*
			...
			$arr[$j]['patient_id']= $pat_nam.' - '.$row_trans_rec['patient_id'];
			$arr[$j]['encounter_id']= $row_trans_rec['encounter_id'];
			$arr[$j]['trans_proc_code'] = $trans_proc_code;
			$arr[$j]['check_no'] = '';
			$arr[$j]['proc_total_amt'] = numberFormat($row_trans_rec['proc_total_amt'],2);
			$arr[$j]['allow_amt'] = numberFormat($allow_amt,2);
			$arr[$j]['deduct_trans_amt'] = numberFormat($deduct_trans_amt,2);
			$arr[$j]['write_trans_amt'] = numberFormat($write_trans_amt,2);
			$arr[$j]['adj_trans_amt'] = str_replace("&nbsp;"," ",numberFormat($adj_trans_amt,2,'yes'));
			$arr[$j]['paid_trans_amt'] = numberFormat($paid_trans_amt,2);
			$arr[$j]['neg_trans_amt'] = numberFormat($neg_trans_amt,2); 
			$arr[$j]['paid_by'] = $paid_by;
			$show_num=$j+1;
			$detail_chk_data .='
			<tr id="details_payment_'.$def_chk_no.'_'.$d.'" class="details_payment_css pointer" style="display:none;">
				<td '.$link.'>'.$d.'</td>			
				<td '.$link.'>'.$pat_nam.' - '.$row_trans_rec['patient_id'].'</td>
				<td '.$link.'>'.$row_trans_rec['encounter_id'].'</td>
				<td '.$link.'>'.$trans_proc_code.'</td>
                <td '.$link.'>&nbsp;</td>
				...
				<td '.$link.' class="text-right">'.numberFormat($adj_trans_amt,2).'</td>
				<td '.$link.' class="text-right">'.numberFormat($paid_trans_amt,2).'</td>
				<td '.$link.' class="text-right">'.numberFormat($neg_trans_amt,2).'</td>
				<td '.$link.'>'.$paid_by.'</td>
				<td>';
				if($post_status==0){
					$detail_chk_data .="		
					<a href=\"javascript:editTrans('$b_id','$trans_id','$sort_by','$chk_view');\" class=\"text_10\"><img src=\"../../library/images/edit.png\" alt=\"Edit\" border=\"0\"></a> 
					<a href=\"javascript:delTrans('$b_id','$trans_id','$sort_by','$chk_view');\" class=\"text_10\"><img src=\"../../library/images/del.png\" alt=\"Del\" border=\"0\"></a>
					";
				}else { $detail_chk_data .='-';} 
				$detail_chk_data .='</td> <td>&nbsp;</td>
			</tr>';
			$j++;
			 } 
			 $arr[$i]['patient_id']= '';
			...
			$arr[$i]['paid_by'] = '';
			 ...
			 $final_neg_proc_amt[]=$total_neg_proc;
			 
			 
			$i = $j+1;
			$j = $i+1;
			 $str .= '<tr class="text_10" style="background-color:#ffffff;">
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
            <td class="text-right"><b>'.$def_chk_no.'</b></td>
			<td class="text-right"><b>'.numberFormat(array_sum($total_proc_charges),2).'</b></td>
			<td class="text-right"><b>'.numberFormat(array_sum($total_allow_amt_arr),2).'</b></td>
			<td class="text-right"><b>'.numberFormat($total_deduct_proc,2).'</b></td>
			<td class="text-right"><b>'.numberFormat($total_write_proc,2).'</b></td>
			<td class="text-right"><b>'.numberFormat($total_adj_proc,2).'</b></td>
			<td class="text-right"><b>'.numberFormat($total_paid_proc,2).'</b></td>
            <td class="text-right"><b>'.numberFormat($total_neg_proc,2).'</b></td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
            <td id="up_arrow_id_'.$def_chk_no.'" style="display:none;cursor:pointer;text-align:center;" onClick="show_payments_div(\''.$def_chk_no.'\',\''.$d.'\',\'hide\');">
                <img src="../../library/images/arr_up.gif" style="border:none;">&nbsp;
            </td>
            <td id="down_arrow_id_'.$def_chk_no.'" style="cursor:pointer;text-align:center;" onClick="show_payments_div(\''.$def_chk_no.'\',\''.$d.'\',\'show\');">
                <img src="../../library/images/arr_dn.gif" style="border:none;">&nbsp;					
            </td>
		</tr>';
		$str .= $detail_chk_data;
		}
		$arr[$j]['patient_id']= '';
		$arr[$j]['encounter_id']= '';
		$arr[$j]['trans_proc_code'] = '';
		$arr[$j]['check_no'] = 'Check Total';
		$arr[$j]['proc_total_amt'] = numberFormat(array_sum($final_tot_proc_amt),2);
		$arr[$j]['allow_amt'] = numberFormat(array_sum($final_allow_proc_amt),2);
		$arr[$j]['deduct_trans_amt'] = numberFormat(array_sum($final_deduct_proc_amt),2);
		$arr[$j]['write_trans_amt'] = numberFormat(array_sum($final_wrt_proc_amt),2);
		$arr[$j]['adj_trans_amt'] = numberFormat(array_sum($final_adj_proc_amt),2);
		$arr[$j]['paid_trans_amt'] =numberFormat(array_sum($final_paid_proc_amt),2);
		$arr[$j]['neg_trans_amt'] =numberFormat(array_sum($final_neg_proc_amt),2); 
		$arr[$j]['paid_by'] = '';
		if($res_rec=="yes"){
			$str .= '<tr class="text_10" style="background-color:#ffffff;">
		<td>&nbsp;</td>
        ...
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td class="text-right purple_bar"><b>Check Total:</b></td>
        <td class="text-right purple_bar"><b>'.numberFormat(array_sum($final_tot_proc_amt),2).'</b></td>
        <td class="text-right purple_bar"><b>'.numberFormat(array_sum($final_allow_proc_amt),2).'</b></td>
        <td class="text-right purple_bar"><b>'.numberFormat(array_sum($final_deduct_proc_amt),2).'</b></td>
        <td class="text-right purple_bar"><b>'.numberFormat(array_sum($final_wrt_proc_amt),2).'</b></td>
        <td class="text-right purple_bar"><b>'.numberFormat(array_sum($final_adj_proc_amt),2).'</b></td>
        <td class="text-right purple_bar"><b>'.numberFormat(array_sum($final_paid_proc_amt),2).'</b></td>
        <td class="text-right purple_bar"><b>'.numberFormat(array_sum($final_neg_proc_amt),2).'</b></td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
    </tr>';
...
function show_payments_div(id,cnt,val){
	if(val=='show'){
		$('#up_arrow_id_'+id).show();
		$('#down_arrow_id_'+id).hide();
		$('.details_payment_css').show();
	}else{
		$('#up_arrow_id_'+id).hide();
		$('#down_arrow_id_'+id).show();
		$('.details_payment_css').hide();
	}	
}
</script>