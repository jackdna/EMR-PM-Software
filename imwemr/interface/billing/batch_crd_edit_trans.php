...
if($_REQUEST['sbmtFrm']){
	...
	$up_trans=imw_query("update manual_batch_creditapplied set amountApplied='$edit_amt',credit_note = '$credit_note' where crAppId='$edit_transid'");
...
$sel_tran=imw_query("select * from manual_batch_creditapplied where crAppId='$edit_transid'");
$fet_tran=imw_fetch_array($sel_tran);
if($fet_tran['crAppliedTo']=="payment"){
	$encounter_id=$fet_tran['crAppliedToEncId'];
	$patient_id=$fet_tran['patient_id'];	
	$charge_list_detaill_id=$fet_tran['charge_list_detail_id'];	
}else{
	$encounter_id=$fet_tran['crAppliedToEncId_adjust'];	
	$patient_id=$fet_tran['patient_id_adjust'];	
	$charge_list_detaill_id=$fet_tran['charge_list_detail_id_adjust'];	
}

$trans_amt=$fet_tran['amountApplied'];	
$credit_note=$fet_tran['credit_note'];
// GET PATIENT INFO
...
// GET DETAILS OF DENIAL
	...
	$sel_tran_allow=imw_query("select proc_allow_amt from manual_batch_transactions
			where encounter_id='$encounter_id' and payment_claims='Allowed'
			and charge_list_detaill_id = '$charge_list_detaill_id'
			and del_status='0' and post_status='0'");
...
?>
<form name="editFrm" method="post" action="batch_crd_edit_trans.php">
...
<table class="table table-bordered">
    ...
    <tr>
        <td colspan="5" class="form-inline">
        	<strong>Cr Note : </strong>
            <input type="text" value="<?php echo $credit_note;?>"  name="credit_note" id="credit_note" class="form-control" style="width:90%;">
        </td>
    </tr>
</table>
<div class="row pt10">	
    <div class="col-sm-12 text-center">
        <input type="submit" name="sbmtFrm" id="sbmtFrm" class="btn btn-primary" value="Update">
