...
?><?php
/*
File: emdeon_electronic_file_i_5010.php
...
$fileData .= 'BHT*0019*00*'.$new_interchange_num.'*'.date('Ymd').'*'.date('hi').'*CH~';
//$fileData .= 'REF*87*005010X223A2~';  commented (removed) in 5010.
...
if(isset($InsuranceComDetails['0']) && is_array($InsuranceComDetails['0']) && in_array(strtoupper($InsuranceComDetails['0']['Payer_id']),array('SKNJ0'))){
...
$chrgs_qry = "select patient_charge_list.*, patient_data.DOB AS patientDOB,
		...
		where (patient_charge_list_details.del_status='0' OR (patient_charge_list.void_notify='1' AND patient_charge_list.void_claim_date='0000-00-00 00:00:00')) and patient_charge_list.submitted = 'true'
...
if($ress && imw_num_rows($ress)>0){
	...
	//	$fileName = $fileNameStart.'i_'.date('mdY').'_00'.$insertData.'.txt';
		...
		$not_cov_proc_arr = isset($notCoverProDetailIdArr[$charge_list_id]) ? $notCoverProDetailIdArr[$charge_list_id] : '';
		$not_cov_proc_str = '';
		if(isset($not_cov_proc_arr) && is_array($not_cov_proc_arr) && count($not_cov_proc_arr) > 0){
		...
		$pcld_q = "select patient_charge_list_details.*, cpt_fee_tbl.cpt4_code, cpt_fee_tbl.cpt_desc, cpt_fee_tbl.cpt_comments, cpt_fee_tbl.unit_of_measure, cpt_fee_tbl.measurement 
				...
				// AND patient_charge_list_details.claim_status='0'";
...
*/		if(trim($subscriberDetails->policy_number) == '' && $invalidClaim == false){
	...
	//		$plan_name = NULL;
			...
			if(trim($recieverId) != '141650868'){
				...
				if($is_delayed>=90 && ((isset($arr_NE_Medicaid_payers) && in_array($otherInsDetail->Payer_id,$arr_NE_Medicaid_payers)) || in_array($bcbs,array('MC')))){
			...
			unset($diagnosisId[0]);
			if(isset($diagnosisId[1]) && trim($diagnosisId[1]) != ''){
				if($enc_icd10==1){
...
//				$fileData .= 'DMG*D8*'.$DOB2.'*'.substr($subscriber_sex2,0,1).'~';
			...
			for($u=0;$u<count($patientChargeDetails);$u++){
				...
				if(isset($mod_code_arr[0]) && $mod_code_arr[0] != ''){
					$modifierCodes = ':';
				...
				if($this->is_not_otherwise_classified_code($patientChargeDetails[$u]['cpt4_code']) && in_array($firstCharOfCPT,array('j','l','6')) && $proc_admin_desc=='' && $proc_acc_notes!=''){
					$proc_admin_desc = $proc_acc_notes;
				}
				if(in_array($firstCharOfCPT,array('j','l')) && $proc_admin_desc!=''){
			...
			// loop 2400	
				...
				$fileData .= 'SV2*'.$rev_code.'*HC:'.$patientChargeDetails[$u]['cpt4_code'].$modifierCodes.$jcode_cpt_desc.'*'.$procCharge.'*UN*';
				$fileData .= $proc_units.'~';
				...
				if($primaryFile == false){		
					...
					if($insPaidAmount >= 0 && $paidForProcByPriIns >=0){
						$fileData .= 'SVD*'.$otherInsDetail2->Payer_id.'*'.$paidForProcByPriIns.'*HC:'.$patientChargeDetails[$u]['cpt4_code'].''.$modifierCodes.'*'.$rev_code.'*1~';
						if($cas_codes_string){
	...
	if($files == true){
		if($createClaims != ''){
			...
			$record_data_arr['Interchange_control'] = $new_interchange_num;
