...
$fileData = 'ISA*00*          *00*          *ZZ*'.$submitterId.''.$submitterSpaceStr.'*ZZ*'.$recieverId.''.$recieveSpaceStr.'*'.date('ymd').'*'.date('hi').'*'.$ISA11_repetition_separator.'*00501*'.$InterchangeControlNumber.'*1*'.$production_code.'*:~';
...
//$fileData .= 'REF*87*004010X098A1~';  commented in 5010 by taran.
...
$chrQryRs = imw_query($query);
$main_charge_list_id_arr = array();
while($chrQryRes = imw_fetch_assoc($chrQryRs)){
	$main_charge_list_id_arr[] = $chrQryRes['charge_list_id'];
}
...
$chrgs_qry = "SELECT patient_charge_list.*, patient_data.fname AS patientFname,
		...
		users.user_npi AS users_npi, users.id AS usersId, refferphysician.NPI,
		...
		WHERE (patient_charge_list_details.del_status='0' OR (patient_charge_list.void_notify='1' AND patient_charge_list.void_claim_date='0000-00-00 00:00:00')) and patient_charge_list.submitted = 'true' AND patient_charge_list.gro_id = '$default_group_id'
...
*/		if(trim($subscriberDetails->policy_number) == '' && $invalidClaim == false){
		...
		if($invalidClaim == false){
			...
			if($selfSubCode != 18){
				...
				$fileData .= 'NM1*QC*1*'.$res[$i]['patientLname'].'*'.$res[$i]['patientFname'].'*'.substr($res[$i]['patientMname'],0,1).'**~';
