...
?><?php 
/*
File: reprint_medicare_electronic_file_5010.php
Purpose: Re-createmm Medicare 837 batch file 
Access Type: Include File 
*/

global $arr_NE_Medicaid_payers;
if(strtolower($billing_global_server_name)=='brian'){
	$var_repetition_separator = '^';
}else{
	$var_repetition_separator = '|';
}
$fileData = 'ISA*00*          *00*          *ZZ*'.$submitterId.''.$submitterSpaceStr.'*ZZ*'.$recieverId.''.$recieveSpaceStr.'*'.date('ymd').'*'.date('hi').'*'.$var_repetition_separator.'*00501*'.$InterchangeControlNumber.'*1*'.$production_code.'*:~';
...
if(trim($billing_global_taxonomy_number) != '' && !in_array($medicare_ins_details->Payer_id_pro,$arr_RRM_payers)){
...
$Chquery = "select charge_list_id from patient_charge_list where del_status='0' and encounter_id in ($encounter_id)";
$chrQryRes = imw_query($Chquery);
$main_charge_list_id_arr = array();
while($charges_RS = imw_fetch_assoc($chrQryRes)){
	$main_charge_list_id_arr[] = $charges_RS['charge_list_id'];
}
...
$chrgs_qry = "select patient_charge_list.*, patient_data.fname AS patientFname,
		...
		trim(users.lname) AS usersLname, users.TaxonomyId AS usersTaxonomyId,
		users.TaxId AS usersTaxId, users.default_group AS users_default_group,
		users.user_npi AS users_npi, refferphysician.NPI,
		users.federaltaxid AS usersFederaltaxid, users.id AS usersId  
...
*/		if(trim($subscriberDetails->policy_number) == '' && $invalidClaim == false){
		...
		if($invalidClaim == false){
			...
			if($selfCode != 18){
				...
				$fileData .= 'NM1*QC*1*'.trim($res[$i]['patientLname']).'*'.trim($res[$i]['patientFname']).'*'.substr($res[$i]['patientMname'],0,1).'**~';
			...
			if(in_array(strtoupper($otherInsDetail->Payer_id_pro),array('61101','11324'))){
				$ref_phy_not_required = 1;
			}
			if($ref_phy_not_required == '0'){
				$fileData .= 'NM1*DN*1*'.trim($reffPhysicianLname).'*';
				$fileData .= ''.trim($reffPhysicianFname).''.$reffPhysicianMname.''.'****XX*'.trim(preg_replace("/-/","",$npiNumber)).'~';
		...
		if(strtolower($billing_global_server_name)=='kung' && $otherInsDetail->Payer_id_pro=='12402'){//for kung and hightmark medicare only
			...
			if(is_object($subscriberDetails2) === true){
				...
				if($subscriberCaseName2 == 'Workman Comp'){
					...
					$MEDICARE2 = explode("MEDICARE",strtoupper($otherInsDetail2->name));
					$MEDICADE2 = explode("MEDICADE",strtoupper($otherInsDetail2->name));
					if(count($MEDICARE2) > 1){
						$insInstance2 = '47';
					}
...
if($files == true){
	...
	$qry = "update batch_file_submitte set file_data = '".strtoupper(addslashes($fileData))."',
