...
include_once($GLOBALS["incdir"]."/printing/common_functions.inc.php"); */
...
ob_start();
//print_hdrTopbar($patientDetails,$date_of_service,$_REQUEST["chart_nopro"],$_REQUEST['chart_exclusion']);
print_hdrTopbar($patientDetails,"",$_REQUEST["chart_nopro"],$_REQUEST['chart_exclusion']);
print_mainHeader($patientDetails,$chartDetails,$_REQUEST["chart_nopro"],$_REQUEST['chart_exclusion']);
/*if($dos){
	$html_print=print_getGenMed($pid,$arrReq,"'".$dos."'",$exclusionArr);
}else{
	$arrReq[]="general_health";
	print_allGen($pid,$arrReq,"",$exclusionArr);
}*/

$arrReq[]="general_health";

$objParser->print_allGen($pid,$arrReq,"",$exclusionArr);
$htmloutput = ob_get_contents();
$data_dir = substr(data_path(), 0, -1);
$patientDirGenHealth = "/PatientId_".$pid;
$patientDir = $patientDirGenHealth;
...
$gen_health_html_file_name ='General_Health'.$_SESSION['authId'];
$putData = file_put_contents($fp,$patientPrintData);

$copyPdfFilePath = $iolinkDirPath.$patientDir."/General_Health.pdf";
if(file_exists($copyPdfFilePath)){unlink($copyPdfFilePath);}

$fp = $data_dir.'/iOLink/'.$gen_health_html_file_name.'.html';
...
ob_clean();
$htmlFlName = $gen_health_html_file_name;

$pdfFileName = 'General_Health.pdf';
...
$data1 = "";
$curNew = curl_init();
//$urlPdfFile = $myHTTPAddress."?op=p&onePage=false&saveOption=F&name=".$gen_health_html_file_name."&pdf_name=".$gen_health_html_file_name;
...
function print_hdrTopbar($patientDetails,$date_of_service="",$arrReq=array(),$exclusion){
...
function print_mainHeader($patientDetails, $chartDetails=array(), $fAct=array(),$exclusion=array()){
	...
	<?php }else{ echo('<td style="width:20%" >&nbsp;</td>');} ?>
		<td style="width:40%" align="right">
			...
			if(!in_array("provider_name_and_contact",$exclusion)){
				...
				print_groupInfo($pat_id,$chartDetails['date_of_service']);
			}
			?>
		</td>
	</tr>
...
//Group info --------------------------
function print_groupInfo($patient_id,$chart_dos){
	...
	//	$res = $GLOBALS['adodb']['db']->Execute($qry) or die("Error in query: ".$GLOBALS['adodb']['db']->errorMsg());
		...
		if($row !== false){
			$groupDetails[0]['name'] = $res->fields["name"];
			$groupDetails[0]['group_Address1'] = $res->fields["group_Address1"];
			$groupDetails[0]['group_Address2'] = $res->fields["group_Address2"];
			$groupDetails[0]['group_City'] = $res->fields["group_City"];
			$groupDetails[0]['group_State'] = $res->fields["group_State"];
			$groupDetails[0]['group_Zip'] = $res->fields["group_Zip"];
			$groupDetails[0]['group_Telephone'] = $res->fields["group_Telephone"];
			$groupDetails[0]['group_Fax'] = $res->fields["group_Fax"];
		}
	}
	//--- Get Default Facility Details -------
