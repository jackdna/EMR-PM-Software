...
$ignoreAuth = true;
require_once(dirname(__FILE__)."/../../interface/chart_notes/common/functions.php");

require_once(dirname(__FILE__)."/../../interface/admin/chart_more_functions.php");		
require_once(dirname(__FILE__)."/../../interface/main/patient_functions.php");
require_once(dirname(__FILE__)."/../../interface/main/main_functions.php");

require_once(dirname(__FILE__)."/../../interface/chart_notes/common/simpleMenu.php");
require_once(dirname(__FILE__)."/../../interface/chart_notes/common/session_chart_view_access.php");	
require_once(dirname(__FILE__)."/../../interface/chart_notes/common/functions.php");
require_once(dirname(__FILE__)."/../../interface/chart_notes/common/ChartApXml.php");

	function getGFSMedication($patientId, $formId){
		//Dos
		$dos = getDateFormatDB(getDateOfServiceFullYear($formId));
		...
		//Remove Last
		//$strPres = (!empty($strPres)) ? substr($strPres, 0, -2)." " : "";
	...
	function insertGlucomaReading($arr)
	{
		if(count($arr) > 0)
		{
			...
			$elem_timeReadingMil = chTimeFormat($elem_timeReading);
	...
	function getChartNotesReading($patient_id)
	{
		...
		{	
			...
			if(empty($row["multiple_pressure"])){ 	
				...
				$arrMiop = getMIop($row["multiple_pressure"], "GFS");
			...
			$arr["elem_gonioOdSummary"]=(!empty($row["gonio_od_summary"])) ? "Done" : "Empty"; //$row["gonio_od_summary"]; 
			$arr["elem_gonioOsSummary"]=(!empty($row["gonio_os_summary"])) ? "Done" : "Empty"; //$row["gonio_os_summary"]; 
			
			//Medication
			$arr["elem_medication"]=getGFSMedication($patient_id,$row["form_id"]);
			
			//$arr["elem_vfExamDate"]=$row["examDateVfNfa"];
			//$arr["elem_iopExamDate"]=$row["examDateIop"];
			//$arr["elem_opticExamDate"]=$row["examDateChartOptic"];
			...
			//$arr["elem_ivfaExamDate"]=$row["examDateIvfa"];
			...
			//$arr["elem_gonioDate"] = FormatDate_show($row["examDateIop"]);
			...
			if(getNumber($row["examDateGonio"]) != 0)
			...
			c5.sel_od as vis_dis_od_sel_3, c5.txt_od as vis_dis_od_txt_3, 
			c5.sel_os as vis_dis_os_sel_3, c5.txt_os as vis_dis_os_txt_3
			...
			LEFT JOIN chart_acuity c5 ON c5.id_chart_vis_master = c2.id AND c5.sec_name = 'Ad. Acuity' AND c5.sec_indx=3
			WHERE c2.form_id = '$add_form_id' AND c2.patient_id = '$patient_id'
			");	
			...
			$oChartApXml = new ChartApXml($patient_id,$add_form_id);
			...
			if(count($list_arr[4])>0){
				$dt_lists_qry = "SELECT title FROM lists WHERE pid = '".$_SESSION["patient"]."' and allergy_status = 'Discontinue'";
				...
				while($row_med=imw_fetch_array($sql_med)){
					/*if(!in_array($row_med["medicine_name"],$dt_lists_arr))
					{*/
	...
	if($elem_activate != "-1")
        {   	
			...
			if(empty($elem_highTaOdOd) || ($elem_highTaOdOd < $arrInitialTop["HighTaOd"]["od"])){
				...
				$elem_dateHighTaOd = checkWrongDate($arrInitialTop["HighTaOd"]["date"]);
		    }
			...
			if(empty($elem_highTaOsOs) || ($elem_highTaOsOs < $arrInitialTop["HighTaOs"]["os"])){
				...
				$elem_dateHighTaOs = checkWrongDate($arrInitialTop["HighTaOs"]["date"]);
		    }
			...
			if(empty($elem_highTxOdOd) || 
				...
				$elem_highTxOdOd = $arrInitialTop["HighTxOd"]["od"];
				$elem_dateHighTxOd = checkWrongDate($arrInitialTop["HighTxOd"]["date"]);
			}
			...
			if(empty($elem_highTxOsOs) || 
				...
				$elem_highTxOsOs = $arrInitialTop["HighTxOs"]["os"];
				$elem_dateHighTxOs = checkWrongDate($arrInitialTop["HighTxOs"]["date"]);
			}
	...
	//Targets	
	$curformId = isChartOpened($patient);
	list($targetOdTmp,$targetOsTmp)=getIopTargets($patient);	
	if($curformId != false){
		//check
