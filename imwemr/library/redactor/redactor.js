	...
	Redactor.fn = $.Redactor.prototype = {
		...
		autosave: function()
		{
			return {
				...
				onChange: function()
				{
					if(typeof(RedactorOnChangeEvenAction)!='undefined'){RedactorOnChangeEvenAction();}
						...
						if (isChrome)
							{
								$tooltip.show();
								$tooltip.css({
									top: ((pos.top*1.4) + (height*1.4))  + 'px',
									left: (pos.left*1.4 + (width*1.4)/2 - $tooltip.innerWidth()/2) + 'px'
									});
						...
						else
							{
								...
								$tooltip.css({
									...
									left: (pos.left + width/2 - $tooltip.innerWidth()/2 +5) + 'px'
									});
			...
			return {
				...
				getData: function()
				{
					...
					if ($el.size() !== 0 && $el[0].tagName === 'A')
					{
						...
						this.link.act_id = $el.attr('act_id');
					}
					else
					{
				...
				smarttag: function(e)
				{
					...
					//Grabs the Link/URL/ID
					this.link.getData();
					this.link.cleanUrl();
					
					//Cuts the ID
					var url_id = this.link.id;
					var	parsed = url_id.split("|");



					var template = '<section>';
					template += '<select id="redactor-smarttag-insert" class="form-control minimal">';
					for(var i = 1; i < parsed.length; i++)
						{
							template+='<option value="'+parsed[i]+'">'+parsed[i]+'</option>';
						}
					template+='</select>';
					template+='</section>';

					//This is going to add the template to the SmartTag Modal
					this.modal.addTemplate('SmartTag',template);

					//Loads the Modal
					this.modal.load('SmartTag', 'Smart Tag', 600);
					
					//Adds a cancel button
					this.modal.createCancelButton();
					
					//This is my button to set call the function					
					this.link.button = this.modal.createActionButton("Insert");
					this.link.button.on('click', $.proxy(this.link.smarttag_load, this));
					
					this.link.text = $('#redactor-smarttag-insert');
					this.link.text.val(this.link.text);
				...
				smarttag_load: function()
				{
					
					var target = '';
					var link = '';
					var text = this.link.text.val(); 
					
					//Act ID
					var act_id = this.link.act_id;
					
					$("[act_id='"+act_id+"']").each(function(){
			...
			return {
				...
				loadTemplates: function()
				{
					this.opts.modal = {
						...
						// smarttag: String()
						// + '<section id="redactor-modal-smarttag-insert">'
				...
				load: function(templateName, title, width, smarttag_array)
				{
					this.modal.templateName = templateName;

					this.modal.width = width;
					this.modal.test = smarttag_array;
		...
		observe: function()
		{
			return {
				...
				showTooltip: function(e)
				{
					...
					var aSmarttag = $('<a href="#">Smart Tag</a>').on('click', $.proxy(this.link.smarttag, this)).addClass('redactor-link-tooltip-action');


					//tooltip.append(aLink).append(' | ').append(aEdit).append(' | ').append(aUnlink).append(' | ').append(aSmarttag);
					tooltip.append(aSmarttag);
					tooltip.css({
						top: (pos.top + 20) + 'px',
						left: pos.left + 'px'
					});
