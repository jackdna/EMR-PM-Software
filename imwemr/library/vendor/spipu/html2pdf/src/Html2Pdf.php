...
<html>
    ...
    protected function _drawImage($src, $subLi = false)
    {
        ...
        $infos=@getimagesize($src);//echo '<br>'.$src;
		
		//START "IMAGE NOT FOUND" HANDLE EXCEPTION MANUALLY
		$imageNotFound = "../../library/images/img_not_found.png";
		if(!file_exists($imageNotFound)) {
			$imageNotFound = dirname(__FILE__)."/../../../../images/img_not_found.png";
		}
		if (!is_array($infos) || count($infos)<2) {
			$infos=@getimagesize($imageNotFound);//echo '<br>'.$src;
		}
		//END "IMAGE NOT FOUND" HANDLE EXCEPTION MANUALLY
    ...
    protected function _tag_open_IMG($param)
    {
		...
		//START "IMAGE NOT FOUND" HANDLE EXCEPTION MANUALLY
		$notFoundImage = "../../library/images/img_not_found.png";
		if(!file_exists($notFoundImage)) {
			$notFoundImage = dirname(__FILE__)."/../../../../images/img_not_found.png";
		}
		if(!file_exists($src)) {
			$src = $notFoundImage;
		}
