...
class StreamIO extends AbstractIO
{
     ...
     * @inheritdoc
     */
	public function trySend($clientObj, &$buffnew) {
		//This function is used send a huge amount of data
		if (strlen($buffnew)) {
			usleep(50000);
			$lenn = fwrite($clientObj, $buffnew);
			//file_put_contents('test1.txt', $buffnew, FILE_APPEND); //."\n============================\n"
			$buffnew = substr($buffnew, $lenn, 8192);
		}
		return $lenn;
	}
	public function write($data)
    {
        ...
        while ($written < $len) {
            ...
            try {
				...
				//$result = $this->trySend($this->sock, $buffer); //This function is used send a huge amount of data
                $this->cleanup_error_handler();
            } catch (\ErrorException $e) {
