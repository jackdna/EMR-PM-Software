...
if (isset($_FILES['upload'])){
    ...
    $name = uniqid().uniqid().$_FILES['upload']['name'];
    
    // File system path for this image   
    $basePath = data_path()."gn_images/";
    
    // It doesn't care if the file already exists, it's simply overwritten.
    move_uploaded_file($_FILES["upload"]["tmp_name"], $basePath.$name);
    
    // Final url of the uploaded image
    $url = $phpHTTPProtocol.$dyn_host.'/'.PRACTICE_PATH.'/data/'.PRACTICE_PATH.'/gn_images/'.$name;
