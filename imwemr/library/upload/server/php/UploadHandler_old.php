...
class UploadHandler
{
    ...
    public function get_unique_filename($name, $type, $index, $content_range) {
    ...
    public function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new stdClass();
		if(strlen($name)>100){
			$arr_file_arr=explode(".",$name);
			$file_extension=end($arr_file_arr);
			$file_name_cut=substr(str_ireplace($file_extension,"",$name),0,100);
			$name=$file_name_cut.".".$file_extension;
		}
		$ret_file_name=$file_name_cut.$file_ext;
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
					...
					//checking if TIFF uploaded. Converting to PDF (TIFF CAN BE MULTI-PAGED)
					$tifPDFpath = false;
					if(substr($file_path,-5)=='.tiff'){$tifPDFpath = substr($file_path,0,-4).'pdf';}
					else if(substr($file_path,-4)=='.tif'){$tifPDFpath = substr($file_path,0,-3).'pdf';}
					if($tifPDFpath){
						$file_path1 = $file_path;
						exec('convert "'.$file_path1.'" "'.$tifPDFpath.'"', $output, $return_var);
						unlink($file_path);
						$file_path = $tifPDFpath;
