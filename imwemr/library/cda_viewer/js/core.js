...
/*function loadcontents(data,reponame,owner,path){
	...
	$('#github').find('.loadrepo').click(function(){
		var url='https://api.github.com/repos/'+$(this).attr('owner')+'/'+$(this).attr('reponame')+'/contents/'+$(this).attr('path'); a=window.open();a.document.write(url);
		var reponame=$(this).attr('reponame')
		var owner=$(this).attr('owner')
	...
	$('#github').find('.transform').off('click').click(function(){
		...
		new Transformation().setXml(cdaxml).setXslt(cdaxml_xsl).transform("viewcda");
	})
}*/
