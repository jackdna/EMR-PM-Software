...
if (isset($GLOBALS['dbh'])==false){
	$conLink = mysqli_connect($sqlconf["host"],$sqlconf["login"],$sqlconf["pass"],constant('IMEDIC_IDOC'));
	$GLOBALS['dbh'] = $conLink;
}
...
// Open a connection to the MySQL server
function imw_connect($server_name,$uname, $pass, $dbname, $port){
	return mysqli_connect($server_name,$uname, $pass, $dbname, $port);
}
...
include_once($srcdir."/classes/common_function.php");
include_once($srcdir."/vendor/autoload.php");
?>