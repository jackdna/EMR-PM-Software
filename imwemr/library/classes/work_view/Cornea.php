<?php

class Cornea extends ChartNote{
	private $examName,$tbl,$xmlFileOd,$xmlFileOs,$divSE;
	public function __construct($pid,$fid=""){
		parent::__construct($pid,$fid);
		$this->tbl="chart_cornea";
		$this->examName="Cornea";
	}
	
	function isRecordExists($a="",$b="",$c="",$d=""){
		return parent::isRecordExists($this->tbl);
	}

	function getRecord($sel=" * ",$a="",$b="",$c="",$d=""){
		return parent::getRecord($this->tbl,$sel);
	}

	function getLastRecord($sel=" * ",$LF="0",$dt="", $tbl="", $b="", $c="" ){
		if(empty($tbl)){ $tbl = $this->tbl; }
		return parent::getLastRecord($tbl,"form_id",$LF,$sel,$dt);
	}
	
	function get_chart_info($elem_dos){
		$oExamXml = new ExamXml();
		$patient_id = $this->pid;
		$elem_formId=$form_id = $this->fid;
		$ar_ret = array();
		
		$sql = "SELECT * FROM ".$this->tbl." WHERE form_Id = '$form_id' AND patient_id ='".$patient_id."' AND purged = '0' ";		
		$row=sqlQuery($sql);
		//------ Check in archive tables for chart_slit_lamp_exam/chart_la/chart_rv ---------
		if(!empty($GLOBALS["CHK_ARCHIVE_TABLE"])){
			if($row==false){
				$sql = str_replace($this->tbl,$this->tbl."_archive",$sql);
				$row = sqlQuery($sql);
			}	
		}
		//-------- End ----------------------------------------------------------------------------------
		if($row==false){
		//if(mysql_num_rows($res)<=0){ /* Show Previous values in finalized chart also ~&& ($finalize_flag == 0)~ */
			//$res = valNewRecordSle($patient_id);	
			$res = $this->getLastRecord(" * ",0,$elem_dos);
			if($res!=false){$row=$res;}else{$row=false;}
			$blDrwaingGray = true;	
			$myflag=true;		
		}

		if(isset($_GET["prevVal"]) && ($_GET["prevVal"] == 1)){
			//$res = valNewRecordSle($patient_id, " * ", "1");
			$res = $this->getLastRecord(" * ",1,$elem_dos);
			if($res!=false){$row=$res;}else{$row=false;}
			$myflag=true;
			$blDrwaingGray = true;
		}
		
		if($row!=false){
		//if(@mysql_num_rows($res)>0){
			//extract(sqlFetchArray($res));
			extract($row);
			
			if($myflag){
				$elem_editMode=0;
				// As last record form_id overwrite current form_id, so retore current form_id
				$form_id = $form_id_cur;		
			}else{
				$elem_editMode=1;
			}
			$ar_ret["elem_cornId_LF"]=$ar_ret["elem_cornId"]=$id;
			$ar_ret["elem_editModeCorn"] = $elem_editMode;
			//$elem_formId=$form_id;
			//$elem_patientId=$patient_id;
			//NC
			if($elem_editMode==1){							
				$ar_ret["elem_ncCorn"] = $ncCorn;				
				$ar_ret["elem_examDateCorn"]=$exam_date;
			}			
			
			$cornea_od= stripslashes($cornea_od);
			$cornea_os= stripslashes($cornea_os);
			
			$arr_vals_cornea_od = $oExamXml->extractXmlValue($cornea_od);
			$ar_ret = array_merge($ar_ret, $arr_vals_cornea_od);
			$arr_vals_cornea_os = $oExamXml->extractXmlValue($cornea_os);
			$ar_ret = array_merge($ar_ret, $arr_vals_cornea_os);			
			
			$ar_ret["elem_wnlCorn"] = $wnlCorn;			
			$ar_ret["elem_posCorn"] = $posCorn;
			
			$ar_ret["elem_wnlCornOd"] = $wnlCornOd;
			$ar_ret["elem_wnlCornOs"] = $wnlCornOs;
			$ar_ret["elem_statusElementsCorn"] = ($elem_editMode==0) ? "" : $statusElem;
			$ar_ret["elem_penLight"]=$pen_light;
			
			//UT Elems //($elem_editMode==1) ?
			$ar_ret["elem_utElemsCorn"] = ($elem_editMode==1) ? $row["ut_elem"] : "" ;
			
			$ar_ret["cornea_od_summary"] = stripslashes($cornea_od_summary);
			$ar_ret["cornea_os_summary"] = stripslashes($cornea_os_summary);
		}
		return $ar_ret;
	}

	public function save_form(){
		$elem_formId = $formId = $this->fid;
		$patientid = $this->pid;
		$oChartNoteSaver = new ChartNoteSaver($patientid, $formId);
		$oExamXml = new ExamXml();	
		$arXmlFiles = $oExamXml->getExamXmlFiles("SLE");
		
		$arrSe = array();
		$tmpd = "elem_chng_div2_Od";
		$tmps = "elem_chng_div2_Os";
		$$tmpd = $_POST[$tmpd];
		$$tmps = $_POST[$tmps];
		$arrSe[$tmpd] = $$tmpd;
		$arrSe[$tmps] = $$tmps;
		
		//
		$statusElem = $oChartNoteSaver->getStrSe($arrSe);
		
		//Cornea ------------
		$wnlCornOd = $wnlCornOs = $wnlCorn = $posCorn = $ncCorn = "0";
		//if(!empty($elem_chng_div2_Od) || !empty($elem_chng_div2_Os)){
			//if(!empty($elem_chng_div2_Od)){
				$menuName = "corneaOd";
				$menuFilePath = $arXmlFiles["cornea"]["od"]; //dirname(__FILE__)."/xml/cornea_od.xml";
				$elem_cornea_od = $oExamXml->newXmlString($menuName,$strMenu,$menuFilePath);
				$wnlCornOd = $_POST["elem_wnlCornOd"];
			//}

			//if(!empty($elem_chng_div2_Os)){
				$menuName = "corneaOs";
				$menuFilePath = $arXmlFiles["cornea"]["os"]; //dirname(__FILE__)."/xml/cornea_os.xml";
				$elem_cornea_os = $oExamXml->newXmlString($menuName,$strMenu,$menuFilePath);
				$wnlCornOs = $_POST["elem_wnlCornOs"];
			//}

			$wnlCorn = (!empty($wnlCornOd) && !empty($wnlCornOs)) ? "1" : "0" ; //$_POST["elem_wnlCorn"];
			$posCorn = $_POST["elem_posCorn"];
			$ncCorn = $_POST["elem_ncCorn"];
		//}
		//Cornea ------------
		
		//SLE ------------
		$examDate = wv_dt("now");
		$pen_light = $_POST["elem_penLight"];
		
		//
		$oUserAp = new UserAp();
		
		//Summary --
		$strExamsAllOd = $strExamsAllOs = "";
		$cornea_od_summary = "";
		$cornea_os_summary = "";

		$cornea_od = $elem_cornea_od;	 // Cornea Od
		
		$arrTemp = $this->getExamSummary($cornea_od);
		$cornea_od_summary = $arrTemp["Summary"];
		$arrExmDone_od = $arrTemp["ExmDone"];
		if(!empty($arrSe["elem_chng_div2_Od"])){
			$strExamsAllOd .= $oUserAp->refineByConsoleSymp("Cornea",$arrExmDone_od,$cornea_od_summary);
		}
		$cornea_os = $elem_cornea_os;	 // Cornea Os
		
		$arrTemp = $this->getExamSummary($cornea_os);
		$cornea_os_summary = $arrTemp["Summary"];
		$arrExmDone_os = $arrTemp["ExmDone"];
		if(!empty($arrSe["elem_chng_div2_Os"])){
			$strExamsAllOs .= $oUserAp->refineByConsoleSymp("Cornea",$arrExmDone_os,$cornea_os_summary);
		}		
		//Summary --
		
		//ut_elems ----------------------
		$elem_utElems = $_POST["elem_utElemsCorn"];
		$elem_utElems_cur = $_POST["elem_utElemsCorn_cur"];
		$ut_elem = $this->getUTElemString($elem_utElems,$elem_utElems_cur);
		//ut_elems ----------------------
		
		//Purge
		if(!empty($_POST["elem_purged"])){
			//Update
			$sql = "UPDATE ".$this->tbl."
				  SET
				  purged=id,
				  purgerId='".$_SESSION["authId"]."',
				  purgetime='".wv_dt('now')."'
				  WHERE form_id='".$elem_formId."' AND patient_id='".$patientid."' AND purged = '0'
				";			
			$row = sqlQuery($sql);
			
		}else{
			$owv = new WorkView();
			
			$elem_cornea_od = sqlEscStr($elem_cornea_od);
			$elem_cornea_os = sqlEscStr($elem_cornea_os);
			
			//check
			$cQry = "select 
					id,last_opr_id,uid,					
					cornea_od_summary, cornea_os_summary, wnlCornOd, wnlCornOs, modi_note_CornArr, wnl_value_Cornea, 
					exam_date				
					FROM ".$this->tbl." WHERE form_id='".$elem_formId."' AND patient_id='".$patientid."' AND purged = '0' ";
			$row = sqlQuery($cQry);
			
			if($row == false){
				$elem_editMode = "0";
				$last_opr_id = $_SESSION["authId"];
			}else{
				$cornId = $cornIDExam = $row["id"];
				$elem_editMode = "1";
				$last_opr_id = $owv->get_last_opr_id($row['last_opr_id'],$row["uid"]);
				//Modifying Notes----------------
				
				$seri_modi_note_CornArr = $owv->getModiNotesArr($row["cornea_od_summary"],$cornea_od_summary,$last_opr_id,'OD',$row["modi_note_ConjArr"],$row['exam_date']);
				$seri_modi_note_CornArr = $owv->getModiNotesArr($row["cornea_os_summary"],$cornea_os_summary,$last_opr_id,'OS',$seri_modi_note_CornArr,$row['exam_date']);
				
				//Modifying Notes----------------					
			}
			
			//
			$sql_con = "
				cornea_od = '".$elem_cornea_od."',
				cornea_os = '".$elem_cornea_os."',
				cornea_od_summary = '".$cornea_od_summary."',
				cornea_os_summary = '".$cornea_os_summary."',
				wnlCorn='".$wnlCorn."',
				posCorn='".$posCorn."',
				ncCorn='".$ncCorn."',
				wnlCornOd='".$wnlCornOd."',
				wnlCornOs='".$wnlCornOs."',
				uid = '".$_SESSION["authId"]."',
				statusElem = '".$statusElem."',
				pen_light = '".$pen_light."',
				ut_elem = '".$ut_elem."',
				last_opr_id = '".$last_opr_id."',
				modi_note_CornArr = '".sqlEscStr($seri_modi_note_CornArr)."'
			";
			
			if($elem_editMode == "0"){
				//
				$wnl_value_Cornea = $this->getExamWnlStr("Cornea");
				
				// Insert
				$sql1 = "insert into ".$this->tbl."
							SET ";
							$sql1 .= "							
							form_id = '".$elem_formId."',
							patient_id='".$patientid."',
							exam_date='".$examDate."',
							wnl_value_Cornea='".sqlEscStr($wnl_value_Cornea)."',
							";
							//print '<pre>';
							//echo $sql;
				$sql = $sql1. $sql_con;				
				$insertId = sqlInsert($sql);
			}
			else if($elem_editMode == "1"){
				//Update
				$sql1 = "UPDATE ".$this->tbl." SET ";
				$sql2 = " WHERE form_id='".$formId."' AND patient_id='".$patientid."' AND purged = '0' ";
				$sql = $sql1. $sql_con. $sql2; 
				$res = sqlQuery($sql);
				$insertId = $cornId;				
			}
			
			// Make chart notes valid
			$this->makeChartNotesValid();			

			//Set Change Date Arc Rec --
			$oChartNoteSaver->setChangeDtArcRec($this->tbl);
			//Set Change Date Arc Rec --			
		}
		
		//
		//combine
		$strExamsAll = $oChartNoteSaver->combineExamFindings($strExamsAllOd, $strExamsAllOs);
		$strExamsAll = $oChartNoteSaver->makeArrString($strExamsAll);
		
		return $strExamsAll;		
	}	
	
	function getWorkViewSummery($post){
		$oOnload =  new Onload();
		//object Chart Rec Archive --
		$oChartRecArc = new ChartRecArc($this->pid,$this->fid,$_SESSION['authId']);
		
		//Get Template Procedures ---
		$arrTempProc=array("All");
		if(isset($post["artemp"])&&!empty($post["artemp"])){
			$arrTempProc=$post["artemp"];
		}			
		//Get Template Procedures ---
		
		$sql ="SELECT ".
			"c10.posCorn,".
			"c10.wnlCorn,".
			"c10.ncCorn,".
			"c10.cornea_od_summary, ".
			"c10.cornea_os_summary,".
			"c10.id, ".
			"c10.wnlCornOd, ".
			"c10.wnlCornOs, ".
			"c10.wnl_value_Cornea, ".
			"c10.pen_light, c10.exam_date, ".
			"c10.statusElem AS se_sle, c10.modi_note_CornArr, 
			c10.last_opr_id AS last_opr_id_sle
			 ".
			"FROM chart_master_table c1 ".
			"INNER JOIN ".$this->tbl." c10 ON c10.form_id = c1.id AND c10.purged='0'  ".
			"WHERE c1.id = '".$this->fid."' AND c1.patient_id='".$this->pid."' ";
		$row = sqlQuery($sql);
		//$row=$oSLE->sqlExe($sql);
		//------ Check in archive tables for chart_slit_lamp_exam/chart_la/chart_rv ---------
		if(!empty($GLOBALS["CHK_ARCHIVE_TABLE"])){
			if($row==false){
				$sql = str_replace($this->tbl,$this->tbl."_archive",$sql);
				$row = sqlQuery($sql);
			}	
		}
		//-------- End ----------------------------------------------------------------------------------
		if($row != false){			
			//SLE
			$elem_posCorn = assignZero($row["posCorn"]);
			$elem_wnlCorn = assignZero($row["wnlCorn"]);
			$elem_ncCorn = assignZero($row["ncCorn"]);
			$elem_wnlCornOd = assignZero($row["wnlCornOd"]);
			$elem_wnlCornOs = assignZero($row["wnlCornOs"]);
			$elem_se_corn = $row["se_sle"];
			$elem_wnl_value_Cornea = $row["wnl_value_Cornea"];
			$elem_cornea_od_summary = $row["cornea_od_summary"];
			$elem_cornea_os_summary = $row["cornea_os_summary"];
			$elem_corn_id = $row["id"];
			$elem_pen_light = $row["pen_light"];
			$examdate = wv_formatDate($row["exam_date"]);
			$modi_note_ConjArr = unserialize($row["modi_note_ConjArr"]);			
			$arrHx = array();
			if(is_array($modi_note_CornArr) && count($modi_note_CornArr)>0 && $row["modi_note_CornArr"]!='')
			$arrHx['Cornea'] = $modi_note_CornArr;
		}

		//Previous 
		if(empty($elem_corn_id)){

			$tmp = "";
			
			$tmp .= " c2.posCorn, ";
			$tmp .= " c2.wnlCorn, ";
			$tmp .= " c2.wnlCornOd,
					c2.wnlCornOs, ";
			
			$tmp .= " c2.ncCorn, ";
			
			$tmp .= "  c2.cornea_od_summary,	 ";
			$tmp .= "  c2.wnl_value_Cornea, ";
			$tmp .= " c2.cornea_os_summary,
						c2.exam_date,
						c2.pen_light, c2.id, 
						c2.statusElem AS se_sle ";
			
			$elem_dos=$this->getDos();
			$elem_dos=wv_formatDate($elem_dos,0,0,"insert");
			$res = $this->getLastRecord($tmp,0,$elem_dos);
			if($res!=false){$row=$res;}else{$row=false;}
			if($row!=false){
			//$res = valNewRecordSle($patient_id, $tmp);
			//for($i=0;$row=sqlFetchArray($res);$i++)	{
				$elem_cornea_od_summary = $row["cornea_od_summary"];
				$elem_cornea_os_summary = $row["cornea_os_summary"];
				$elem_posCorn = assignZero($row["posCorn"]);
				$elem_wnlCorn = assignZero($row["wnlCorn"]);
				$elem_ncCorn = assignZero($row["ncCorn"]);				
				$elem_wnlCornOd = assignZero($row["wnlCornOd"]);
				$elem_wnlCornOs = assignZero($row["wnlCornOs"]);				
				$elem_wnl_value_Cornea = $row["wnl_value_Cornea"];				
				$elem_pen_light = $row["pen_light"];
				$examdate = wv_formatDate($row["exam_date"]);
				$elem_se_corn_prev = $row["se_sle"];
			}
			//BG
			$bgColor_SLE = "bgSmoke";
		}
		
		//---------

		//is Change is made in new chart -----		
		$flgSe_Corn_Od = $flgSe_Corn_Os = "0";
		
		if(!isset($bgColor_SLE)){
			if(!empty($elem_se_corn)){
				$tmpArrSe = $this->se_elemStatus("SLE","0",$elem_se_corn);
				$flgSe_Corn_Od = $tmpArrSe["2"]["od"];
				$flgSe_Corn_Os = $tmpArrSe["2"]["os"];
			}
		}else{
			if(!empty($elem_se_corn_prev)){
				$tmpArrSe_prev = $this->se_elemStatus("SLE","0",$elem_se_corn_prev);
				$flgSe_Corn_Od_prev = $tmpArrSe_prev["2"]["od"];
				$flgSe_Corn_Os_prev = $tmpArrSe_prev["2"]["os"];				
			}
		}
		//is Change is made in new chart -----

		//WNL
		
		//Cornea --
		$wnlString_Cornea = !empty($elem_wnl_value_Cornea) ? $elem_wnl_value_Cornea : $this->getExamWnlStr("Cornea");
		$wnlStringOd_Cornea = $wnlStringOs_Cornea = $wnlString_Cornea; 
		
		if(empty($flgSe_Corn_Od) && empty($flgSe_Corn_Od_prev) && !empty($elem_wnlCornOd)){ $tmp = $this->getExamWnlStr_fromPrvExm("Cornea", "OD"); if(!empty($tmp)){ $wnlStringOd_Cornea = $tmp;}  }
		if(empty($flgSe_Corn_Os) && empty($flgSe_Corn_Os_prev) && !empty($elem_wnlCornOs)){  $tmp = $this->getExamWnlStr_fromPrvExm("Cornea", "OS"); if(!empty($tmp)){ $wnlStringOs_Cornea = $tmp;}  }
		
		list($elem_cornea_od_summary,$elem_cornea_os_summary) = $oOnload->setWnlValuesinSumm(array("wValOd"=>$wnlStringOd_Cornea,"wValOs"=>$wnlStringOs_Cornea,
																	"wOd"=>$elem_wnlCornOd,"sOd"=>$elem_cornea_od_summary,
																	"wOs"=>$elem_wnlCornOs,"sOs"=>$elem_cornea_os_summary));
		
		//Nochanged
		if(!empty($elem_se_corn)&&strpos($elem_se_corn,"=1")!==false){
			$elem_noChangeCorn=1;
		}
		
		//Archived SLE --
		if($bgColor_SLE != "bgSmoke"){
		
		$arrDivArcCmn=array();
		$oChartRecArc->setChkTbl($this->tbl);
		$arrInpArc = array("elem_sumOdCorn"=>array("cornea_od_summary",$elem_cornea_od_summary,"smof","wnlCornOd",$wnlString_Cornea,$modi_note_CornOd),
						"elem_sumOsCorn"=>array("cornea_os_summary",$elem_cornea_os_summary,"smof","wnlCornOs",$wnlString_Cornea,$modi_note_CornOs)
							);
		$arTmpRecArc = $oChartRecArc->getArcRec($arrInpArc);
		
		//Cornea --
		//OD
		if(!empty($arTmpRecArc["div"]["elem_sumOdCorn"])){
			//echo $arTmpRecArc["div"]["elem_sumOdCorn"];
			$arrDivArcCmn["Cornea"]["OD"]=$arTmpRecArc["div"]["elem_sumOdCorn"];
			$moeArc["od"]["Corn"] = $arTmpRecArc["js"]["elem_sumOdCorn"];
			$flgArcColor["od"]["Corn"] = $arTmpRecArc["css"]["elem_sumOdCorn"];
			if(!empty($arTmpRecArc["curText"]["elem_sumOdCorn"])) $elem_cornea_od_summary = $arTmpRecArc["curText"]["elem_sumOdCorn"];
		}else{
			$moeArc["od"]["Corn"] = $flgArcColor["od"]["Corn"]="";
		}
		//OS
		if(!empty($arTmpRecArc["div"]["elem_sumOsCorn"])){
			//echo $arTmpRecArc["div"]["elem_sumOsCorn"];
			$arrDivArcCmn["Cornea"]["OS"]=$arTmpRecArc["div"]["elem_sumOsCorn"];
			$moeArc["os"]["Corn"] = $arTmpRecArc["js"]["elem_sumOsCorn"];
			$flgArcColor["os"]["Corn"] = $arTmpRecArc["css"]["elem_sumOsCorn"];
			if(!empty($arTmpRecArc["curText"]["elem_sumOsCorn"])) 
				$elem_cornea_od_summary = $arTmpRecArc["curText"]["elem_sumOsCorn"];
		}else{
			$moeArc["os"]["Corn"] = $flgArcColor["os"]["Corn"]="";
		}
		//Cornea --
		
		}//
		
		//Archived SLE --
		
		$arr=array();							
		
		//if(in_array("Cornea",$arrTempProc) || in_array("All",$arrTempProc)){
		//Cornea
		$arr["subExm"][] = $oOnload->getArrExms_ms(array("enm"=>"Cornea",
											"sOd"=>$elem_cornea_od_summary,"sOs"=>$elem_cornea_os_summary,
											"fOd"=>$flgSe_Corn_Od,"fOs"=>$flgSe_Corn_Os,"pos"=>$elem_posCorn,
											//"arcJsOd"=>$moeArc["od"]["Corn"],"arcJsOs"=>$moeArc["os"]["Corn"],
											"arcCssOd"=>$flgArcColor["od"]["Corn"],"arcCssOs"=>$flgArcColor["os"]["Corn"],
											//"mnOd"=>$moeMN["od"]["Corn"],"mnOs"=>$moeMN["os"]["Corn"],
											"enm_2"=>"Corn"));
		//}
		
		//Sub Exam List
		$arr["seList"] = 	array(
					"Corn"=>array("enm"=>"Cornea","pos"=>$elem_posCorn,
						"wOd"=>$elem_wnlCornOd,"wOs"=>$elem_wnlCornOs)
					);
		$arr["nochange"] = $elem_noChangeCorn;
		$arr["bgColor"] = "".$bgColor_SLE;
		$arr["penLight"] = $elem_pen_light;		
		$arr["examdate"] = $examdate;		
		$arr["moeMN"] = $moeMN;			
		$arr["exm_flg_se"] = array($flgSe_Corn_Od,$flgSe_Corn_Os);
		
		//$arr["arrDivArcCmn"] = $arrDivArcCmn;
		$arr["arrHx"] = $arrHx;
		//---------
		
		return $arr;
	}

	function getPurgedExm(){
	
		//Get Template Procedures ---
		$arrTempProc=array("All");
		if(isset($post["artemp"])&&!empty($post["artemp"])){
			$arrTempProc=$post["artemp"];
		}			
		//Get Template Procedures ---
	
		$oOnload =  new Onload();
		$arPurge = array();
		
		$sql ="SELECT ".
			"c10.posCorn,".
			"c10.wnlCorn,".
			"c10.ncCorn,".
			"c10.cornea_od_summary, ".
			"c10.cornea_os_summary,".
			"c10.id, ".
			"c10.wnlCornOd, ".
			"c10.wnlCornOs, ".
			"c10.wnl_value_Cornea, ".
			"c10.pen_light, c10.exam_date, ".
			"c10.statusElem AS se_sle, c10.modi_note_CornArr, 
			c10.last_opr_id AS last_opr_id_sle, c10.purgerId, c10.purgeTime, c10.purgerId AS purgerId_sle, c10.purgeTime AS purgeTime_sle
			 ".
			"FROM chart_master_table c1 ".
			"INNER JOIN ".$this->tbl." c10 ON c10.form_id = c1.id AND c10.purged!='0'  ".
			"WHERE c1.id = '".$this->fid."' AND c1.patient_id='".$this->pid."' ";
			
		$rez=sqlStatement($sql);
		//------ Check in archive tables for chart_slit_lamp_exam/chart_la/chart_rv ---------
		if(!empty($GLOBALS["CHK_ARCHIVE_TABLE"])){
			if(imw_num_rows($rez) <= 0){
				$sql = str_replace($this->tbl,$this->tbl."_archive",$sql);
				$rez=sqlStatement($sql);
			}	
		}
		//-------- End ----------------------------------------------------------------------------------
		for($i=0;$row=sqlFetchArray($rez);$i++){		
			//SLE
			$elem_posCorn = assignZero($row["posCorn"]);
			$elem_wnlCorn = assignZero($row["wnlCorn"]);
			$elem_ncCorn = assignZero($row["ncCorn"]);
			$elem_wnlCornOd = assignZero($row["wnlCornOd"]);
			$elem_wnlCornOs = assignZero($row["wnlCornOs"]);
			$elem_se_corn = $row["se_sle"];
			$elem_wnl_value_Cornea = $row["wnl_value_Cornea"];
			$elem_cornea_od_summary = $row["cornea_od_summary"];
			$elem_cornea_os_summary = $row["cornea_os_summary"];
			$elem_corn_id = $row["id"];
			$elem_pen_light = $row["pen_light"];
			$examdate = wv_formatDate($row["exam_date"]);
			$modi_note_ConjArr = unserialize($row["modi_note_ConjArr"]);			
			$arrHx = array();
			if(count($modi_note_CornArr)>0 && $row["modi_note_CornArr"]!='')
			$arrHx['Cornea'] = $modi_note_CornArr;
			
			//is Change is made in new chart -----		
			$flgSe_Corn_Od = $flgSe_Corn_Os = "0";
			
			if(!isset($bgColor_SLE)){
				if(!empty($elem_se_corn)){
					$tmpArrSe = $this->se_elemStatus("SLE","0",$elem_se_corn);
					$flgSe_Corn_Od = $tmpArrSe["2"]["od"];
					$flgSe_Corn_Os = $tmpArrSe["2"]["os"];
				}
			}else{
				if(!empty($elem_se_corn_prev)){
					$tmpArrSe_prev = $this->se_elemStatus("SLE","0",$elem_se_corn_prev);
					$flgSe_Corn_Od_prev = $tmpArrSe_prev["2"]["od"];
					$flgSe_Corn_Os_prev = $tmpArrSe_prev["2"]["os"];				
				}
			}
			//is Change is made in new chart -----

			//WNL
			
			//Cornea --
			$wnlString_Cornea = !empty($elem_wnl_value_Cornea) ? $elem_wnl_value_Cornea : $this->getExamWnlStr("Cornea");
			$wnlStringOd_Cornea = $wnlStringOs_Cornea = $wnlString_Cornea; 
			
			if(empty($flgSe_Corn_Od) && empty($flgSe_Corn_Od_prev) && !empty($elem_wnlCornOd)){ $tmp = $this->getExamWnlStr_fromPrvExm("Cornea", "OD"); if(!empty($tmp)){ $wnlStringOd_Cornea = $tmp;}  }
			if(empty($flgSe_Corn_Os) && empty($flgSe_Corn_Os_prev) && !empty($elem_wnlCornOs)){  $tmp = $this->getExamWnlStr_fromPrvExm("Cornea", "OS"); if(!empty($tmp)){ $wnlStringOs_Cornea = $tmp;}  }
			
			list($elem_cornea_od_summary,$elem_cornea_os_summary) = $oOnload->setWnlValuesinSumm(array("wValOd"=>$wnlStringOd_Cornea,"wValOs"=>$wnlStringOs_Cornea,
																		"wOd"=>$elem_wnlCornOd,"sOd"=>$elem_cornea_od_summary,
																		"wOs"=>$elem_wnlCornOs,"sOs"=>$elem_cornea_os_summary));
			
			//Nochanged
			if(!empty($elem_se_corn)&&strpos($elem_se_corn,"=1")!==false){
				$elem_noChangeCorn=1;
			}
			
			$arr=array();							
			
			if(in_array("Cornea",$arrTempProc) || in_array("All",$arrTempProc)){
			//Cornea
			$arr["subExm"][] = $oOnload->getArrExms_ms(array("enm"=>"Cornea",
												"sOd"=>$elem_cornea_od_summary,"sOs"=>$elem_cornea_os_summary,
												"fOd"=>$flgSe_Corn_Od,"fOs"=>$flgSe_Corn_Os,"pos"=>$elem_posCorn,
												//"arcJsOd"=>$moeArc["od"]["Corn"],"arcJsOs"=>$moeArc["os"]["Corn"],
												"arcCssOd"=>$flgArcColor["od"]["Corn"],"arcCssOs"=>$flgArcColor["os"]["Corn"],
												//"mnOd"=>$moeMN["od"]["Corn"],"mnOs"=>$moeMN["os"]["Corn"],
												"enm_2"=>"Corn"));
			}
			
			//Sub Exam List
			$arr["seList"] = 	array(
						"Corn"=>array("enm"=>"Cornea","pos"=>$elem_posCorn,
							"wOd"=>$elem_wnlCornOd,"wOs"=>$elem_wnlCornOs)
						);
			$arr["nochange"] = $elem_noChangeCorn;
			$arr["bgColor"] = "".$bgColor_SLE;
			$arr["penLight"] = $elem_pen_light;		
			$arr["examdate"] = $examdate;		
			$arr["moeMN"] = $moeMN;			
			$arr["exm_flg_se"] = array($flgSe_Corn_Od,$flgSe_Corn_Os);
			
			//$arr["arrDivArcCmn"] = $arrDivArcCmn;
			$arr["arrHx"] = $arrHx;
			
			$arr["purgerId"] = $row["purgerId_sle"];
			$arr["purgeTime"] = $row["purgeTime_sle"];
			//---------			
			
			$arPurge[] = $arr;
		}
		return $arPurge;
		
	}

	function insertNew(){
		if(!empty($this->pid) && !empty($this->fid)){
		//WNL
		$wnl_value_Cornea = $this->getExamWnlStr("Cornea");		
		
		$sql = "INSERT INTO ".$this->tbl." (id, form_id, patient_id, exam_date, uid, wnl_value_Cornea)
				VALUES (NULL, '".$this->fid."','".$this->pid."','".wv_dt("now")."','".$this->uid."', '".sqlEscStr($wnl_value_Cornea)."') ";
		$return=sqlInsert($sql);
		
		}else{ $return=0; }
		return $return;
	}
	
	function carryForward(){
		$res = $this->getLastRecord(" c2.id ","1");
		if($res!=false){
			$Id_LF = $res["id"];
		}
		//Insert
		$insertId = $this->insertNew();
		//CopyLF
		$ignoreFlds = "form_id,exam_date,uid,statusElem,". 
					"ncCorn,".
					"ncCorn_od,".
					"ncCorn_os,".
					"wnl_value_Cornea";
		if(!empty($Id_LF)) $this->carryForwardExe($this->tbl,$insertId,$Id_LF,$ignoreFlds,"",'id');
	}
	
	function save_wnl($op="get", $arv=array()){
		$form_id = $this->fid;
		$patientId = $this->pid;
		$exmEye = $_POST["elem_exmEye"];
		
		$w = strtoupper($_POST["w"]);
		//Get Template Procedures ---
		$arrTempProc=array("All");
		if(isset($_POST["artemp"])&&!empty($_POST["artemp"])){
			//$arrTempProc=$_POST["artemp"];
			$arrTempProc = json_decode(gzuncompress(base64_decode($_POST["artemp"])));
		}			
		//Get Template Procedures ---
		
		if($op=="get"){
			$ar_ret=array();
			//
			$wnl_value_Cornea="";
			$wnl_value_Cornea_phrase="";
			$flgCarry=0;	
			
			//Fundus
			if(!$this->isRecordExists()){
				$this->carryForward();
				$flgCarry=1;
			}
			
			$cQry = "select 
					wnlCorn, ".					
					"wnlCornOd,wnlCornOs,
					".					
					"posCorn, ".					
					"cornea_od_summary,cornea_os_summary,						
					".					
					"statusElem, uid, wnl_value_Cornea, 
					id
					FROM ".$this->tbl." WHERE form_id='".$form_id."' AND patient_id='".$patientId."'  AND purged = '0' ";
			$row = sqlQuery($cQry);
			if($row == false){				
			}else{
				$ar_ret=$row;
			}
			return $ar_ret;			
		}else if($op=="set" && count($arv)){
			extract($arv);
			$elem_wnlCornOd = $wnlCornOd;
			$elem_wnlCornOs = $wnlCornOs;
			$elem_wnlCorn = $wnlCorn;
		
			$oWv = new WorkView();
			
			if(in_array("Cornea",$arrTempProc)||in_array("All",$arrTempProc)){
			if((!empty($statusElem)&&strpos($statusElem,"0")===false)||	
				(empty($cornea_od_summary) && empty($elem_wnlCornOd))||
				(empty($cornea_os_summary) && empty($elem_wnlCornOs))
				){
			//Toggle Corn
			list($elem_wnlCornOd,$elem_wnlCornOs,$elem_wnlCorn) =
									$oWv->toggleWNL($posCorn,$cornea_od_summary,$cornea_os_summary,
													$elem_wnlCornOd,$elem_wnlCornOs,$elem_wnlCorn,$exmEye);
			}
			}
			
			//Status
			$statusElem_prev=$statusElem;
			$statusElem = $this->setEyeStatus($w, $exmEye,$statusElem,0);
			
			if(empty($wnl_value_Cornea)){
				$wnl_value_Cornea=$this->getExamWnlStr("Cornea");
				$wnl_value_Cornea_phrase = ", wnl_value_Cornea='".sqlEscStr($wnl_value_Cornea)."' ";
			}
			
			//Fundus
			$sql = "UPDATE ".$this->tbl." SET  
				  wnlCorn='".$elem_wnlCorn."', ".
				  "wnlCornOd='".$elem_wnlCornOd."',
				  wnlCornOs='".$elem_wnlCornOs."',				  
				  ".
				  " exam_date='".date("Y-m-d H:i:s")."', uid='".$_SESSION["authId"]."',
				  statusElem='".$statusElem."'
				  ".				  
				  "				  
				  ";			
			$sql .= " ".$wnl_value_Cornea_phrase." ";
			
			$sql .=  "WHERE form_id='".$form_id."' AND patient_id='".$patientId."'  AND purged = '0' ";			
			$res = sqlQuery($sql);
		}
	
	}
	
	function resetVals(){
		$is_cryfd=0;
		if(!$this->isRecordExists()){
			//In
			$this->carryForward();
			$is_cryfd=1;
		}
		
		//if($this->isRecordExists()){
			$statusElem = "";
			$res1 = $this->getRecord(" id,statusElem ");
			if($res1!=false){
				$Id = $res1["id"];
				if($is_cryfd==0){$statusElem = $res1["statusElem"];}
			}

			//CopyLF
			$ignoreFlds = "form_id,exam_date,uid,patient_id,";
			if(!empty($Id)){
			
				if(empty($_POST["site"]) || $_POST["site"]=="OU"){ $statusElem = "";  }
				if($_POST["site"]=="OS"){ 
					$ignoreFlds .= "cornea_od_summary,
								cornea_od,
								wnlCornOd,
								ncCorn_od,
								"; 
					if(!empty($statusElem)){ $statusElem = str_replace("_Os=1","_Os=0",$statusElem);  }			
				}else if($_POST["site"]=="OD"){ 
					$ignoreFlds .= "cornea_os,cornea_os_summary,
								wnlCornOs,
								ncCorn_os,
								"; 
					if(!empty($statusElem)){ $statusElem = str_replace("_Od=1","_Od=0",$statusElem);  }				
				}
				if($_POST["site"]=="OD" || $_POST["site"]=="OS"){
					$ignoreFlds .= "posCorn,";
					if($is_cryfd==0){$ignoreFlds .= "wnl_value_Corn,ut_elem,";}
				}
				$ignoreFlds = trim($ignoreFlds,",");
			
			
				$this->resetValsExe($this->tbl,$Id,$ignoreFlds);
				$this->setStatus($statusElem,$this->tbl);
			}
		//}else{
			//
		//	$this->insertNew();
		//}
	}
	
	function set2PrvVals(){
		if($this->isRecordExists()){
			$res1 = $this->getRecord(" id ");
			if($res1!=false){
				$Id = $res1["id"];
			}
			
			$res = $this->getLastRecord(" c2.id ","1");		
			if($res!=false){
				$Id_LF = $res["id"];
			}

			//CopyLF
			$ignoreFlds = "form_id,exam_date,uid,statusElem,". 
						"ncCorn,".
						"ncCorn_od,".
						"ncCorn_os,".
						"wnl_value_Corn";
			if(!empty($Id_LF)&&!empty($Id)){
				$this->carryForwardExe($this->tbl,$Id,$Id_LF,$ignoreFlds);
				$this->setStatus("",$this->tbl);
			}else if(!empty($Id)){ //when no previous exam
				$this->resetVals(); //empty exam values 
			}
		}
	}
	
	function isNoChanged(){
		$res= $this->getRecord("ncCorn,statusElem");
		if($res!=false){
			if( !empty($res["statusElem"]) && strpos($res["statusElem"],"=1")!==false){
				return true;
			}
			if( !empty($res["ncCorn"]) ){
				return true;
			}		
		}
		return false;
	
	}
	
	function save_no_change(){
		$form_id = $this->fid;
		$patientId = $this->pid;
		$w = strtoupper($_POST["w"]);
		$exmEye = $_POST["elem_exmEye"];
		
		$tmpNC=0;
		$elem_ncCorn="0";			
		$elem_ncCorn_od="0";			
		$elem_ncCorn_os="0";
		
		//
		$oFE=$this; //new FundusExam($patientId,$form_id);
		if(!$oFE->isRecordExists()){
			$oFE->carryForward();
			$tmpNC=1;
		}else if(!$oFE->isNoChanged()){
			$tmpNC=1;
		}else{
			$oFE->set2PrvVals();
			$tmpNC=0;
		}
		
		// ---
		//Set NC
		if($tmpNC==1){
			if($exmEye=="OU"){				
				$elem_ncCorn="1";
				$elem_ncCorn_od="1";
				$elem_ncCorn_os="1";
			}else if($exmEye=="OD"){				
				$elem_ncCorn_od="1";
			}else if($exmEye=="OS"){
				$elem_ncCorn_os="1";
			}
		}			
		// ---
		
		//Get status string --
		$statusElem="";
		if($elem_ncCorn_od==1||$elem_ncCorn_os==1){$statusElem=$this->se_elemStatus($this->examName,"1","",$elem_ncCorn_od,$elem_ncCorn_os,0);}
		//Get status--		
		
		//
		$sql = "UPDATE ".$this->tbl."
			  SET			  
			  ncCorn = '".$elem_ncCorn."',
			  ncCorn_od='".$elem_ncCorn_od."', ncCorn_os='".$elem_ncCorn_os."',			  
			  exam_date='".wv_dt("now")."', uid='".$_SESSION["authId"]."',
			  statusElem='".$statusElem."'
			 WHERE form_id='".$form_id."' AND patient_id='".$patientId."'  AND purged = '0' ";
		$res = sqlQuery($sql);
	}
	
	function getSubExamInfo(){
		$arr=array();
		$arr["db"]["xmlOd"] = "cornea_od";
		$arr["db"]["xmlOs"] = "cornea_os";
		$arr["db"]["wnlSE"] = "wnlCorn";
		$arr["db"]["wnlOd"] = "wnlCornOd";
		$arr["db"]["wnlOs"] = "wnlCornOs";
		$arr["db"]["posSE"] = "posCorn";
		$arr["db"]["summOd"] = "cornea_od_summary";
		$arr["db"]["summOs"] = "cornea_os_summary";
		$arr["divSe"] = "2";
		
		$oExamXml = new ExamXml();
		$tmp = $oExamXml->getExamXmlFiles("SLE", $this->examName);
		$arr["xmlFile"]["OD"] = $tmp["od"];
		$arr["xmlFile"]["OS"] = $tmp["os"];
		return $arr;
	}
	
	function smartChart($arr){
		if(!$this->isRecordExists()){
			//In
			$this->carryForward();
		}
		$arrSEInfo = $this->getSubExamInfo();
		$res=$this->getRecord();
		if($res!=false){
			$xmlOd = $res[$arrSEInfo["db"]["xmlOd"]];
			$xmlOs = $res[$arrSEInfo["db"]["xmlOs"]];
			$wnlSE = $res[$arrSEInfo["db"]["wnlSE"]];
			$wnlOd=$res[$arrSEInfo["db"]["wnlOd"]];
			$wnlOs=$res[$arrSEInfo["db"]["wnlOs"]];
			$posSE=$res[$arrSEInfo["db"]["posSE"]];
			$statusElem = $res["statusElem"];	
			$ut_elem=$res["ut_elem"];

		}else{
			$desc = "";
		}
		
		//
		$idSE = $arrSEInfo["divSe"];
		$statusElemCur=$this->getCurStatusFromPost($arr, "elem_chng_div".$idSE."_Od", "elem_chng_div".$idSE."_Os", $statusElem);
		
		//Edit Xml --
		$arrIn["xOd"]=$xmlOd;
		$arrIn["xOs"]=$xmlOs;
		$arrIn["xFileOd"]=$arrSEInfo["xmlFile"]["OD"];
		$arrIn["xFileOs"]=$arrSEInfo["xmlFile"]["OS"];
		$arrIn["arrSc"] = $arr;
		$arrIn["ut_elem"] = $ut_elem;		
		$arrOut= $this->editXml($arrIn);

		$xmlOd=$arrOut["xOd"];
		$xmlOs=$arrOut["xOs"];
		$siteExm=$arrOut["siteExm"];
		$desc.=$arrOut["desc"];
		$sumOd = $arrOut["sumOd"];
		$sumOs = $arrOut["sumOs"];
		$arrElemEd=$arrOut["elemEd"];
		$ut_elem = $arrOut["ut_elem"];
		//Edit Xml --

		//Set WNL --
		list($wnlSE,$wnlOd,$wnlOs,$pos,$wnl,$posSE) = $this->setExmWnlPos($sumOd,$sumOs,$wnlSE,$wnlOd,$wnlOs,$pos,$wnl,$posSE);
		//Set WNL --

		//Set Status Fields--
		if(!empty($statusElemCur)){ 
			$statusElem=$statusElemCur;			
		}else{		
			$statusElem = $this->setStatusElem($arrSEInfo["divSe"],$statusElem,$sumOd,$sumOs,$siteExm);
		}
		//Set Status Fields--
		
		//UTteleme --
		$elem_utElems_cur="";
		if(count($arrElemEd)>0){
			$elem_utElems_cur=implode(",", $arrElemEd);
			$elem_utElems_cur=$elem_utElems_cur.",";
		}
		$ut_elem = $this->getUTElemString($ut_elem,$elem_utElems_cur);
		//UTteleme --		

		//Update Records--
		$sql="UPDATE ".$this->tbl." ".
			"SET ".
			"exam_date = '".wv_dt("now")."', ".		

			$arrSEInfo["db"]["xmlOd"]."  = '".sqlEscStr($xmlOd)."', ".
			$arrSEInfo["db"]["xmlOs"]." = '".sqlEscStr($xmlOs)."', ".
			$arrSEInfo["db"]["summOd"]." = '".sqlEscStr($sumOd)."', ".
			$arrSEInfo["db"]["summOs"]." = '".sqlEscStr($sumOs)."', ".
			$arrSEInfo["db"]["wnlOd"]."='".$wnlOd."', ".
			$arrSEInfo["db"]["wnlOs"]."='".$wnlOs."', ".
			$arrSEInfo["db"]["wnlSE"]."='".$wnlSE."', ".
			$arrSEInfo["db"]["posSE"]."='".$posSE."', ".
			
			"uid = '".$this->uid."', ".
			"statusElem = '".sqlEscStr($statusElem)."', ".
			"ut_elem = '".sqlEscStr($ut_elem)."' ".
			"WHERE form_id = '".$this->fid."' AND patient_id = '".$this->pid."' AND purged = '0' ";
		$res=sqlQuery($sql);
	
	}
	
}


?>