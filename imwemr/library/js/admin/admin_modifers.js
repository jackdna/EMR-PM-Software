...
var totalRecords	   = 0;
var formObjects		   = new Array('modifiers_id','modifier_code');
 ...
 //$('#result_set').html(r+'<hr>end of Response');exit;
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='modifiers_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='modifier_code'){
	...
	top.show_loading_image('hide');
	type_options = '';
	type_options += '<option value="Active">Active</option>';
	type_options += '<option value="Inactive">Inactive</option>';
	$('#status').html(type_options);
...
function saveFormData(){
	...
	msg="";
	if($.trim($("#modifier_code").val())==""){
		msg+="<br> &bull; Modifier Code";
	}
	if($.trim($("#mod_prac_code").val())==""){
		msg+="<br> &bull; Practice Code";
	}
	if($.trim($("#mod_description").val())==""){
...
function fillEditData(pkId){
	...
	for(i=0;i<e.length;i++){
		o = e[i];
		if($.inArray(o.modifier_code,formObjects)){
			on	= o.name;
...
var ar = [["add_new","Add New","top.fmain.addNew();"],["mod_del","Delete","top.fmain.deleteSelectet();"]];
top.btn_show("ADMN",ar);
$(document).ready(function(){
	LoadResultSet();
	check_checkboxes();
	set_header_title('Modifier');
});
