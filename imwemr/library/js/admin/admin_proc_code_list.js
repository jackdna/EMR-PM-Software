...
var totalRecords	   = 0;
var formObjects		   = new Array('proc_code_id','proc_code');
function LoadResultSet(p,f,s,so,currLink){
	...
	ajaxURL = "proc_code_list_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
	  success: function(r) {
		showRecords(r);
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='proc_code_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='proc_code'){
					row	+= '<td class="leftborder" onclick="addNew(1,\''+pkId+'\');">'+tdVal+'</td>';
				}
				if(y=='proc_code_description'){
...
function saveFormData(){
	...
	msg="";
	if($.trim($("#proc_code").val())==""){
		msg+="<br> &bull; Proc Code";
	}
	if($.trim($("#proc_code_description").val())==""){
		msg+="<br> &bull; Proc Code Description";
	}
	...
	$.ajax({
		type: "POST",
		url: "proc_code_list_ajax.php",
		data: frm_data,
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert("Proc Code Already Exist.");
				return false;
			}
...
function fillEditData(pkId){
	...
	for(i=0;i<e.length;i++){
		o = e[i];
		if($.inArray(o.proc_code,formObjects)){
			on	= o.name;
