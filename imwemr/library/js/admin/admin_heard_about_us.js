...
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	top.show_loading_image('show','300', 'Loading Heard About US...');
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){no_record='no';
			...
			for(y in s){
				tdVal = s[y];
				if(y=='heard_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='heard_options'){
					row	+= '<td onclick="addNew(1,\''+pkId+'\');">&nbsp;'+tdVal+'</td>';
				}
				if(y=='status'){
					if(tdVal=='1'){tdVal="Inactive";}else{tdVal="Active";}
					row	+= '<td onclick="addNew(1,\''+pkId+'\');">&nbsp;'+tdVal+'</td>';
				}
			}
			totalRecords++;
...
function saveFormData(){
	...
	if($.trim($('#heard_options').val())==""){
		top.fAlert("&bull; Heard type is required.<br>");
		top.show_loading_image('hide');
		return false;
	}
	$.ajax({
...
$(document).ready(function(){
	LoadResultSet();
	check_checkboxes();
	set_header_title('Heard About Us');	
});
