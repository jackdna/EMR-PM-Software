...
var totalRecords	   = 0;
var formObjects		   = new Array('id','name');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "lens_used_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
	  success: function(r) {
		showRecords(r);
...
function saveFormData(){
	...
	if($.trim($('#name').val())==""){
		top.fAlert("Enter Lens used");
		top.show_loading_image('hide');
		return false;
	}
	$.ajax({
		type: "POST",
		url: "lens_used_ajax.php",
		data: frm_data,
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert('Lens used "<b>'+$('#name').val()+'</b>" already exist.');		
				return false;
			}
