...
var totalRecords	   = 0;
var formObjects		   = new Array('d_id','d_code');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "discount_code_list_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='d_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='d_code'){
...
function saveFormData(){
	...
	msg="";
	if($.trim($("#d_code").val())==""){
		msg+=" &bull; Enter the Discount Code";
	}
	if($.trim($("#d_desc").val())==""){
	...
	$.ajax({
		type: "POST",
		url: "discount_code_list_ajax.php",
		data: frm_data,
		success: function(d){
			top.show_loading_image('hide');
			if(d=='enter_unique'){
...
LoadResultSet();
check_checkboxes();
set_header_title('Discount Codes');
});
