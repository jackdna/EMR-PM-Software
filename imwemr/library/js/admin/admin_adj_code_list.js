...
var totalRecords	   = 0;
var formObjects		   = new Array('a_id','a_code');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "adj_code_list_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
	  success: function(r) {
		showRecords(r);
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='a_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='a_code'){
...
function saveFormData(){
	...
	$.ajax({
		type: "POST",
		url: "adj_code_list_ajax.php",
		data: frm_data,
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert("Adj Code Already Exist.");
				return false;
			 }
...
function fillEditData(pkId){
	...
	for(i=0;i<e.length;i++){
		o = e[i];
		if($.inArray(o.a_code,formObjects)){
			on	= o.name;
...
LoadResultSet();
check_checkboxes();
set_header_title('Adjustment Codes');
});
