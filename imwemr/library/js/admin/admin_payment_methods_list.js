...
var totalRecords	   = 0;
var formObjects		   = new Array('pm_id','pm_name');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "payment_methods_list_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
	  success: function(r) {//document.write(r); ///*dataType: "json",*/
		showRecords(r);
	  }
	});
}
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				...
				if(s['default_method'] > 0){
					if(y=='pm_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"></td>';}
					rowData[y] = tdVal;
					if(y=='pm_name'){
						row	+= '<td>'+tdVal+'</td>';
					}
				}else{
					if(y=='pm_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
					rowData[y] = tdVal;
					if(y=='pm_name'){
...
function saveFormData(){
	...
	msg="";
	var check_pattern = /-/;
	var check_exists = check_pattern.test($.trim($("#pm_name").val()));
	if($.trim($("#pm_name").val())==""){
		msg+=" &bull; Enter the Payment Method.";
	}else if(check_exists==true){
		msg+=" &bull; Hyphen (-) is not Allowed.";
	}
	if(msg!=""){
		top.show_loading_image('hide');	
		var amsg="<b>Please Correct the Following :-</b><br>"+msg;
		top.fAlert(amsg);
		return false;
	}
	$.ajax({
		type: "POST",
		url: "payment_methods_list_ajax.php",
		data: frm_data,
		success: function(d){
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert("Payment Method Already Exist.");
				return false;
			}
...
LoadResultSet();
check_checkboxes();
set_header_title('Payment Methods');
});
