...
var totalRecords	   = 0;
var formObjects		   = new Array('pos_id','pos_code','pos_prac_code','pos_description','facility','admit_discharge','nocopay','category_code','status');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	top.show_loading_image('hide');
	top.show_loading_image('show','300', 'Loading POS codes...');
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='pos_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="chk_sel" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y!='pos_id' && y!='facility' && y!='facilityName' && y!='pos_description'){
					row	+= '<td onclick="addNew(1,\''+pkId+'\');">'+tdVal+'</td>';
				}else if(y=='facilityName' || y=='pos_description'){
					row	+= '<td onclick="addNew(1,\''+pkId+'\');">'+tdVal+'</td>';
				}
			}
			totalRecords++;
	...
	for(x in facilities){
		ff = facilities[x];
		fac_options += '<option value="'+ff.id+'">'+ff.name+'</option>';
	}			
	$('#facility').html(fac_options);
}
...
function saveFormData(){
	...
	msg="";
	if($.trim($("#pos_code").val())==""){
		msg+=" &bull; Enter the POS Code";
	}
	if($.trim($("#pos_prac_code").val())==""){
		msg+="<br> &bull; Enter the Practice Code";
	}
	if($.trim($("#pos_description").val())==""){
