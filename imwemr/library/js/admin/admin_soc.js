...
var totalRecords	   = 0;
var formObjects		   = new Array('id','soc');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "soc_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
		success: function(r) {
			showRecords(r);
...
function saveFormData(){
	...
	if($.trim($('#soc').val())==""){
		top.fAlert("Please enter standards of care");
		top.show_loading_image('hide');
		return false;
	}
	$.ajax({
		...
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert('Standards of Care "<b>'+$('#soc').val()+'</b>" already exist.');		
				return false;
			}
...
$(document).ready(function(){
	LoadResultSet();
	check_checkboxes();
	set_header_title('Standards of Care');
});
