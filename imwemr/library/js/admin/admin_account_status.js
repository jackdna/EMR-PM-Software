...
function showRecords(r){
	...
	top.show_loading_image('hide');
	type_options = '';
	type_options += '<option value="">Normal</option>';
	type_options += '<option value="collection">Collection</option>';
	$('#status_type').html(type_options);
}
...
function saveFormData(){
	var status_nm = $("#status_name").val();
	if($.trim(status_nm)=="")
	{
		top.fAlert('Please Enter the Account Status Name');
		return false;	
	}
	top.show_loading_image('hide');
...
function fillEditData(pkId){
	...
	for(i=0;i<e.length;i++){
		o = e[i];
		if($.inArray(o.status_name,formObjects)){
			on = o.name;
