...
var totalRecords	   = 0;
var formObjects		   = new Array('case_id','case_name');
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='case_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='case_name'){
...
function fillEditData(pkId){
	...
	for(i=0;i<e.length;i++){
		o = e[i];
		if($.inArray(o.case_name,formObjects)){
			on	= o.name;
