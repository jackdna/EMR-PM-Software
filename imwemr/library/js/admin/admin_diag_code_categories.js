...
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "diag_categories_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
 ...
 //$('#result_set').html(r+'<hr>end of Response');exit;
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='diag_cat_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='category'){
...
function saveFormData(){
	var cat_nm = $("#category").val();
	if($.trim(cat_nm)=="")
	{
		top.fAlert('Please Enter the Catagory Name');
		return false;	
	}
	top.show_loading_image('hide');
...
function daigList(){
	...
	top.fmain.location.href = "../admin/billing/diagnosis_code/index.php";
}
