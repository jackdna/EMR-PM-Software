...
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "visit_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
		success: function(r) {
			showRecords(r);
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){no_record='no';
			...
			for(y in s){
				tdVal = s[y];
				if(y=='tech_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="fu_id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='ptVisit'){
...
function saveFormData(){
	...
	if($.trim($('#ptVisit').val())==""){
		top.fAlert("Please enter the visit");
		top.show_loading_image('hide');
		return false;
	}
	$.ajax({
		...
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert('Visit "<b>'+$('#ptVisit').val()+'</b>" already exist.');		
				return false;
			}
