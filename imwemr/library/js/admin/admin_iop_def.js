...
var totalRecords	   = 0;
var formObjects		   = new Array('id','def_mthd');
function LoadResultSet(p,f,s,so,currLink){
	...
	ajaxURL = "iop_def_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
		success: function(r) {
			showRecords(r);
...
function saveFormData(){
	...
	var msg = "";
	if($.trim($('#def_mthd').val())==""){
		msg += "<br> - Default IOP Method";
	}
	if($.trim($('#phy_id').val())==""){
		msg += "<br> - Provider";
	}
	...
	$.ajax({
		type: "POST",
		url: "iop_def_ajax.php",
		data: frm_data,
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert('Default IOP Methos for "<b>'+$('#phy_id option:selected').text()+'</b>" already exist.');
				return false;
			}
