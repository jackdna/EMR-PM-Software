...
var totalRecords	   = 0;
var formObjects		   = new Array('scp_id','allergie_name');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	top.show_loading_image('hide');
	top.show_loading_image('show','300', 'Loading SCP Reasons...');
	...
	ajaxURL = "SCPReasons_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
	  success: function(r){
		showRecords(r);
...
//$('#result_set').html(r+'<hr>end of Response');exit;
	...
	if(r != null){
		...
		for(x in result){no_record='no';
			...
			for(y in s){
				tdVal = s[y];
				if(y=='scp_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='reason_code'){
...
function saveFormData(){
	...
	if($.trim($('#reason_code').val())==""){
		top.fAlert("Enter The reason code");
		top.show_loading_image('hide');
		return false;
	}
	$.ajax({
		type: "POST",
		url: "SCPReasons_ajax.php",
		data: frm_data,
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert('Reason code "<b>'+$('#reason_code').val()+'</b>" already exist.');		
				return false;
			}
