...
var totalRecords	   = 0;
var formObjects		   = new Array('DepartmentId','DepartmentCode');
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){
			...
			for(y in s){
				tdVal = s[y];
				if(y=='DepartmentId'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='DepartmentCode'){
...
function saveFormData(){
	...
	if($.trim($("#DepartmentCode").val())==""){
		msg+="<br> &bull; Department Code";
	}
	if($.trim($("#DepartmentDesc").val())==""){
		msg+="<br> &bull; Department Desc";
	}
...
function fillEditData(pkId){
	...
	for(i=0;i<e.length;i++){
		o = e[i];
		if($.inArray(o.DepartmentCode,formObjects)){
			on	= o.name;
...
var ar = [["add_new","Add New","top.fmain.addNew();"],["dep_del","Delete","top.fmain.deleteSelectet();"]];
top.btn_show("ADMN",ar);
$(document).ready(function(){
	LoadResultSet();
	check_checkboxes();
	set_header_title('Department');
});
