...
var totalRecords	   = 0;
var formObjects		   = new Array('allergies_id','allergie_name');
function LoadResultSet(p,f,s,so,currLink,alpha,page,record_limit,searchStr,cont_num){//p=practice code, f=fac code, s=status, so=sort by;
	...
	top.show_loading_image('show','300', 'Loading Allergies...');
...
function showRecords(r,cont_num){
	...
	if(r != null){
		...
		for(x in result){no_record='no';
			...
			for(y in s){
				tdVal = s[y];
				if(y=='allergies_id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='allergie_name'){
...
function saveFormData(){
	...
	if($.trim($('#allergie_name').val())==""){
		top.fAlert("Enter The allergy name");
		top.show_loading_image('hide');
		return false;
	}
	$.ajax({
...
$(document).ready(function(){
	check_checkboxes();
	set_header_title('Allergies');
	
	$("#search").keypress(function (evt){
			if(evt.keyCode==13){	srh_records();}
	});
