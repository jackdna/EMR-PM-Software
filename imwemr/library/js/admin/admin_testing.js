...
var totalRecords	   = 0;
var formObjects		   = new Array('id','testing_nm');
function LoadResultSet(p,f,s,so,currLink){//p=practice code, f=fac code, s=status, so=sort by;
	...
	ajaxURL = "testing_ajax.php?task=show_list"+s_url+p_url+f_url+so_url;
	$.ajax({
	  url: ajaxURL,
		success: function(r) {
			showRecords(r);
...
function showRecords(r){
	...
	if(r != null){
		...
		for(x in result){no_record='no';
			...
			for(y in s){
				tdVal = s[y];
				if(y=='id'){pkId = tdVal; row += '<td style="width:20px; padding-left:10px;"><div class="checkbox"><input type="checkbox" name="fu_id" id="'+tdVal+'" class="chk_sel" value="'+pkId+'"><label for="'+tdVal+'"></label></div></td>';}
				rowData[y] = tdVal;
				if(y=='testing_nm'){
...
function saveFormData(){
	...
	if($.trim($('#testing_nm').val())==""){
		top.fAlert("Please enter testing");
		top.show_loading_image('hide');
		return false;
	}
	$.ajax({
		...
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='enter_unique'){
				top.fAlert('Testing "<b>'+$('#testing_nm').val()+'</b>" already exist.');		
				return false;
			}
...
function deleteModifiers(pos_id) {
	...
	$.ajax({
		...
		success: function(d) {
			top.show_loading_image('hide');
			if(d=='1'){top.alert_notification_show('Record Deleted'); LoadResultSet(); $("#chk_sel_all").prop("checked", false);}
