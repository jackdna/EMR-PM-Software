				...
				for (var i = 0; i < options.calendars; i++) {
					...
					switch (tblCal[0].className) {
						...
						case 'datepickerViewYears':
							left = (date.getFullYear()-6);
							right = (date.getFullYear()+5);
							if(right > now.getFullYear())
							right = now.getFullYear();
							dow = left  + ' - ' + right
							break;
					} 
					...
					for ( var j = 0; j < 12; j++) {//alert(dow + j);break;
						if(dow + j <= now.getFullYear())
				...
				if (el.is('a')) {
					...
					if (parentEl.is('th')) {
						if (parentEl.hasClass('datepickerWeek') && options.mode == 'range' && !parentEl.next().hasClass('datepickerDisabled')) {
							...
							switch (tblEl.get(0).className) {
								...
								case 'datepickerViewMonths':
									...
									el.find('span').text((tmp.getFullYear()-6) + ' - ' + (tmp.getFullYear()));
									break;
								case 'datepickerViewYears':
							...
							switch (tblEl.get(0).className) {
								case 'datepickerViewDays':
									next = 1 ;
									if(options.current.getFullYear() >= now.getFullYear() && options.current.getMonth()+1 > now.getMonth())
									next = 0;
									options.current.addMonths(parentEl.hasClass('datepickerGoPrev') ? -1 : next);
									break;
								case 'datepickerViewMonths':
									next = 1 ;
									if(options.current.getFullYear() >= now.getFullYear())
									next = 0;
									options.current.addYears(parentEl.hasClass('datepickerGoPrev') ? -1 : next);
									break;
								case 'datepickerViewYears':
									next = 12 ;
									if(options.current.getFullYear() >= now.getFullYear())
									next = 0;
									options.current.addYears(parentEl.hasClass('datepickerGoPrev') ? -12 : next);
									break;
							}
							fillIt = true;
						}
						...
						switch (tblEl.get(0).className) {
							case 'datepickerViewMonths':
								options.currentDate = '';
								options.currentMonth = parseInt(tblEl.find('tbody.datepickerMonths td').index(parentEl))+1;
								options.current.setMonth(tblEl.find('tbody.datepickerMonths td').index(parentEl));
								options.current.setFullYear(parseInt(tblEl.find('thead th.datepickerMonth span').text(), 10));
								options.current.addMonths(Math.floor(options.calendars/2) - tblIndex);
								tblEl.get(0).className = 'datepickerViewDays';
								//alert(options.current.getFullYear())
								/*if(parseInt(tblEl.find('tbody.datepickerMonths td').index(parentEl))+1 > parseInt(now.getMonth())+1){
									alert("Can't select future date")
									options.currentMonth = '';
									return false;
								}else if(options.currentYear!='' && options.currentYear <= now.getFullYear() && options.current.getFullYear()<= now.getFullYear()){
									options.currentMonth = parseInt(tblEl.find('tbody.datepickerMonths td').index(parentEl))+1;
								}
								else{
									alert("Can't select future date2")
									options.currentMonth = '';
									options.currentYear = '';
								}
*/
							...
							case 'datepickerViewYears':
								options.currentDate = '';
								options.currentMonth = '';
								/*if(parseInt(el.text(), 10) > now.getFullYear()){
									alert("Can't select future date")
									options.currentYear = '';
									return false;
								}
								else*/
								options.currentYear = parseInt(el.text(), 10);
		...
		return {
			init: function(options){
                ...
                if(typeof(options.current)==='undefined')options.current="";
...
(function($){
		var initLayout = function() {
			...
			if(date_global_format == "null" || date_global_format == "undefined")
			{
				GLOBALDATEFORMAT = "";
			}
			else
			{
				GLOBALDATEFORMAT = date_global_format;
			}
			
			top.date_format = GLOBALDATEFORMAT;
		...
		$('.date-pick1').DatePicker({
				...
				onChange: function(formated, date){
					...
					if(options.current.getFullYear() > now.getFullYear() && options.current.getMonth() > now.getMonth()){
						...
						return false;
							...
							if(options.currentYear != ''){
									arrDate[2] = options.current.getFullYear();
									arrDate[2] = (arrDate[2]<10)?'0'+arrDate[2]:arrDate[2];
									if(options.currentMonth != ''){
											if(options.current.getFullYear() < now.getFullYear() || (options.current.getFullYear() == now.getFullYear() && options.current.getMonth() <= now.getMonth())){
													selMonth = parseInt(options.current.getMonth())+1;
													if(selMonth != ''){
														if(top.date_format == "mm-dd-yyyy" || top.date_format == "" || typeof(top.date_format) == "undefined"){
															arrDate[0] = (selMonth<10)?'0'+selMonth:selMonth;
															if(options.currentDate != ''){
																arrDate[1] = options.currentDate;
																arrDate[1] = (arrDate[1]<10)?'0'+arrDate[1]:arrDate[1];
															}
														}else if (top.date_format == "dd-mm-yyyy"){
															arrDate[1] = (selMonth<10)?'0'+selMonth:selMonth;
															if(options.currentDate != ''){
																arrDate[0] = options.currentDate;
									...
									arrDate = jQuery.grep(arrDate,function(n){if(n!='' && n!='undefined') return n;});
									$("#"+eleID).val(arrDate.join('-'));
								}else
									$("#"+eleID).val(formated);
						insertSxProIdVizChange('',ele,event,document.getElementById('sg_id'+tab))			
				},
