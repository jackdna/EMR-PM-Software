...
function getSchData(forceRefresh1){
	...
	if(typeof(page_loader2)=='number') clearInterval(page_loader2);
	if(typeof(forceRefresh1)=='undefined') forceRefresh1 = false;
	...
	if(fac != ""){
		url_var = "../remote/remote_appt_data.php?fac="+fac+"&prov="+prov+"&local_tz="+encodeURI(local_tz);
		IAmBusyNow = true;
		$.ajax({
			type: "POST",
			url: url_var,
			...
			success: function(xml){
						...
						*/
						...
						$(xml).find("pt").each(function(){
							...
							if(typeof(IMM_LIST_NOSHOW_APPTS)!='undefined' && IMM_LIST_NOSHOW_APPTS==0 && appt_status == '3'){
								return true;
							}
							
							pt_id			= $(this).find("pid").text();
							pt_name			= $(this).find("name").text();
							pt_name			= pt_name.JSstripSlashes();
							if(view_mode!='private' && pt_name!=''){//Robinson, Judith M
							...
							scribeortech	= $(this).find("scribeortech").text();
							...
							if(pt_with=='11' || pt_with=='19' || pt_with=='13'){pt_with = 3;}
							...
							if(roomopcolor.length==7) roomopcolor = roomopcolor.substr(1,6);
							waitStartFrm	= $(this).find("locCurrTime").text();
							waitStartFrm1	= $(this).find("locCurrTime1").text();
							if(waitStartFrm1=='') waitStartFrm1 = waitStartFrm;
							...
							//coloring
							var color_class = ""; var ReadyForClass = "";
							...
							if(prov != '' && prov != '0' && prov == doc_id && fac == sch_fac && baseName!='main_rooms.php'){
								if(sch_ci_time=='N/A'){
									...
									scheduled_pts_val += '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+sno+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" onDblclick="LoadWorkView(\''+pt_id+'\')" title="Patient At a Glance"></span></td><td>'+sch_st_time+'</td><td>';
									if(appt_status == '3'){scheduled_pts_val += '<font color="#ff0000"><b title="No Show"><i>NS</i></b></font> ';}
									scheduled_pts_val += pt_name+'</td><td class="private">'+sch_proc+'</td><td>'+doc_name+'</td><td class="message_div private">'+sch_msg_text+'&nbsp;</td></tr>';
									sno++;
								}
								if(appt_status == '13' && pt_with!='6'){//patient is checked-in.
									if((room == "" || roomoptype=="") && waitingPt != "Y" && sch_ci_time != "N/A" && (pt_with == '' || pt_with == '0')){
										...
										checked_in_pts_val += '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snochin+'<span class="icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td class="">'+sch_ci_time+'<br>('+sch_st_time+')'+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+'</td><td class="private">'+sch_proc+'</td><td>'+doc_name+'</td><td class="message_div private">'+checked_in_msg_text+'&nbsp;</td><td class="showTime private">&nbsp;</td></tr>';
										snochin++;
										str_chk_in += pt_name+"; ";
									}else if((waitingPt != "Y" && room != "" && (roomoptype == "Physician" || roomoptype == "Test")  && pt_with!='2' && pt_with!='3' && pt_with!='4' && pt_with!='5') || pt_with=='1'){
										if(pt_with=='1'){ReadyForClass=' icon16_readyforP';}else{ReadyForClass='';}
										if(pt_with=='6'){DoneMarked='<span class="icon20 icon20_done"></span>';}else{DoneMarked='';}
										ready4doc_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
										str_doc_pt_val = '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snophy+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td class="'+ReadyForClass+'">'+sch_ci_time+'<br>('+sch_st_time+')'+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+DoneMarked+'<span class="span_priority">'+pt_priority+'</span></td><td class="private">'+sch_proc+'</td><td>'+room+ready_for_status_text+'&nbsp;</td><td>'+doc_name+'</td><td>'+scribeortech+'</td><td class="message_div private">'+ready4doc_msg_text+'&nbsp;</td><td class="showTime private"></td></tr>';
										if(pt_with=='1'){
										...
										if(pt_with=='2'){ReadyForClass=' icon16_readyforT';}else{ReadyForClass='';}
										if(pt_with=='6'){DoneMarked='<span class="icon20 icon20_done"></span>';}else{DoneMarked='';}
										techactive_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
										str_tech_pt_val = '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snotech+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td class="'+ReadyForClass+'">'+sch_ci_time+'<br>('+sch_st_time+')'+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+DoneMarked+'<span class="span_priority">'+pt_priority+'</span></td><td class="private">'+sch_proc+'</td><td>'+room+ready_for_status_text+'&nbsp;</td><td>'+doc_name+'</td><td>'+roomopnm+'</td><td class="message_div private">'+techactive_msg_text+'&nbsp;</td><td class="showTime private">&nbsp;</td></tr>';
										if(pt_with=='2'){
										...
										if(waitingIcon=="icon16_dilation"){dilationTime = '<span style="display:none" class="pickDTime">'+waitStartFrm1+'</span>';dilationElapsed='<span class="greenText"></span>';}else{dilationTime='<span style="display:none" class="pickDTime"></span>'; dilationElapsed='';}
										if(pt_with=='3'){ReadyForClass=' icon16_readyfor';}else{ReadyForClass='';}
										waitingpt_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
										waiting_patients_val += '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snowaitp+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td>'+dilationTime+ready_for_status_text+'<span class="icon16 '+waitingIcon+'"></span><span class="showDTime">'+dilationElapsed+'</span></td><td>'+sch_st_time+'</td><td class="'+ReadyForClass+'">'+sch_ci_time+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+'<span class="span_priority">'+pt_priority+'</span></td><td class="private">'+sch_proc+'</td><td>'+doc_name+'&nbsp;</td><td>'+roomopnm+'</td><td class="message_div private">'+waitingpt_msg_text+'&nbsp;</td><td class="showTime private">&nbsp;</td></tr>';
										str_chk_in += pt_name+"; ";
										snowaitp++;
									}
								}
								...
								if(sch_ci_time == 'N/A'){
									if((prov != '' && prov != '0' && prov == doc_id && baseName=='main_rooms.php') || ((prov == '' || prov == '0') && (baseName=='main_rooms.php' || baseName=='main.php'))){
										sch_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
										scheduled_pts_val += '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+sno+'<span class="icon20 icon20_pag" onDblclick="LoadWorkView(\''+pt_id+'\')" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span></td><td>'+sch_st_time+'</td><td>';
										if(appt_status == '3'){scheduled_pts_val += '<font color="#ff0000"><b title="No Show"><i>NS</i></b></font> ';}
										scheduled_pts_val += pt_name+'</td><td class="private">'+sch_proc+'</td><td>'+doc_name+'</td><td class="message_div private">'+sch_msg_text+'&nbsp;</td></tr>';
										sno++;
									}
								...
								if(appt_status == '13' && pt_with!='6'){//patient is checked-in.
									if((room == "" || (roomoptype == "" && baseName!='main_rooms.php') || (room=="N/A" && baseName=='main_rooms.php')) && waitingPt != "Y" && sch_ci_time != "N/A" && (pt_with == '' || pt_with == '0')){
										if((prov != '' && prov != '0' && prov == doc_id && baseName=='main_rooms.php') || ((prov == '' || prov == '0') && (baseName=='main_rooms.php' || baseName=='main.php'))){
											if(pt_priority == '' || pt_priority == '0'){span_priority_html = '';}
											else{span_priority_html = '<span class="span_priority">'+pt_priority+'</span>';}
											checked_in_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
											checked_in_pts_val += '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snochin+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td>'+sch_ci_time+'<br>('+sch_st_time+')'+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+span_priority_html+'</td><td class="private">'+sch_proc+'</td><td>'+doc_name+'</td><td class="message_div private">'+checked_in_msg_text+'&nbsp;</td><td class="showTime private">&nbsp;</td></tr>';
											snochin++;
											str_chk_in += pt_name+"; ";
										}
										...
										ready4doc_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
										str_doc_pt_val = '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snophy+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td class="'+ReadyForClass+'">'+sch_ci_time+'<br>('+sch_st_time+')'+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+DoneMarked+'<span class="span_priority">'+pt_priority+'</span></td><td class="private">'+sch_proc+'</td><td>'+room+'<br>'+ready_for_status_text+'</td><td>'+doc_name+'</td><td>'+scribeortech+'</td><td class="message_div private">'+ready4doc_msg_text+'&nbsp;</td><td class="showTime private"></td></tr>';
										if(pt_with=='1'){
											waiting_ready_for_doc_pts_val += str_doc_pt_val;
										}else{
											ready_for_doc_pts_val += str_doc_pt_val;
										...
										techactive_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
										str_tech_pt_val = '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snotech+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td class="'+ReadyForClass+'">'+sch_ci_time+'<br>('+sch_st_time+')'+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+DoneMarked+'<span class="span_priority">'+pt_priority+'</span></td><td class="private">'+sch_proc+'</td><td>'+room+'<br>'+ready_for_status_text+'</td><td>'+doc_name+'</td><td>'+roomopnm+'</td><td class="message_div private">'+techactive_msg_text+'&nbsp;</td><td class="showTime private">&nbsp;</td></tr>';
										if(pt_with=='2'){
											waiting_tech_active_list_val += str_tech_pt_val;
										}else{
											tech_active_list_val += str_tech_pt_val;
										...
										if(baseName!='main_rooms.php'){
											waiting_patients_val += '<tr sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td>'+snowaitp+'<span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span><span class="pickTime hide">'+sch_ci_time1+'</span></td><td>'+dilationTime+ready_for_status_text+'<span class="icon16 '+waitingIcon+'"></span><span class="showDTime">'+dilationElapsed+'</span></td><td>'+sch_st_time+'</td><td class="'+ReadyForClass+'">'+sch_ci_time+'</td><td class="pt_nameid_td" id="nametd'+sch_id+'">'+pt_name+'<span class="span_priority">'+pt_priority+'</span></td><td class="private">'+sch_proc+'</td><td>'+doc_name+'&nbsp;</td><td>'+roomopnm+'</td><td class="message_div private">'+waitingpt_msg_text+'&nbsp;</td><td class="showTime private">&nbsp;</td></tr>';
										}else if(baseName=='main_rooms.php'){
											wr_appt_data_html = '';
											wr_appt_data_html += '<div class="ApptDetails section hide"><div class="section_header"><img src="../css/images/icon_close.png" style="width:15px;" class="fr">Appointment Details</div><table>';
											...
											wr_appt_data_html += '<tr><td>Pt. At a Glance</td><td><span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span></td></tr>';
											...
											waiting_patients_val += '<tr sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td class="pt_nameid_td" id="nametd'+sch_id+'"><img src="../css/images/icon_message.png" class="fr msg_icon">'+pt_name+ready_for_status_text+'<span class="span_priority">'+pt_priority+'</span></td><td class="private"><span class="pickTime hide">'+sch_ci_time1+'</span>'+sch_proc+wr_appt_data_html+'</td><td class="showTime private">&nbsp;</td></tr>';
										}
										str_chk_in += pt_name+"; ";
										snowaitp++;
									}else if(baseName=='main_rooms.php' && room_no!='' && room_no!= '0'){
										if(waitingIcon=="icon16_dilation"){dilationTime = '<span style="display:none" class="pickDTime">'+dilated_time+'</span>';dilationElapsed='<span class="greenText"></span>';}else{dilationTime='<span style="display:none" class="pickDTime"></span>'; dilationElapsed='';}
										waitingpt_msg_text = (doc_msg == '' || doc_msg == 'N/A') ? sch_msg : doc_msg;
										pt_status_icon = '';
										ReadyForClass='';
										DoneMarked='';
										
										if(pt_with=='6'){pt_status_icon='<span class="icon20 icon20_done"></span>';}
										else if(pt_with=='1'){pt_status_icon = '<span class="icon16 icon16_readyforP"></span>';}
										else if(pt_with=='2'){pt_status_icon = '<span class="icon16 icon16_readyforT"></span>';}
										else if(pt_with=='3'){pt_status_icon = '<span class="icon16 icon16_readyfor"></span>';}
										else if(waitingPt=="Y" && waitingIcon=="icon16_dilation"){pt_status_icon = '<span class="icon16 '+waitingIcon+'"></span>';}
										...
										if(typeof(room_wise_pts_arr[room_no])=='undefined' || room_wise_pts_arr[room_no]==''){
											...
											if(pt_priority == '0') room_wise_priority[room_no] = pt_priority;
											else if(pt_priority != '0') room_wise_priority[room_no] = pt_priority;
											
											if(sch_proc.length > 12) {span_room_proc_val = '<span title="'+sch_proc+'">'+sch_proc.substr(0,12)+'..</span>';}
											else{span_room_proc_val = '<span title="'+sch_proc+'">'+sch_proc+'</span>';}
										...
										room_data_html = '<tr class="'+color_class+'" sch_id="'+sch_id+'" doc_id="'+doc_id+'"><td class="pt_nameid_td" id="nametd'+sch_id+'" style="width:auto; cursor:pointer;"><img src="../css/images/icon_message.png" class="fr msg_icon">'+room_appt_move_up+pt_status_icon+'<span class="span_ptname_text">'+ptNameVal+'<div class="ready4box">'+doc_name+'</div></span><span class="pickTime hide">'+sch_ci_time1+'</span>';
										
										room_data_html += '<div class="ApptDetails section hide"><div class="section_header"><img src="../css/images/icon_close.png" style="width:15px;" class="fr">Appointment Details</div><table>';
										...
										room_data_html += '<tr><td>Pt. At a Glance</td><td><span class="icon20 icon20_pag" onclick="showPAG(\''+pt_id+'\')" title="Patient At a Glance"></span><span class="icon20_chart" onclick="LoadWorkView(\''+pt_id+'\')" title="Open Patient Chart"></span></td></tr>';
										...
										if(ready4doc_msg_text!='' && view_mode=='private'){
							...
							if($('#refresh_mode').val()=='auto' || forceRefresh1){
								...
								$('.span_priority').each(function(index, element) {
									if($(this).text()=='' || $(this).text()=='0'){$(this).hide();$(this).text('');}
											...
											if($(this).text().indexOf('Priority')== -1)$(this).text('Priority '+$(this).text());
											if($(this).text()=='Priority 1'){
												$(this).css({'background-color':'#f00'});
											}else if($(this).text()=='Priority 2' || $(this).text()=='Priority 3'){
												$(this).css({'background-color':'#0c0'});
...
function load_pts(fac, prov){
	...
	if(fac != ""){
		...
		if(prov == "get"){
			if(baseName=='main_rooms.php'){prov = $("#hidd_sel_provider").val();}else{prov = $("#active_pro").val();}
		}else{
			$('#active_pro').val(prov);
		}
