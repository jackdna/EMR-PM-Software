...
class send_api_call extends sendApiClient{
	...
	function CREATE_PATIENT(){
		...
		//========EXAMPLE FIELDS OF PATIENT DATA ======//
		/*$fields = array(
...
/*		$fields  = array(
	...
	function BOOK_APPOINTMENT($row_patient=array()){
		...
		if(sizeof($row_patient)<=0){
			...
			//GET MD IDS
	/*		$api_doctor_id=$api_facility_id='';
			if($row_patient['sa_doctor_id']>0){
				$qry="Select api_id FROM users WHERE id='".$row_patient['sa_doctor_id']."'";
				$rs=imw_query($qry);
				$res=imw_fetch_assoc($rs);
				$api_doctor_id=$res['api_id'];
				$rs.close;
			}
			if($row_patient['sa_facility_id']>0){
				$qry="Select api_id FROM facility WHERE id='".$row_patient['sa_facility_id']."'";
				$rs=imw_query($qry);
				$res=imw_fetch_assoc($rs);
				$api_facility_id=$res['api_id'];
				$rs.close;
			}
			if($row_patient['procedureid']>0){
				$qry="Select api_id FROM slot_procedures WHERE id='".$row_patient['procedureid']."'";
				$rs=imw_query($qry);
				$res=imw_fetch_assoc($rs);
				$api_procedure_id=$res['api_id'];
				$rs.close;
			}*/
...
/*			$row_patient  = array(
				...
				,'apptStartDate' => '2016-11-23 09:30:00'
				,'visitTypeId' => '1'
				,'locationId' => '1'
				,'doctorId' => '1'
