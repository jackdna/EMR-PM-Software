...
$router->respond(function ($request, $response, $service, $app) {
    ...
    //Check Patient
    $service->addValidator('patient', function ($str, $app) {
	...
	//Check Guarantor
    $service->addValidator('guarantor', function ($str, $app) {
    ...
    $service->addValidator('ptinsurance', function ($str, $klein) {
		...
		$sql = '
			...
			FROM 
				...
				LEFT JOIN insurance_case_types ins_types ON (ins_types.case_id = ins_case.ins_case_type) AND ins_types.case_id in (1, 10)
			WHERE 
...
$router->respond('*', function($request, $response, $service, $app) use($router) {
	...
	$sql = "INSERT INTO `fmh_iportal_api_call_log`
			SET
				`path` = '".$path."',
				`method` = '".$method."',
				`headers` = '".$headers."',
...
$router->afterDispatch(function() use($router){
	...
	$sql = "UPDATE `fmh_iportal_api_call_log`
			SET
