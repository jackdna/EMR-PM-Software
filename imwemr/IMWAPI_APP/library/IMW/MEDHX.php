...
class MEDHX
{
	...
	public function getFieldsName($str = '',$fields_for = 1,$section = 1,$view = false){
		...
		$qry = "SELECT id,field_key,name FROM fmh_iportal_med_fields
 WHERE field_key IN (".$str.") AND type = ".$fields_for." AND sec_id = ".$section."";
	...
	public function getGenHealth($data = array()){
		...
		/*field Keys*/
				...
				$sqlKey = 'SELECT `id`, `name`, `field_key` FROM `fmh_iportal_med_fields
` WHERE `type`=1 AND `sec_id`=2';
