...
if($totrec == 0){
	...
	while($rowQryAppts = imw_fetch_array($rsQryAppts)){
		...
		while($st_time < $ed_time){
			...
			$qry3 = "select id, l_text, l_show_text, labels_replaced, label_group from scheduler_custom_labels where start_date = '".$rowQryAppts["sa_app_start_date"]."' and start_time = '".$match_st_time.":00' and provider = '".$rowQryAppts["sa_doctor_id"]."' and facility = '".$rowQryAppts["sa_facility_id"]."' and (l_type = 'Procedure' or l_type = 'Information') LIMIT 1";
			//echo "<br>".$qry3."<br>";
			$res3 = imw_query($qry3);
			if(imw_num_rows($res3)>0){
				...
				if($boooooooooool === true){
					...
					$str_replace = "";
					$bl_do = false;
					$labels_replaced = "::".$rowQryAppts["id"].":".$rowQryAppts["acronym"];
					if(count($arr_l_text) > 0){
						foreach($arr_l_text as $this_replaced){	
							if(trim($this_replaced) == trim($rowQryAppts["acronym"])){
								$bl_do = true;	
							}	
							if(trim($this_replaced) != trim($rowQryAppts["acronym"])){
								$arr_replace[] = trim($this_replaced);
							}
						}
						$match = 0;
						foreach($arr_l_text as $this_replaced){	
						...
						if($match > 1){
							for($hitesh = 0; $hitesh < $match-1; $hitesh++){
								$arr_replace[] = trim($rowQryAppts["acronym"]);
							}
						}
					}
					if($bl_do == true){
						$str_replace = implode("; ", $arr_replace);
						//echo "<br>";
