...
class APPOINTMENT
{
				...
				foreach($arr_prov_sch as $key => &$val){
					if(count($val) > 0){
						...
						if(empty($facilityName) == false && isset($val['iportal_enable_slot']) && empty($val['iportal_enable_slot']) == false && isset($val['sch_tmp_id']) && empty($val['sch_tmp_id']) == false){
							...
							$excludeSlots = $this->excludeSlots($arrTemplates, $FacfacilityId, $val['provider'], $dateObj);
	...
	public function bookAppointment($patientId = '', $timeSlotId = '', $physicianId = '', $facilityId = '', $procedureId = '', $apptInfo = '', $ScheduleTemplateId = ''){
		...
		if($dbh->imw_num_rows($chkAppt) > 0) return $returnData['errorStr'] = 'An appointment already exists. Unable to proceed.';
		
		$timeSlotDt = strtotime(date('Y-m-d H:i a', $timeSlotId));
		
		if($this->currentDate > $timeSlotDt) return $returnData['errorStr'] = 'Selected Slot Date is invalid';
		...
		if($timeSlots === false) return $returnData['errorStr'] = 'Please provide a valid time Slot ID.';
		...
		if(empty($appDuration) == false){
			$apptStart = strtotime(date('H:i a', $timeSlotDt));
		...
		if(
			...
			if($apptId){
				...
				if(count($replaceLabels)>0){
					...
					foreach($replaceLabels as $lbl){
						...
						{
							imw_query($sql2);
						}else custom_lbl_log('imwapi\library\imw\appointment.php');
					}
				}
			}
			...
			$chkMsgs = $dbh->imw_query("SELECT pd.fname, pd.mname, pd.lname, pm.sender_id, pm.msg_subject, pm.msg_data, DATE_FORMAT(pm.msg_date_time,'%m %h:%i %p') AS msg_date_time FROM patient_messages pm INNER JOIN patient_data pd ON(pm.sender_id=pd.id) WHERE pt_msg_id = '' ");
			...
			if(empty($autoResponder) == false && isset($autoResponder['data'])){
				...
				if($req_qry_obj){
					...
					return $returnData = 'Appointment Request Send successfully.';
				}else{
					return $returnData['errorStr'] = 'Unable to process the request.Please Try Again.';
				}
			}
		}else{
			return $returnData['errorStr'] = 'Invalid Request. Try Again.';
		}
	}
