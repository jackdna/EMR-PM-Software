...
class athena{
		...
		if($this->authenticate() === true){
			...
			switch($this->hl7_method){
				...
				case "SIU^S14":		//message type 7 and 8
					...
					break;
				default:
					$this->response .= "<error>Invalid Method (".$this->hl7_method.").</error>";
					break;
			}
	...
	*/
	function database_log(){
		...
		$thisMsgTblID = imw_insert_id();
		$this->curr_msg_id = $thisMsgTblID;
		if($blInsert === false){
			$this->error_logs();
		}
		//writing post in a file
		if(POSTS_LOG == 1){
			$posts_file = fopen(dirname(__FILE__)."/".ATHENA_POSTS_PATH."DO_NOT_DELETE_Athena_Posts_".date('dmY').".inc","a+");
	...
	*/
	function error_logs(){
		if(ERROR_LOG == 1){
			$err_file = fopen(dirname(__FILE__)."/".ERROR_LOG_PATH."ATHENA_ERR_".date('dmY').".inc","a+");
	...
	*/
	function send_response(){
		$q = "UPDATE tblathenaposts SET response = '".htmlentities($this->response)."' WHERE fldPostId = '".$this->curr_msg_id."'";
		imw_query($q);
		echo $this->response;
	}
	...
	*/
	function add_update_patient(){
		...
		$tempFname = explode(' ',$arrTempName[1]);
		if(count($tempFname)==2){
			$arrFields[] = "fname";					$arrValues[] = ucfirst(strtolower(addslashes($tempFname[0])));
			$arrFields[] = "mname";					$arrValues[] = ucfirst(strtolower(addslashes($tempFname[1])));
		}else{
		...
		$strTempSex = ($strSex == "M") ? "Male" : ($strSex == "F" ? "Female" : "");
		
		$arrFields[] = "sex";					$arrValues[] = $strTempSex;
		
		/*--RACE--*/
		$arrRace = array("American Indian or Alaska Native", "Asian", "Black or African American", 
					"Native Hawaiian or Other Pacific Islander", "Latin American", "White",	"Declined to Specify");
		$strRaceOther = $strRace = '';
		$strRaceTemp = trim($this->obj_hl7->getSegmentFieldAsString($PIDIndex,10));
		if($strRaceTemp!='' && strstr($strRaceTemp,'^')){
			$strRaceTempArr = explode('^',$strRaceTemp);
			$strRace = $strRaceTempArr[1];
		}else{
			$strRace = trim($this->obj_hl7->getSegmentFieldAsString($PIDIndex,10));
		}
		if($strRace != ''){
			if(in_array($strRace,$arrRace)){
				$arrFields[] = "race";					$arrValues[] = $strRace;
				$arrFields[] = "otherRace";				$arrValues[] = '';
			}else{
				$arrFields[] = "race";					$arrValues[] = 'Other';
				$arrFields[] = "otherRace";				$arrValues[] = $strRace;						
			}
		}

		/*--ETHNIC GROUP--*/
		$ptEthOth = $ptEthnicity = '';
		$ptEthnicityTemp = trim($this->obj_hl7->getSegmentFieldAsString($PIDIndex,22));
		if($ptEthnicityTemp!='' && strstr($ptEthnicityTemp,'^')){
			$ptEthnicityTempArr = explode('^',$ptEthnicityTemp);
			$ptEthnicity	= $ptEthnicityTempArr[1];
		}
		if($ptEthnicity != ''){
			$arrEthnicity = array("African Americans", "American", "American Indians", "Chinese",  "European Americans",  "Hispanic or Latino", 
							"Jewish", "Not Hispanic or Latino", "Unknown", "Declined to Specify");
			if(in_array($ptEthnicity,$arrEthnicity)){
				$ptEthOth = '';
				$arrFields[] = "ethnicity";					$arrValues[] = $ptEthnicity;
				$arrFields[] = "otherEthnicity";			$arrValues[] = '';
			}else{
				$ptEthOth = 'Other';
				$arrFields[] = "ethnicity";					$arrValues[] = $ptEthOth;
				$arrFields[] = "otherEthnicity";			$arrValues[] = $ptEthnicity;
			}
		}
		...
		$strAddress = strrev($this->obj_hl7->getSegmentFieldAsString($PIDIndex,11));
		
		$strTempAddress 	= explode("^",$strAddress);
		//$country			= ucwords(strtolower(addslashes(strrev($strTempAddress[0]))));
		$ADR_index_From		= 0;
		if(intval($strTempAddress[0])==0){$ADR_index_From = 1;}
		$strTempStreet1 	= ucwords(strtolower(addslashes(strrev($strTempAddress[$ADR_index_From+3]))));
		$strTempStreet2 	= ucwords(strtolower(addslashes(strrev($strTempAddress[$ADR_index_From+4]))));
		$strTempCity 		= ucfirst(strtolower(addslashes(strrev($strTempAddress[$ADR_index_From+2]))));
		$strTempState 		= strtoupper(addslashes(strrev($strTempAddress[$ADR_index_From+1])));
		$strTempZipCode 	= str_replace('-','',trim(strrev($strTempAddress[$ADR_index_From])));
		$strTempZipCodeExt	= '';
		...
		if($strAddress != '' && strstr($strAddress,'^')){
		...
		/*--PATIENT LANGUAGE--*/
		$arrLanguage = array('English','Spanish','French','German','Russian','Japanese','Portuguese','Italian', 'Declined to Specify', 'Other');
		$ptLanguage		= '';
		$ptLanguage		= trim($this->obj_hl7->getSegmentFieldAsString($PIDIndex,15));
		if($ptLanguage!='' && strstr($ptLanguage,'^')!==false){
			$ptLanguageTempArr = explode('^',$ptLanguage);
			$ptLanguage = $ptLanguageTempArr[1];
		}
		if($ptLanguage != ''){
			if(!in_array($ptLanguage,$arrLanguage)){
				$ptLanguage = 'Other -- '.ucfirst($ptLanguage);	
			}
			$arrFields[] = "language";					$arrValues[] = $ptLanguage;
		}
		...
		$strPatQry = "SELECT id, fname, lname FROM patient_data WHERE athenaID = '".$athena_id."' AND LOWER(patientStatus) != 'inactive' ORDER BY id LIMIT 0,1"; 
		$rsPatData = imw_query($strPatQry);	
		
		if(imw_num_rows($rsPatData) == 1){
			$this->response .= "<info>Patient with Athena ID ".$athena_id." found.</info>";
			$arrPatientId = imw_fetch_assoc($rsPatData);
			$this->patient_id = $intPatientId = $arrPatientId['id'];
			if($intPatientId == $athena_id || constant('DIFFERENT_ATHENA_ID')=='ALLOWED'){
				if($this->execution_status){
					$strQry = $this->makeArrayToQuery($arrFields,$arrValues);
		...
		}else if(imw_num_rows($rsPatData) > 1){
			$this->response .= "<error>Multiple patient records found with athena ID ".$athena_id.".</error>";
			$this->resp_log_mode = "ALERT";
			$this->execution_status = false;
		}else{
			$strPatQry2 = "SELECT id FROM patient_data WHERE LOWER(fname) = '".trim(strtolower(addslashes($arrTempName[1])))."' 
						and LOWER(lname) = '".trim(strtolower(addslashes($arrTempName[0])))."' and 
						LOWER(sex) = '".trim(strtolower(addslashes($strTempSex)))."' and LOWER(DOB) = '".trim(strtolower(addslashes($strTempDOB)))."' AND LOWER(patientStatus) != 'inactive' ORDER BY id LIMIT 0,1";
			$rsPatData2 = imw_query($strPatQry2);
			if($rsPatData2 && imw_num_rows($rsPatData2)==1){
				//UPDATING PATIENT DATA AND SETTING ATHENA_ID ALSO.
				$PatData2 = imw_fetch_assoc($rsPatData2);
				$this->patient_id = $intPatientId = $PatData2['id'];
				if($this->execution_status){
					$strQry = $this->makeArrayToQuery($arrFields,$arrValues);
					$strQry = "UPDATE patient_data SET ".$strQry." WHERE id = '".$intPatientId."'";
					$blRESULT = imw_query($strQry);
			...
			}else if($rsPatData2 && imw_num_rows($rsPatData2)>1){
				//MULTIPLE PATIENTS FOUND FOR RECEIVED INFO. QUITTING.
				$this->response .= "<error>Multiple patient records found with same basic demographics info.</error>";
				$this->resp_log_mode = "ALERT";
				$this->execution_status = false;
			}else if($rsPatData2 && imw_num_rows($rsPatData2)==0){
				//RECEIVED DATA IS A NEW PATIENT. REGISTERING..
				if($this->execution_status){
					$strQry = $this->makeArrayToQuery($arrFields,$arrValues);
					$strQry = "INSERT INTO patient_data SET ".$strQry;
					$blRESULT = imw_query($strQry);
					$this->patient_id = $new_patient_id	= imw_insert_id();
					if($blRESULT === false){
						$this->error_logs();
					}else{
						$strQry = "UPDATE patient_data SET pid = '".$new_patient_id."' WHERE id = '".$new_patient_id."'";
						$blRESULT = imw_query($strQry);
						if($blRESULT === false){
							$this->error_logs();
						}
...
/*
			if($this->obj_hl7->getSegmentFieldAsString(7, 0) == "IN1"){
				$this->add_update_ins_company(7);
				$this->add_update_pt_ins_case(7);
				$this->add_update_pt_insurance(7);
			}*/
		}
	}
	...
	*/
	function add_update_ins_company($segment_no){
		...
		if(imw_num_rows($rsData) > 0){
			$this->response .= "<info>Insurance Company with ID ".$intInsCompId." already exists.</info>";
	...
	function add_update_pt_ins_case($segment_no){
		if($this->execution_status == true && !empty($this->ins_comp_id)){
			...
			if(imw_num_rows($res) > 0){
				...
				$ins_case_type_id = DEFAULT_INSURANCE_CASE_TYPE;
			}
	...
	*/
		...
		if($this->execution_status == true && !empty($this->ins_comp_id) && !empty($this->ins_case_id)){
			...
			if(imw_num_rows($rsData) > 0){
				...
				if($blItsNewDetail == true){
					...
					$arrUpdOldInsFields[] = "actInsComp";						$arrUpdOldInsValues[] = 1;
	...
	function add_doctor(){
		if($this->execution_status == true){
			...
			$arrAthenaProvider = explode("^",$this->obj_hl7->getSegmentFieldAsString($PV1Index,7));
			$ProviderNPI		 = trim(str_replace('-','',$arrAthenaProvider[0]));
			//$intAthenaProviderId = $arrAthenaProvider[0];
			if(count($arrAthenaProvider) > 3){
				$strLname = ucwords(strtolower(trim($arrAthenaProvider[1])));
				$strFname = ucwords(strtolower(trim($arrAthenaProvider[2])));
				$strMname = trim($arrAthenaProvider[3]);
			}else{
				$strLname = ucwords(strtolower(trim($arrAthenaProvider[1])));
				$strFname = ucwords(strtolower(trim($arrAthenaProvider[2])));
				$strMname = '';
			...
			$strUQry = "SELECT id, fname, lname FROM users WHERE user_npi  = '".$ProviderNPI."' AND lname = '".$strLname."' AND delete_status = 0";
			$rsUData = imw_query($strUQry);
			if($rsUData === false){
				$this->error_logs();
			}
			//var_dump(imw_num_rows($rsUData));die;
			if(imw_num_rows($rsUData) == 1){				
				$strMessage .= "<error>Provider with Lname '".$strLname."' already exists.</error>";
				...
				return;
				$this->response .= "<info>Adding new Provider with NPI ".$ProviderNPI.".</info>";
	...
	function add_operator(){
		$q = "SELECT id,username FROM users WHERE LOWER(fname) = 'athena' AND LOWER(lname) = 'athena' AND delete_status = '0' LIMIT 0,1";
		$res = imw_query($q);
		if($res && imw_num_rows($res)==1){
			...
			//$this->operator_id = 1; //Athena Operator User ID.
		//	$this->operator_username = 'athenaHL7';
		}
	...
	function add_facility(){	
		if($this->execution_status == true){
			...
			$strAthenaFacilityName = stripslashes($arrAthenaFacility[1]);
			$strQry = "SELECT id FROM facility WHERE athenaID = '".$intAthenaFacilityId."' ORDER BY id LIMIT 0,1";
			$rsData = imw_query($strQry);
			
			if($rsData && imw_num_rows($rsData) == 1){
				$this->response .= "<info>Facility with ID ".$intAthenaFacilityId." already exists.</info>";
				$arrFacilityId = imw_fetch_assoc($rsData);
				$intFacilityId = $arrFacilityId['id'];
				$this->facility_id = $intFacilityId;
			}else if($rsData && imw_num_rows($rsData)==0){
				$q = "SELECT id FROM facility WHERE LOWER(name) = '".strtolower(addslashes($strAthenaFacilityName))."'";
				$res = imw_query($q);
				if($res && imw_num_rows($res)==1){
					$rs = imw_fetch_assoc($res);
					$intFacilityId = $rs['id'];
			...
			if(intval($this->facility_id) > 0){
				$strGQry = "UPDATE facility SET athenaID = '".$intAthenaFacilityId."' WHERE id = '".$this->facility_id."'";
	...
	function add_update_appointment($strMode){
		if($this->execution_status == true && !empty($this->procedure_id) && !empty($this->facility_id) && !empty($this->doctor_id) && !empty($this->operator_id)){
			...
			$SCHIndex = $this->getSegmentByName('SCH');
			$intAthenaApptId = $this->obj_hl7->getSegmentFieldAsString($SCHIndex,2);
			$strDateTime = str_replace('\^','',$this->obj_hl7->getSegmentFieldAsString($SCHIndex,11));
			...
			$strApptQry = "SELECT id, sa_patient_app_status_id, status_update_operator_id FROM schedule_appointments WHERE athenaID = '".$intAthenaApptId."' ORDER BY id LIMIT 0,1";		
			$rsApptData = imw_query($strApptQry);
			$appt_status_id = $appt_status_change_operator_id = '';
			if($rsApptData && imw_num_rows($rsApptData) == 1){
				$arrApptId = imw_fetch_assoc($rsApptData);
				$intApptId = $arrApptId['id'];
				$this->appt_id = $intApptId;
				$appt_status_id		 = $arrApptId['sa_patient_app_status_id'];
				$appt_status_change_operator_id = $arrApptId['status_update_operator_id'];		
			}else if($rsApptData && imw_num_rows($rsApptData) == 0){
				$q = "SELECT id, sa_app_start_date, sa_app_starttime, sa_patient_app_status_id, status_update_operator_id FROM schedule_appointments WHERE 
					  sa_doctor_id = '".$this->doctor_id."' AND 
					  sa_app_start_date = '".$dtStartDate."' AND sa_app_end_date = '".$dtStartDate."' AND 
					  sa_app_starttime = '".$tmStartTime."' AND sa_app_endtime = '".$tmEndTime."' AND 
					  sa_patient_id = '".$this->patient_id."' ORDER BY id DESC LIMIT 0,1";/* AND 
					  procedureid = '".$this->procedure_id."' AND 
					  sa_facility_id = '".$this->facility_id."'";*/
				$res = imw_query($q);
				if($res && imw_num_rows($res)==1){
					$rs = imw_fetch_assoc($res);
					$intApptId = $rs['id'];
					$this->appt_id = $intApptId;
					$appt_status_id		 = $rs['sa_patient_app_status_id'];
					$appt_status_change_operator_id = $rs['status_update_operator_id'];	
			...
			if($appt_status_id=='18') {$this->execution_status=false; return false;}
			...
			if(intval($this->appt_id) > 0){//APPT FOUND, UPDATING....	
				if($strMode == "addorupdate"){
					$strQry = "UPDATE schedule_appointments SET ".$strQry;						
					$strQry .= " WHERE id = '".$intApptId."'";		
					imw_query($strQry);
					//rescheduling
					$strMessage .= $this->changeStatus($this->appt_id, $this->patient_id, $arrApptId['sa_app_start_date'], $arrApptId['sa_app_starttime'], $this->operator_id, constant("DEFAULT_RESCHEDULE_STATUS_ID"));		
				}			
			}else{
			...
			if($strMode != "addorupdate"){
				switch ($strMode){
					case "cancel":		//cancel
						$strMessage .= $this->changeStatus($this->appt_id, $this->patient_id, $dtStartDate, $tmStartTime, $this->operator_id, constant("DEFAULT_CANCEL_STATUS_ID"));
						break;
					case "check":
						if($this->operator_id!=$appt_status_change_operator_id && $appt_status_id=='18') {$this->execution_status=false; return false;}
						$strCheckStatus = $this->obj_hl7->getSegmentFieldAsString($SCHIndex,25);
						if($strCheckStatus == "COMPLETED")	//check out
							$strMessage .= $this->changeStatus($this->appt_id, $this->patient_id, $dtStartDate, $tmStartTime, $this->operator_id, constant("DEFAULT_CHECKOUT_STATUS_ID"));
						elseif ($strCheckStatus == "ARRIVED")	//check in
							$strMessage .= $this->changeStatus($this->appt_id, $this->patient_id, $dtStartDate, $tmStartTime, $this->operator_id, constant("DEFAULT_CHECKIN_STATUS_ID"));					
						break;				
				}
			}
		}
	...
	function changeStatus($intApptId, $intPatientId, $dtOldDate, $tmOldTime, $intOperatorId, $strMode){
		...
		if(empty($intPrevStatusId)){
			$vquery_ind = " insert into previous_status set sch_id='$intApptId', patient_id='$intPatientId', status_time=current_time(),status_date=current_date(),status='$strMode',old_date='$dtOldDate',old_time='$tmOldTime' ";
		}else{
			$vquery_ind = " update previous_status set sch_id='$intApptId',patient_id='$intPatientId', status_time=current_time(),status_date=current_date(),status='$strMode', old_date='$dtOldDate', old_time='$tmOldTime' where id = '$intPrevStatusId'";
		}
		
		$vsql_ind = imw_query($vquery_ind);
		
		//updating status in main table
		$vquery_update = " update schedule_appointments set status_update_operator_id = '$intOperatorId', sa_patient_app_status_id='$strMode', status_date=NOW() where id='$intApptId' ";		
		$vsql_update = @imw_query($vquery_update);
		
		//status specific updates
		if($strMode == constant("DEFAULT_RESCHEDULE_STATUS_ID")){
		...
		if($strMode == constant("DEFAULT_CHECKIN_STATUS_ID")){
			
			//null
			
			$this->response .= "<info>Appointment Checked In successfully.</info>";
		}
		if($strMode == constant("DEFAULT_CHECKOUT_STATUS_ID")){
			...
			for($i=0;$i<=3;$i++){
				...
				$vquery_d = "SELECT min(end_date) , reff_id
							 FROM `patient_reff` 
							 WHERE end_date >= current_date() and effective_date <= current_date() and reff_type='$i'
		...
		if($strMode == constant("DEFAULT_CANCEL_STATUS_ID")){
	...
	function makeArrayToQuery($arrFields,$arrValues){
		$strQry = "";
		$jk = 0;
		foreach($arrFields as $strColumn){
			$strQry .= " ".$strColumn." = '".$arrValues[$jk]."', ";
