...
// MIT License 2019
var actb_prtDiv="";
function actb(obj,ca,sfw,fclr,objHidden,objHiddenVal,cptPOS,objHidden2,objHiddenVal2,objHidden3,objHiddenVal3,objHidden4,objHiddenVal4,actionVal, functionFor, functionMaterial, searchFromAnywhere){
	/* ---- Public Variables ---- */
	...
	*/
	function setTypeAheadTopPosition(objElem,divA)
	{
		...
		*/
		var minus=divOffsetHeight+parseInt(20);
		...
		if(toper > thisClientHeight && isChrome)
		return bottomY-minus;
		else
	...
	*/
	...
	function actb_insertword(a,hiddenVal,hiddenVal2,hiddenVal3,hiddenVal4){
		if(actionVal=="remove_hiffen_appolicy") {
			if(a.search(" - ")>=0) {
				tempVal = a.split(" - ");
				a = tempVal[1];	
			}
		...
		if (actb_self.actb_delimiter.length > 0){
			...
			if(actionVal=="remove_round_bracket") {
				tempVal = str.split("(");
				str = tempVal[0];	
			}

			var tempVal='';
			if(obj.id == 'insprovider' && fclr==true) {
				tempVal = str.split(" - ");
				str = tempVal[1];
			}
			actb_curr.value = str;
			...
			if(actb_curr_hidden4 && hiddenVal4) {
				actb_curr_hidden4.value = hiddenVal4;
			}
			//Test
				if(actb_curr) 
				{
			...
			//FUNCTION CALLING TO CALCULATE ORDER TOTAL PRICE FOR CL ORDER FORM
			if(functionFor=='clOrderPrice'){
				if(functionMaterial!=''){
					arrArgu=functionMaterial.split('~~');
	...
	function actb_penter(){
		...
		var word = '';
		var c = 0;
		var hiddVal = hiddVal2  = hiddVal3 = hiddVal4 ="";
		for (var i=0;i<=actb_self.actb_keywords.length;i++){
			...
			if (c == actb_pos){
				...
				if(objHiddenVal4) {
					hiddVal4 = objHiddenVal4[i];
				}
				break;
			}
		}
		actb_insertword(word,hiddVal,hiddVal2,hiddVal3,hiddVal4);
