...
function putInsImgContentToImw($scan_card,$insuranceDataScanContent,$demoPtId,$imwDirNme) {
	...
	if($scan_card) {
		...
		{
			$iolinkInsScanFolder = $rootServerPath.'/'.$imwDirNme.'/data/'.strtolower($imwPracName).'/PatientId_'.$demoPtId;
		}
		else
		{
		...
		{		
			mkdir( $iolinkInsScanFolder, 0755, true );
			chown( $iolinkInsScanFolder, 'apache' );
		}
	...
	if($patient_image_path) {
		...
		if(trim($imwPatientImageName)) {
			...
			if( file_exists($scemrPtImagePath) )
			{
				...
				{
					$imwPtImageFolder		= $rootServerPath.'/'.$imwDirectoryName.'/data/'.strtolower($imwPracName).'/PatientId_'.$imw_patient_id;
				}
				else
				{
				...
				if(!is_dir($imwPtImageFolder)){
					mkdir( $imwPtImageFolder, 0755, true );
					chown( $imwPtImageFolder, 'apache' );
				}
...
$procQry = "SELECT * FROM procedures order by `name` ";
...
if($surgeonNumRow>0) {
	while($surgeonRow 		= imw_fetch_array($surgeonRes)) {
		$surgeonNpiFname	= $surgeonRow['fname'];
		$surgeonNpiMname	= $surgeonRow['mname'];
		$surgeonNpiLname	= $surgeonRow['lname'];
...
//START TO GET PATIENTS FROM patient_in_waiting_tbl
	...
	if($getPatientInWaitingTblNumRow>0) {
		...
		while($getPatientInWaitingTblRow=imw_fetch_array($getPatientInWaitingTblInfoRes)) {
			...
			$surgeon_npi				= $surgeonNpiArr[$getPatientInWaitingTblRow['surgeon_fname']][$getPatientInWaitingTblRow['surgeon_mname']][$getPatientInWaitingTblRow['surgeon_lname']];
			$surgeon_mname_space		= '';
...
/*================================================================================================*/
	...
	*/
	...
	/*End IF and WHILE for the iteration fo waiting_tbl*/
		}
	}
