...
if($_REQUEST['sig_data']){
	...
	$sigFileNamePath = $sig_directory.'no_image.jpg'; 
	//check	

	if($rqSigFor == "ptHealth"){
		$sigFileName = 'signHealthIpad_iolink_'.$ptWaitingId.'_'.date('d_m_y_h_i_s').'_1.jpg';
		$sigFileNamePath = $sig_directory.$sigFileName; 
	}else if($rqSigFor == "ptConsent"){
		$sigFileName = 'signIpadConsent_iolink_'.$ptWaitingId.'_'.date('d_m_y_h_i_s').'_'.$patient_id.'_1.jpg';
		$sigFileNamePath = $sig_directory.$sigFileName; 
	}else if($rqSigFor == "ptInstruction"){
		$sigFileName = 'signIpadInstruction_iolink_'.$ptWaitingId.'_'.date('d_m_y_h_i_s').'_'.$patient_id.'_1.jpg';
		$sigFileNamePath = $sig_directory.$sigFileName; 
	}else if($rqSigFor == "phy"){
		$sigFileName = 'signIpadPhy_iolink'.$user_id.'_'.date('d_m_y_h_i_s').'_1.jpg';
		$sigFileNamePath = $sig_directory.$sigFileName; 
	}
	$_SESSION['IPadImage'] = NULL;
	$_SESSION['IPadImage'] = "";					
	...
	// Insert Record into iolink consent Form signature table
	if($rqSigFor == "ptConsent"){
		
		$fldQry = " SET signature_content = '', consent_template_id = '".$consentTemplateID."', patient_in_waiting_id = '".$ptWaitingId."', patient_id = '".$patient_id."', signature_count = '".$signSeqNum."', signature_image_path = '".addslashes($sigFileNamePath)."' ";
		
		$chk_sig_data_qry = "select consent_form_signature_id from iolink_consent_form_signature 
				where patient_in_waiting_id = '".$ptWaitingId."'
				and consent_template_id = '$consentTemplateID'
				and signature_count = ".$signSeqNum."";
		...
		if($chk_sig_data_num_row>0) {	
			...
			//UDPATE RECORD
			$upQry = "Update iolink_consent_form_signature ".$fldQry." Where consent_form_signature_id = '".$consent_form_signature_id."'";
			imw_query($upQry);
		}
		else{
			$insQry = "Insert Into iolink_consent_form_signature ". $fldQry;
			imw_query($insQry);
			$sig_insert_id = imw_insert_id();
		}
	}
...
<html>
	...
	<body onLoad="init();" style="overflow: hidden; -ms-touch-action: none;" ontouchmove="BlockMove(event);" onmousemove="BlockMove(event);" onUnload="unload();"> 
        ...
        <form name="sig" id="sig" action="" method="post">            
            ...
            <input type="hidden" name="ptWaitingId" 	id="ptWaitingId" 	 value="<?php echo $ptWaitingId; ?>"/>
            ...
            <input type="hidden" name="consentTemplateID" id="consentTemplateID" value="<?php echo $consentTemplateID; ?>"/>
            <table>
            	<tr>
                	<td>
