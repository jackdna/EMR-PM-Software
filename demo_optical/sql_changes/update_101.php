...
if($start_val==0){
	...
	imw_query("update in_item set qty_on_hand='0',amount='0'");
}
$item_qry = imw_query("select in_item.id from in_item join in_item_loc_total on in_item.id=in_item_loc_total.item_id where in_item.del_status='0' group by in_item.id order by in_item.id asc limit $start_val , $end");
