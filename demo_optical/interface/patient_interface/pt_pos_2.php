...
<!--  onChange="changeQty('<?php echo $sel_records['module_type_id']; ?>', this.value, '<?php echo $pro_cont; ?>');" -->
			...
			?>
				...
				?>
					...
					<?php if($pageName=="frameLensSelection"): ?>
					<td>
						<input type="text" id="itemDescription_frame_1" style="width:100%" class="itemnameDisp" value="<?php echo $sel_records['item_name']; ?>" />
					</td>
					<?php endif; ?>
                    <td>
				...
				if($sel_order_data[$i]['module_type_id']==1){
					...
					if(imw_num_rows($sel_lens_qry)>0){
						...
						for($l=0;$l<count($show_lens_value_arr);$l++){
							...
							if(($sel_records_lens[$show_lens_value_arr[$l]] > 0) || ($sel_records_lens[$show_lens_value_arr[$l]]!="" && $show_itemized_name_arr[$l]=="other") || in_array($show_itemized_name_arr[$l], $multipleLensVals)){
								...
								//select * from in_order_lens_price_detail where order_id ='15' and order_detail_id='31' and patient_id='6' and itemized_name LIKE 'material%' and del_status='0' ORDER BY id ASC
...
<?
	...
	/*Dininfectent POS Row for Contact Lens*/
			...
			?>
				...
				<?php } ?>
					...
					<td>
						 <input type="text" style="width:100%;text-align:right;" class="qty_cls" id="pos_qty_<?php echo $pro_cont; ?><?php echo $pos_lensd; ?>_os" name="pos_qty_<?php echo $pro_cont; ?><?php echo $pos_lensd; ?>_os" value="<?php echo $sel_records['qty']; ?>" onChange="changeQty('<?php echo $sel_records['module_type_id']; ?>', this.value, '<?php echo $pro_cont; ?>');" autocomplete="off" <?php echo ($sel_records['module_type_id']==3)?'readonly ':'';?> onKeyUp="validate_qty(this);" />
...
<?php endforeach; ?>
	...
	/*End Dininfectent POS Row for Contact Lens*/
			} 
		}
...
?>
			<td style="text-align:right;font-weight:bold;" colspan="<?php echo $colspan; ?>">
				...
				<label style="floa:right;font-weight:bold;">Total <?php echo $taxLabel; ?> Payable: </label></td>
			<td>
...
?>
    	...
    	<tr>
			<td colspan="<?php echo $colspan; ?>" style="font-weight:bold;text-align:right;"><label style="float:right;">Total Payment:</label></td>
            <td>
            	<input class="currency" type="text" name="show_pos_pat_pos_grand_payed" id="show_pos_pat_pos_grand_payed" value="<?php echo $sel_rec['pt_paid']; ?>" style="width:100%;text-align:right;" onchange="dev_pay_in_prac();">
            </td>
...
function calculate_all_Grand_POS(){
	...
	var price_cls=allowed_cls=qty_cls=rqty_cls=payed_cls=ins_amt_cls=disc_val=tot_qty=0;
	...
	$(qtys).each(function(i, obj){
		var qtyElem = $(obj).val();
		grand_qty_count = grand_qty_count+parseInt(qtyElem);
	});
	...
	if(!mflag || mflag == 'chnageVal'){
		if(overall_discount=="" || overall_discount=="0"){
			...
			$.each(pos_rows, function(i, row){
				...
				tax_v = (amt*tax_p)/100;
				tax_v = tax_v.toFixed(2);	/*Tax payable for line Item*/
