<?php
/*
File: cont_lens_style.php
Coded in PHP7
Purpose: Add/Edit/Delete: Contact Lense Style
...
$date = date("Y-m-d");
$time = date("h:i:s");
//UPDATE AND INSERT CONTACT LENS STYLE//
...
{	
	if(count($_POST['select_record'])>0)
	{
		for($v=0;$v<count($_POST['select_record']);$v++)
		{
			if(trim($_POST['del_hidden'])=="")
			{
				...
				if($rec_supplyname!="")
				{			
					$updateQry = "update in_contact_style set style_name = '".imw_real_escape_string($rec_supplyname)."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
					...
					{
						$edit_time_insert_query = "insert in_contact_style set style_name = '".imw_real_escape_string($_POST['input_val'])."' ";					
					}
				}
			}
			else
			{
				...
				$rec_id = $_POST['select_record'][$v];
				$updateQry = "";
				$updateQry = "update in_contact_style set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
				imw_query($updateQry);					
			}
	...
	else
	{
		if(trim($_POST['input_val'])!="")
		{
			$msg = "Record(s) Saved Successfully";
			$msg_stat = "block";
			imw_query("insert in_contact_style set style_name = '".imw_real_escape_string($_POST['input_val'])."' ");
		}
	}
...
$targetpage = "cont_lens_style.php"; 	
$limit = 50; 
$query = "SELECT COUNT(*) as num FROM in_contact_style where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_contact_style',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_contact_style',"+rowid+",'1','del_status',this)");
				}
...
function open_addnew_popup()
{
	...
	var Add_new_popup=top.WindowDialog.open('Add_new_popup','../add_new.php?module_name=contact_style&col_name=style_name&heading=Contact_Lenses_Style','Add_new_popup','width=320,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addframe" id="firstform" action="" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td width="560">Contact Lens Style<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
	...
	<div id="listing_record" style="height:<?php echo $_SESSION['wn_height']-$aprxHght;?>px; overflow-y:auto;">
        ...
        <tbody class="table_cell_padd2">
            <?php 
                ...
                $sql="select * from in_contact_style where del_status != '2' $whr order by style_name asc";
                $res = imw_query($sql);
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td width="560">
                <input type="text" value="<?php echo $row['style_name']; ?>" class="supplyname_field" id="supplyname_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="supplyname[<?php echo $row['id']; ?>]" style="width:250px;"/>
              </td>
              <td align="center" width="40">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_contact_style','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_contact_style','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
	...
	<ul style="float:left;  margin:10px 0 10px 170px; <?php if($numrows==1 && !isset($_REQUEST['alpha'])){ ?>display:none;<?php } ?>">
	<li class="fl"><a style="border-radius:5px; padding:2px 6px; color:#fff; text-decoration:none; font-weight:bold; text-transform:uppercase; margin:3px; background:#09F;" href="?alpha=az">A-Z</a></li>
	<?php foreach($alpha as $key=>$value) 
	{ ?>
	<li class="fl"><a style="border-radius:5px; padding:2px 6px; color:#fff; text-decoration:none; font-weight:bold; text-transform:uppercase; margin:2.3px; background:#09F;" href="?alpha=<?php echo $value; ?>"><?php echo $value; ?></a></li>
	<?php } ?>
	</ul>
    </div>
</form>
