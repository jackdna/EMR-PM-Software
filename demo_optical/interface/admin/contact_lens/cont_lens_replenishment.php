...
Coded in PHP7
Purpose: Add/Edit/Delete: Contact Lense Replenishment
	...
	if(count($_POST['select_record'])>0){
		for($v=0;$v<count($_POST['select_record']);$v++){
			if(trim($_POST['del_hidden'])==""){
				...
				if($opt_val!="" || $rec_id!=""){
					$updateQry = "UPDATE `in_options` SET `opt_val`='".imw_real_escape_string($opt_val)."', `modified_date`='$date', `modified_time`='$time', `modified_by`='$opr_id' WHERE `id` = '".$rec_id."' AND `opt_type`='4' and module_id='3'";
					imw_query($updateQry);
				...
				$rec_id = $_POST['select_record'][$v];
				$updateQry = "";
				$updateQry = "UPDATE `in_options` SET `del_status`='2', `del_date`='".$date."', `del_time`='".$time."', `del_by`='".$opr_id."' WHERE `id`= '".$rec_id."' AND `opt_type`='4' and module_id='3'";
				imw_query($updateQry);					
			}
		}
	}
...
if($_REQUEST['alpha'] && $_REQUEST['alpha']!='az'){
	...
	$limit = 50; 
	$query = "SELECT COUNT(*) AS 'num' FROM `in_options` WHERE `del_status`!='2' AND `opt_type`='4' and module_id='3'".$whr;
...
function open_addnew_popup(){
	...
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=contact_lens_replenishment&col_name=opt_val&heading=Contact_Lenses_Replenishment&m_id=3','Add_new_popup','width=320,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addcontactLensUsage" id="firstform" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td width="650">Contact Lens Replacement<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
...
<?php 
	...
	$sql="SELECT * FROM `in_options` WHERE `del_status`!='2' ".$whr." AND `opt_type`='4' and module_id='3' ORDER BY `opt_val` ASC LIMIT ".$start.", ".$limit;
	$res = imw_query($sql);
...
?>
            <tr class="<?php echo $rowbg;?>">
              ...
              <td width="620">
                <input type="text" value="<?php echo $row["opt_val"]; ?>" class="opt_val_field" id="opt_val_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="option[<?php echo $row['id']; ?>]" style="width:600px;"/>
              </td>
