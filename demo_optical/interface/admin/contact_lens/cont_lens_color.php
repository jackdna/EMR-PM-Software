<?php
	/*
	File: cont_lens_color.php
	Coded in PHP7
	Purpose: Add/Edit/Delete: Contact Lense Color
	...
	*/
	...
	$date = date("Y-m-d");
	$time = date("h:i:s");
	//UPDATE AND INSERT CONTACT LENS COLOR//
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					{		
						$updateQry = "update in_color set color_name = '".imw_real_escape_string($rec_colorname)."',  color_code	= '".imw_real_escape_string($rec_colorcode)."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						...
						{
							$edit_time_insert_query = "insert in_color set color_name = '".imw_real_escape_string($_POST['input_val'])."', color_code	= '".imw_real_escape_string($_POST['color_code'])."' ";				
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_color set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="" && trim($_POST['color_code'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_color set color_name = '".imw_real_escape_string($_POST['input_val'])."', color_code	= '".imw_real_escape_string($_POST['color_code'])."' ");
			}
		}
	...
	$targetpage = "cont_lens_color.php"; 	
	$limit = 50; 
	$query = "SELECT COUNT(*) as num FROM in_color where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_color',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_color',"+rowid+",'1','del_status',this)");
				}
...
function open_addnew_popup()
{
	...
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=color&col_name=color_name&heading=Contact_Lenses_Color','Add_new_popup','width=580,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
	...
	<div id="listing_record" style="height:<?php echo $_SESSION['wn_height']-$aprxHght;?>px; overflow-y:auto;">
        ...
        <tbody class="table_cell_padd2">
            <?php 
                ...
                $sql="select * from in_color where del_status != '2' $whr order by color_name asc LIMIT $start, $limit";
                $res = imw_query($sql);
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td width="240">
                <input type="text" value="<?php echo $row["color_code"]; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="colorcode[<?php echo $row['id']; ?>]" style="width:230px;"/>
              </td>
              <td align="center" width="70">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_color','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_color','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
