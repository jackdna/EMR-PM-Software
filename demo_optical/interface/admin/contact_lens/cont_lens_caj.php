<?php
	/*
	File: cont_lens_caj.php
	Coded in PHP7
	Purpose: Add/Edit/Delete: Contact Lense Wear Time
	...
	*/
	...
	$date = date("Y-m-d");
	$time = date("h:i:s");
	//UPDATE AND INSERT CONTACT LENS WEAR TIME//
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			...
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					$rec_catname = trim($_POST['catname'][$rec_id]);
					if($rec_catname!="")
					{			
						$updateQry = "update in_contact_cat set cat_name = '".imw_real_escape_string($rec_catname)."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						...
						{
							$edit_time_insert_query = "insert in_contact_cat set cat_name = '".imw_real_escape_string($_POST['input_val'])."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_contact_cat set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_contact_cat set cat_name = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
	...
	{
		$whr = " and cat_name like '".$_REQUEST['alpha']."%' ";	
	}
	$targetpage = "cont_lens_caj.php"; 	
	$limit = 50; 
	$query = "SELECT COUNT(*) as num FROM in_contact_cat where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_contact_cat',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_contact_cat',"+rowid+",'1','del_status',this)");
				}
...
function refrsh()
{
	window.location.href='cont_lens_caj.php';
}

function open_addnew_popup()
{
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=contact_cat&col_name=cat_name&heading=Contact_Lenses_Wear_Time','Add_new_popup','width=320,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
	...
	<div id="listing_record" style="height:<?php echo $_SESSION['wn_height']-$aprxHght;?>px; overflow-y:auto;">
                <table class="table_collapse">
                    <tr class="listheading">
                      ...
                      <td width="620">Contact Lens Wear Scheduler<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
                      <td align="center" width="100">Status</td>
                    </tr>
                    <tbody class="table_cell_padd2">
						<?php 
							
                            $sql="select * from in_contact_cat where del_status != '2' $whr order by cat_name asc LIMIT $start, $limit";
                            $res = imw_query($sql);
                        ...
                        <tr class="<?php echo $rowbg;?>">
                          ...
                          <td width="620">
                            <input type="text" value="<?php echo $row["cat_name"]; ?>" class="catname_field" id="catname_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="catname[<?php echo $row['id']; ?>]" style="width:600px;"/>
                          </td>
                          <td align="center" width="70">
                            <?php if($status=="1") 
                            { ?>	
                            <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_contact_cat','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                            <?php } else { ?>
                            <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_contact_cat','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                            <?php 
                            } ?>
                          </td>
                        </tr>
...
$(document).ready(function()
{
	selectCurrentCheck = function(ab)
	{
	   ...
	   var currentval = $('#catname_field_'+ab);
	
		if($.trim(currentval.val()) == "")
		{
				top.falert('Please Enter Contact Lens Category');	
				setTimeout(function(){$( currentval ).focus(); },0);
		}
		else
		{
			$(".catname_field").each(function( index ) 
			{
