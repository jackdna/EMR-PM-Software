<?php
	/*
	File: medicines_type.php
	Coded in PHP7
	Purpose: Add/Edit/Delete: MEDICINE Type
	...
	*/
	...
	$date = date("Y-m-d");
	$time = date("h:i:s");
	//----------- UPDATE AND INSERT MEDICINE TYPE ----------//
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					if($rec_typename!="")
					{			
						$updateQry = "update `in_medicines_types` set type_name = '".imw_real_escape_string($rec_typename)."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						...
						{
							$edit_time_insert_query = "insert `in_medicines_types` set type_name = '".imw_real_escape_string($_POST['input_val'])."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update `in_medicines_types` set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert `in_medicines_types` set type_name = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
	...
	$targetpage = "medicines_type.php"; 	
	$limit = 50; 
	$query = "SELECT COUNT(*) as num FROM `in_medicines_types` where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_medicines_types',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_medicines_types',"+rowid+",'1','del_status',this)");
				}
...
function refrsh()
{
	window.location.href='medicines_type.php';
}

function open_addnew_popup()
{
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=medicines_types&col_name=type_name&heading=Medicines_Type','Add_new_popup','width=320,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<div class="mt10" style="height:<?php echo $_SESSION['wn_height']-360;?>px;">
    <form onSubmit="return validateform()" name="addmedicines" id="firstform" action="" method="post">
        <table class="table_collapse">
            <tr class="listheading">
              <td style="width:20px;">
                <input type="hidden" id="del_hidden" name="del_hidden" value="" />
                <input type="checkbox" id="selectall" value="" /></td>
              <td style="width:auto;">Medicines Type<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
        ...
        <table class="table_collapse">
            <tbody class="table_cell_padd2">
                <?php 
                    ...
                    $sql="select * from `in_medicines_types` where del_status != '2' $whr order by type_name asc LIMIT $start, $limit";
                    $res = imw_query($sql);
                ...
                <tr class="<?php echo $rowbg;?>">
                  ...
                  <td align="center" style="width:70px;">
                    <?php if($status=="1") 
                    { ?>	
                    <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_medicines_types','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                    <?php } else { ?>
                    <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_medicines_types','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                    <?php 
                    } ?>
                  </td>
                </tr>
...
$(document).ready(function(){
	...
	selectCurrentCheck = function(ab)
	{
	   ...
	   var currentval = $('#typename_field_'+ab);
		...
		{
			top.falert('Please Enter Medicine Type');	
