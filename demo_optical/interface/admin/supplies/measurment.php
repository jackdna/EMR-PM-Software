<?php
	...
	Coded in PHP7
	Purpose: View/Update Supplies Measurment
	...
	*/
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					$rec_measurmentname = trim($_POST['measurmentname'][$rec_id]);
					if($rec_measurmentname!="")
					{			
						$updateQry = "update in_supplies_measurment set measurment_name = '".imw_real_escape_string($rec_measurmentname)."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						...
						{
							$edit_time_insert_query = "insert in_supplies_measurment set measurment_name = '".imw_real_escape_string($_POST['input_val'])."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_supplies_measurment set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_supplies_measurment set measurment_name = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
	...
	{
		$whr = " and measurment_name like '".$_REQUEST['alpha']."%' ";
	}
	$targetpage = "measurment.php"; 	
	$limit = 50;
	$query = "SELECT COUNT(*) as num FROM in_supplies_measurment where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_supplies_measurment',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_supplies_measurment',"+rowid+",'1','del_status',this)");
				}
			}
		}
	});
...
function open_addnew_popup()
{
	...
	var addwin=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=supplies_measurment&col_name=measurment_name&heading=Supplies_Measurment','Add_new_popup','width=320,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	addwin.focus();
}
...
<form onSubmit="return validateform()" name="addframe" id="firstform" action="" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td style="width:auto;">Supplies Measurement<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
    ...
    <table class="table_collapse">
        <tbody class="table_cell_padd2">
            <?php
                ...
                $sql="select * from in_supplies_measurment where del_status != '2' $whr order by measurment_name asc LIMIT $start, $limit";
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td style="width:auto;">
                <input type="text" value="<?php echo $row["measurment_name"]; ?>" class="measurmentname_field"  id="measurmentname_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="measurmentname[<?php echo $row['id']; ?>]" style="width:600px;"/>
              </td>
              <td align="center" style="width:70px;">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_supplies_measurment','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_supplies_measurment','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
...
$(document).ready(function()
{
	selectCurrentCheck = function(ab)
	{
	   ...
	   var currentval = $('#measurmentname_field_'+ab);
	
		if($.trim(currentval.val()) == "")
		{
				top.falert('Please Enter Mearsurment');	
				setTimeout(function(){$( currentval ).focus(); },0);
		}
		else
		{
			$(".measurmentname_field").each(function( index ) 
			{
