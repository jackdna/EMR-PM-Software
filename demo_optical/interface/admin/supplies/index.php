...
File: index.php
Coded in PHP7
Purpose: View/Edit and Delete Supplies
Access Type: Direct access
*/
require_once(dirname('__FILE__')."/../../../config/config.php");
require_once(dirname('__FILE__')."/../../../library/classes/functions.php"); 

$stringAllUpc = get_upc_name_id('5');
...
function add_qty_fun(type){
	...
	var addwin=top.WindowDialog.open('Add_new_popup','../lens/location_lot_popup.php?item_add='+type+'&item_id='+item_id,'location_popup','width=820,height=500,left=600,scrollbars=no,top=150');
	addwin.focus();
}
...
var vendor = document.getElementById('vendor').value;
	top.WindowDialog.closeAll();
	var win=top.WindowDialog.open('Add_new_popup','../stock_search.php?srch_id='+type+'&manuf_id='+manuf_id+'&vendor='+vendor,'location_popup','width=1237,height=500,left=180,scrollbars=no,top=150');
	win.focus();
}
...
if(isset($_REQUEST['save']))
{
	...
	$savedId = supply_stock($edit_item_id,$manufacturer,$upc_name,$item_prac_code,$module_type,$name,$vendor,$type_desc,$num_size,$measurement,$char_size,$other,$hazardous,$qty_on_hand,$amount,$retail_price,$discount,$disc_date,$formula_save,$retailpriceFlag);
...
<div class="listheading mt10">
	<div style="width:1045px; float:left;">Supplies</div>
	<div>
...
<div style="height:<?php echo $_SESSION['wn_height']-450;?>px width: 0px background-color: #008000;">
	...
	<input type="hidden" name="module_type" id="module_type" value="5">
	<table class="table_collapse table_cell_padd5">
		<tr>
            ...
            <td style="width:229px;">
            	...
            	<?php $rows="";
                          $rows = data("select id, manufacturer_name from in_manufacturer_details where supplies_chk='1' and del_status='0' order by manufacturer_name asc");
                          foreach($rows as $r)
                          { ?>
	...
	<div class="module_border mt15">
		<table class="table_collapse table_cell_padd5">
			<tr>
				...
				<td colspane="2" style="width: 526px;">
			...
			<tr>
				<td colspan="2" style="">
					...
					<select name="measurement" id="measurement" style="width:90px;">
						<option value="">Select</option>
						<?php $sql="select id,measurment_name from in_supplies_measurment where del_status = '0' order by measurment_name asc";
						$res = imw_query($sql);
						while($row = imw_fetch_array($res)) {
						?>
						<option value="<?php echo $row['id']; ?>"><?php echo ucfirst($row['measurment_name']); ?> </option>
						<?php } ?>
					</select>
			...
			<tr>
				<td colspan="2">
					<label for="char_size" style="margin-right: 20px;">Size</label>
					<select name="char_size" id="char_size" style="width: 90px; margin-right: 15px;">
						<option value="">Select</option>
						<?php $sql="select id,size_name from in_supplies_size where del_status = '0'";
						$res = imw_query($sql);
						while($row = imw_fetch_array($res)) {
						?>
						<option value="<?php echo $row['id']; ?>"><?php echo ucfirst($row['size_name']); ?> </option>
						<?php } ?>
					</select>
					<label for="other" style="margin-right: 72px;">Other</label>
					<input type="text" name="other" id="other" style="width:82px;" />
				...
				<td valign="top" style="width: 220px;">
					<span id="qty_on_hand_td" style="font-weight: bold; display: inline-block; margin-right: 46px;">0</span>			<label for="amount">Amount</label> <input class="supply_amt currency" name="amount" id="amount" type="text" style="width: 80px; margin: 0 5px 0 0; position: inherit;" readonly />
				</td>
...
<?php /*			
			<tr>
				...
				<td>
					<input name="wholesale_cost" id="wholesale_cost" type="text" style="width:147px;" onChange="parse_float(this);formula_change(false);" class="currency" />
				</td>
			...
			<tr>
				...
				<td>
					<input name="purchase_price" id="purchase_price" style="width:147px" type="text" onChange="parse_float(this);formula_change(false);" class="currency" />
				</td>
				<td valign="top" colspan="2"></td>
			</tr>
 */ ?>			
			<tr>
				<td>
					<label for="purchase_price">Formula</label>
				</td>
				<td>
					<input name="formula" id="formula" style="width:147px" type="text" class="currency" onChange="formula_change(true);" />
			...
			<tr>
				...
				<td>
					<input name="retail_price" id="retail_price" type="text" style="width:147px;" onChange="price_total_fun();retailPriceChanged();" class="currency" />
				</td>
			</tr>
...
function newForm(){
	window.location.href= WEB_PATH+'/interface/admin/supplies/index.php';
}
