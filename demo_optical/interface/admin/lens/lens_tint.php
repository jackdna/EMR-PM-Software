<?php
	...
	File: lens_tint.php
	Coded in PHP7
	Purpose: Add/Edit/Delete: Lens Tint
	...
	*/
	...
	$q1=imw_query("SELECT * FROM  `in_prac_codes` where module_id=2 and sub_module='tint'");
	$d1=imw_fetch_object($q1);
	$defaultPracCode=back_prac_id($d1->prac_code);
	if($defaultPracCode){
	imw_query("update in_lens_tint set prac_code ='$defaultPracCode' where prac_code='' OR prac_code=0")or die(imw_error());
	}
	//-------- UPDATE AND INSERT NEW LENS TINT-------//
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					$rec_tinttype = trim($_POST['tinttype'][$rec_id]);
					$rec_prac_code = trim($_POST['item_prac_code'][$rec_id]);
					$procedureId = back_prac_id($rec_prac_code);
					if($rec_tinttype!="")
					{			
						$updateQry = "update in_lens_tint set tint_type = '".imw_real_escape_string($rec_tinttype)."', prac_code='$procedureId', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						imw_query($updateQry);
						...
						{
							$edit_time_insert_query = "insert in_lens_tint set tint_type = '".imw_real_escape_string($_POST['input_val'])."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_lens_tint set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_lens_tint set tint_type = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
...
//------------------------	START GETTING DATA FOR MENUS TO Procedure CATEGORY-----------------------//
	...
	{
		$whr = " and tint_type like '".$_REQUEST['alpha']."%' ";	
	}
	$targetpage = "lens_tint.php"; 	
	$limit = 50; 
	$query = "SELECT COUNT(*) as num FROM in_lens_tint where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_tint',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_tint',"+rowid+",'1','del_status',this)");
				}
...
function refrsh()
{
	window.location.href='lens_tint.php';
}

function open_addnew_popup()
{
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=lens_tint&col_name=tint_type&heading=Lens_Tint_Name','Add_new_popup','width=580,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addframe" id="firstform" action="" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td width="650">Lens Tint<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
	...
	<div id="listing_record" style="height:<?php echo $_SESSION['wn_height']-$aprxHght;?>px; overflow-y:auto;">
        ...
        <tbody class="table_cell_padd2">
            <?php 
                ...
                $sql="select * from in_lens_tint where del_status != '2' $whr order by tint_type asc LIMIT $start, $limit";
                $res = imw_query($sql);
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td width="620">
                <input type="text" value="<?php echo $row["tint_type"]; ?>" class="tinttype_field" id="tinttype_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="tinttype[<?php echo $row['id']; ?>]" style="width:600px;"/>
              </td>
			  <td width="240">
                <input type="text" value="<?php echo $proc_code_arr[$row['prac_code']];?>" id="item_prac_code_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>'); show_price_from_praccode(this,'','admin');" name="item_prac_code[<?php echo $row['id']; ?>]" style="width:230px;" title="<?php echo $proc_code_desc_arr[$row['prac_code']];?>" autocomplete="off" />
              </td>
              <td align="center" style="width:70px;">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_tint','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_tint','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
...
$(document).ready(function()
{
	selectCurrentCheck = function(ab)
	{
	   ...
	   var currentval = $('#tinttype_field_'+ab);
	
		if($.trim(currentval.val()) == "")
		{
				top.falert('Please Enter Tint Type');	
				setTimeout(function(){$( currentval ).focus(); },0);
		}
		else
		{
			$(".tinttype_field").each(function( index ) 
			{
