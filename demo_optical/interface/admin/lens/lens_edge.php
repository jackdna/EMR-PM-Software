<?php
	...
	File: lens_edge.php
	Coded in PHP7
	Purpose: Add/Edit/Delete: Lens Edge
	...
	*/
	...
	$q1=imw_query("SELECT * FROM  `in_prac_codes` where module_id=2 and sub_module='edge'");
	$d1=imw_fetch_object($q1);
	$defaultPracCode=back_prac_id($d1->prac_code);
	if($defaultPracCode){
	imw_query("update in_lens_edge set prac_code ='$defaultPracCode' where prac_code='' OR prac_code=0")or die(imw_error());
	}
	//-------- UPDATE AND INSERT LENS EDGE-------//
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			...
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				...
				if(trim($_POST['del_hidden'])=="")
				{
					...
					$rec_edgename = trim($_POST['edgename'][$rec_id]);
					$rec_prac_code = trim($_POST['item_prac_code'][$rec_id]);
					$procedureId = back_prac_id($rec_prac_code);
					
					if($rec_edgename!="")
					{			
						$updateQry = "update in_lens_edge set edge_name = '".imw_real_escape_string($rec_edgename)."', prac_code='$procedureId', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						...
						{
							$edit_time_insert_query = "insert in_lens_edge set edge_name = '".imw_real_escape_string($rec_edgename_new)."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_lens_edge set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
		...
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_lens_edge set edge_name = '".imw_real_escape_string($rec_edgename_new)."' ");
			}
		}
...
//------------------------	START GETTING DATA FOR MENUS TO Procedure CATEGORY-----------------------//
	...
	{
		$whr = " and edge_name like '".$_REQUEST['alpha']."%' ";	
	}
	$targetpage = "lens_edge.php"; 	
	$limit = 50; 
	$query = "SELECT COUNT(*) as num FROM in_lens_edge where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_edge',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_edge',"+rowid+",'1','del_status',this)");
				}
...
function refrsh()
{
	window.location.href='lens_edge.php';
}

function open_addnew_popup()
{
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=lens_edge&col_name=edge_name&heading=Lens_Edge_Name', 'Add_new_popup', 'width=580,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addframe" id="firstform" action="" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td width="650">Edge Name<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
	...
	<div id="listing_record" style="height:<?php echo $_SESSION['wn_height']-$aprxHght;?>px; overflow-y:auto;">
        ...
        <tbody class="table_cell_padd2">
            <?php 
                ...
                $sql="select * from in_lens_edge where del_status != '2' $whr order by edge_name asc LIMIT $start, $limit";
                $res = imw_query($sql);
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td width="620">
                <input type="text" value="<?php echo $row["edge_name"]; ?>" class="edgename_field" id="edgename_field_<?php echo $i; ?>"  onChange="selectCurrentCheck('<?php echo $i; ?>')" name="edgename[<?php echo $row['id']; ?>]" style="width:600px;"/>
              </td>
			  <td width="240">
              ...
              <td align="center" style="width:80px;">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_edge','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_edge','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
...
$(document).ready(function()
{
	selectCurrentCheck = function(ab)
	{
	   ...
	   var currentval = $('#edgename_field_'+ab);
	
		if($.trim(currentval.val()) == "")
		{
				top.falert('Please Enter Edge Name');	
				setTimeout(function(){$( currentval ).focus(); },0);
		}
		else
		{
			$(".edgename_field").each(function( index ) 
			{
