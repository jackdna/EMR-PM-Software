<?php
	...
	*/
	...
	$q1=imw_query("SELECT `prac_code` FROM  `in_prac_codes` where module_id=2 and sub_module='design'");
	$d1=imw_fetch_object($q1);
	$defaultPracCode=back_prac_id($d1->prac_code);
	if($defaultPracCode){
	imw_query("update in_lens_design set prac_code ='$defaultPracCode' where prac_code='' OR prac_code=0")or die(imw_error());
	}
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					$rec_vw_code = trim($_POST['vw_code'][$rec_id]);
					$rec_vw_code_type = trim($_POST['vw_code_type'][$rec_id]);
					...
					{			
						$updateQry = "update in_lens_design set design_name = '".imw_real_escape_string($rec_treatmentname)."', prac_code='$procedureId', vw_code='$rec_vw_code', lens_vw_code='$rec_vw_code_type', wholesale_price='".$rec_wholesale_price."', purchase_price='".$rec_purchase_price."', retail_price='".$rec_retail_price."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";
						...
						{
							$edit_time_insert_query = "insert in_lens_design set design_name = '".imw_real_escape_string($_POST['input_val'])."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_lens_design set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_lens_design set design_name = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
...
//------------------------	START GETTING DATA FOR MENUS TO Procedure CATEGORY-----------------------//
	...
	if($_REQUEST['alpha']=='09'){
		$whr = "  and (design_name LIKE '1%' OR design_name LIKE '2%'  OR design_name LIKE '3%'  OR design_name LIKE '4%' OR design_name LIKE '5%' OR design_name LIKE '6%' OR design_name LIKE '6%' OR design_name LIKE '8%' OR design_name LIKE '9%' OR design_name LIKE '0%')";
	}else if($_REQUEST['alpha'] && $_REQUEST['alpha']!='az')
	{
		$whr = " and design_name like '".$_REQUEST['alpha']."%' ";	
	}
	if(isset($_REQUEST['filter_type']) && $_REQUEST['filter_type']!="" && $_REQUEST['filter_type']!="0"){
		$whr .= " and lens_vw_code='".$_REQUEST['filter_type']."'";
	}
	$targetpage = "lens_design.php"; 	
	$limit = 50; 
	$query = "SELECT COUNT(*) as num FROM in_lens_design where del_status != '2' $whr";
...
/*Type/Seg Type List*/
$seg_types = array();
$seg_qry = "SELECT `type_name`, `vw_code` FROM `in_lens_type` WHERE `del_status`='0' ORDER BY FIELD(`vw_code`, 'SV','PAL','BFF','TFF')";
$seg_resp = imw_query($seg_qry);
while($row = imw_fetch_object($seg_resp)){
	$seg_types[$row->vw_code] = $row->type_name;
}
?>
<!DOCTYPE html>
<html>
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_design',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_design',"+rowid+",'1','del_status',this)");
				}
...
function refrsh()
{
	window.location.href='lens_design.php';
}

function open_addnew_popup()
{
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=lens_design&col_name=design_name&heading=Lens_Design_Name', 'Add_new_popup', 'width=1048,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<body>
<form method="GET" id="filter_form" name="filter_form" action="">
	<input type="hidden" name="filter_type" id="filter_type" />
	<input type="hidden" name="param" id="param" value="<?php echo $_REQUEST['param']; ?>" />
	<input type="hidden" name="alpha" id="alpha" value="<?php echo $_REQUEST['alpha']; ?>" />
...
<form onSubmit="return validateform()" name="addframe" id="firstform" action="" method="post" class="mt10">
    ...
    <table class="table_collapse">
		<thead style="position:fixed;padding-right:15px;z-index:1;">
			<tr class="listheading">
			  <td style="width:20px;">
				<input type="hidden" id="del_hidden" name="del_hidden" value="" />
				<input type="checkbox" id="selectall" value="" /></td>
			  <td style="width:225px;">Lens Design</td>
			  <td style="width:169px;">Type
				<select name="filter_dd" id="filter_dd" style="width:124px;" onChange="filter_vals()">
				<option value="0">All</option>
				<?php  foreach($seg_types as $key=>$type): 
					$selected = (isset($_REQUEST['filter_type']) && $_REQUEST['filter_type']==$key)?"selected":"";
				?>
					<option value="<?php echo $key; ?>" <?php echo $selected; ?>><?php echo $type; ?></option>
				<?php endforeach; ?>
				</select>
			  </td>
			  <td style="width:151px;">VisionWeb Code</td>
			  <td style="width:152px;">Billing Prac. Code</td>
			  <td style="width:119px;">Wholesale Price</td>
			  <td style="width:108px;">Purchase Price</td>
        ...
        <tbody style="padding-top:25px;display:block;">
            ...
            <?php
                $sql="select * from in_lens_design where del_status != '2' $whr order by design_name asc LIMIT $start, $limit";
                $res = imw_query($sql);
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td>
                <input type="text" value="<?php echo $row["design_name"]; ?>" class="treatmentname_field" id="treatmentname_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="treatmentname[<?php echo $row['id']; ?>]"  style="width:95%;"/>
              </td>
			  <td>
			  	<select name="vw_code_type[<?php echo $row['id']; ?>]" id="vw_code_type<?php echo $i; ?>" style="width:98%;" onChange="selectCurrentCheck('<?php echo $i; ?>');">
	                <option value="">Please Select</option>
					<?php  
						foreach($seg_types as $key=>$type){
							$selected = ($row["lens_vw_code"]==$key)?'selected="selected"':'';
					?>
						<option value="<?php echo $key; ?>" <?php echo $selected ; ?>><?php echo $type; ?></option>
					<?php
						}
              ...
              <td>
                <input type="text" value="<?php echo $row['vw_code'];?>" id="vw_code_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>');" name="vw_code[<?php echo $row['id']; ?>]" style="width:94%;"/>
              </td>
			  <td>
                <input type="text" value="<?php echo $proc_code_arr[$row['prac_code']];?>" id="item_prac_code_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>');" name="item_prac_code[<?php echo $row['id']; ?>]" style="width:94%;" title="<?php echo $proc_code_desc_arr[$row['prac_code']];?>" autocomplete="off"/>
              </td>
			  <td>
			  ...
			  <td>
			  	<input type="text" value="<?php echo $row['retail_price']; ?>" id="retail_price_<?php echo $i; ?>" name="retail_price[<?php echo $row['id']; ?>]" onChange="convert_float(this);selectCurrentCheck('<?php echo $i; ?>');" style="width:94%;" />
			  </td>
              <td align="center">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_design','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_design','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
...
<div class="btn_cls">
	...
	<li class="fl"><a style="border-radius:5px; padding:2px 6px; color:#fff; text-decoration:none; font-weight:bold; text-transform:uppercase; margin:3px; background:<?php if($_REQUEST['alpha']=="az") { echo "#fb9608"; } else { echo "#09F"; } ?>" href="?alpha=az&filter_type=<?php echo $_REQUEST['filter_type']; ?>">A-Z</a></li>
	<?php foreach($alpha as $key=>$value) 
	{ ?>
	<li class="fl"><a style="border-radius:5px; padding:2px 6px; color:#fff; text-decoration:none; font-weight:bold; text-transform:uppercase; margin:2.3px; background:<?php if($_REQUEST['alpha']==$value) { echo "#fb9608"; } else { echo "#09F"; } ?>" href="?alpha=<?php echo $value; ?>&filter_type=<?php echo $_REQUEST['filter_type']; ?>"><?php echo $value; ?></a></li>
	<?php } ?>
	</ul>
</div>
</form>
...
function filter_vals(){
	var filter_val = $("#filter_dd").val();
	$("#filter_type").val(filter_val);
