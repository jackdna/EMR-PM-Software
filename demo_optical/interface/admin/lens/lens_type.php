<?php
	...
	File: lens_type.php
	Coded in PHP7
	Purpose: Add/Edit/Delete: Lense Type
	...
	*/
	...
	$q1=imw_query("SELECT * FROM  `in_prac_codes` where module_id=2 and sub_module='type'");
	$d1=imw_fetch_object($q1);
	$defaultPracCode=back_prac_id($d1->prac_code);
	if($defaultPracCode){
	imw_query("update in_lens_type set prac_code ='$defaultPracCode' where prac_code='' OR prac_code=0")or die(imw_error());
	}
	
	//------------- UPDATE AND INSERT NEW LENS TYPE --------------//
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					$rec_typename = trim($_POST['typename'][$rec_id]);
					...
					if($rec_typename!="")
					{			
						$updateQry = "update in_lens_type set type_name = '".imw_real_escape_string($rec_typename)."', prac_code='$procedureId',vw_code='$rec_vw_code_type', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						imw_query($updateQry);
						...
						{
							$edit_time_insert_query = "insert in_lens_type set type_name = '".imw_real_escape_string($_POST['input_val'])."',vw_code='$rec_vw_code_type' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_lens_type set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_lens_type set type_name = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
...
//------------------------	START GETTING DATA FOR MENUS TO Procedure CATEGORY-----------------------//
	...
	{
		$whr = " and type_name like '".$_REQUEST['alpha']."%' ";	
	}
	$targetpage = "lens_type.php"; 	
	$limit = 50; 
	$query = "SELECT COUNT(*) as num FROM in_lens_type where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_type',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_lens_type',"+rowid+",'1','del_status',this)");
				}
...
function refrsh()
{
	window.location.href='lens_type.php';
}

function open_addnew_popup()
{
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=lens_type&col_name=type_name&heading=Lens_Type_Name','Add_new_popup','width=580,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addframe" id="firstform" action="" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td width="630">Lens<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
		   <td width="170">Type</td>
           <td width="150">Prac Code</td>
          <td width="100" align="center" style="width:100px;">Status</td>
        </tr>
        </table>
        <?php
        ...
        <tbody class="table_cell_padd2">
            <?php 
                ...
                $sql="select * from in_lens_type where del_status != '2' $whr order by type_name asc LIMIT $start, $limit";
                $res = imw_query($sql);
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td style="width:630px;">
                <input type="text" value="<?php echo $row["type_name"]; ?>" class="typename_field" id="typename_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="typename[<?php echo $row['id']; ?>]" style="width:600px;"/>
              </td>
               <td style="width:170px;">
                <select name="vw_code_type[<?php echo $row['id']; ?>]" id="vw_code_type_<?php echo $i; ?>" style="width:150px;" onChange="selectCurrentCheck('<?php echo $i; ?>');">
                	<option value="">Type</option>
                	<option value="BFF" <?php if($row["vw_code"]=="BFF"){echo"selected";} ?>>BiFocal</option>
                    <option value="PAL" <?php if($row["vw_code"]=="PAL"){echo"selected";} ?>>Progressive</option>
                    <option value="SV" <?php if($row["vw_code"]=="SV"){echo"selected";} ?>>Single Vision</option>
                    <option value="TFF" <?php if($row["vw_code"]=="TFF"){echo"selected";} ?>>TriFocal</option>
                </select>
              </td>
			  <td style="width:170px;">
                <input type="text" value="<?php echo $proc_code_arr[$row['prac_code']];?>" id="item_prac_code_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>'); show_price_from_praccode(this,'','admin');" name="item_prac_code[<?php echo $row['id']; ?>]" style="width:170px;" title="<?php echo $proc_code_desc_arr[$row['prac_code']];?>"/>
              </td>
              <td align="center">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_type','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_lens_type','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
...
$(document).ready(function()
{
	selectCurrentCheck = function(ab)
	{
	   ...
	   var currentval = $('#typename_field_'+ab);
	
		if($.trim(currentval.val()) == "")
		{
				top.falert('Please Enter Lens Type');	
				setTimeout(function(){$( currentval ).focus(); },0);
		}
		else
		{
			$(".typename_field").each(function( index ) 
			{
