<?php
	/*
	File: min_seg_ht.php
	Purpose: Add/Edit/Delete: Min Segment Hight
	...
	*/
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					if($rec_labname!="")
					{		
						$updateQry = "update in_min_seg_ht set min_seg_name = '".imw_real_escape_string($rec_labname)."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						imw_query($updateQry);
						...
						{
							$edit_time_insert_query = "insert in_min_seg_ht set min_seg_name = '".imw_real_escape_string($_POST['input_val'])."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_min_seg_ht set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_min_seg_ht set min_seg_name = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
	...
	{
		$whr = " and min_seg_name like '".$_REQUEST['alpha']."%' ";	
	}
	
	$targetpage = "min_seg_ht.php"; 	
	$limit = 30; 
	$query = "SELECT COUNT(*) as num FROM in_min_seg_ht where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_min_seg_ht',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_min_seg_ht',"+rowid+",'1','del_status',this)");
				}
...
function refrsh()
{
	window.location.href='min_seg_ht.php';
}

function open_addnew_popup()
{
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=min_seg_ht&col_name=min_seg_name&heading=Min_Seg_ht_Name', 'Add_new_popup', 'width=320,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addframe" id="firstform" action="" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td style="width:auto;">Min Seg Ht <div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
	...
	<div id="listing_record" style="height:<?php echo $_SESSION['wn_height']-$aprxHght;?>px; overflow-y:auto;">
        ...
        <tbody class="table_cell_padd2">
            <?php 
                ...
                $sql="select * from in_min_seg_ht where del_status != '2' $whr order by min_seg_name asc LIMIT $start, $limit";
                $res = imw_query($sql);
            ...
            <tr class="<?php echo $rowbg;?>">
              ...
              <td style="width:auto;">
                <input type="text" value="<?php echo $row["min_seg_name"]; ?>" class="materialname_field" id="materialname_field_<?php echo $i; ?>"  onChange="selectCurrentCheck('<?php echo $i; ?>')" name="labname[<?php echo $row['id']; ?>]" style="width:600px;"/>
              </td>
              <td align="center" style="width:70px;">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_min_seg_ht','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_min_seg_ht','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
