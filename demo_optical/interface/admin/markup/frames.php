...
/*Manufacturers List*/
$manufacturers = array();
$sql_manuf = 'SELECT `id`, `manufacturer_name` AS \'name\' FROM `in_manufacturer_details` WHERE `del_status`=0 AND `frames_chk`=1 ORDER BY `manufacturer_name` ASC';
...
<div class="tab_container" style="float:left; width:100%;margin:10px 0 0 0;">
	<h3 class="listheading" style="margin-top:0;">Frames Retail Price Markup</h3>
...
$sql = 'SELECT 
			...
			`pm`.`brand_id`, 
			`brand`.`frame_source` AS \'brand_name\', 
			`pm`.`style_id`, 
			`style`.`style_name`, 
			`pm`.`formula` 
		FROM 
			...
			LEFT JOIN `in_frame_sources` `brand` ON(`pm`.`brand_id` = `brand`.`id`) 
			LEFT JOIN `in_frame_styles` `style` ON(`pm`.`style_id` = `style`.`id`) 
		WHERE 
			`pm`.`module_type_id` = 1 
			AND `pm`.`del_status` = 0 
		ORDER BY 
...
?>
				<tr class="<?php echo $row_class; ?>" id="markup_<?php echo $sr_no; ?>">
					...
					<td><?php echo ( $row->brand_id == 0 ) ? 'ALL' : html_entity_decode($row->brand_name); ?></td>
					<td><?php echo ( $row->style_id == 0 ) ? 'ALL' : html_entity_decode($row->style_name); ?></td>
					<td>
...
/*Brands List*/
	var brand = $('<select>').attr({name: 'item_brand['+sr_no+']'}).prop('disabled', true);
	/*Options*/
	var brand_opt = $('<option/>').val(0).text('ALL');
	$(brand).html(brand_opt);
	$('<td/>').html(brand).appendTo(tr);
	
/*Styles List*/
	var style = $('<select>').attr({name: 'item_style['+sr_no+']'}).prop('disabled', true);
	/*Options*/
	var style_opt = $('<option/>').val(0).text('ALL');
	$(style).html(style_opt);
	$('<td/>').html(style).appendTo(tr);
...
/*Bind Events to the dropDowns*/
	...
	$(manuf).on('change', function(){
		...
		if( manuf_val == 0 ){
			$(brand).html(brand_opt).prop('disabled', true);
			$(style).html(style_opt).prop('disabled', true);
			$(formula_field).val('').prop('disabled', true);
			$(chkbx).prop('checked', false);
			/*Mark Chekcbox*/
		}
		else{
			var formData = {action: 'get_brand', mid: manuf_val, return_json: true};
			$.ajax({
				method	: 'POST',
				...
				success: function(resp){
					...
					$.each(resp, function(i, val){
						$('<option/>').val(val).text(i).appendTo(brand);
					});
	...
	$(brand).on('change', function(){
		...
		if( brand_val == 0 ){
			...
			var formData = {action: 'get_style', bid: brand_val, return_json: true};
			$.ajax({
				method	: 'POST',
				url		: '<?php echo $GLOBALS['WEB_PATH']; ?>/interface/admin/ajax.php',
				data	: formData,
				...
				success: function(resp){
					...
					resp = $.parseJSON(resp);
					$.each(resp, function(i, val){
						$('<option/>').val(val).text(i).appendTo(style);
					});
...
function submitForm(){
	...
	if(checkedRows.length == 0){
		top.falert('Please select records to save.');
		return;
	}
	...
	$.each(checkedRows, function(index, obj){
		...
		if( uid!='' ){
			...
			data.brand	= $('select[name="item_brand['+srNo+']"]').val();
			data.style	= $('select[name="item_style['+srNo+']"]').val();
			data.formula= $('input[name="formula['+srNo+']"]').val();
			formData.add.push(data)
		}
		formData.moduleTypeId = 1;
	});
