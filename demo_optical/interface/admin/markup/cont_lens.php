...
/*Manufacturers List*/
$manufacturers = array();
$sql_manuf = 'SELECT `id`, `manufacturer_name` AS \'name\' FROM `in_manufacturer_details` WHERE `del_status`=0 AND `cont_lenses_chk`=1 ORDER BY `manufacturer_name` ASC';
...
<div class="tab_container" style="float:left; width:100%;margin:10px 0 0 0;">
	<h3 class="listheading" style="margin-top:0;">Contact Lens Retail Price Markup</h3>
...
$sql = 'SELECT 
		...
		FROM 
			...
			LEFT JOIN `in_contact_brand` `brand` ON(`pm`.`brand_id` = `brand`.`id`)
		WHERE 
			`pm`.`module_type_id` = 3 
			AND `pm`.`del_status` = 0 
		ORDER BY 
...
/*Bind Events to the dropDowns*/
	...
	$(manuf).on('change', function(){
		...
		if( manuf_val == 0 ){
			...
			var formData = {action: 'get_brand_contact', mid: manuf_val, return_json: true};
			$.ajax({
				method	: 'POST',
