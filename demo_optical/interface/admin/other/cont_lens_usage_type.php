...
Coded in PHP7
Purpose: Add/Edit/Delete: Contact Lense Type
	...
	if(count($_POST['select_record'])>0){
		for($v=0;$v<count($_POST['select_record']);$v++){
			if(trim($_POST['del_hidden'])==""){
				...
				if($opt_val!="" || $rec_id!=""){
					$updateQry = "UPDATE `in_options` SET `opt_val`='".imw_real_escape_string($opt_val)."', `modified_date`='$date', `modified_time`='$time', `opt_sub_type`='".$opt_sub_type."', `modified_by`='$opr_id' WHERE `id` = '".$rec_id."' AND `opt_type`='2' and module_id='0'";
					imw_query($updateQry);
				...
				$rec_id = $_POST['select_record'][$v];
				$updateQry = "";
				$updateQry = "UPDATE `in_options` SET `del_status`='2', `del_date`='".$date."', `del_time`='".$time."', `del_by`='".$opr_id."' WHERE `id`='".$rec_id."' AND `opt_type`='2' and module_id='0'";
				imw_query($updateQry);					
			}
		}
	}
...
if($_REQUEST['alpha'] && $_REQUEST['alpha']!='az'){
	...
	$limit = 50; 
	$query = "SELECT COUNT(*) AS 'num' FROM `in_options` WHERE `del_status`!='2' and module_id='0' AND `opt_type`='2' ".$whr;
...
function open_addnew_popup(){
	...
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=lens_type_common&col_name=opt_val&heading=Lens_Type_Common&m_id=0','Add_new_popup','width=450,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addcontactLensUsage" id="firstform" method="post" class="mt10">
  ...
  <?php
	...
	$sql="SELECT * FROM `in_options` WHERE `del_status`!='2' ".$whr." AND `opt_type`='2' and module_id='0' ORDER BY `opt_val` ASC LIMIT ".$start.", ".$limit;
	$res = imw_query($sql);
