...
Coded in PHP7
Purpose: Add/Edit/Delete: Contact Lense Usage Options
	...
	if(count($_POST['select_record'])>0){
		for($v=0;$v<count($_POST['select_record']);$v++){
			if(trim($_POST['del_hidden'])==""){
				...
				$opt_sub_type = trim($_POST['sub_type'][$rec_id]);
				if($opt_val!="" || $rec_id!=""){
					$updateQry = "UPDATE `in_options` SET `opt_val`='".imw_real_escape_string($opt_val)."', `opt_sub_type`='".$opt_sub_type."', `modified_date`='$date', `modified_time`='$time', `modified_by`='$opr_id' WHERE `id` = '".$rec_id."' AND `opt_type`='1' and module_id='0'";
					imw_query($updateQry);
				...
				$rec_id = $_POST['select_record'][$v];
				$updateQry = "";
				$updateQry = "UPDATE `in_options` SET `del_status`='2', `del_date`='".$date."', `del_time`='".$time."', `del_by`='".$opr_id."' WHERE `id`= '".$rec_id."' AND `opt_type`='1' and module_id='0'";
				imw_query($updateQry);					
			}
		}
	}
...
if($_REQUEST['alpha'] && $_REQUEST['alpha']!='az'){
	...
	$limit = 50; 
	$query = "SELECT COUNT(*) AS 'num' FROM `in_options` WHERE `del_status`!='2' and module_id='0' AND `opt_type`='1' ".$whr;
...
function refrsh(){
	window.location.href='cont_lens_usage.php';
}

function open_addnew_popup(){
	top.WindowDialog.closeAll();
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=lens_usage&col_name=opt_val&heading=Lenses_Usage&m_id=0','Add_new_popup','width=450,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
  ...
  <table class="table_collapse">
    ...
    <table class="table_collapse">
      <tbody class="table_cell_padd2">
        <?php 
	$usage_options = array('Both', 'Contact Lens', 'Lens');
	$sql="SELECT * FROM `in_options` WHERE `del_status`!='2' ".$whr." AND `opt_type`='1' and module_id='0' ORDER BY `opt_val` ASC LIMIT ".$start.", ".$limit;
	$res = imw_query($sql);
...
?>
        <tr class="<?php echo $rowbg;?>">
          ...
          <td width="520"><input type="text" value="<?php echo $row["opt_val"]; ?>" class="opt_val_field" id="opt_val_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="option[<?php echo $row['id']; ?>]" style="width:600px;"/></td>
		  <td width="100">
			<select name="sub_type[<?php echo $row['id']; ?>]" id="sub_type_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')">
				<?php 
					foreach($usage_options as $key=>$val){
						$sel = ($key==$row['opt_sub_type'])?'selected="selected"':'';
...
<script type="text/javascript">
function submitFrom(){
	document.addcontactLensUsage.submit();
}
$(document).ready(function(){
