<?php
	...
	File: reason.php
	Coded in PHP7
	Purpose: View Reason for return orders
	...
	*/
	...
	{	
		if(count($_POST['select_record'])>0)
		{
			for($v=0;$v<count($_POST['select_record']);$v++)
			{
				if(trim($_POST['del_hidden'])=="")
				{
					...
					if($rec_reasonname!="")
					{			
						$updateQry = "update in_reason set reason_name = '".imw_real_escape_string($rec_reasonname)."', modified_date='$date', modified_time='$time', modified_by='$opr_id' where id = '".$rec_id."' ";		
						...
						{
							$edit_time_insert_query = "insert in_reason set reason_name = '".imw_real_escape_string($_POST['input_val'])."' ";					
						}
					}
				}
				else
				{
					...
					$rec_id = $_POST['select_record'][$v];
					$updateQry = "";
					$updateQry = "update in_reason set del_status = '2', del_date='$date', del_time='$time', del_by='$opr_id' where id = '".$rec_id."' ";
					imw_query($updateQry);					
				}
			}
		}
		else
		{
			if(trim($_POST['input_val'])!="")
			{
				$msg = "Record(s) Saved Successfully";
				$msg_stat = "block";
				imw_query("insert in_reason set reason_name = '".imw_real_escape_string($_POST['input_val'])."' ");
			}
		}
	...
	{
		$whr = " and reason_name like '".$_REQUEST['alpha']."%' ";	
	}
	$targetpage = "reason.php"; 	
	$limit = 50;
	$query = "SELECT COUNT(*) as num FROM in_reason where del_status != '2' $whr";
		...
		success: function(response)
		{
			if(response=="true")
			{
				if(value==1)
				{
					...
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_reason',"+rowid+",'0','del_status',this)");
				}
				else if(value==0)
				{
					$('#status'+rowid).attr('src','../../../images/on.png');
					$('#status'+rowid).attr('title','Active');
					$('#status'+rowid).attr("onclick","javascript:setStatus('in_reason',"+rowid+",'1','del_status',this)");
				}
...
function open_addnew_popup()
{
	...
	var Add_new_popup=top.WindowDialog.open('Add_new_popup',WEB_PATH+'/interface/admin/add_new.php?module_name=reason&col_name=reason_name&heading=Reason','Add_new_popup','width=320,height=380,left=300,scrollbars=no,top=80,fullscreen=0,resizable=0');
	Add_new_popup.focus();
}
...
<form onSubmit="return validateform()" name="addframe" id="firstform" method="post" class="mt10">
    <table class="table_collapse">
        <tr class="listheading">
          ...
          <td style="width:620px;">Reason<div class="success_msg" style="display:<?php echo $msg_stat;?>;"><?php echo $msg; ?></div></td>
    ...
    <table class="table_collapse">
        <tbody class="table_cell_padd2">
            <?php 
                ...
                $sql="select * from in_reason where del_status != '2' $whr order by reason_name asc LIMIT $start, $limit";
                $res = imw_query($sql);
                ...
                if($num>0)
                {
                    ...
                    while($row = imw_fetch_array($res))
                    {
						...
						$disabled="";
						if($row['not_editable']==1){
							$disabled="disabled";
						}
						
            ?>
            <tr class="<?php echo $rowbg;?>">
              <td style="width:20px;"><input type="checkbox" class="getchecked" value="<?php echo $row['id']; ?>" id="checked_<?php echo $i; ?>" name="select_record[]" <?php echo $disabled;?> /></td>
              <td style="width:620px;">
                <input type="text" value="<?php echo $row["reason_name"]; ?>" class="reasonname_field" id="reasonname_field_<?php echo $i; ?>" onChange="selectCurrentCheck('<?php echo $i; ?>')" name="reasonname[<?php echo $row['id']; ?>]" style="width:600px;"  <?php echo $disabled;?>/>
              </td>
              <td align="center" style="width:70px;">
                <?php if($status=="1") 
                { ?>	
                <img id="status<?php echo $row['id']; ?>" title="InActive" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_reason','<?php echo $row['id']; ?>','0','del_status',this)" src="../../../images/off.png" complete="complete"/>
                <?php } else { ?>
                <img id="status<?php echo $row['id']; ?>" title="Active" class="noborder" style="cursor: pointer;" onClick="javascript:setStatus('in_reason','<?php echo $row['id']; ?>','1','del_status',this)" src="../../../images/on.png" complete="complete"/>
                <?php 
                } ?>
              </td>
            </tr>
