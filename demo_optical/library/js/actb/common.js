...
function loadXMLDoc(XMLname)
{
	...
	xmlDoc.open("POST",XMLname,false);
	xmlDoc.send();
...
function readXMLFile(XMLname,ele,hidID,refPhyfax){
	...
	for(i=0; i < fNameTag.length; i++){
		if(typeof(fNameTag[i].text) != "undefined"){
			...
			fname = fNameTag[i].textContent;
			mname = mNameTag[i].textContent;		
			lname = lNameTag[i].textContent;
			title = titleTag[i].textContent;	
			phyID = phyIDNameTag[i].textContent;
			phyFax= phyFaxTag[i].textContent;
		}
		var name = "";
