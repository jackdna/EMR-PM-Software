...
function reportpop(path,report_format){
	var date=document.nc_state_report.date1.value;
	...
	if(date==""){
		...
		document.nc_state_report.action = 'nc_state_report.php';
		document.nc_state_report.submit();
	}
...
function date12(){ 
	var s_date=document.nc_state_report.date1.value;
	if(s_date==""){
...
function resetfields(){
	document.nc_state_report.surgeon.value='';
	document.nc_state_report.date1.value='';
	document.nc_state_report.date2.value='';
	document.nc_state_report.asc.value=""
...
var flPath = '';
	...
	$qry_nc_state_asc="";
	if($asc){
		$qry_nc_state_asc=" AND st.iasc_facility_id = '".$ascArr['facility_id'][$asc]."' ";
	}
	...
	if($date!="") {
		...
		$query = "select pc.patientConfirmationId FROM patientconfirmation pc  
				   ...
				   WHERE  ( pc.dos between '".$selected_date."' AND '".$selected_date2."' ) AND pc.ascId !='0' AND pc.patient_primary_procedure_id !='0' ".$qry_nc_state_asc.$surgeonQry."
		...
		if($patientConfirmationId){
			$pgNme = 'nc_state_report_print.php';
			$winNme = 'nc_state_report_wind';
			
			if($_REQUEST["hidd_report_format"]=="csv") {
				$winNme = 	"_self";
			}
			?>
			<form name="nc_state_report_sub" id="nc_state_report_sub" method="post" action="<?php echo $pgNme;?>" target="<?php echo $winNme;?>" >
			...
			<script>
				...
				if(hidd_report_format_new == 'csv') {
					...
					document.getElementById('nc_state_report_sub').submit();
				}
			</script>
			<?php
		}
   ...
   ?>
<form name="nc_state_report" id="nc_state_report" action="nc_state_report.php" method="post" > 
...
$(document).ready(function() {
	if(hidd_report_format_new == 'csv') {
		if(document.getElementById('nc_state_report_sub')) {
			document.getElementById('nc_state_report_sub').submit();	
		}
	}
});
