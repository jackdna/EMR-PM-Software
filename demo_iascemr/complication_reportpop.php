...
if($_REQUEST['proc_save']=='yes') {
	...
	$qry = "SELECT vs.*,
			...
			AND vs.complication!='' ".$physicianQry."
	...
	$pDetail = array();
	$surgeonUsed = array();
	$total_better_complication = array();
	if(imw_num_rows($res)>0) {
		...
		while($row = imw_fetch_assoc($res)) 
		{
			...
			if($row['fname'] && $recExist == true)
			{
				...
				if(!in_array($surgeon_name,$surgeonUsed)) array_push($surgeonUsed,$surgeon_name);
				$tmpPatientProcNames = '';
				...
				if(!array_key_exists($surgeon_name,$total_better_complication)) $total_better_complication[$surgeon_name] = 0;
				if($row['complication']=='Yes'){
				$total_better_complication[$surgeon_name]+=1;}
		...
		$better_complication_average = array();
		foreach($total_better_complication as $surgeon=>$better_complication)
		{
			if($better_complication)
			{
				$value=0;
				$better_complication_average[$surgeon]=number_format(($better_complication/$total_proc[$surgeon]),2,'.','');
			}	
		}
...
if($_REQUEST['action'] == 'csv')
{
		...
		$csvData	=	array();
		array_push($csvData,array($name, '', 'Surgery Center Complication Report '.date("m-d-Y"),'' ));
		...
		array_push($csvData,array('','','',''));
		array_push($csvData,array('','','',''));
		array_push($csvData,array('Surgeon','Total Procedures','Total Complication','Average Complication'));

		foreach($total_better_complication as $surgeon=>$better_complication)
		{
			$csvDataNode	=	array($surgeon,$total_proc[$surgeon],$better_complication,$better_complication_average[$surgeon]);
			array_push($csvData,$csvDataNode);
		}

		
		$file_name	=	'complication_report.csv';
...
<title>Complication Report</title>    
...
<div class="main_wrapper analysis-top-bar">
		<form name="complication_reportpop_csv" id="complication_reportpop_csv" method="post" action="complication_reportpop.php">
                      ...
                      <div class="wrap_inside_admin">
                        <div class=" subtracting-head">
                            <div class="head_scheduler new_head_slider padding_head_adjust_admin">
                                ...
                                <a href="javascript:void(0)" class="btn btn-info analysis-csv-btn" id="generate_csv" onclick="javascript:document.complication_reportpop_csv.submit();" >
...
<div class="col-md-12 col-sm-12 col-xs-12 col-lg-12 margin_top_5">
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4">
        ...
        <span>Total Surgeries with Complication</span></div>
    ...
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 ">
    	<div class="middle_sub_head text-left">
        <span>Average  Number of Surgeries with Complication</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
    	<div id="average_cost_div" class="col-md-12 col-sm-12 col-xs-12 col-lg-12" style="height:400px"></div> 
	</div>
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 ">
    	<div class="middle_sub_head text-left">
        <span>Mean Number of Surgeries with Complication</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
    	<div id="mean_cost_div" class="col-md-12 col-sm-12 col-xs-12 col-lg-12" style="height:450px"></div> 
	</div>
...
<?php
//create json array to total complication 
$key=0;
$total_complication_arr = array();
foreach($surgeonUsed as $s_name)
{
	$val = array_key_exists($s_name,$total_better_complication) ? $total_better_complication[$s_name] : 0;
	$total_complication_arr[$key]["kee"]=$s_name;
	$total_complication_arr[$key]["val"]=$val;
	$key++;	
}
//create json array for average complication
$key=0;
$average_complication_arr = $mean_complication_arr = array();
foreach($surgeonUsed as $s_name)
{
	$val = array_key_exists($s_name,$better_complication_average) ? $better_complication_average[$s_name] : 0;
	$average_complication_arr[$key]["kee"]=$s_name;
	$average_complication_arr[$key]["val"]=$val;
	
	$mean_complication_arr[$key]["kee"]=$s_name;
	$mean_complication_arr[$key]["val"]=$val;
	$key++;	
}

$total_complication_js_arr=json_encode($total_complication_arr);
$average_complication_js_arr=json_encode($average_complication_arr);
$mean_complication_arr=json_encode($mean_complication_arr);

//create json array for mean
$key=0;
$totalSugeons=0;
foreach($better_complication_average as $surgeon_name=>$complication_average)
{
	$totalSugeonsAverComplication+=$complication_average;
	$totalSugeons++;
}

$meanComplication=number_format($totalSugeonsAverComplication/$totalSugeons,2,'.','');
?>

<script type="text/javascript">
	window.focus();
	<?php if($total_complication_js_arr){?>pie_chart('pie','total_cost_div','<?php echo $total_complication_js_arr; ?>','');<?php }?>
	<?php if($average_complication_js_arr){?>pie_chart('pie','average_cost_div','<?php echo $average_complication_js_arr; ?>','');<?php }?>
	//bar_chart('serial','mean_cost_div','<?php echo $total_cost_js_arr; ?>','','');
	<?php if($mean_complication_arr){?>line_chart2(<?php echo $mean_complication_arr; ?>,'Mean Complication',<?php echo $meanComplication;?>,'mean_cost_div','15','','Complication');<?php }?>
</script>
<!--
---------------------
TOP TEN SURGEON
...
if(imw_num_rows($res)>0) {
	...
	while($row = imw_fetch_assoc($res)) 
	{
		if($row['fname'])
		{
			...
			if($row['complication']=='Yes')
			{
				$total_complication_true[$surgeon_name]+=1;
				//else{$total_complication_false[$surgeon_name]+=1;}
			}
			if( !array_key_exists($surgeon_name,$total_comp_proc)) $total_comp_proc[$surgeon_name] = 0;
			$total_comp_proc[$surgeon_name]++;
		}
	}
}
...
<div class="col-md-12 col-sm-12 col-xs-12 col-lg-12">
    <div class="col-md-6 col-sm-12 col-xs-12 col-lg-6 ">
        <div class="middle_sub_head text-left">
        <span>Top Ten Sugeon by Fewest Procedures with Complication</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
        <div id="highest_div" class="col-md-12 col-sm-12 col-xs-12 col-lg-12" style="height:450px"></div>
    </div>
    <div class="col-md-6 col-sm-12 col-xs-12 col-lg-6 ">
        <div class="middle_sub_head text-left">
        <span>Top Ten Sugeon by Most Procedures with Complication</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
...
$key=0;
$better_arr = array();
foreach($total_comp_proc as $surgeon_name=>$procedures)
{
	$average_cost = array_key_exists($surgeon_name,$total_complication_true) ? $total_complication_true[$surgeon_name] : 0;
	$better_arr[$key]["surgeon"]=$surgeon_name;
	$better_arr[$key]["average_cost"]=$average_cost;
	$better_arr[$key]["no_of_procedures"]=$procedures;
	$key++;
}//$key=0;
//foreach($total_proc as $surgeon_name=>$complications)
//{
	//$worse_arr[$key]["surgeon"]=$surgeon_name;
	//$worse_arr[$key]["average_cost"]=$total_complication_true[$surgeon_name];
...
array_sort_by_column($better_arr, 'average_cost',SORT_ASC);
...
for($i=0;$i<=10;$i++)
{
	if($better_arr[$i])$lowest_arr[]=$better_arr[$i];
}
$lowest_js_arr=json_encode($lowest_arr);
?>
<script type="text/javascript">
	<?php if($highest_js_arr){?>columnAndLine_chart(<?php echo $highest_js_arr; ?>,'highest_div','',15, 'No. of Complications');<?php }?>
	<?php if($lowest_js_arr){?>columnAndLine_chart(<?php echo $lowest_js_arr; ?>,'lowest_div','',15, 'No. of Complications');<?php }?>
</script> 
<!--
---------------------
TABLE FOR DATA
...
<div class="col-md-12 col-sm-12 col-xs-12 col-lg-12 well-sm">
   ...
   <tbody>
      <tr>
         ...
         <th class="col-md-1 col-sm-1 col-xs-1 col-lg-1">Total Complication</th>
         <th class="col-md-1 col-sm-1 col-xs-1 col-lg-1">Ave. Complication</th>
   ...
   <tr>
      <?php
				...
				$pdfSurgeonArray[]	=	array('Surgeon','Total Procedure','Total Complication','Ave. Complication');
				
				foreach($total_better_complication as $surgeon=>$better_complication)
				{
					$pdfSurgeonArray[]	=	array($surgeon,(string)$total_proc[$surgeon],(string)$better_complication,$better_complication_average[$surgeon]);
		?>
         ...
         <td class="col-md-1 col-sm-1 col-xs-1 col-lg-1"><?php echo $better_complication;?></td>
         <td class="col-md-1 col-sm-1 col-xs-1 col-lg-1"><?php echo $better_complication_average[$surgeon];?></td>
      <?php 
	  $rowCount++;
...
{	
		...
		var _pdfReportTitle	=	' Complication Report <?=date('m-d-Y')?> ';
		var _pdfSaveAsName	=	'complication_report.pdf';
		...
		_pdfContent.push({
			...
			[
					{
						width: "33%",
						text: 'Total Surgeries with Complication',
						style:'boxTitle'
					},
					{
						width: "33%",
						text: 'Average Number of Surgeries with Complication',
						style:'boxTitle'
						
					},
					{
						width: "*",
						text: 'Mean Number of Surgeries with Complication',
						style:'boxTitle'
					}
			],
			columnGap: 5
		...
		_pdfContent.push({
			...
			[
				{
					width: "50%",
					text: "Top Ten Sugeon by Fewest Procedures with Complication",
					style: "boxTitle",
				},
				{
					width: "*",
					text: "Top Ten Sugeon by Most Procedures with Complication",
					style: "boxTitle",
				}
			],
			columnGap: 5,
		...
		_pdfContent.push({ 
			text:' Complication Data ', style:'boxTitle', margin:[0,20,0,0]
		});
