...
//select the detail from database
	$ViewlaserprocedureQry = "select *, date_format(signSurgeon1DateTime,'%m-%d-%Y %h:%i %p') as signSurgeon1DateTimeFormat, date_format(signNurseDateTime,'%m-%d-%Y %h:%i %p') as signNurseDateTimeFormat from `laser_procedure_patient_table` where  confirmation_id = '".$pConfId."'";
...
//SHOW DETAIL OF PATIENT PRE OP MEDICATION
	...
	$preOpPatientDetails = $objManageData->getArrayRecords('patientpreopmedication_tbl', 'patient_confirmation_id', $pConfId);
//SHOW DETAIL OF PATIENT PRE OP MEDICATION
