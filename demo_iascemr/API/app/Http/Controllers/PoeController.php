...
class PoeController extends Controller {
    ...
    public function construct($pid = "", $eid = "") {
        $this->pId = $pid;
        $this->eId = $eid;
        $this->arPoeId = array();
        $this->arPoeCpt = array();
    ...
    public function isPoeCode($cpt) {
        ...
        if (!empty($cpt)) {
            ...
            $row = DB::connection('DB_REGISTER_CONNECTION')->selectone($sql); //$this->db->Execute($sql) or die("Error In Poe :: isPoeCode");
            if ($row) {
    ...
    private function getDOS() {
        ...
        if (!empty($this->eId)) {
            ...
            $row = DB::connection('DB_REGISTER_CONNECTION')->selectone($sql); //$this->db->Execute($sql) or die("Error In Poe :: getDOS-1");
            if ($row) {
            ...
            if ($ret == false) {
                ...
                $row = DB::connection('DB_REGISTER_CONNECTION')->selectone($sql); //$this->db->Execute($sql) or die("Error In Poe :: getDOS-2");
                if ($row) {
    ...
    function get_poe_days($poe_id, $poe_cpt) {
        if (!empty($poe_cpt)) {
            ...
            $row = DB::connection('DB_REGISTER_CONNECTION')->selectone($sql);
            $poe_id_n = 0;

            if ($row) {
                if (!empty($row->elem_poe))
                    $poe_id_n = $row->elem_poe;
            }
            if (!empty($poe_id_n)) {
                $poe_id = $poe_id_n;
            }
    ...
    private function getDOSSaved($flgNotCurEnc = 0) {
        ...
        $poe_id = "";
        $phrse_no_cur_enc = "";
        if (!empty($flgNotCurEnc) && !empty($this->eId)) {
            $phrse_no_cur_enc = " AND c1.poe_eId != '" . $this->eId . "'  ";
        }
    ...
    private function getPoeId2Attach() {
        ...
        if ($tmp >= 1) {
            ...
            $res = DB::connection('DB_REGISTER_CONNECTION')->select($sql);
            if ($res) {
                $mxDys = 0;
                foreach ($res as $row) {
    ...
    function rem_poe() {
        ...
        $row =DB::connection('DB_REGISTER_CONNECTION')->select($sql);
    }
