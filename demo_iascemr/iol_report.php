...
function reportpop(path,report_format){
	var date=document.iol_report.date1.value;
	...
	if(date==""){
		...
		document.iol_report.action = 'iol_report.php';
		document.iol_report.submit();
	}
...
function date12(){ 
	var s_date=document.iol_report.date1.value;
	if(s_date==""){
...
function resetfields(){
	document.iol_report.surgeon.value='';
	document.iol_report.date1.value='';
	document.iol_report.date2.value='';
	document.iol_report.iol_serial_no.value=""
	document.iol_report.summary.checked=false;
	document.iol_report.detail.checked=false;
...
var flPath = '';
	...
	$iol_serial_no=trim($_REQUEST['iol_serial_no']);
	$vision_status=trim($_REQUEST['vision_status']);
	
	$qry_iol_serial="";
	if($iol_serial_no){
		$qry_iol_serial=" and opr.iol_serial_number like '%".$iol_serial_no."%' ";
	}
	$qry_vision_status = "";
	$qry_vision_join = "";
	if($vision_status){
		if($vision_status == "better") {
			$qry_vision_status	=	" and vs.vision_20_40 = 'Yes' ";	
		}else if($vision_status == "worse") {
			$qry_vision_status	=	" and vs.vision_20_40 = 'No' ";	
		}else if($vision_status == "complication") {
			$qry_vision_status	=	" and vs.complication = 'Yes' ";	
		}
		$qry_vision_join = " INNER JOIN vision_success vs ON (vs.confirmation_id = pc.patientConfirmationId) ";
	}
	...
	if($date!="") {
		...
		if($surgeon==""){
			$query = "select pc.patientConfirmationId from patientconfirmation pc  
					   ...
					   opr.form_status='not completed') $qry_iol_serial ".$qry_vision_status."
		...
		elseif($surgeon!=""){
			$query = "select pc.patientConfirmationId from patientconfirmation pc  
					   ...
					   AND (opr.manufacture!='' OR opr.model!='' OR opr.lensBrand!='' OR opr.Diopter!='')
					    ".$qry_vision_join."
					   WHERE  ( pc.dos between '$selected_date' and '$selected_date2' ) and (opr.form_status='completed' or
					   opr.form_status='not completed') and pc.surgeonId in($surgeon) $qry_iol_serial ".$qry_vision_status."
					   ORDER BY pc.surgeon_name, pc.surgery_time
					   ";
		}
		...
		if($patientConfirmationId){
			$pgNme = 'iol_report_print_summary.php';
			$winNme = 'iol_report_summary_wind';
			if($_REQUEST['report_typechkbx']=="detail") {
				$pgNme = 'iol_report_print_detail.php';
				$winNme = 'iol_report_detail_wind';	
			}
			if($_REQUEST["hidd_report_format"]=="csv") {
				$winNme = 	"_self";
			}
			?>
			<form name="iol_report_sub" id="iol_report_sub" method="post" action="<?php echo $pgNme;?>" target="<?php echo $winNme;?>" >
			...
			<script>
				...
				if(hidd_report_format_new == 'csv') {
					...
					window.open('<?php echo $winNme;?>',winNme,'width='+parWidth+',height='+parHeight+' top=100,left=100,resizable=yes,scrollbars=1');
					document.getElementById('iol_report_sub').submit();
				}
			</script>
			<?php
		}
   ...
   ?>
<form name="iol_report" id="iol_report" action="iol_report.php" method="post" > 
							   ...
							   <Div class="row">
                                   ...
                                   <div class="col-md-6 col-sm-12 col-lg-6 col-xs-12">
									   <div class="form_reg">
											 <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
											   ...
											   <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
												   <Select class="selectpicker form-control" name="vision_status" id="vision_status" data-size="10">
													  <option value="" selected>--All--</option>
													  <option value="better" <?php if($_REQUEST["vision_status"]=="better") { echo "selected"; }?>>20/40 Better</option>
                                                      <option value="worse" <?php if($_REQUEST["vision_status"]=="worse") { echo "selected"; }?>>Worse</option>
                                                      <option value="complication" <?php if($_REQUEST["vision_status"]=="complication") { echo "selected"; }?>>Complication</option>
												   </Select> 
												</div> <!----------------------- Full Inout col-12    ------------------------------>
									   </div>
								   </div>
									   ...
									   <div class="form_reg">
										...
										<div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
											   <div id="datetimepicker1" class="input-group">
												   <input autocomplete="off" type="text" tabindex="1" id="date1" name="date1" value="<?php echo $_REQUEST['date1'];?>" class="form-control">
									   ...
									   <div class="form_reg">
										...
										<div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
											   <div id="datetimepicker2" class="input-group">
												   <input autocomplete="off" type="text" tabindex="1" id="date2" name="date2" value="<?php echo $_REQUEST['date2'];?>" class="form-control">
												   <div class="input-group-addon datepicker">
													   <a href="javascript:void(0)"><span class="glyphicon glyphicon-calendar"></span></a>
												   </div>
											   </div>
								 ...
								 <div class="clearfix margin_adjustment_only"></div>
								  ...
								  <div class="col-md-6 col-sm-12 col-lg-6 col-xs-12">
									   <div class="form_reg">
											 <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
											   <label class="" for="iol_serial_no">
												   Serial Number
											   </label>
											 </div>
											 <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
												<input class="form-control" type="text" id="iol_serial_no" name="iol_serial_no" value="<?php echo $_REQUEST['iol_serial_no'];?>" />
											 </div> <!----------------------- Full Inout col-12    ------------------------------>
									   </div>	
								  </div>
								  <Div class="clearfix margin_adjustment_only visible-sm"></Div>
...
$(document).ready(function() {
	if(hidd_report_format_new == 'csv') {
		if(document.getElementById('iol_report_sub')) {
			document.getElementById('iol_report_sub').submit();	
		}
	}
});
