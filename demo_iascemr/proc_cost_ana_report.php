...
if($_REQUEST['proc_save']=='yes') {
	...
	$qry = "SELECT pc.labor_cost, pc.supply_cost, pc.cost_procedure_id as proc_id, pc.cost_procedure_name as proc_name, pc.patientConfirmationId, users.fname, users.mname, users.lname
			...
			/*AND (pc.supply_cost>0 || pc.labor_cost>0)*/ ".$physicianQry;	//die($qry);
	...
	else
	{
		...
		?>
		<form name="proc_phy_report_sub" id="proc_phy_report_sub" method="post" action="proc_cost_ana_reportpop.php" target="proc_cost_ana_report_win" >
		...
		<script>
			...
			var win=window.open('proc_cost_ana_reportpop.php','proc_cost_ana_report_win','width='+parWidth+',height='+parHeight+' top=10,left=10,resizable=yes,scrollbars=1');
			win.focus();
			document.getElementById('proc_phy_report_sub').submit();
			/*
			var win=window.open('proc_cost_ana_reportpop.php?<?php echo "proc_save=$_REQUEST[proc_save]&date1=$_REQUEST[date1]&date2=$_REQUEST[date2]&physician=$physicianImp&procedure=$procedure_id";?>','Report_result','width='+parWidth+',height='+parHeight+' top=10,left=10,resizable=yes,scrollbars=1');
			win.focus();
			*/
		</script>
        <?php
                                                	...
                                                	<div class="form_reg">
                                                        ...
                                                        <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12">
                                                        	<select class="selectpicker form-control" name="physician[]" id="physician" multiple="multiple" title="Select Physicians" >
                                                            	<option value="all" data-attending= "0" <?php if(in_array('all',$reqstPhyArr) || !$reqstPhyArr) { echo "selected";}?> > All Physicians</option>
															<?PHP
