...
<script>
			...
			var bodySize = function()
			{
				...
				$("#iframeScanUpload").attr('height',(BH-70) + 'px')
        ...
        <div class="body">
			<div class="folder_wrap">
				...
				<?php 
						...
						$scanUploadSrc = "uploader/index.php?upload_url=".urlencode("uploader/save_upload_user.php?method=upload")."&upload_from=user_scan_docs&user_id=$user_id&pconfirmId=$pConfirmId&patient_id=$patient_id&ptStubId=$ptStubId&dosScan=$dosScan&formName=$formName&folderId=$folderId&scanIOL=$scanIOL&IOLScan=$IOLScan&scanDISCHARGE=$scanDISCHARGE&DISCHARGEScan=$DISCHARGEScan";
						$arrBrow = browser();
						
						if($arrBrow['name'] == 'msie' && $arrBrow['version'] < 10)
						{
								$scanUploadSrc = "csxthumbupload.php?upload_url=".urlencode("/$surgeryCenterDirectoryName/admin/uploader/save_upload_user.php?method=upload&activex=1&upload_from=user_scan_docs&user_id=$user_id&pconfirmId=$pConfirmId&patient_id=$patient_id&ptStubId=$ptStubId&dosScan=$dosScan&formName=$formName&folderId=$folderId&scanIOL=$scanIOL&IOLScan=$IOLScan&scanDISCHARGE=$scanDISCHARGE&DISCHARGEScan=$DISCHARGEScan");
						}
						//echo $arrBrow['name'].$arrBrow['version'].$scanUploadSrc;	
			
				?>
                
                <div style="margin:35px auto 5px; border:dashed 1px #DDD; width:75%; display:block ">
                		<iframe name="iframeScanUpload" id="iframeScanUpload" src="<?php echo $scanUploadSrc;?>" width="100%" scrolling="yes" frameborder="0" > </iframe>
    			</div>
