...
$col			=	($RT  == 'pre')			?	'laser_preop_medication'	:	($RT == 'post' ? 'laser_postop_medication' : '' ) ;

	if( $PID > 0  )
		$_REQUEST['laser_templateID']		=	$PID		;
	
	if($RID > 0  && $PID > 0 && !empty($col) )
	{
			$laserDetails	=		$objManageData->getRowRecord('laser_procedure_template
', 'laser_templateID', $_REQUEST['laser_templateID'] );
			...
			$objManageData->UpdateRecord($updateArray, 'laser_procedure_template', 'laser_templateID',$_REQUEST['laser_templateID'] )	;
		
	}
	
	include 'laser_'.$RT.'op_order_sheet.php';
