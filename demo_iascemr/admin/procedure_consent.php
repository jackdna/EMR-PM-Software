...
if($_REQUEST['pro_id']<>"" && $_REQUEST['sub']=='yes'){
	...
	$updat="update surgeonprofileprocedure set 
		consentTemplateId = '$elem_consentIdArrImplode'
		where id = '$pro_id'";
	
	$updat_qry = imw_query($updat) or die(imw_error());		
	echo "<script>window.opener.document.getElementById('pro_cons".$cont."').value = "."'$elem_consentIdArrImplode'".";</script>";
...
if($_REQUEST['pref_card'])
{
	...
	if(imw_num_rows($query)>=1)
	{
		...
		$pref_card_temp_id=explode(',',$pref_card->consentTemplateId);
	}
}
?>
<!DOCTYPE html>
 ...
 <!-- Loader -->
        ...
        <div class="dialog box-sizing">
            <div class="content box-sizing">
                    ...
                    <table class="col-xs-12 col-md-12 col-lg-12 col-sm-12 table-bordered  table-condensed cf  table-striped">
                        ...
                        foreach($consentFormTemplates as $templates){
                            if($_REQUEST['pro_id']<>'') {
                                ...
                                $consentTemplateId = $selectSurgeonProfileProcedureRow['consentTemplateId'];
                            }
                            ...
                            <tr style="height:20px;background-color:<?php if(($seq%2)!=0) echo '#FFFFFF';?>;">
                                <td class="text_10" style="width:10px;">
                                    <input type="checkbox"  name="elem_consentIdList[]" value="<?php echo $templates->consent_id; ?>" <?php if(in_array($templates->consent_id,$consentTemplateIdExplode) || in_array($templates->consent_id, $pref_card_temp_id)) { echo "checked"; } ?>>
                                </td>
				...
				<a class="btn btn-primary" href="javascript:void(0)" onClick="document.pro.submit();" id="saveButton"><b class="fa fa-floppy-o" ></b>&nbsp;Save</a>
