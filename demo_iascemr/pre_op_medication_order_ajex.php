...
$objManageData= new manageData;
	...
	$preDefined		= addslashes($_GET['medication_name']);
	$strength			= addslashes($_GET['strength']);
	$directions		= addslashes($_GET['directions']);
	$sourcePage 	=	isset($_GET['sourcePage'])	?	$_GET['sourcePage']	:	0	;
	$timemed			= $_GET['timemeds'];
	
	$preOpPhysicianOrdersIdQry=("select * from preopphysicianorders where patient_confirmation_id ='".$pConfId."'");
	$preOpPhysicianOrdersIdRes=imw_query($preOpPhysicianOrdersIdQry);
	$Id=imw_fetch_array($preOpPhysicianOrdersIdRes);
	$PreOpPhysicianOrdersId=$Id['preOpPhysicianOrdersId'];
	
	//TIME saved in database 
	$meds_times=$timemed;
	$time_split = explode(" ",$meds_times);
	
	if($time_split[1]=="PM" || $time_split[1]=="pm") {
			
		$time_split = explode(":",$time_split[0]);
		$medsTimeIncr=$time_split[0]+12;
		 $meds_time = $medsTimeIncr.":".$time_split[1].":00";
		
	}elseif($time_split[1]=="AM" || $time_split[1]=="am") {
			$time_split = explode(":",$time_split[0]);
		$meds_time=$time_split[0].":".$time_split[1].":00";
		
		if($time_split[0]=="00" && $time_split[1]=="00") {
			$meds_time=$time_split[0].":".$time_split[1].":01";
		}
	}
	//TIME saved in database
	
	imw_query("INSERT INTO 
				patientpreopmedication_tbl set	
				medicationName = '$preDefined',
                                        ...
                                        <div class="<?=$containerCSS?> col-sm-12 col-xs-12">
                                            ...
                                            <div class="row" id="row<?php echo $seq; ?>">
                                                <div class="col-md-3 col-lg-3 col-xs-3 col-sm-3">
                                                    <input class="form-control" type="text" placeholder="Medication"/>
                                                </div>
                                                <div class="col-md-3 col-lg-3 col-xs-3 col-sm-3">
                                                    <input class="form-control" type="text" placeholder="Direction" />                                                                            </div>
                                                <div class="col-md-3 col-lg-3 col-xs-3 col-sm-3">
                                                ...
                                                <div class="col-md-3 col-lg-3 col-xs-3 col-sm-3">
                                                     <div class="row">
                                                        ...
                                                        <div class="col-md-4 col-lg-4 col-xs-4 col-sm-4 text-center">
                                                           <a style="margin:0" class="btn btn-danger" href="javascript:void(0)">X</a>			                                            			</div>	
                                                     </div>
                                                </div>
                                            </div>
                                        </div>	   
