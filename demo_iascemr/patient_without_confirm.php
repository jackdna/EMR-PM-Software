...
//START ANOTHER CHECK FOR PATIENT CONFIRMATION ID (EXIST OR NOT)

$stubTableChkConfIDExistQry = "select patient_confirmation_id from stub_tbl where stub_id = '".$stub_id."'";
$stubTableChkConfIDExistRes = imw_query($stubTableChkConfIDExistQry);
$stubTableChkConfIDExistNumRow = imw_num_rows($stubTableChkConfIDExistRes);
if($stubTableChkConfIDExistNumRow>0) {
	$stubTableChkConfIDExistRow = imw_fetch_array($stubTableChkConfIDExistRes);
	$pConfId=$stubTableChkConfIDExistRow['patient_confirmation_id'];
}
//END ANOTHER CHECK FOR PATIENT CONFIRMATION ID (EXIST OR NOT)
...
if($pConfId){
	...
	<script>				
		...
		var stub_id = '<?php echo $stub_id; ?>';
		location.href='mainpage.php?patient_id='+pId+'&pConfId='+pConfId+'&multiwin='+multiwin+'&stub_id='+stub_id;
	</script>	
	<?php
}
	...
	//START INSERT/UPDATE IN CONFIRMATION TABLE
		...
		// UPDATE SCAN DOCUMENTS, SCAN UPLOAD 
			...
			$updateScanUploadQry = "update `scan_upload_tbl` set confirmation_id = '".$insertConfirmationId."' 
										 WHERE patient_id='".$insertPatientDataId."' AND confirmation_id='0' AND dosOfScan = '$patient_dos_temp' AND stub_id = '".$_REQUEST["stub_id"]."' AND stub_id != '0'";
		...
		//INSERT NEW ENTRY OF SCAN DOCUMENT WITH PATIENT ID
			$chk_insert_scan_document_qry1 = "select document_id from scan_documents where document_name = 'Pt. Info' AND patient_id = '$insertPatientDataId' AND confirmation_id = '0' AND dosOfScan = '$patient_dos_temp' AND stub_id = '".$_REQUEST["stub_id"]."' AND stub_id != '0'";
			...
			if($chk_insert_scan_document_numrow1>0) {
				...
				$update_scan_document_qry1 = "update `scan_documents` set 
											...
											AND stub_id = '".$_REQUEST["stub_id"]."' AND stub_id != '0'
											";
				$update_scan_document_res1 = imw_query($update_scan_document_qry1) or die(imw_error());										
			
			}else {	
				$chk_update_scan_document_qry1 = "select document_id from scan_documents where document_name = 'Pt. Info' AND patient_id = '".$insertPatientDataId."' AND confirmation_id = '".$insertConfirmationId."'  AND stub_id = '".$_REQUEST["stub_id"]."' AND stub_id != '0'";
