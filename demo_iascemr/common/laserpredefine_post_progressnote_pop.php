...
$qry_post_progressnote = "select * from laserpredefine_postprogressnotes_tbl order by `name`";
$res_post_progressnote = imw_query($qry_post_progressnote) or die(imw_error());
$totalRows_post_progressnote = imw_num_rows($res_post_progressnote);
?>
<script>
function getInnerHTMLpost_progressnote(obj){
	var  val = obj.innerHTML;
	var obj2 = document.getElementById('txtarea_Post_ProgressNote');
	if(obj2.value==''){
		obj2.value = val;
	}else{
		obj2.value += ', '+val;
...
<div id="evaluationPost_ProgressNote" onMouseOver="stopCloseEkg();" onMouseOut="closeEkg('evaluationPost_ProgressNote');" style="position:absolute; background:#FFF; display:none; overflow:auto; padding:0px;margin:235px 0;border:1px solid #CCC;border-radius:2px;z-index:999;" class="col-md-5 col-lg-4 col-xs-5 col-sm-5">
    <div  class="col-md-12 col-lg-12 col-xs-12 col-sm-12" style="height:30px; width:100%;  background:#d9534f;  padding-top:5px;color:#FFF; text-align:left;font-weight:bold;padding-left:5px;">Post Op Order<span onClick="document.getElementById('evaluationPost_ProgressNote').style.display='none';" style="float:right; color:#FFF; cursor:pointer; font-family:Verdana;">X</span>
    </div>
    <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12" style="height:150px; overflow:auto">
<?php
	$rows = 5; 
	$counter_post_progressnote=0;
	while ($res_post_progressnote_row = imw_fetch_assoc($res_post_progressnote)){
		$counter_post_progressnote++;
?>
		<div class="row hoverdiv" style="cursor:pointer; background:#FFF; padding:5px 0 5px 0 ; border-bottom:1px solid #CCC;" id="post_progressnote_tr<?php echo $counter_post_progressnote; ?>">
            <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12" onClick="return getInnerHTMLpost_progressnote(this)"><?php echo stripslashes($res_post_progressnote_row['name']).''; ?></div>
        </div>
		<?php
	}
?>
