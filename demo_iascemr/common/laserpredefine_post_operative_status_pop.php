...
$qry_post_operative_status = "select * from laserpredefine_postoperativestatus_tbl  order by `name`";
$res_post_operative_status = imw_query($qry_post_operative_status) or die(imw_error());
$totalRows_post_operative_status = imw_num_rows($res_post_operative_status);
?>
<script>
function getInnerHTMLpost_operative_status(obj){
	var  val = obj.innerHTML;
	var obj2 = document.getElementById('txtarea_Post_Operative_Status');
	if(obj2.value==''){
		obj2.value = val;
	}else{
		obj2.value += ', '+val;
...
<div id="evaluationpost_operative_status" onMouseOver="stopCloseEkg();" onMouseOut="closeEkg('evaluationpost_operative_status');" style="position:absolute; background:#FFF; display:none; overflow:auto; padding:0px;margin:235px 0;border:1px solid #CCC;border-radius:2px;z-index:999;" class="col-md-5 col-lg-4 col-xs-5 col-sm-5">
    <div  class="col-md-12 col-lg-12 col-xs-12 col-sm-12" style="height:30px; width:100%;  background:#d9534f;  padding-top:5px;color:#FFF; text-align:left;font-weight:bold;padding-left:5px;">Progress Notes<span onClick="document.getElementById('evaluationpost_operative_status').style.display='none';" style="float:right; color:#FFF; cursor:pointer; font-family:Verdana;">X</span>
    </div>
    <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12" style="height:150px; overflow:auto">
	<?php
	$rows = 5;
	$counter_post_operative_status=0;
	while ($res_post_operative_status_row = imw_fetch_assoc($res_post_operative_status)){
		$counter_post_operative_status++;
		?>
		
		<div class="row hoverdiv" style="cursor:pointer; background:#FFF; padding:5px 0 5px 0 ; border-bottom:1px solid #CCC;" id="post_operative_status_tr<?php echo $counter_post_operative_status; ?>">
            <div class="col-md-12 col-lg-12 col-xs-12 col-sm-12" onClick="return getInnerHTMLpost_operative_status(this)"><?php echo stripslashes($res_post_operative_status_row['name']).''; ?></div>
        </div>
	<?php
	}
	//}
