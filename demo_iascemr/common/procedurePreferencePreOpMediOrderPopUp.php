...
?>
<script>
function getInnerHTMLProcedurePreference(objvalue,objstrength,objdirections,mediId,mediCatID){
	...
	if(!isNaN(objectCounter)){
		for(i=1;i<=objectCounter; i++){
			if(top.frames[0].frames[0].document.getElementById(objectName+i)){
				if(top.frames[0].frames[0].document.getElementById(objectName+i).value==''){
					var str = objvalue;		
					if(str.indexOf(" - ")!=-1){
							posStr1 = str.indexOf(" - ");
					...
					top.frames[0].frames[0].document.getElementById(objectName+i).value = str;		
					top.frames[0].frames[0].document.getElementById(objSecondary+i).value = objstrength;					
					top.frames[0].frames[0].document.getElementById(objtertiary+i).value = objdirections;					
					top.frames[0].frames[0].document.getElementById(objmediID+i).value = mediId;
					top.frames[0].frames[0].document.getElementById(objmediCatID+i).value = mediCatID;
...
//alert(top.frames[0].frames[0].document.getElementById("hiddPreDefineId"));
	if(top.frames[0].frames[0].document.getElementById("hiddPreDefineId").value=="preDefineOpenYes") {
		if(top.frames[0].frames[0].document.getElementById('ProcPrefPreOpMediOrderDiv').style.display == "block"){
			top.frames[0].frames[0].document.getElementById('ProcPrefPreOpMediOrderDiv').style.display = "none";
...
<div class="modal fade" id="ProcPrefPreOpMediOrderDiv" style="overflow:hidden !important;">
	...
	<?php		
			while($cat_fetch = imw_fetch_array($sub_category)){
				//echo $procedure_seq;
				 $procedure_seq++;
			?>
				<tr style="height:25px; cursor:pointer;" id="procedure<?php echo $id;?>_tr<?php echo $procedure_seq; ?>" >
						<td style="padding-left:2px; display:inline-block;" class="nowrap text_10" onClick="return getInnerHTMLProcedurePreference('<?php echo $cat_fetch['medicationName'];?>','<?php echo $cat_fetch['strength'];?>','<?php echo $cat_fetch['directions'];?>','<?php echo $cat_fetch['preOpMedicationOrderId'];?>','<?php echo $cat_fetch['mediCatId'];?>')">
