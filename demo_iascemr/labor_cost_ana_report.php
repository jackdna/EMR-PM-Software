...
<?php 
  ...
  $fac_qry	=	" and st.iasc_facility_id='$_SESSION[iasc_facility_id]' ";
	...
	$qry = "SELECT pc.labor_cost, pc.supply_cost, pc.cost_procedure_id as proc_id, pc.cost_procedure_name as proc_name,
			...
			/* AND pc.labor_cost>0 */ ".$physicianQry;		//die($qry);
	...
	else
	{
		...
		?>
		<form name="labor_cost_ana_report_sub" id="labor_cost_ana_report_sub" method="post" action="labor_cost_ana_reportpop.php" target="labor_cost_ana_report_win" >
    ...
    <script>
			...
			var win=window.open('labor_cost_ana_reportpop.php','labor_cost_ana_report_win','width='+parWidth+',height='+parHeight+' top=10,left=10,resizable=yes,scrollbars=1');
			win.focus();
			document.getElementById('labor_cost_ana_report_sub').submit();
			/*
			var win=window.open('labor_cost_ana_reportpop.php?<?php echo "proc_save=$_REQUEST[proc_save]&date1=$_REQUEST[date1]&date2=$_REQUEST[date2]&physician=$physicianImp&procedure=$procedure_id";?>','Report_result','width='+parWidth+',height='+parHeight+' top=10,left=10,resizable=yes,scrollbars=1');
			win.focus();
			*/
		</script>
    <?php
