...
if(empty($totalCount))
{
	...
	// Get Total Count Where surgeryStartTime Field contains value like 24:13:00
	$qry	=	"Select count(*) as totalRecords From ".$tbl." Where surgeryStartTime LIKE '24%'";
...
if($totalCount > 0 )
{
		$qry	=	"Select operatingRoomRecordsId, surgeryStartTime From ".$tbl." Where surgeryStartTime LIKE '24%' LIMIT 0, ".$fetchRecords;
		$sql	=	imw_query($qry) or $msg_info[] = imw_error();
		...
		if(imw_num_rows($sql)>0)
		{
			while($row	=	imw_fetch_assoc($sql))
			{
					...
					$old_surgeryStartTime	=	$row['surgeryStartTime'];
					list($hour,$minute,$second)	=	explode(':',$old_surgeryStartTime);
					$new_surgeryStartTime	=	((int)$hour-12).':'.$minute.':'.$second;
					
					$updtQry	=	"Update ".$tbl." Set surgeryStartTime = '".$new_surgeryStartTime."' Where operatingRoomRecordsId = '".$recordID."' ";
...
$msg_info[] = "Update Surgery Start Time in Operating Room Records for previous record - run OK";
