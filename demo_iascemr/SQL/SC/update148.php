...
$qry = "SELECT scan_upload_id,image_type,img_content,confirmation_id,stub_id,iolink_scan_consent_id FROM ".$archiveScanDbName."scan_upload_tbl where `img_content`!='' ORDER BY `scan_upload_id` DESC LIMIT 0,1";
...
if(imw_num_rows($res)>0) {
	while($row = imw_fetch_array($res)) {
		...
		$pdfFolderName = $rootServerPath.'/'.$surgeryCenterDirectoryName.'/admin/pdfFiles/'.$surgeonName;
