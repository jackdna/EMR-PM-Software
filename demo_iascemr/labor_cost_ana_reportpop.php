...
$objManageData = new manageData;//create array for procedure acro
...
if($_REQUEST['proc_save']=='yes') {
	...
	$proc_JOIN = '';
	if( defined('STRING_SEARCH')=='YES' && constant('STRING_SEARCH')=='YES')
	...
	$qry = "SELECT pc.labor_cost, pc.supply_cost, pc.cost_procedure_id as proc_id, pc.cost_procedure_name as proc_name,
			pc.patientConfirmationId, users.usersId, users.fname, users.mname, users.lname,
	...
	if(imw_num_rows($res)>0) {
		...
		while($row = imw_fetch_assoc($res)) 
		{
			...
			if($row['fname'] && $recExist == true )
			{
				...
				if($row['labor_cost']>0)
				{
					$total_cost[$surgeon_name]+=$row['labor_cost'];
					$total_proc_cost[$proc_alias[$row['proc_id']]]+=$row['labor_cost'];
...
if($_REQUEST['action'] == 'csv')
{
		...
		$csvData	=	array();
		array_push($csvData,array($name, '', 'Surgery Center Labor Cost Analysis Report '.date("m-d-Y"),'' ));
		...
		array_push($csvData,array('','','',''));
		array_push($csvData,array('','','',''));
		array_push($csvData,array('Surgeon','Total Procedures','Total Labor Cost','Average Labor Cost'));
		...
		$file_name	=	'labor_cost_analysis_report.csv';
...
<title>Labor Cost Analysis Report</title>    
...
<div class="main_wrapper analysis-top-bar">
		<form name="labor_cost_ana_reportpop_csv" id="labor_cost_ana_reportpop_csv" method="post" action="labor_cost_ana_reportpop.php">
                      ...
                      <div class="wrap_inside_admin">
                        <div class=" subtracting-head">
                            <div class="head_scheduler new_head_slider padding_head_adjust_admin">
                                <span>Labor Cost Analysis Report</span>
                                ...
                                <a href="javascript:void(0)" class="btn btn-info analysis-csv-btn" id="generate_csv" onclick="javascript:document.labor_cost_ana_reportpop_csv.submit();" >
...
<div class="col-md-12 col-sm-12 col-xs-12 col-lg-12">
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 ">
        ...
        <span>Total Cost Of <?php echo $proc_name;?> Procedure</span></div>
    ...
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 ">
        <div class="middle_sub_head text-left">
        <span>Average Cost Of <?php echo $proc_name;?> Procedure</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
    	<div id="average_cost_div" class="col-md-12 col-sm-12 col-xs-12 col-lg-12" style="height:400px"></div> 
	</div>
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 ">
        <div class="middle_sub_head text-left">
        <span>Mean Cost Of <?php echo $proc_name;?> Procedure</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
...
$meanCost=str_replace(',','',$meanCost);
$total_cost_js_arr=json_encode($total_cost_arr);


/*echo"<pre>";
print_r($total_cost_arr);
print_r($average_cost_arr);
echo "</pre>";*/
?>
...
foreach($total_proc_cost as $proc_name=>$cost)
{
	...
	{
		$value=0;
		$proc_cost_average[$proc_name]=number_format(($cost/$no_of_proc+1),2,'.','');
	}	
}
//create json array for average cost
$key=0;
...
<div class="col-md-12 col-sm-12 col-xs-12 col-lg-12">
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 ">
        <div class="middle_sub_head text-left">
        <span>Total Labor Cost Per Procedure</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
    ...
    <div class="col-md-4 col-sm-12 col-xs-12 col-lg-4 ">
        <div class="middle_sub_head text-left">
        <span>Average Labor Cost Per Procedure</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
        ...
        <div class="inner_surg_middle ">
                ...
                <div style="" id="" class="all_content1_slider ">	         
                      ...
                      <div class="wrap_inside_admin">
                        <div class=" subtracting-head">
                            <div class="head_scheduler new_head_slider padding_head_adjust_admin">
                                <span>Top Ten Surgeon By Labor Cost</span>
                            </div>
                        </div>
                </div> 
              </div>  
...
$qry = "SELECT COUNT(*) AS total_rec, SUM(pc.labor_cost) AS total_cost,  pc.patientConfirmationId,
		...
		WHERE (pc.dos BETWEEN '".$from_date."' AND '".$to_date."') ".$fac_con."
		AND pc.finalize_status='true'
		/* AND pc.labor_cost>0 */ ".$physicianQry."
		AND pc.surgeonId<>0
...
if(imw_num_rows($res)>0) {
	...
	foreach($total_cost_top_ten as $surgeon=>$cost)
	{
		...
		{
			$value=0;
			$value=number_format(($cost/$total_proc_top_ten[$surgeon]),2,'.','');
			$proc_average_top_ten[$surgeon]=str_replace(',','',$value);
		}	
	}
}
...
<div class="col-md-12 col-sm-12 col-xs-12 col-lg-12">
    <div class="col-md-6 col-sm-12 col-xs-12 col-lg-6 ">
        <div class="middle_sub_head text-left">
        <span>Top Ten Sugeon By Labor Average Cost (lowest to highest)</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
        <div id="highest_div" class="col-md-12 col-sm-12 col-xs-12 col-lg-12" style="height:450px"></div>
    </div>
    <div class="col-md-6 col-sm-12 col-xs-12 col-lg-6 ">
        <div class="middle_sub_head text-left">
        <span>Top Ten Sugeon By Labor Average Cost (highest to lowest)</span></div>
        <div class="clearfix margin_adjustment_only margin_line"></div>
  ...
  <table class = "table table-bordered">
  	<tbody>
    	<tr>
         ...
         <th class="col-md-1 col-sm-1 col-xs-1 col-lg-1">Total Labor Cost</th>
         <th class="col-md-1 col-sm-1 col-xs-1 col-lg-1">Average Labor Cost</th>
      ...
      <?php
				...
				$pdfSurgeonArray[]	=	array('Surgeon','Total Procedure','Total Labor Cost','Average Labor Cost');
				foreach($total_cost as $surgeon=>$cost)
				{
...
{	
		...
		var _pdfReportTitle	=	' Labor Cost Analysis Report <?=date('m-d-Y')?> ';
		var _pdfSaveAsName	=	'labor_cost_analysis_report.pdf';
		...
		_pdfContent.push({
			...
			[
					{
						width: "33%",
						text: 'Total Cost Of Selected Procedure',
						style:'boxTitle'
					},
					{
						width: "33%",
						text: 'Average Cost Of Selected Procedure',
						style:'boxTitle'
						
					},
					{
						width: "*",
						text: 'Mean Cost Of Selected Procedure',
						style:'boxTitle'
					}
			],
			columnGap: 5
		...
		_pdfContent.push({
			...
			[
				{
					width: "33%",
					text: "Total Labor Cost Per Procedure",
					style:'boxTitle'
				},
				{
					width: "33%",
					text: "Average Labor Cost Per Procedure",
					style:'boxTitle' 
		...
		_pdfContent.push({ 
			text:' Top Ten Surgeon By Labor Cost ',style:'subHeader'
		});
		...
		_pdfContent.push({
			...
			[
				{
					width: "50%",
					text: "Top Ten Sugeon By Labor Average Cost (lowest to highest)",
					style: "boxTitle",
				},
				{
					width: "*",
					text: "Top Ten Sugeon By Labor Average Cost (highest to lowest)",
					style: "boxTitle",
				}
			],
			columnGap: 5,
