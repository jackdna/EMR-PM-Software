...
//CODE TO DISABLE SLIDER LINK AT SINGLE CLICK
	...
	$fieldName = "hippa_form";
	$pageName = "conset_hippa_form.php?patient_id=$patient_id&pConfId=$pConfId&ascId=$ascId";
...
//GET PATIENT DETAIL
	$HIPPA_patientName_tblQry = "SELECT * FROM `patient_data_tbl` WHERE `patient_id` = '".$_REQUEST['patient_id']."'";
...
if($_POST['SaveRecordForm']=='yes'){
	...
	//SET FORM STATUS ACCORDING TO MANDATORY FIELD
		...
		if($_POST["consentHippa_patient_sign"]=="" || $_POST["consentHippa_patient_sign"]=="255-0-0:;") {
			$form_status = "not completed";
		}
	//END SET FORM STATUS ACCORDING TO MANDATORY FIELD
	$chkConsentHippaQry = "select * from `hippa_consent_form` where  confirmation_id = '".$_REQUEST["pConfId"]."'";
	$chkConsentHippaRes = imw_query($chkConsentHippaQry) or die(imw_error()); 
	$chkConsentHippaNumRow = imw_num_rows($chkConsentHippaRes);
	if($chkConsentHippaNumRow>0) {
	  	//CODE START TO CHECK FORM STATUS (IF EMPTY THEN REFRESH SLIDER ON SAVE)
			$chkFormStatusRow = imw_fetch_array($chkConsentHippaRes);
			$chk_form_status = $chkFormStatusRow['form_status'];
		//CODE START TO CHECK FORM STATUS (IF EMPTY THEN REFRESH SLIDER ON SAVE)
		
		$SaveConsentHippaQry = "update `hippa_consent_form` set 
									hippa_consent_data = '".addslashes($_POST["hippa_consent_data"])."',
									hippa_consent_sign = '".$_POST["consentHippa_patient_sign"]."', 
									form_status ='".$form_status."'
									WHERE confirmation_id='".$_REQUEST["pConfId"]."'";
	}else {
		$SaveConsentHippaQry = "insert into `hippa_consent_form` set 
									hippa_consent_data = '".addslashes($_POST["hippa_consent_data"])."',
									hippa_consent_sign = '".$_POST["consentHippa_patient_sign"]."', 
									form_status ='".$form_status."',
									confirmation_id='".$_REQUEST["pConfId"]."'";
	}
	//echo $SaveConsentHippaQry;
	$SaveConsentHippaRes = imw_query($SaveConsentHippaQry) or die(imw_error());
	...
	//REFRESH SLIDER (IF FORM STATUS IS EMPTY OR CHANGED IN DATABASE ON SAVE)
		if($form_status == "completed" && ($chk_form_status=="" || $chk_form_status=="not completed")) {
			echo "<script>top.frames[0].location='blankform.php?frameHref=conset_hippa_form.php&SaveForm_alert=true$saveLink';</script>";
		}else if($form_status=="not completed" && ($chk_form_status==""  || $chk_form_status=="completed")) {
			echo "<script>top.frames[0].location='blankform.php?frameHref=conset_hippa_form.php&SaveForm_alert=true$saveLink';</script>";
		}
	//REFRESH SLIDER (IF FORM STATUS IS EMPTY OR CHANGED IN DATABASE ON SAVE)

	/* echo "<script> location.href='conset_hippa_form.php?formStatus=filled$saveLink';</script>";*/
}
//END SAVE RECORD IN DATABASE

//VIEW RECORD FROM DATABASE
	$ViewConsentHippaQry = "select * from `hippa_consent_form` where  confirmation_id = '".$_REQUEST["pConfId"]."'";
...
function get_App_Coords(objElem){
	var coords,appName;
	var objElemSign = document.frm_consent_hippa.consentHippa_patient_sign;
...
function getCoords(){		
	var coords = document.applets["app_ConsentHippaSignature"].getSign();
	return coords;
}
function getclear_os(){
	document.applets["app_ConsentHippaSignature"].clearIt();
	changeColorThis(255,0,0);
	document.applets["app_ConsentHippaSignature"].onmouseout();
}
function changeColorThis(r,g,b){				
	document.applets['app_ConsentHippaSignature'].setDrawColor(r,g,b);								
}
//Applet
...
<?php 
	$query_rsNotes = "SELECT * FROM eposted WHERE table_name = 'hippa_consent_form' AND patient_conf_id = '$pConfId' ";
	$rsNotes =imw_query($query_rsNotes);
	$totalRows_rsNotes =imw_num_rows($rsNotes);
?> 
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	 <form name="frm_consent_hippa" class="wufoo topLabel" enctype="multipart/form-data" method="post" style="margin:0px; " action="conset_hippa_form.php?saveRecord=true&SaveForm_alert=true<?php echo $saveLink;?>">
			...
			<input type="hidden" name="frmAction" value="conset_hippa_form.php">
			<tr>
				...
				<td  valign="top" align="center">
					<table width="30%" border="0" cellpadding="0" cellspacing="0">
						<tr>
							...
							<td valign="middle" bgcolor="#BCD2B0" align="center" class="text_10b" >HIPPA Consent Form</td>
			  ...
			  <td bgcolor="#ECF1EA" align="left">
				<table width="99%"  align="center" border="0" cellpadding="0" cellspacing="0" class="all_border">
					<tr height="22" bgcolor="#D1E0C9">
						<td width="1%"></td>
						<td colspan="2" class="text_10b">HIPPA Consent</td>
					</tr>
					...
					<tr height="300" bgcolor="#FFFFFF">
						...
						<td height="22" width="95%" class="text_10" valign="top">
							...
							if($img_content){
								...
								?>
								<!-- <input type="hidden" name="hippa_consent_data" value="<?php //echo $hippa_consent_data;?>"> -->
								<?php echo $hippa_consent_data;?>
								<textarea name="hippa_consent_data" style=" height:1px;visibility:hidden; "><?php echo $hippa_consent_data;?></textarea>
								<?php
							}
							?>
						</td>
		   ...
		   <tr>
				<td width="99%"  valign="top" align="center">
					<table cellpadding="0"  cellspacing="0" rules="none" border="1" bgcolor="#F1F4F0" bordercolor="#D1E0C9"   width="99%">
						...
						<tr height="80">
							<td width="41"></td>
							<td class="text_10b" nowrap><?php echo $HIPPA_patientName.' : ';?></td>
							<td width="780" colspan="1">
								<!-- <input type="text"  class="all_border" style="height:50px; width:200px; " disabled> -->
								<input type="hidden" name="consentHippa_patient_sign" value="<?php echo $consentHippa_patient_sign;?>">
								<applet name="app_ConsentHippaSignature" code="MyCanvasColored.class" 
									...
									<param name="strpixls" value="<?php echo $consentHippa_patient_sign;?>">
	...
	<form name="frm_return_BlankMainForm" class="wufoo topLabel" enctype="multipart/form-data" method="post" style="margin:0px; " action="conset_hippa_form.php?cancelRecord=true<?php echo $saveLink;?>" target="_self">
	</form>
...
//CODE FOR FINALIZE FORM
	$finalizePageName = "conset_hippa_form.php";
